...
//    , opt_allIncPresets("PCASSO - INIT", "pAllSetup", "add to all incarnations (after other setups)", 0, optionListPtr)
    ...
    , opt_storageSize("PCASSO - INIT", "pcasso-storageSize", "Number of clauses in one ring buffer (0 => 4000 x threads)", 0, IntRange(0, INT32_MAX), optionListPtr)
    ...
    , opt_sizeChange("SEND",  "pcasso-sizeChange",  "set to value greater than 0 to see dynamic limit changes! (e.g. 0.05)", 0,   DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
    , opt_lbdChange("SEND",   "pcasso-lbdChange",   "set to value greater than 0 to see dynamic limit changes! (e.g. 0.02)", 0,   DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
    , opt_sendRatio("SEND",   "pcasso-sendRatio",   "", 0.1, DoubleRange(0, true, 1, true), optionListPtr)

    , opt_doBumpClauseActivity("SEND", "pcasso-bumpSentCA", "bump activity of received clauses", false, optionListPtr)
    ...
    , opt_useDynamicLimits("SEND", "pcasso-dynLimits", "update sharing limits dynamically", false, optionListPtr)
