...
void normalizeDiv ( vector<double>& values, vector<double>& divisors ){
...
int getNearestPoint( vector<pair<int, double> >& distances, int& amountClasses ){
...
double euclideanDistance ( vector<double>& a, vector<double>& b ){
  ...
  if (a.size() != b.size()) exit(1); //TODO assert
