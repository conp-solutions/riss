...
class ProofMaster {
    ...
    vector<vector<CRef>>
        hashTable;      // for each hash have a list of clauses, the LBD of the clause is re-used as the counter for the number of occurrences
...
public:
     ...
     * @param extraClauseLit additional literal of the clause that is written to the proof at the first position (and might be presend in the clause
     * again)
     * @param ownerID id of the calling thread, for the clause sharing pool the ID has to match the thread number
     * @param local add the clauses only to the local storage, and do not submit them to the global proof (can be done later, when the next clauses
     * are sent)
     */
    template <class T>
     ...
     * @param clause clause that should be added (can be vector or clause)
     * @param ownerID id of the calling thread, for the clause sharing pool the ID has to match the thread number
     * @param local delete the clauses only from the local storage, and do not ubdate the global proof (can be done later, when the next clauses are
     * sent)
     */
    template <class T>
...
private:
    template <class T>
    unsigned long long getHash(const T& clause, const Lit& extraClauseLit, int startIndex,
                               int endIndex);    // hash function for a single clause, extraLit might be present in clause again
...
inline void ProofMaster::updateGlobalProof(int ownerID, bool useExtraLock) {
    ...
    if (localClauses[ownerID].size() == 0) {
        return;
    } // if there is nothing to do, return, and do not lock!
    // lock
    if (useExtraLock) {
        ownLock.lock();
    }
    ...
    for (int i = 0; i < currentList.size(); ++i) {
        if (currentList[i] == lit_Undef) { // found the next clause
            ...
            deleteClause = false;
            if (currentList[start] == lit_Error) {
                deleteClause = true;
                start++;
            } // check whether this clause should be deleted, update start pointer

            // add the clause to the global proof
            if (deleteClause) {
                removeGlobalClause(currentList, lit_Undef, ownerID, start, end);
            } else {
                addGlobalClause(currentList, lit_Undef, ownerID, start, end);
            }
        }
    }
    currentList.clear();
...
inline void ProofMaster::addUnitToGlobalProof(const Lit& unit, int ownerID) {
    ...
    if (useCounting) { // only if the counting feature is enabled
        ...
        if (list.size() > 0) { // there are clauses that have to be matched
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (c.size() != 1 || c[0] != unit) {
                    continue;
                } // not the same clause, if the size check fails
                hashClause = list[i];
                break; // store current reference, stop loop
            }
        }
    }
    ...
    if (hashClause == CRef_Undef) {
        ...
        if (opc != 0) { // check proof
            if (!opc->addClause(unit)) {
                cerr << "c adding unit clause " << unit << " to global proof fails" << endl;
                assert(false);
                exit(13);
            }
        }
        ...
        // write to file
        if (opt_verboseProof) {
            fprintf(drupProofFile, "c add clause by %i\n", ownerID);
        }
        ...
        if (useCounting) {
            ...
            if (opt_verboseProof) {
                cerr << "c add clause " << ca[ref] << " to hash table "
                     << " (hash(" << thisHash << "): " << getHash(unit) << " ) list:" << hashTable[thisHash] << endl;
            }
        }
        ...
        if (useCounting) {
            ...
            if (opt_verboseProof) {
                cerr << "c set counter of " << ca[hashClause] << " to " << ca[hashClause].lbd() << " (hash: " << getHash(unit)
                     << " ) list:" << hashTable[thisHash] << endl;
            }
        }
    }
}
...
inline void ProofMaster::addGlobalClause(const T& clause, const Lit& extraClauseLit, int ownerID, int startIndex, int endIndex) {
    ...
    if (useCounting) {                                                    // check whether the clause already exists
        ...
        if (list.size() > 0) { // there are clauses that have to be matched
            ...
            if (extraClauseLit != lit_Undef) {
                clauseSize++;
                matchArray.setCurrentStep(toInt(extraClauseLit));
            } // mark all literals of the current clause
    ...
    if (hashClause == CRef_Undef) {
        ...
        if (opc != 0) { // perform check for the global proof only
            opcTmpCls.clear();
            if (extraClauseLit != lit_Undef) {
                opcTmpCls.push(extraClauseLit);
            }
            for (int i = startIndex; i < endIndex; i++) {
                if (clause[i] != lit_Undef && clause[i] != extraClauseLit)
                    opcTmpCls.push(clause[i]);
            }
            if (!opc->addClause(opcTmpCls)) {
                cerr << "c adding clause " << opcTmpCls << " to global proof fails" << endl;
                assert(false);
                exit(13);
            }
        }
        ...
        if (extraClauseLit != lit_Undef) {
            fprintf(drupProofFile, "%i ", (var(extraClauseLit) + 1) * (-2 * sign(extraClauseLit) + 1));
        ...
        if (useCounting) {
            ...
            tmpCls.clear();
            if (extraClauseLit != lit_Undef) {
                tmpCls.push(extraClauseLit);
            }
            for (int i = 0; i < clause.size(); ++i)
                if (clause[i] != extraClauseLit) {
                    tmpCls.push(clause[i]);
                }
            ...
            if (opt_verboseProof) {
                cerr << "c add clause " << ca[ref] << " to hash table "
                     << " (hash(" << thisHash << "): " << getHash(clause, extraClauseLit, startIndex, endIndex) << " ) list:" << hashTable[thisHash]
                     << endl;
            }
        } else {
            if (opt_verboseProof) {
                cerr << "c do not add to hash table" << endl;
            }
        }
    } else { // else increase the counter of that clause by 1
        if (useCounting) {
            ca[hashClause].setLBD(ca[hashClause].lbd() + 1); // re-use LBD
            if (opt_verboseProof) {
                cerr << "c set counter of clause " << ca[hashClause] << " to " << ca[hashClause].lbd() << " (hash(" << thisHash << ": "
                     << getHash(clause, extraClauseLit, startIndex, endIndex) << " ) list:" << hashTable[thisHash] << endl;
            }
        } else {
            if (opt_verboseProof) {
                cerr << "c do not add to hash table" << endl;
...
inline void ProofMaster::removeGlobalClause(const T& clause, const Lit& extraClauseLit, int ownerID, int startIndex, int endIndex) {
    ...
    if (useCounting) { // check whether the clause is in the proof multiple times
        ...
        const vector<CRef>& list = hashTable[thisHash];
        if (opt_verboseProof) {
            cerr << "c list with hash " << thisHash << ": " << list << endl;
        }
        if (list.size() > 0) { // there are clauses that have to be matched
            matchArray.nextStep();
            int clauseSize = 0;
    ...
    if (!useCounting || hashClause != CRef_Undef) { // for the safety of the proof do not remove clauses that are not present!
        ...
        if (useCounting) {
            ...
            if (opt_verboseProof) {
                cerr << "c decrease counter of clause " << ca[hashClause] << " to " << ca[hashClause].lbd()
                     << " (hash: " << getHash(clause, extraClauseLit, startIndex, endIndex) << " )" << endl;
            }
        }
        if (!useCounting || ca[hashClause].lbd() == 0) {
            ...
            // write to file
            if (opt_verboseProof) {
                fprintf(drupProofFile, "c remove clause by %i\n", ownerID);
            }
            ...
            if (useCounting) {
                ...
                if (opt_verboseProof) {
                    cerr << "c remove clause " << ca[list[hashListEntry]] << " from hash table "
                         << " (hash: " << getHash(clause, extraClauseLit, startIndex, endIndex) << " )" << endl;
                }
...
inline void ProofMaster::addToProof(const T& clause, const Lit& extraClauseLit, int ownerID, bool local) {
    ...
    if (local) { // simply extend the local pool
        if (extraClauseLit != lit_Undef) {
            localClauses[ownerID].push_back(extraClauseLit);
        } // have the extra literal first!
        for (int i = 0; i < clause.size(); ++i)
            if (clause[i] != extraClauseLit) {
                localClauses[ownerID].push_back(clause[i]);
            }                                       // do not add the extra lit twice!
        localClauses[ownerID].push_back(lit_Undef); // mark the end of the clause with "lit_Undef"
...
inline void ProofMaster::addInputToProof(const T& clause, int ownerID, int numberOfOccurrence, bool isNewClause) {
    ...
    if (useCounting) {
        thisHash = getHash(clause, lit_Undef, 0, clause.size());
    } // get hash for the current clause
    ...
    if (hashClause == CRef_Undef) {
        ...
        for (int nrCopy = 0; nrCopy < numberOfOccurrence; ++nrCopy) {
            ...
            if (useCounting) {
                break;
            } // if use counting, then stop after the first copy!
        }
        ...
        if (useCounting) {
            ...
            // add to hash table
            if (opt_verboseProof) {
                cerr << "c add clause " << ca[ref] << " to hash table" << endl;
            }
            assert(thisHash != HASHMAX && "the hash cannot be HASHMAX if counting is used");
            hashTable[thisHash].push_back(ref);
            if (opt_verboseProof) {
                cerr << "c set counter of clause " << ca[ref] << " to " << ca[ref].lbd() << " (hash: " << getHash(clause, lit_Undef, 0, clause.size())
                     << " ) list:" << hashTable[thisHash] << endl;
            }
        } else {
            if (opt_verboseProof) {
        ...
        if (useCounting) {
            ...
            if (opt_verboseProof) {
                cerr << "c set counter of clause " << ca[hashClause] << " to " << ca[hashClause].lbd() << " (hash(" << thisHash
                     << "): " << getHash(clause, lit_Undef, 0, clause.size()) << " )" << endl;
            }
        } else {
            if (opt_verboseProof) {
                cerr << "c do not add to hash table" << endl;
...
inline void ProofMaster::delFromProof(const Lit& unit, int ownerID, bool local) {
    ...
    if (local) { // simply extend the local pool
        ...
        if (useCounting) {
            ...
            if (list.size() > 0) { // there are clauses that have to be matched
                for (int i = 0; i < list.size(); ++i) {
                    ...
                    hashListEntry = i;
                    hashClause = list[i]; // store current reference
                    break;                // stop loop
                }
            }
        ...
        if (hashClause != CRef_Undef) { // for the safety of the proof do not remove clauses that are not present!
            ...
            ca[hashClause].setLBD(ca[hashClause].lbd() - 1); // re-use LBD, decrease presence of clause
            if (opt_verboseProof) {
                cerr << "c decrease the counter of clause " << ca[hashClause] << " to " << ca[hashClause].lbd() << endl;
            }
            if (ca[hashClause].lbd() == 0) {
                ...
                // write to file
                if (opt_verboseProof) {
                    fprintf(drupProofFile, "c delete clause by %i\n", ownerID);
                }
                ...
                vector<CRef>& list = hashTable[thisHash];
                if (opt_verboseProof) {
                    cerr << "c remove clause from hash table: " << ca[list[hashListEntry]] << " (hash (" << thisHash << "): " << getHash(unit) << " )"
                         << endl;
                }
                list[hashListEntry] = list[list.size() - 1];
                list.pop_back();
...
inline void ProofMaster::delFromProof(const T& clause, const Lit& extraClauseLit, int ownerID, bool local) {
    ...
    if (local) { // simply extend the local pool
        ...
        if (clause.size() == 0) {
            assert(false && "empty clauses should not be removed");
        }
