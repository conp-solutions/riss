...
static IntOption opt_printEvery("PROOC-CHECK", "pc-print-every", "number of clauses until next output is printed during input (0=off)", 0,
...
ProofChecker::~ProofChecker() {
    if (forwardChecker != 0) {
        delete forwardChecker;
    }
    if (backwardChecker != 0) {
        delete backwardChecker;
    }
}
...
void ProofChecker::setDRUPproof() {
    checkDrat = false;
    if (checkBackwards) {
        backwardChecker->setDRUPproof();
    } else {
    } // nothing to be done for the forward checker after parsing the full proof
}
...
bool ProofChecker::addClause_(vec<Lit>& ps, bool isDelete) {
    if (ps.size() == 0) {
        parsedEmptyClause = true;
    } // memorize that the empty clause has been added
    ...
    if (opt_printEvery != 0 && addedClauses % opt_printEvery == 0) {
        ...
        cerr << "c [PC] " << checkClock.getRunningCpuTime() << " , " << checkClock.getRunningWallTime() << ": clauses: " << addedClauses << endl
             << " (per sec: global:" << addedClauses / checkClock.getRunningWallTime() << " relative: " << (addedClauses - lastAddedClauses) / relTime
             << " memory: " << memUsed() << endl;
    ...
    if (!checkBackwards) {
        if (receiveFormula) {
            ...
            if (isDelete) {
                ok = ok && forwardChecker->removeClause(ps);
            } else {
                ok = ok && forwardChecker->addClause(ps);
            }
        }
    } else {
        if (receiveFormula) {
            ok = ok && backwardChecker->addProofClause(ps, false);
        } // might fail if DRAT is enabled, because than verifying RAT might become unsound (if clauses have been added)
        else {
            ok = ok && backwardChecker->addProofClause(ps, true, isDelete);
        }
    }
    return ok;
}
...
bool ProofChecker::checkClauseDRUP(vec<Lit>& clause, bool add) {
    ...
    if (receiveFormula) {
        return true;
    } // clause could obviously be added
    if (!checkBackwards) {
        return forwardChecker->addClause(clause, true);
    } else {
        // work on the formula, but do not touch the clauses
        bool ret =
            backwardChecker->checkClause(clause, true, true); // only use DRUP, do not build too expensive data structures undo marks afterwards!
...
bool ProofChecker::verifyProof() {
    ...
    cerr << "c [PC] verify proof with " << addedClauses << " elements, after " << checkClock.getRunningCpuTime() << " , "
         << checkClock.getRunningWallTime() << " seconds, memory: " << memUsed() << endl;
    ...
    cerr << "c [PC] proof verification took " << checkClock.getRunningCpuTime() << " , " << checkClock.getRunningWallTime()
         << " seconds, memory: " << memUsed() << endl;
    return ret;
}
...
void ProofChecker::setVerbosity(int verbosity) {
    if (forwardChecker != 0) {
        forwardChecker->setVerbosity(verbosity);
    }
}
