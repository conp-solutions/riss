...
namespace Riss {
    ...
    protected:
        ...
        ClauseAllocator ca;
        vec<CRef> clauses;
        vec<Lit> unitClauses; // store how many unit clauses have been seen already for a given literal (simply count per literal, use for propagation
                              // initialization!)
    ...
    inline void OnlineProofChecker::setVerbosity(int newVerbosity) {
    ...
    inline int OnlineProofChecker::nVars() const {
        return assigns.size();
    }
    ...
    inline lbool OnlineProofChecker::value(Var x) const {
        return assigns[x];
    }
    inline lbool OnlineProofChecker::value(Lit p) const {
        ...
        while (qhead < trail.size()) {
            ...
            for (i = j = (Watcher*)ws, end = i + ws.size(); i != end;) {
                ...
                if (value(blocker) == l_True) { // keep binary clauses, and clauses where the blocking literal is satisfied
                    DOUT(if (verbose > 8) std::cerr << "c [DRAT-OTFC] clause is true by blocker " << blocker << " in " << ca[i->cref()]
                                                    << std::endl;);
                    *j++ = *i++;
                    continue;
                }
                ...
                *j++ = w;
                // if( config.opt_printLhbr )  std::cerr << "c keep clause (" << cr << ")" << c << " in watch list while propagating " << p <<
                // std::endl;
    ...
    template <class T>
    inline bool OnlineProofChecker::removeClause(const T& cls, const Lit& remLit) {
        tmpLits.clear();
        if (remLit != lit_Undef) {
            tmpLits.push(remLit);
        }
    ...
    template <class T>
    inline bool OnlineProofChecker::hasClause(const T& cls) {
        if (verbose > 3) {
        ...
        if (!conflict) {
            ...
            if (propagate()) {
                ...
                if (proof == dratProof) { // are we checking DRAT?
                    ...
                    assert(
                        cls.size() > 0 &&
                        "checking the empty clause cannot reach here (it can only be a RUP clause, not a RAT clause -- empty clause is not entailed");
                        ...
                        if (!conflict) {
                            ...
                            for (int i = 0; i < list.size(); ++i) {
                                ...
                                for (; j < d.size(); ++j) {
                                    ...
                                    if (!ma.isCurrentStep(toInt(d[j]))) {
                                        // ma.setCurrentStep( toInt(d[j]) );  // do not do this, because there are multiple resolvents! // this step
                                        // might be redundant -- however, here we can ensure that also clauses with duplicate literals can be handled,
                                        // as well as tautologic clauses!
                                        lits.push(d[j]);
                                    }
                                }
                                if (j != d.size()) {
                                ...
                                if (!resovleConflict && !propagate()) {
                                    ...
                                    if (verbose > 1) {
                                        std::cerr << "c [DRAT-OTFC] the clause " << cls << " is not a DRAT clause -- resolution on " << resolveLit
                                                  << " with " << d << " failed! (does not result in a conflict with UP)" << std::endl;
                                    }
                                    printState();
                                    assert(false && "added clause has to be a DRAT clause");
                                    return false;
                                } else {
                                    DOUT(if (verbose > 6) std::cerr << "c [DRAT-OTFC] the clause " << cls << " conflicts during enqueing/propagating"
                                                                    << std::endl;);
                                }
                            }
                    ...
                    if (verbose > 1) {
                        std::cerr << "c [DRAT-OTFC] the clause " << cls << " is not a DRUP clause, and we do not check DRAT" << std::endl;
                    }
                    printState();
    ...
    inline void OnlineProofChecker::printState() {
        ...
        for (int i = 0; i < clauses.size(); ++i) {
            ...
            if (clause.mark() != 0 || clause.can_be_deleted()) {
                continue;
        ...
        for (Var v = 0; v < nVars(); ++v) {
            for (int p = 0; p < 2; ++p) {
                ...
                for (int j = 0; j < ws.size(); ++j) {
                    ...
                    if (c[0] != ~l && c[1] != ~l) {
                        std::cerr << "wrong literals for clause [" << wcr << "] " << c << " are watched. Found in list for " << l << std::endl;
                    }
                }
            }
        }
