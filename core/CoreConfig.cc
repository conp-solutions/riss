...
// extra 
 ...
 opt_ecl_as_learned("EXTENDED RESOLUTION ECL", "ecl-l", "add ecl clauses as learned clauses", false),
 opt_ecl_as_replaceAll("EXTENDED RESOLUTION ECL", "ecl-r", "run through formula and replace all disjunctions in the ECL (only if not added as learned)", false),
 ...
 opt_rer_as_replaceAll("EXTENDED RESOLUTION RER", "rer-r", "replace all disjunctions of the RER extension (only, if not added as learned, and if full - RER adds a conjunction)", false), 
 ...
 opt_rer_maxSize("EXTENDED RESOLUTION RER", "rer-min-size", "minimum size of learned clause to perform rer", INT32_MAX, IntRange(2, INT32_MAX) ),
