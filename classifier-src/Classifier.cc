...
vector<int>& Classifier::classifyJava(const char* wekaFile) {
	...
	scmd << "java -Xmx4G -cp \""<< predictorLocation.c_str() << "\":\"" << wekaLocation.c_str() << "\" predictor.Predictor ";
	scmd << "\"" << wekaFile << "\" ";
	scmd << "\"" << prefix << "\" ";
...
vector<int>& Classifier::classify(const char* wekaFile) {
	...
	for  (int config=0; config<configurations.getSize(); config++ ){
		...
		scmd << "java -Xmx4G -cp \"" << wekaLocation << "\" weka.classifiers.meta.FilteredClassifier -l " << classifiersNames[config];
		scmd << " -T " << wekaFile;
		scmd << " -c " << index;
		stringstream name;
		if (useTempFiles) {
