...
namespace Riss {
    ...
    class PSolver {
        ...
        public:
            ...
            OriginalFormula(const vec<Lit>& originaltrail, const vec<CRef>& originalclauses, const ClauseAllocator& originalca, const int vars,
                            const vec<double>& originalactivity, const Heap<Solver::VarOrderLt>& originalorder_heap,
                            const vec<Solver::VarFlags>& originalvarFlags, const vec<Solver::VarData>& originalvardata)
    ...
    public:
        ...
        Riss::vec<Riss::Lit> conflict; // If problem is unsatisfiable (possibly under assumptions),  this std::vector represent the final conflict
        ...
        void setGlobalSimplifierConfig(const std::string& _config) {
            defaultSimplifierConfig = _config;
        }
