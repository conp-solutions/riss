...
namespace Riss {
    ...
    PSolver::PSolver(Riss::PfolioConfig* externalConfig, const char* configName, int externalThreads)
        ...
        if (externalThreads != -1) {
            threads = externalThreads > 0 ? externalThreads : 1;
        }
        ...
        for (int i = 0; i < sizeof(cpu_set_t) << 3; ++i) // add all available cores to the system
            if (CPU_ISSET(i, &mask)) {
                hardwareCores.push_back(i);
            }
        if (hardwareCores.size() > threads) {
            hardwareCores.resize(threads);
        } // use only the first required available cores!
    ...
    PSolver::~PSolver() {
        ...
        if (globalSimplifier != 0) {
            delete globalSimplifier;
        }
        if (globalSimplifierConfig != 0) {
            delete globalSimplifierConfig;
        }

        for (int i = 0; i < solvers.size(); ++i) {
            delete solvers[i]; // free all solvers
            if (communicators != 0 && communicators[i] != 0) {
        ...
        if (data != 0 && data != externalData) {
            delete data;
        } // only delete, if we created this object
        data = 0;
    ...
    bool PSolver::addClause_(vec<Lit>& ps) {
        ...
        for (int i = 0; i < solvers.size(); ++i) {
            ...
            //         }
            bool ret2 = solvers[i]->addClause_(ps, i != 0); // if a solver failed adding the clause, then the state for all solvers is bad as well,
                                                            // avoid redundancy check for all but the first solver
            if (pfolioConfig.opt_verbosePfolio)
                if (i == 0) {
                    cerr << "c parsed clause " << ps << endl;
    ...
    lbool PSolver::simplifyFormula() {
        DOUT(cerr << "c call of PSolver object " << std::hex << this << std::dec << " simplifyFormula with default config: "
                  << defaultSimplifierConfig << " and winning solver: " << winningSolver << " and simplified: " << simplified << endl;);
        if (winningSolver != -1) {
            return l_Undef;
        } // simplify only, if there is no winning solver already
        lbool ret = l_Undef;
        /*
        ...
        if (!simplified && defaultSimplifierConfig.size() != 0) {
            ...
            globalSimplifier =
                new Coprocessor::Preprocessor(solvers[0], *globalSimplifierConfig, threads); // use simplifier with the given number of threads
            ...
            if (verbosity > 0) {
                cerr << "c solver0 with " << solvers[0]->nVars() << " variables, and " << solvers[0]->clauses.size() << " clauses" << endl;
            }
            ...
            if (ret == l_False) {
                ...
                extendModel(model);
                if (ret == l_True && verbosity > 0) {
                    cerr << "c solved formula with simplification" << endl;
                }
                return ret;
            }
        }
    ...
    void PSolver::extendModel(Riss::vec<Riss::lbool>& externalModel) {
        if (globalSimplifier != 0) {
            globalSimplifier->extendModel(externalModel);
        }
    }
    ...
    lbool PSolver::solveLimited(const vec<Lit>& assumps) {
        ...
        if (!initialized) { // check whether some solver wants to work on the original formula
            ...
            if (keepOriginal) {
                ...
                originalFormula = new OriginalFormula(solvers[0]->trail, solvers[0]->clauses, solvers[0]->ca, solvers[0]->nVars(),
                                                      solvers[0]->activity, solvers[0]->order_heap, solvers[0]->varFlags,
                                                      solvers[0]->vardata); // memorize current state for initialization
            }
        }
        ...
        if (!initialized) { // distribute the formula, if this is the first call to this method!
            ...
            if (modelMaster != nullptr) {
                if (globalSimplifier != nullptr) {
                    modelMaster->setPreprocessor(globalSimplifier);
                }
            ...
            if (proofMaster != 0 && pfolioConfig.opt_verboseProof > 0) {
                proofMaster->addCommentToProof("c initialized parallel solvers", -1);
            }
            ...
            if (!simplified) {
                solvers[0]->solve_(Solver::SolveCallType::initializeOnly);
            }
            ...
            for (int i = 1; i < solvers.size(); ++i) {
                ...
                if (!configs[i].opt_useOriginal) {
                    ...
                    while (solvers[i]->nVars() < solvers[0]->nVars()) {
                        solvers[i]->newVar();
                    }
                    ...
                    solvers[0]->order_heap.copyOrderTo(
                        solvers[i]->order_heap); // rebuild order heap (use configuration of other heap, but use own acticities)
                    ...
                    solvers[i]->solve_(Solver::SolveCallType::initializeOnly); // let solve initialize itself, if it does not want to perform the full
                                                                               // solving process on its own
                } else {                                                       // initialize based on original formula

                    communicators[i]->setDoSend(
                        false); // disable sending hard   // TODO might be disabled once sharing and simplification works nicely together
                    communicators[i]->setDoReceive(
                        false); // disable receiving hard // TODO might be disabled once sharing and simplification works nicely together

                    assert(originalFormula != nullptr && "had to be collected before");
                    solvers[i]->reserveVars(originalFormula->nVars);
                    while (solvers[i]->nVars() < originalFormula->nVars) {
                        solvers[i]->newVar();
                    }
                    ...
                    originalFormula->order_heap.copyOrderTo(
                        solvers[i]->order_heap); // rebuild order heap (use configuration of other heap, but use own acticities)
                ...
                if (verbosity > 1) {
                    cerr << "c Solver[" << i << "] has " << solvers[i]->nVars() << " vars, " << solvers[i]->clauses.size() << " cls, "
                         << solvers[i]->learnts.size() << " learnts" << endl;
                }
                solvers[i]->setPreprocessor(&ppconfigs[i]); // tell solver incarnation about preprocessor

                if (modelMaster != nullptr) {
                    solvers[i]->setEnumnerationMaster(modelMaster);
                }
            }
            ...
            if (proofMaster != 0) { // if a proof is generated, add all clauses that are currently present in solvers[0]
                if (pfolioConfig.opt_verboseProof > 0) {
        ...
        for (int i = 0; i < threads; ++i) {
            for (int j = 0; j < assumps.size(); ++j) { // make sure, everybody knows all the variables
                while (solvers[i]->nVars() <= var(assumps[j])) {
                    solvers[i]->newVar();
                }
            }
            // assumps.copyTo(communicators[i]->assumptions);
            communicators[i]->setFormulaVariables(solvers[i]->nVars()); // for incremental calls, no ER is supported, so that everything should be
                                                                        // fine until here! Note: be careful with this!
        ...
        if (proofMaster != 0 && pfolioConfig.opt_verboseProof > 0) {
            proofMaster->addCommentToProof("c start all solvers", -1);
        }
        ...
        winningSolver = 0;
        for (int i = 0; i < threads; ++i) {
            if (communicators[i]->isWinner() && communicators[i]->getReturnValue() != l_Undef) {
                winningSolver = i;
                break;
            }
        }
        ...
        if (verbosity > 0) {
            if (verbosity > 0) {
                cerr << "c MASTER found winning thread (" << communicators[winningSolver]->isWinner() << ") as " << winningSolver << " / " << threads
                     << " model= " << solvers[winningSolver]->model.size() << endl;
            }
        }
        ...
        if (winningSolver < threads) {
            ...
            if (ret == l_True) {
                ...
                if (false && verbosity > 2) {
                    cerr << "c units: " << endl;
                    for (int i = 0; i < solvers[winningSolver]->trail.size(); ++i) {
                        cerr << " " << solvers[winningSolver]->trail[i] << " 0" << endl;
                    }
                    cerr << endl;
                    cerr << "c clauses: " << endl;
                    for (int i = 0; i < solvers[winningSolver]->clauses.size(); ++i) {
                        cerr << "c " << solvers[winningSolver]->ca[solvers[winningSolver]->clauses[i]] << endl;
                    }
                    cerr << "c assumptions: " << endl;
                ...
                for (int i = 0; i < solvers[winningSolver]->conflict.size(); ++i) {
                    conflict.push(solvers[winningSolver]->conflict[i]);
                }
            } else {
                if (verbosity > 0) {
        ...
        if (verbosity > 0) {
            if (verbosity > 0) {
                cerr << "c thread  S  \t|\t R   \t|\t sRej \t|\t lRej \t|\t vivLits \t|" << endl;
            }
            for (int i = 0; i < threads; ++i) {
                if (verbosity > 0)
                    cerr << "c " << i << " : " << communicators[i]->nrSendCls << "  \t|\t" << communicators[i]->nrReceivedCls << "  \t|\t"
                         << communicators[i]->nrRejectSendSizeCls << "  \t|\t" << communicators[i]->nrRejectSendLbdCls << "  \t|\t"
                         << communicators[i]->vivifiedLiterals << "\t|" << endl;
            }
            if (verbosity > 0) {
            ...
            for (int i = 0; i < threads; ++i) {
                if (verbosity > 0)
                    cerr << "c " << i << " : " << communicators[i]->nrSendEEs << "  \t|\t" << communicators[i]->nrReceivedEEs << "  \t|\t"
                         << communicators[i]->nrSendMultiUnits << "  \t|\t" << communicators[i]->nrReceivedMultiUnits << endl;
            }
            if (verbosity > 0) {
                cerr << "c attempts:" << endl;
            }
            if (verbosity > 0) {
                cerr << "c thread  SCls\t|\tS-EE\t|\tS-U\t|\tRec\t" << endl;
            }
            for (int i = 0; i < threads; ++i) {
                if (verbosity > 0)
                    cerr << "c " << i << " : " << communicators[i]->nrSendCattempt << "  \t|\t" << communicators[i]->nrSendMattempt << "  \t|\t"
                         << communicators[i]->nrSendEattempt << "  \t|\t" << communicators[i]->nrReceiveAttempts << endl;
            }
            if (verbosity > 0) {
                cerr << "c search data:" << endl;
            }
            for (int i = 0; i < threads; ++i) {
                if (verbosity > 0)
                    cerr << "c " << i << " : cons: " << communicators[i]->getSolver()->conflicts
                         << "  dec: " << communicators[i]->getSolver()->decisions << "  units: "
                         << (communicators[i]->getSolver()->trail_lim.size() == 0 ? communicators[i]->getSolver()->trail.size()
                                                                                  : communicators[i]->getSolver()->trail_lim[0])
                         << "  models: " << communicators[i]->getSolver()->enumerationClient.getModels()
                         << "  dup-models: " << communicators[i]->getSolver()->enumerationClient.getDupModels() << endl;
            }
        }
    ...
    void PSolver::createThreadConfigs() {
        const char* Configs[] = {
            "STRONGUNSAT:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 -cp3_uhdPrSize=5 -rlevel=2 -bve_early -revMin "
            "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uepgxv -no-dense -up -refRec -shareTime=1 -sendAll",
            "-no-usePP -revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec",
            "-no-usePP -revMin -init-act=3 -actStart=2048 -keepWorst=0.01 -refRec "
            "-revMin -init-act=4 -actStart=2048 -refRec",
            ...
            "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 "
            "-inprocess -cp3_inp_cons=1000000 -cp3_itechs=uegsv -no-dense -up -refRec ",
            "",                 // 0
            "PLAINBIASSERTING", // 1
            "LLA",              // 2
            "AUIP",             // 3
        ...
        if (defaultConfig.size() == 0) {
            for (int t = 0; t < threads; ++t) {
                if (pfolioConfig.opt_verbosePfolio) {
                    cerr << "c PFOLIO set incarnation presets or default presets" << endl;
                }
            ...
            if (pfolioConfig.opt_verbosePfolio) {
                cerr << "c PFOLIO set BMC presets" << endl;
            }
            // thread 1 runs with empty (default) configurations
            if (threads > 1) {
                configs[1].setPreset("DECLEARN");
            }
            if (threads > 2) {
                configs[2].setPreset("FASTRESTART");
            }
            if (threads > 3) {
                configs[3].setPreset("SUHLE");
            }
            ...
            if (pfolioConfig.opt_verbosePfolio) {
                cerr << "c PFOLIO set PLAIN (no) presets" << endl;
            }
            for (int t = 0; t < threads; ++t) {
            ...
            // configs[1].setPreset("");
            if (threads > 1) {
                configs[1].setPreset("-K=0.7 -R=1.5");
            }
            if (threads > 2) {
                configs[2].setPreset("-var-decay-b=0.85 var-decay-e=0.85");
            }
            if (threads > 3) {
                configs[3].setPreset("-K=0.7 -R=1.5 -var-decay-b=0.85 var-decay-e=0.85");
            }
            ...
            if (threads > 3) {
                ppconfigs[3].setPreset("-enabled_cp3 -shareTime=2 -unhide -cp3_uhdIters=5 -cp3_uhdEE -cp3_uhdTrans -cp3_uhdProbe=4 -cp3_uhdPrSize=3 "
                                       "-inprocess -cp3_inp_cons=10000");
            }
            ...
            if (threads > 1) {
                ppconfigs[1].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec");
                configs[1].setPreset("-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec "
                                     "-resRefRec -shareTime=1");
            }
            if (threads > 2) {
                ppconfigs[2].setPreset("Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin "
                                       "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uev -no-dense -up -refRec ");
                configs[2].setPreset("Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin "
                                     "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uev -no-dense -up -refRec -shareTime=1");
            }
            if (threads > 3) {
            ...
            if (threads > 5) {
                ...
                configs[5].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec  -shareTime=2");
            }
            if (threads > 6) {
            ...
            if (threads > 7) {
                ppconfigs[7].setPreset("Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin "
                                       "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uev -no-dense -up -refRec ");
                configs[7].setPreset(
                    "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 "
                    "-actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uev -no-dense -up -refRec  -shareTime=2");
            }
            for (int t = 8; t < threads; ++t) { // set configurations for remaining (beyond 8)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 8]);
                } // assign preset, if no cmdline was specified
                else {
                    configs[t].setPreset(incarnationConfigs[t]);
                } // otherwise, use commandline configuration
            }
            ...
            if (threads > 2) {
                ...
                configs[2].setPreset(
                    "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 "
                    "-actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uev -no-dense -up -refRec -shareTime=1 -sendAll");
            }
            if (threads > 3) {
                ppconfigs[3].setPreset("-revMin -init-act=3 -actStart=2048 -keepWorst=0.01 -refRec ");
            ...
            if (threads > 5) {
                ppconfigs[5].setPreset("-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec ");
                configs[5].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec  -shareTime=0 -dynLimits");
            }
            if (threads > 6) {
                ppconfigs[6].setPreset("-revMin -init-act=3 -actStart=2048 -longConflict -refRec ");
            ...
            if (threads > 7) {
                ...
                configs[7].setPreset(
                    "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 "
                    "-actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uev -no-dense -up -refRec  -shareTime=2 -sendAll");
            }
            for (int t = 8; t < threads; ++t) { // set configurations for remaining (beyond 8)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 8]);
            ...
            if (threads > 1) {
                ppconfigs[1].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec");
                configs[1].setPreset("-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec "
                                     "-resRefRec -shareTime=1 -sendAll -rnd-freq=0.01");
            }
            if (threads > 2) {
                ppconfigs[2].setPreset("Riss427:plain_XOR:plain_PRB:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 "
                                       "-cp3_uhdPrSize=5 -rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 "
                                       "-cp3_itechs=uepgxv -no-dense -up -refRec -shareTime=1 -sendAll");
                configs[2].setPreset("Riss427:plain_XOR:plain_PRB:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 "
                                     "-cp3_uhdPrSize=5 -rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 "
                                     "-cp3_itechs=uepgxv -no-dense -up -refRec -shareTime=1 -sendAll");
            }
            if (threads > 3) {
            ...
            if (threads > 4) {
                ppconfigs[4].setPreset("-revMin -init-act=4 -actStart=2048 -refRec ");
                configs[4].setPreset("-revMin -init-act=4 -actStart=2048 -refRec -shareTime=2 -dynLimits -lbd-core-th=5 -var-decay-b=0.8 "
                                     "-var-decay-e=0.99  -rnd-freq=0.01");
            }
            if (threads > 5) {
                ppconfigs[5].setPreset("-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec ");
                configs[5].setPreset("AUIP:LLA:SUHD:-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec "
                                     "-shareTime=0 -dynLimits");
            }
            if (threads > 6) {
                ppconfigs[6].setPreset("-revMin -init-act=3 -actStart=2048 -longConflict -refRec ");
                configs[6].setPreset("-revMin -init-act=3 -actStart=2048 -longConflict -refRec  -shareTime=2 -sendAll");
            ...
            if (threads > 7) {
                ppconfigs[7].setPreset("Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin "
                                       "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uegv -no-dense -up -refRec ");
                configs[7].setPreset(
                    "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 "
                    "-actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uegv -no-dense -up -refRec  -shareTime=2 -sendAll");
            }
            for (int t = 8; t < threads; ++t) { // set configurations for remaining (beyond 8)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 8]);
            ...
            if (threads > 1) {
                ppconfigs[1].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec");
                configs[1].setPreset(
                    "-revMin -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec -shareTime=1 "
                    "-sendAll -rnd-freq=0.01 -init-pol=6 -init-act=4 -biAsserting -minSizeMinimizingClause=0 -no-revMin -rmf");
            }
            if (threads > 2) {
                ppconfigs[2].setPreset("Riss427:plain_XOR:plain_PRB:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 "
                                       "-cp3_uhdPrSize=5 -rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 "
                                       "-cp3_itechs=uepgxv -no-dense -up -refRec -shareTime=1 -sendAll");
                configs[2].setPreset(
                    "Riss427:plain_XOR:plain_PRB:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 -cp3_uhdPrSize=5 "
                    "-rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uepgxv -no-dense -up -refRec "
                    "-shareTime=1 -sendAll -init-pol=3 -init-act=6 -lbdIgnL0 -otfss -otfssL");
            }
            if (threads > 3) {
                ppconfigs[3].setPreset("-revMin -init-act=3 -actStart=2048 -keepWorst=0.01 -refRec ");
                configs[3].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -keepWorst=0.01 -refRec -shareTime=1 -sendAll -rnd-freq=0.05 -init-pol=4 -init-act=5");
            }
            if (threads > 4) {
                ppconfigs[4].setPreset("-revMin -init-act=4 -actStart=2048 -refRec ");
                configs[4].setPreset("-revMin -init-act=4 -actStart=2048 -refRec -shareTime=2 -dynLimits -lbd-core-th=5 -var-decay-b=0.8 "
                                     "-var-decay-e=0.99  -rnd-freq=0.01 -init-pol=5");
            }
            if (threads > 5) {
                ppconfigs[5].setPreset("-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec ");
                configs[5].setPreset("AUIP:LLA:SUHD:-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -refRec "
                                     "-shareTime=0 -dynLimits -biAsserting");
            }
            if (threads > 6) {
                ppconfigs[6].setPreset("-revMin -init-act=3 -actStart=2048 -longConflict -refRec ");
                configs[6].setPreset("-revMin -init-act=3 -actStart=2048 -longConflict -refRec  -shareTime=2 -sendAll");
            ...
            if (threads > 1) {
                ppconfigs[1].setPreset("STRONGUNSAT:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 -cp3_uhdPrSize=5 "
                                       "-rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uepgxv "
                                       "-no-dense -up -refRec -shareTime=1 -sendAll");
                configs[1].setPreset("STRONGUNSAT:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -cp3_uhdProbe=4 -cp3_uhdPrSize=5 "
                                     "-rlevel=2 -bve_early -revMin -init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=30000 -cp3_itechs=uespgxv "
                                     "-no-dense -up -refRec -shareTime=1 -sendAll -init-pol=3 -init-act=6 -lbdIgnL0 -otfss -otfssL");
            }
            if (threads > 2) {
                ppconfigs[2].setPreset(
                    "-revMin -init-act=3 -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec");
                configs[2].setPreset(
                    "-revMin -actStart=2048 -firstReduceDB=200000 -rtype=1 -rfirst=1000 -rinc=1.5 -act-based -refRec -resRefRec -shareTime=1 "
                    "-sendAll -rnd-freq=0.01 -init-pol=6 -init-act=4 -biAsserting -minSizeMinimizingClause=0 -no-revMin -rmf");
            ...
            if (threads > 7) {
                ppconfigs[7].setPreset("Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin "
                                       "-init-act=3 -actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uegsv -no-dense -up -refRec ");
                configs[7].setPreset(
                    "Riss427:plain_XOR:-no-usePP -cp3_iters=2 -up -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early -revMin -init-act=3 "
                    "-actStart=2048 -inprocess -cp3_inp_cons=1000000 -cp3_itechs=uegv -no-dense -up -refRec  -shareTime=2 -sendAll");
            }
            ...
            for (int t = 3; t < threads; ++t) { // set configurations for remaining (beyond 3)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 3]);
                } // assign preset, if no cmdline was specified
                else {
                    configs[t].setPreset(incarnationConfigs[t]);
                } // otherwise, use commandline configuration
            ...
            for (int t = 4; t < threads; ++t) { // set configurations for remaining (beyond 4)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 4]);
                } // assign preset, if no cmdline was specified
                else {
                    configs[t].setPreset(incarnationConfigs[t]);
                } // otherwise, use commandline configuration
            ...
            if (threads > 4) {
                ...
                configs[4].setPreset("RealTime.data2:-no-receive -shareTime=0 -dynLimits -no-usePP -no-useIP"); // same pp as above, hence share
                                                                                                                // clauses, but do not receive!
            }
            if (threads > 5) {
            ...
            for (int t = 6; t < threads; ++t) { // set configurations for remaining (beyond 6)
                if (incarnationConfigs[t].size() == 0) {
                    configs[t].setPreset(Configs[t - 6]);
                } // assign preset, if no cmdline was specified
                else {
                    configs[t].setPreset(incarnationConfigs[t]);
                } // otherwise, use commandline configuration
        ...
        if (pfolioConfig.addExtraSetup) {
            if (pfolioConfig.opt_verbosePfolio) {
                cerr << "c PFOLIO add default presets" << endl;
            }
        ...
        if ((const char*)pfolioConfig.opt_allIncPresets != 0) {
            if (pfolioConfig.opt_verbosePfolio) {
                cerr << "c PFOLIO add all inc presets" << endl;
            }
            for (int t = 0; t < threads; ++t) {
    ...
    bool PSolver::initializeThreads() {
        ...
        if (externalData != nullptr) {
            ...
            data = new CommunicationData(
                privateConfig->opt_storageSize == 0 ? 4000 * threads : privateConfig->opt_storageSize); // space for clauses, dynamic or static
        }
        ...
        if (drupProofFile != 0) {
            proofMaster = new ProofMaster(drupProofFile, threads, nVars(), pfolioConfig.opt_proofCounting,
                                          pfolioConfig.opt_verboseProof > 1); // use a counting proof master
        ...
        for (unsigned i = 0; i < threads; ++i) {
            ...
            if (!pfolioConfig.opt_share) {
                communicators[i]->setDoSend(false);
            } // no sending
            if (!pfolioConfig.opt_receive) {
                communicators[i]->setDoReceive(false);
            } // no sending
            ...
            if (verbosity > 0) {
                cerr << "c [MASTER] setup thread " << i << " with data at " << std::hex << communicators[i] << std::dec << endl;
            }
    ...
    void PSolver::start() {
        ...
        for (unsigned i = 0; i < threads; ++i) {
            ...
            if (verbosity > 1) {
                cerr << "c [MASTER] set thread " << i << " state to working" << endl;
            }
            communicators[i]->setState(Communicator::working);
            communicators[i]->ownLock->unlock();
            if (verbosity > 1) {
                cerr << "c [MASTER] awake thread " << i << endl;
            }
    ...
    void PSolver::waitFor(const WaitState waitState) {
        ...
        while (true) {
            ...
            for (unsigned i = 0; i < threads; ++i) {
                if (verbosity > 2) {
                    cerr << "c [WFI" << waitForIterations << "] MASTER checks thread " << i << " / " << threads
                         << " finished: " << communicators[i]->isFinished() << endl;
                }
                if (waitState == oneIdle) {
                    if (communicators[i]->isIdle()) {
                        threadOfInterest = i;
                        break;
                    }
    ...
    void PSolver::kill() {
        if (!initialized) {
            return;
        } // child threads have never been created - no need to kill them
        if (killed) {
            return;
        } // killed already
        killed = true;
        ...
        for (unsigned i = 0; i < threads; ++i) {
            ...
            if (err != 0) {
                cerr << "c joining a thread resulted in a failure with status " << *status << endl;
            }
        }
        if (verbosity > 1) {
    ...
    void* runWorkerSolver(void* data) {
        ...
        if (verbose) {
            cerr << "c started thread with id " << info.getID() << endl;
        }
        // initially, wait until master provides work!
        info.ownLock->lock();
        if (verbose) {
            cerr << "c thread id " << info.getID() << " state= " << (info.isIdle() ? "idle" : "nonIdle") << endl;
        }
        ...
        while (!info.isAborted()) {
            if (verbose) {
                cerr << "c [THREAD] " << info.getID() << " start " << endl;
            }
            ...
            if (verbose) {
                cerr << "c [THREAD] " << info.getID() << " solve task with " << assumptions.size() << " assumed literals:" << assumptions << endl;
            }

            // do work
            lbool result l_Undef;
            if (!info.getSolver()->okay()) {
                result = l_False;
            } else {
                if (info.independent()) {
                    result = info.getSolver()->solveLimited(
                        assumptions,
                        Solver::SolveCallType::full); // as we are working on the original formula, have the change to initialize amd simplify
                } else {
                    result = info.getSolver()->solveLimited(
                        assumptions, Solver::SolveCallType::afterSimplification); // we work on a simplified formula, do not simplify/initialize again
                }
            }

            info.setReturnValue(result);
            if (verbose)
                cerr << "c [THREAD] " << info.getID() << " result " << (result == l_Undef ? "UNKNOWN" : (result == l_True ? "SAT" : "UNSAT")) << endl;
            ...
            if (verbose) {
                cerr << "c [THREAD] " << info.getID() << " change state to finished" << endl;
            }
            ...
            if (verbose) {
                cerr << "c [THREAD] " << info.getID() << " wake up master" << endl;
            }
            // wake up master so that the master can do something with the solver
            info.awakeMaster();

            if (verbose) {
                cerr << "c [THREAD] " << info.getID() << " wait for next round (sleep)" << endl;
            }
            // wait until master changes the state again to working!
            info.ownLock->lock();
            while (!info.isWorking() && !info.isAborted()) { // wait for work or being aborted
                if (verbose) {
                    cerr << "c [THREAD] " << info.getID() << " sleeps until next work ... " << endl;
                }
                info.ownLock->sleep();
                if (verbose) {
                    cerr << "c [THREAD] " << info.getID() << " awakes after work-sleep ... " << endl;
                }
            }
            info.ownLock->unlock();
        }
