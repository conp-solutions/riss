...
namespace Riss {
    ...
    inline void PfolioConfig::setPreset(const std::string& optionSet) {
    ...
    inline bool PfolioConfig::addPreset(const std::string& optionSet) {
        parsePreset = true;
        bool ret = true;

        if (optionSet == "alpha") {
            parseOptions("-ppconfig=Riss427:plain_XOR:-cp3_iters=2:-ee:-cp3_ee_level=3:-cp3_ee_it:-rlevel=2:-bve_early -psetup=alpha "
                         "-storageSize=32000 -pAllSetup=-keepLonger:-no-cp3_stats:-recLBDf=-1",
                         false);
        } else if (optionSet == "beta") {
            parseOptions("-ppconfig=Riss427:plain_XOR:-cp3_iters=2:-ee:-cp3_ee_level=3:-cp3_ee_it:-rlevel=2:-bve_early -psetup=beta "
                         "-storageSize=32000 -pAllSetup=-keepLonger:-no-cp3_stats:-recLBDf=-1",
                         false);
        } else if (optionSet == "gamma") {
            parseOptions("-ppconfig=Riss427:plain_XOR:-cp3_iters=2:-ee:-cp3_ee_level=3:-cp3_ee_it:-rlevel=2:-bve_early -psetup=gamma "
                         "-storageSize=32000 -pAllSetup=-keepLonger:-no-cp3_stats:-recLBDf=-1",
                         false);
        } else if (optionSet == "delta") {
            parseOptions("-ppconfig=Riss427:plain_XOR:-cp3_iters=2:-ee:-cp3_ee_level=3:-cp3_ee_it:-rlevel=2:-bve_early -psetup=delta "
                         "-storageSize=32000 -pAllSetup=-keepLonger:-no-cp3_stats:-recLBDf=-1",
                         false);
        } else if (optionSet == "epsilon") {
            parseOptions("-ppconfig=STRONGUNSAT:-no-cp3_stats -psetup=epsilon -storageSize=32000 -pAllSetup=-keepLonger:-no-cp3_stats:-recLBDf=-1",
                         false);
