...
namespace Riss {
    ...
    PfolioConfig::PfolioConfig(const std::string& presetOptions) // add new options here!
        ...
        , opt_verboseProof("PFOLIO - PROOF", "pv", "verbose proof (2=with comments to clause authors,1=comments by master only, 0=off)", 1,
        ...
        , opt_storageSize("PFOLIO - INIT", "storageSize", "Number of clauses in one ring buffer (0 => 4000 x threads)", 0, IntRange(0, INT32_MAX),
                          optionListPtr)
        ...
        , opt_protectAssumptions("SEND", "protectA", "should assumption variables not be considered for calculating send-limits", false,
                                 optionListPtr)
        , opt_sendSize("SEND", "sendSize", "Minimum Lbd of clauses to send  (also start value)", 10, DoubleRange(0, true, HUGE_VAL, true),
                       optionListPtr)
        , opt_sendLbd("SEND", "sendLbd", "initial value, also minimum limit (first checks size)", 5, DoubleRange(0, true, HUGE_VAL, true),
                      optionListPtr)
        , opt_sendMaxSize("SEND", "sendMaxSize", "upper bound for clause size (larger clause is never shared)", 128,
                          DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
        , opt_sendMaxLbd("SEND", "sendMaxLbd", "upper bound for clause size (larger clause is never shared)", 32,
                         DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
        , opt_sizeChange("SEND", "sizeChange", "set to value greater than 0 to see dynamic limit changes! (e.g. 0.05)", 0,
                         DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
        , opt_lbdChange("SEND", "lbdChange", "set to value greater than 0 to see dynamic limit changes! (e.g. 0.02)", 0,
                        DoubleRange(0, true, HUGE_VAL, true), optionListPtr)
        ...
        , opt_checkLiterals("SEND", "checkLits", "control allowing sending and receiving information based on literal instead of variables", false,
                            optionListPtr)
