 ...
 */
void* priss_init(int& threads, const char* presetConfig) {
    if (threads < 1) {
        threads = 1;
    } // make sure that the number of available threads makes sense ...
    else if (threads > 64) {
        threads = 64;
    }
...
void priss_assume(void* priss, const int& lit) {
    libpriss* solver = (libpriss*)priss;
 ...
 */
int priss_sat(void* priss, const int& nOfConflicts) {
    libpriss* solver = (libpriss*)priss;
    if (nOfConflicts == -1) {
...
int priss_deref(const void* priss, const int& lit) {
