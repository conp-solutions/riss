 ...
 void oddEvenMerge(std::vector<Formula>& fs, int begin, int end)
{
    ...
    if (end - begin == 2)
        ...
        riffle(tmp);
        for (int i = 1; i < ((int)tmp.size()) - 1; i += 2)
            cmp2(tmp,i);
...
{
    int orig_sz = fs.size();
    int sz; for (sz = 1; sz < (int)fs.size(); sz *= 2);
...
void buildConstraint(std::vector<Formula>& ps, std::vector<int64_t>& Cs, std::vector<Formula>& carry, std::vector<int>& base, int digit_no, std::vector<std::vector<Formula> >& out_digits)
{
    ...
    if (digit_no == (int)base.size()){
        ...
        // Get carry bits:
        carry.clear();
        for (int i = B-1; i < (int)result.size(); i += B)
            carry.push_back(result[i]);

        out_digits.push_back(std::vector<Formula>());
        for (int i = 0; i < B-1; i++){
            Formula out = _false_;
            for (int j = 0; j < (int)result.size(); j += B){
                int n = j+B-1;
                if (j + i < (int)result.size())
                    out = OR(out, AND(result[j + i] , ((n >= (int)result.size()) ? _true_ : ~result[n])));
            }
...
Formula lexComp(int sz, std::vector<int>& num, std::vector<std::vector<Formula> >& digits)
{
    if (sz == 0)
        ...
        Formula gt = ((int)digit.size() > dig) ? digit[dig] : _false_;       // This digit is greater than the "dig" of 'num'.
        Formula ge = (dig == 0) ? _true_ :
                     ((int)digit.size() > dig-1) ? digit[dig-1] : _false_;   // This digit is greater than or equal to the "dig" of 'num'.
