...
//  Instance Status ExitCode RealTime CpuTime Memory Decisions Conflicts CP3PPtime CP3IPtime CP3remcls
    ...
    if (!handled[0] || !handled[5] || (Status == "")) { Status = "fail"; }
    else {
        if (SolverCode != "10" && SolverCode != "20" && SolverCode != "30") { Status = "solverFail"; }
//     if( ToolCode != "10" && ToolCode != "20" && ToolCode != "30" ) Status = "parseToolFail";
    }
