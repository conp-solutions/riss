#!/bin/bash
#
# This script starts a smac scenario with the given parameters as a slurm job or without slurm
#
# example call if the smac_scenarios folder is placed within the smac folder:
#        with slurm:  sbatch --exclusive ./start-smac.sh .. solvers/MiniSATi scenarios/minisat-scenario.txt
#        local:       ./start-smac.sh .. solvers/MiniSATi scenarios/example-scenario.txt [--wallclock-limit 30] [--cutoff-time 10] [--executionMoade ROAR] ...
#
...
usage="
Usage as slurm-job:
    sbatch --exclusive ./start-smac.sh <smac-path> <solver-dir> <scenario name> <--additional smac-parameter>
Usage as local:
    ./start-smac.sh <smac-path> <solver-dir> <scenario name> <--additional smac-parameter>"
...
if [ -n "$SLURM_JOB_ID" ]; then
    ...
    echo "The output can be found in: smac_output/local-$(date +%s)"
