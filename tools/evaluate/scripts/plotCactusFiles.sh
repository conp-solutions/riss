...
#
# setup first part of the plot
#
echo "set terminal postscript eps color enhanced" > tmp.plt
#echo "set terminal postscript eps" > tmp.plt
#echo "set key samplen 2 spacing .5 font \",8\"" >> tmp.plt
echo "set key right bottom" >> tmp.plt
...
#	cat tmp.dat | awk "{ if( $sep$exitCol == 20 && $sep$cutcol + 0 > 0 && $sep$cutcol <= $failValue ) print $sep$cutcol }" | sort -g > tmp$col.dat
# without exit code check
	cat tmp.dat | awk "{ if( $sep$cutcol + 0 > 0 && $sep$cutcol <= $failValue ) print $sep$cutcol }" | sort -g > tmp$col.dat
# without exit code check, go for industrial / crafted
#	cat tmp.dat | grep "2013/industrial" | awk "{ if( $sep$cutcol + 0 > 0 && $sep$cutcol <= $failValue ) print $sep$cutcol }" | sort -g > tmp$col.dat
#	cat tmp.dat | grep "2013/crafted" | awk "{ if( $sep$cutcol + 0 > 0 && $sep$cutcol <= $failValue ) print $sep$cutcol }" | sort -g > tmp$col.dat
done
...
#
# define all the styles for differnt runs
#
