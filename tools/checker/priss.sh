...
#./priss $1 -threads=2 -showUnusedParam -ppconfig="Riss427:plain_XOR:-cp3_iters=2 -ee -cp3_ee_level=3 -cp3_ee_it -rlevel=2 -bve_early" -psetup=delta -mem-lim=2048 $param > /tmp/riss-out-$$  #> /dev/null 2> /dev/null
echo "c      ./priss $1 -threads=2 -showUnusedParam -ppconfig= -psetup= -mem-lim=2048 $param "
./priss $1 -threads=2 -ppconfig="" -psetup="" -mem-lim=2048 $param > /tmp/riss-out-$$  #> /dev/null 2> /dev/null
