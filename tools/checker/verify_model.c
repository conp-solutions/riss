...
static inline literal_t     variable_to_literal(variable_t var, polarity_t pol) { return (literal_t)var * ((pol == POS) ? (1) : (-1)); }
...
static inline polarity_t    literal_get_polarity(literal_t lit) { return (lit > 0) ? (POS) : ((lit < 0) ? (NEG) : (UNDEF)); }
