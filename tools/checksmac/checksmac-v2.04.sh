...
grep -e "]i" -e "\] \["  $PARAMFILE | awk '{print $1}' > $INTERVALOPTIONS
...
#		FAILTIME=`grep " Algorithm Reported: Result for ParamILS" $SMACOUTPUT | tail -n 1 | awk '{print $9}' | sed  's/\./,/' | awk -v f=$FAILFACTOR '{ print $1 ; }'`
		FAILTIME=`grep " Algorithm Reported: Result for ParamILS" $SMACOUTPUT | tail -n 1 | awk '{print $9}' | awk -v f=$FAILFACTOR '{ print $1 ; }'`
