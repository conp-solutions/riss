...
//
//
void BoundedVariableElimination::bve_worker(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, int64_t& bveChecks,
        const bool force, const bool doStatistics)
{
    ...
    while (!data.isInterupted()
        ...
        if (config.opt_bve_verbose > 1) {
             cerr << "c considering variable: " << v + 1 << endl; 
	    if (config.opt_bve_verbose > 1 ) {
	      cerr << "c remaining BVE heap: " << endl;
        ...
        if (config.opt_bve_verbose > 1) {
            ...
            cerr << "c Clauses with Literal Â¬" << v + 1 << ":" << endl;
        ...
        if (!force) {
            ...
            if (anticipateResult == l_Undef && config.opt_bve_bc) {
                ...
                if (config.opt_bve_verbose > 1) { cerr << "c removing blocked clauses from F_Â¬" << v + 1 << endl; }
        ...
        if (resolvents > pos_count + neg_count) {
            ...
            if (config.opt_bve_growTotal != INT32_MAX && totallyAddedClauses > config.opt_bve_growTotal) { reducedClss = false; } // stop increasing!
        } else {
        ...
        if (config.opt_bve_verbose > 2) { cerr << "c resolve: " << doResolve << " res: " << resolvents << " pos: " << pos_count << " neg: " << neg_count << endl; }
        // anticipateResult == l_True -> !reduceClss (we hit the early limit, do not eliminate!)
        assert((anticipateResult != l_True || !reducedClss) && "in case of an early abort, we do not resolve");
        ...
        if ((force || doResolve) // clauses or literals should be reduced and we did
            ...
            removeClauses(data, heap, pos, lit_Undef, p_limit,
                          doStatistics);      // do not add these clauses to the undo stack!
            if (data.value(mkLit(v, false)) == l_Undef) {
                removeClauses(data, heap, neg, mkLit(v, true), n_limit,
...
//     config.opt_bve_growTotal
	...
	if( config.opt_bve_growTotal != INT32_MAX) { clausesToUse += (config.opt_bve_growTotal -
    ...
    for (int cr_p = 0; cr_p < positive.size(); ++cr_p) {
        ...
        for (int cr_n = 0; cr_n < negative.size(); ++cr_n) {
		  ...
		  if (config.opt_bve_verbose > 2) { cerr << "c reached clauses to use limit (" << clausesToUse << ") with resolvents: " << resolvents << endl;}
            ...
            if (config.opt_bve_verbose > 3) { cerr << "c ------------------------------------------" << endl; }
        }
    }
    if (config.opt_bve_verbose > 3) {
        for (int i = 0; i < positive.size(); ++i)
        { cerr << "c pos stat(" << i << "): " << (unsigned) pos_stats[i] << endl; };
        for (int i = 0; i < negative.size(); ++i)
        { cerr << "c neg stat(" << i << "): " << (unsigned) neg_stats[i] << endl; };

        cerr << "c finished anticipate_bve normally (reached early limit: " << binariesOnly << " )" << endl;
    }
    return (binariesOnly) ? l_True : l_Undef; // return whether we found limit already
}
 ...
 */
lbool BoundedVariableElimination::resolveSet(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap,
