...
namespace Coprocessor {
    ...
    class Circuit {
        ...
        struct ternary {
            Riss::Lit l1, l2;
            ternary(Riss::Lit _l1, Riss::Lit _l2)
                : l1(_l1)
                , l2(_l2) {
            }
        };
        struct quad {
            ...
            quad(Riss::Lit _l1, Riss::Lit _l2, Riss::Lit _l3)
                ...
                , l3(_l3) {
                if (l1.x > l2.x) {
                    const Riss::Lit tmp = l1;
                    l1 = l2;
                    l2 = tmp;
                } // first two are okay!
                if (l2.x > l3.x) {
                    const Riss::Lit tmp = l2;
                    l2 = l3;
                    l3 = tmp;
                } // also check last candidate!
                if (l1.x > l2.x) {
                    const Riss::Lit tmp = l1;
                    l1 = l2;
                    l2 = tmp;
                } // now, first two have to be compared again!
            }
        };
    ...
    public:
        ...
        public:
            ...
            enum Encoded { // which clauses have been found in the formula
                ...
                NEG_BLOCKED  // clauses with the negative output literal occurences are blocked TODO: can these gates have even more inputs than
                             // encoded?
            };
            ...
            Gate(const Riss::Lit& x, const Riss::Lit& s, const Riss::Lit& t, const Riss::Lit& f, const Coprocessor::Circuit::Gate::Type& _type,
                 const Coprocessor::Circuit::Gate::Encoded e); // ITE, FA_SUM
            Gate(const Riss::Lit& x, const Riss::Lit& a, const Riss::Lit& b, const Coprocessor::Circuit::Gate::Type& _type,
                 const Coprocessor::Circuit::Gate::Encoded& e); // AND, XOR
            ~Gate();
            ...
            Riss::Lit getOutput() const {
                return (type != GenAND && type != ExO) ? (const Riss::Lit)x() : data.e.x;
            }
        ...
        public:
            Riss::Lit& x() {
                assert(type != XOR && type != GenAND && type != ExO && "gate cannot be XOR");
                return data.lits[0];
            } // output
            Riss::Lit& a() {
                assert(type != ITE && "gate cannot be ITE");
                return data.lits[1];
            } // AND, FA_SUM
            Riss::Lit& b() {
                assert(type != ITE && "gate cannot be ITE");
            ...
            Riss::Lit& c() {
                assert((type == FA_SUM || type == XOR) && "gate has to be FA_SUM");
                return data.lits[3];
            } // FA_SUM
            Riss::Lit& s() {
                assert(type == ITE && "gate has to be ITE");
                return data.lits[1];
            } // ITE selector
            Riss::Lit& t() {
                assert(type == ITE && "gate has to be ITE");
            ...
            Riss::Lit& get(const int index) {
                assert(type == ExO || type == GenAND);
                return data.e.externLits[index];
            }

            const Riss::Lit& x() const {
                return data.lits[0];
            } // output
            const Riss::Lit& a() const {
                return data.lits[1];
            } // AND, FA_SUM
            const Riss::Lit& b() const {
                return data.lits[2];
            } // AND, FA_SUM
            const Riss::Lit& c() const {
                return data.lits[3];
            } // FA_SUM
            const Riss::Lit& s() const {
                return data.lits[1];
            } // ITE selector
            const Riss::Lit& t() const {
                return data.lits[2];
            } // ITE true branch
            const Riss::Lit& f() const {
                return data.lits[3];
            } // ITE false branch
            const Riss::Lit& get(const int index) const {
                return data.e.externLits[index];
            }
            int size() const {
                assert(type == ExO || type == GenAND);
            ...
            void putInQueue() {
                assert(inQueue == false && "cannot put twice in a queue");
                inQueue = true;
            }
            void takeFromQueue() {
                inQueue = false;
