...
namespace Coprocessor {
    ...
    void CoprocessorData::printFormula(FILE* fd, bool clausesOnly) {
        ...
        // print clauses
        for (int i = 0; i < clauses.size(); ++i) {
            if (!solver->ca[clauses[i]].can_be_deleted()) {
    ...
    int Preprocessor::parseModel(const string& filename) {
        ...
        if (!foundS) {
            cerr << "c WARNING: could not find any solution information!" << endl;
        }
        if (solution == 10 && !foundS) {
            cerr << "c WARNING: could not find any variable value information!" << endl;
        }
        if (!standartSAT) {
            cerr << "c WARNING: result based on non-standart solution!" << endl;
        }
