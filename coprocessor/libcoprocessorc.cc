...
void* CPinit(const char* presetConfig) {
...
void CPdestroy(void** preprocessor) {
...
void CPpreprocess(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->cp3->preprocess();
}

int CPhasNextOutputLit(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    if (cp3->outputCls < cp3->solver->clauses.size() + cp3->solver->trail.size()) {
        return 1;
    } // there are more clauses to go
    if (cp3->outputCls >= cp3->solver->clauses.size() + cp3->solver->trail.size()) {
        return 0;
    } // all clauses have been skipped
    // here, only the very last clause needs to be considered!
    if (cp3->outputCls == cp3->solver->trail.size() + cp3->solver->clauses.size() && cp3->outputLit <= 1) {
        return 1;
    } // have seen at most one literal of the very last clause yet (there is at least the termination literal)
    if (cp3->outputCls > cp3->solver->trail.size() && // there is a longer clause
        cp3->outputLit <= cp3->solver->ca[cp3->solver->clauses[cp3->solver->clauses.size() - 1]]
                              .size() // have seen all its literals, but not the termination symbol
    ) {
        return 1;
    }
    return 0;
...
int CPnextOutputLit(void* preprocessor) {
    if (!CPhasNextOutputLit(preprocessor)) {
        return 0;
    }
    libcp3* cp3 = (libcp3*)preprocessor;
    // cerr << "c ask for literal " << cp3->outputLit << " if cls " << cp3->outputCls << " (trail: " << cp3->solver->trail.size() << ", clss: " <<
    // cp3->solver->clauses.size() << ")" << endl;
    ...
    if (cp3->outputCls >= cp3->solver->trail.size() + cp3->solver->clauses.size()) {
        return INT32_MAX;
    } // undefined behavior!
...
void CPresetOutput(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->outputCls = 0;
    cp3->outputLit = 0;
}
...
void CPresetModel(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->model.clear();
    cp3->modelLit = 0;
}
...
void CPgiveModelLit(void* preprocessor, int literal) {
    libcp3* cp3 = (libcp3*)preprocessor;
    const Lit l = mkLit(literal < 0 ? -literal + 1 : literal - 1, literal < 0);
    const Var v = var(l);
    while (cp3->model.size() <= v) {
        cp3->model.push(l_Undef);
 ...
 */
void CPpushModelBool(void* preprocessor, int pol) {
    libcp3* cp3 = (libcp3*)preprocessor;
    if (pol == 0) {
        cp3->model.push(l_False);
    } else {
        cp3->model.push(l_True);
    }
}

/** pass a (satisfied) literal of the current model to the preprocessor */
void CPpostprocessModel(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->cp3->extendModel(cp3->model);
}

/** extract the next model literal from the postprocessed model*/
int CPgetFinalModelLit(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
...
int CPmodelVariables(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    return cp3->model.size();
}

void CPfreezeVariable(void* preprocessor, int variable) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->cp3->freezeExtern(variable);
}

int CPgetReplaceLiteral(void* preprocessor, int oldLit) {
    libcp3* cp3 = (libcp3*)preprocessor;
    int ret = cp3->cp3->importLit(oldLit);
    return ret == 0 ? oldLit : ret; // if there is no replacement, return the literal itself
}

void CPparseOptions(void* preprocessor, int* argc, char** argv, int strict) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->cp3config->parseOptions(*argc, argv, strict != 0);
}

void CPparseOptionString(void* preprocessor, char* argv) {
    libcp3* cp3 = (libcp3*)preprocessor;
    cp3->cp3config->parseOptions(argv);
}

void CPsetPresetConfig(void* preprocessor, const char* presetConfig) {
    libcp3* cp3 = (libcp3*)preprocessor;
    /*
    if( configNr == 1 ) {
...
void CPaddLit(void* preprocessor, int lit) {
    libcp3* cp3 = (libcp3*)preprocessor;
    if (lit != 0) {
        cp3->currentClause.push(lit > 0 ? mkLit(lit - 1, false) : mkLit(-lit - 1, true));
...
float CPversion(void* preprocessor) {
#ifdef TOOLVERSION
...
int CPok(void* preprocessor) {
    libcp3* cp3 = (libcp3*)preprocessor;
    return cp3->solver->okay();
}

int CPlitFalsified(void* preprocessor, int lit) {
    libcp3* cp3 = (libcp3*)preprocessor;
