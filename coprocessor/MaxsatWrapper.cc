...
void Mprocessor::setSpecs(int specifiedVars, int specifiedCls) {
    ...
    while (S->nVars() < specVars) {
        newVar();
    } // are there variables missing, add them
}

void Mprocessor::setProblemType(int type) {
    problemType = type;
    if (debugLevel > 2) {
        cerr << "c set problem type to " << type << endl;
    }
}
...
Riss::Var Mprocessor::newVar(bool polarity, bool dvar, char type) {
    ...
    if (debugLevel > 2) {
        cerr << "c set highest var to " << v << endl;
    }
}
...
void Mprocessor::setHardWeight(int weight) {
    if (debugLevel > 2) {
        cerr << "c set hard weight to " << weight << endl;
    }
    hardWeight = weight;
}
...
void Mprocessor::setCurrentWeight(int weight) {
    if (debugLevel > 2) {
        cerr << "c set current weight to " << weight << endl;
    }
...
void Mprocessor::updateSumWeights(int weight) {
    if (debugLevel > 2) {
        cerr << "c update sum weight " << weight << endl;
    }
    sumWeights = weight;
}
...
void Mprocessor::addHardClause(Riss::vec<Riss::Lit>& lits) {
    if (debugLevel > 1) {
        cerr << "c added hard clause to MSW: " << lits << endl;
    }
    S->addClause_(lits);
}
...
bool Mprocessor::addSoftClause(int weight, Riss::vec<Riss::Lit>& lits) {
    if (debugLevel > 1) {
        cerr << "c added soft clause to MSW: " << weight << " ; " << lits << endl;
    }
    ...
    if (lits.size() == 1) {
        ...
        if (debugLevel > 1) {
            cerr << "c added soft unit clause: " << lits << " with current total weight: " << literalWeights[toInt(lits[0])] << endl;
        }
        return true;
    }
    ...
    if (debugLevel > 2) {
        cerr << "c crate relax variable " << relaxVariables + 1 << endl;
    }

    literalWeights.push(0);
    literalWeights.push(0);                                 // weights for the two new literals!
    literalWeights[toInt(mkLit(relaxVariables))] += weight; // assign the weight of the current clause to the relaxation variable!

    if (debugLevel > 2) {
        cerr << "c new weight for variable " << relaxVariables + 1 << " : " << literalWeights[toInt(mkLit(relaxVariables))] << endl;
    }

    if (lits.size() == 0) { // found empty weighted clause
        // to ensure that this clause is falsified, add the compementary unit clause!
        if (debugLevel > 2) {
            cerr << "c add as HARD clause to PREPROCESSOR: " << mkLit(relaxVariables) << endl;
        }
    ...
    if (debugLevel > 2) {
        cerr << "c add as HARD clause to PREPROCESSOR: " << lits << endl;
    }
    S->addClause(lits);
    return false;
}
