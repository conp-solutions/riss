...
namespace Coprocessor {
    ...
    void VarShuffler::setSeed(uint32_t s) {
        seed = s;
        randomizer.set(seed);
    }
    ...
    void VarShuffler::shuffle(vec<Lit>& lits, bool shuffleOrder) {
        ...
        DOUT(if (config.shuffle_debug_out > 2) { cerr << "c rewritten vector: " << lits << endl; });
    }
