...
namespace Coprocessor {
    ...
    protected:
        ...
        public:
            Stepper(int limit, int steps = 0)
                : limit(limit)
                , steps(steps) {
            }

            inline int64_t getCurrentSteps() const {
                return steps;
            } // returns the number of consumed steps
            inline int64_t getCurrentLimit() const {
                return limit;
            } // returns budget of steps
            inline void reset() {
                steps = 0;
            } // set step counter to zero, persists current budget
            inline void increaseLimit(int additionalBudget) {
                limit += additionalBudget;
            }
            inline void increaseSteps(int _steps = 1) {
                steps += _steps;
            } // you can also decrease the steps by using a negative argument
            inline bool inLimit() const {
                return steps < limit;
            }
             ...
             */
            inline bool inLimit(uint64_t offset) const {
                return steps + offset < limit;
            }
        };
