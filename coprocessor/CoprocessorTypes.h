...
namespace Coprocessor {
    ...
    public:
        MethodTimer(double* timer)
            : pointer(timer) {
            *pointer = Riss::cpuTime() - *pointer;
        }
        ~MethodTimer() {
            *pointer = Riss::cpuTime() - *pointer;
        }
        ...
        int countLitOcc(Riss::Lit l) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (occurs) {
                    ...
                    assert(false && "dirty occurrence list!!!");
                }
            }
            return count;
        }
    ...
    public:
        ...
        CoprocessorData(Riss::ClauseAllocator& _ca, Riss::Solver* _solver, Coprocessor::Logger& _log, bool _limited = true, bool _randomized = false,
                        bool _debug = false);
        ...
        uint32_t nTotLits() const {
            return solver->nTotLits();
        }
        ...
        Riss::lbool
        enqueue(const Riss::Lit& l,
                const unsigned dependencyLevel = 0); // enqueue literal l to current solver structures, adopt to extraInfo of solver, if needed
        ...
        void addClause(const Riss::CRef& cr, Riss::Heap<Coprocessor::VarOrderBVEHeapLt>* heap, const bool update = false,
                       const Riss::Var ignore = (-1), SpinLock* data_lock = 0,
                       SpinLock* heap_lock = 0); // add clause to data structures, update counters
        bool removeClauseFrom(const Riss::CRef& cr,
                              const Riss::Lit& l); // remove clause reference from list of clauses for literal l, returns true, if successful
        void removeClauseFrom(const Riss::CRef& cr, const Riss::Lit& l,
                              const int index); // remove clause reference from list of clauses for literal l, returns true, if successful
        inline bool
        removeClauseFromThreadSafe(const Riss::CRef& cr,
                                   const Riss::Lit& l); // replaces clause reference from clause list by Riss::CRef_Undef, returns true, if successful
        ...
        void checkGarbage(std::vector<Riss::CRef>** updateVectors = 0, int size = 0) {
            return checkGarbage(solver->garbage_frac, updateVectors, size);
        }
        void checkGarbage(double gf, std::vector<Riss::CRef>** updateVectors = 0, int size = 0) {
            if (ca.wasted() > ca.size() * gf) {
                garbageCollect(updateVectors, size);
            }
        }
        ...
        void lock() {
            dataLock.lock();
        } // lock and unlock the data structure
        void unlock() {
            dataLock.unlock();
        } // lock and unlock data structure

        // formula statistics with HeapUpdate and LockHandling

        void addedLiteral(const Riss::Lit& l, const int32_t diff = 1, Riss::Heap<VarOrderBVEHeapLt>* heap = nullptr, const bool update = false,
                          const Riss::Var ignore = var_Undef, SpinLock* data_lock = nullptr,
                          SpinLock* heap_lock = nullptr); // update counter for literal
        void removedLiteral(const Riss::Lit& l, const int32_t diff = 1, Riss::Heap<VarOrderBVEHeapLt>* heap = nullptr, const bool update = false,
                            const Riss::Var ignore = var_Undef, SpinLock* data_lock = nullptr,
                            SpinLock* heap_lock = nullptr); // update counter for literal
        void addedClause(const Riss::CRef& cr, Riss::Heap<Coprocessor::VarOrderBVEHeapLt>* heap = 0, const bool update = false,
                         const Riss::Var ignore = (-1), SpinLock* data_lock = 0,
                         SpinLock* heap_lock = 0); // update counters for literals in the clause

        void removedClause(const Riss::CRef& cr, Riss::Heap<VarOrderBVEHeapLt>* heap = nullptr, const bool update = false,
                           const Riss::Var ignore = var_Undef, SpinLock* data_lock = nullptr,
                           SpinLock* heap_lock = nullptr);            // update counters for literals in the clause
...
// for DRUP / DRAT proofs
        ...
        Riss::vec<Riss::Lit>& replacedBy() {
            return solver->eqInfo.replacedBy;
        }
        ...
        void sendUnits(Riss::vec<Riss::Lit>& literalStorage, uint32_t beginIndex, int unitsToShare) {
            ...
            Riss::Lit* headPointer =
                &(literalStorage[beginIndex]); // pointer to the actual element in the vector. as vectors use arrays to store data, the trick works
#ifdef PCASSO
            share(&headPointer, unitsToShare, currentDependencyLevel(),
    ...
    public:
        ...
        uint32_t getVars() const {
            return duringCreationVariables;
        }
        ...
        uint32_t getStart(const Riss::Lit& l) {
            return start != 0 && var(l) < duringCreationVariables ? start[Riss::toInt(l)] : 0;
        }
        /** get indexes of BIG scan algorithm */
        uint32_t getStop(const Riss::Lit& l) {
            return stop != 0 && var(l) < duringCreationVariables ? stop[Riss::toInt(l)] : 0;
        }
    };
    ...
    struct VarOrderBVEHeapLt {
        ...
        bool operator()(Riss::Var x, Riss::Var y) const {
            ...
            if (heapOption == 0) {
                ...
                double rx = 0;
                if (xp != 0 || xn != 0) {
                    rx = xp > xn ? (xn != 0 ? xp / xn : xp * 1000) : (xp != 0 ? xn / xp : xn * 1000);
                }
                double ry = 0;
                if (yp != 0 || yn != 0) {
                    ry = yp > yn ? (yn != 0 ? yp / yn : yp * 1000) : (yp != 0 ? yn / yp : yn * 1000);
                }

                if (heapOption == 3) {
                    return (rx < ry) || (rx == ry && data[x] < data[y]);
                    ...
                    return (data[x] < data[y]) || (rx < ry && data[x] == data[y]);
                ...
                assert(false && "In case of random order no heap should be used");
                return false;
            }
            return false;
        }
        VarOrderBVEHeapLt(CoprocessorData& _data, int _heapOption)
            : data(_data)
            , heapOption(_heapOption) {
        }
    };
    ...
    struct LitOrderHeapLt {
        ...
        bool operator()(int ix, int iy) const {
            /* assert (data != nullptr && "Please assign a valid data object before heap usage" );*/
            const Riss::Lit x = Riss::toLit(ix);
            const Riss::Lit y = Riss::toLit(iy);
            if (heapOption == 0) {
                return data[x] < data[y];
            } else if (heapOption == 1) {
                return data[x] > data[y];
                ...
                assert(false && "In case of random order no heap should be used, or wrong parameter for heap comparison");
                return false;
            }
            return false;
        }
        LitOrderHeapLt(CoprocessorData& _data, int _heapOption, bool allowRandom = true)
            : data(_data)
            , heapOption(allowRandom ? _heapOption : (_heapOption > 1 ? _heapOption + 1 : _heapOption)) {
            assert((allowRandom || heapOption != 2) && "only allow heap option 2 if random selection is allowed");
        }
    };

    inline CoprocessorData::CoprocessorData(Riss::ClauseAllocator& _ca, Riss::Solver* _solver, Coprocessor::Logger& _log, bool _limited,
                                            bool _randomized, bool _debug)
        : ca(_ca)
        , solver(_solver)
        , numberOfVars(0)
        , numberOfCls(0)
    ...
    inline void CoprocessorData::moveVar(Riss::Var from, Riss::Var to, bool final) {
        if (from != to) { // move data only if necessary
            solver->varFlags[to].assigns = solver->varFlags[from].assigns;
            solver->varFlags[from].assigns = l_Undef;
            solver->vardata[to] = solver->vardata[from];
            solver->vardata[from] = Riss::Solver::VarData();
            solver->activity[to] = solver->activity[from];
            solver->activity[from] = 0;
            solver->varFlags[to].seen = solver->varFlags[from].seen;
            solver->varFlags[to].seen = 0;
            solver->varFlags[to].polarity = solver->varFlags[from].polarity;
            solver->varFlags[from].polarity = 0;
            solver->varFlags[to].decision = solver->varFlags[from].decision;
            solver->varFlags[from].decision = false;
            solver->varFlags[to].frozen = solver->varFlags[from].frozen;
            solver->varFlags[from].frozen = false;

            solver->eqInfo.replacedBy[to] = solver->eqInfo.replacedBy[from];
            solver->eqInfo.replacedBy[from] = Riss::mkLit(from, false);
    ...
    inline bool CoprocessorData::isInterupted() {
        return solver->asynch_interrupt ||
               (0 != solver->terminationCallbackMethod && 0 != solver->terminationCallbackMethod(solver->terminationCallbackState));
    }

    inline Riss::lbool CoprocessorData::enqueue(const Riss::Lit& l, const unsigned int dependencyLevel) {
        if (false || global_debug_out) {
            std::cerr << "c enqueue " << l << " with previous value "
                      << (solver->value(l) == l_Undef ? "undef" : (solver->value(l) == l_False ? "unsat" : " sat ")) << std::endl;
        }
    ...
    inline void CoprocessorData::addClause(const Riss::CRef& cr, bool check) {
        ...
        if (c.size() > 1) {
            solver->updatePosNeg(somePosInClause, somNegInClause);
        } // tell solver whether we can still use the polarity information, only for large clauses
        numberOfCls++;
    }

    inline void CoprocessorData::addClause(const Riss::CRef& cr, Riss::Heap<VarOrderBVEHeapLt>* heap, const bool update, const Riss::Var ignore,
    ...
    inline void CoprocessorData::reSetupSolver() {
    ...
    inline void CoprocessorData::removedClause(const Riss::Lit& l1, const Riss::Lit& l2) {
        ...
        for (int i = 0; i < list(searchLit).size(); ++i) {
            ...
            if (cl.can_be_deleted() || cl.size() != 2) {
                continue;
            }
    ...
    inline void CoprocessorData::addedLiteral(const Riss::Lit& l, const int32_t diff, Riss::Heap<VarOrderBVEHeapLt>* heap, const bool update,
                                              const Riss::Var ignore, SpinLock* data_lock, SpinLock* heap_lock) {
        if (heap == nullptr && data_lock == nullptr && heap_lock == nullptr) {
    ...
    inline void CoprocessorData::removedLiteral(const Riss::Lit& l, const int32_t diff, Riss::Heap<VarOrderBVEHeapLt>* heap, const bool update,
                                                const Riss::Var ignore, SpinLock* data_lock, SpinLock* heap_lock) // update counter for literal
    {
        if (heap == nullptr && data_lock == nullptr && heap_lock == nullptr) {
            deletedVar(var(l));
            lit_occurrence_count[Riss::toInt(l)] -= diff;
    ...
    inline void CoprocessorData::addedClause(const Riss::CRef& cr, Riss::Heap<VarOrderBVEHeapLt>* heap, const bool update, const Riss::Var ignore,
                                             SpinLock* data_lock, SpinLock* heap_lock) // update counters for literals in the clause
    {
        const Riss::Clause& c = ca[cr];
        if (heap == nullptr && data_lock == nullptr && heap_lock == nullptr) {
            for (int l = 0; l < c.size(); ++l) {
    ...
    inline void CoprocessorData::removedClause(const Riss::CRef& cr, Riss::Heap<Coprocessor::VarOrderBVEHeapLt>* heap, const bool update,
                                               const Riss::Var ignore, SpinLock* data_lock,
                                               SpinLock* heap_lock) // update counters for literals in the clause
    {
        const Riss::Clause& c = ca[cr];
        if (heap == nullptr && data_lock == nullptr && heap_lock == nullptr) {
            for (int l = 0; l < c.size(); ++l) {
    ...
    inline void CoprocessorData::correctCounters() {
        ...
        for (int i = 0; i < solver->clauses.size(); ++i) {
            ...
            numberOfCls++;
            for (int j = 0; j < c.size(); j++) {
                lit_occurrence_count[Riss::toInt(c[j])]++;
            } // increment all literal counters accordingly
        }
        for (int i = 0; i < solver->learnts.size(); ++i) {
    ...
    inline void CoprocessorData::garbageCollect(std::vector<Riss::CRef>** updateVectors, int size) {
        ...
        DOUT(if (solver->verbosity != 0) std::cerr << "c Garbage collection: " << ca.size() * Riss::ClauseAllocator::Unit_Size << " bytes => "
                                                   << to.size() * Riss::ClauseAllocator::Unit_Size << " bytes " << std::endl;);
    ...
    inline void CoprocessorData::relocAll(Riss::ClauseAllocator& to, std::vector<Riss::CRef>** updateVectors, int size) {
        ...
        for (int i = 0; i < solver->trail.size(); i++) {
            ...
            if (solver->level(v) == 0) {
                solver->vardata[v].reason = Riss::CRef_Undef;
            } // take care of reason clauses for literals at top-level
            else if (!solver->reason(v).isBinaryClause() && solver->reason(v).getReasonC() != Riss::CRef_Undef &&
    ...
    inline void CoprocessorData::extendModel(Riss::vec<Riss::lbool>& model) {
        ...
        for (int j = 0; j < model.size(); ++j) {
            if (model[j] == l_Undef) {
                model[j] = l_True;
            } // set free variables to some value
        }
        ...
        for (int i = undo.size() - 1; i >= 0; --i) {
            ...
            if (global_debug_out || local_debug) {
                std::cerr << "c read literal " << c << std::endl;
            }
            if (c == Riss::lit_Undef) { // found clause delimiter, without jumping over it in the SAT case (below)
                if (!isSat) { // this condition is always satisfied -- the current clause has to be unsatisfied (otherwise, would have been ignored
                              // below!)
                    ...
                    if (local_debug) {
                        std::cerr << "c set literal " << undo[i + 1] << " to true " << std::endl;
                    }
            ...
            if (var(c) >= model.size()) {
                model.growTo(var(c) + 1, l_True);
            }                                                    // model is too small? this will also take care of extended resolution variables!
            if (model[var(c)] == (sign(c) ? l_False : l_True)) { // satisfied
                ...
                while (undo[i] != Riss::lit_Undef) {             // skip literal until hitting the delimiter - for loop will decrease i once more
                    if (global_debug_out || local_debug) {
                        std::cerr << "c skip because SAT: " << undo[i] << std::endl;
                    }
                    --i;
                }
                if (local_debug) { // print intermediate state!
    ...
    inline void CoprocessorData::addEquivalences(const Riss::Lit& l1, const Riss::Lit& l2) {
        ...
        if (global_debug_out) {
            std::cerr << "c [DATA] set equi: " << l1 << " == " << l2 << std::endl;
        }
    ...
    inline void BIG::create(Riss::ClauseAllocator& ca, uint32_t nVars, Riss::vec<Riss::CRef>& list) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            const Riss::Lit l0 = c[0];
            const Riss::Lit l1 = c[1];
        ...
        for (Riss::Var v = 0; v < maxVar; ++v) {
            for (int p = 0; p < 2; ++p) {
                ...
                for (int i = 1; i < getSize(l); ++i) {
                    ...
                    if (getArray(l)[i] != getArray(l)[j]) {
                        getArray(l)[++j] = getArray(l)[i];
                    } // keep elements, if they are not equal to the last element!
                }
    ...
    inline void BIG::generateImplied(CoprocessorData& data) {
        ...
        if (start == 0) {
            start = (uint32_t*)malloc(maxVar * sizeof(uint32_t) * 2);
        } else {
        ...
        if (stop == 0) {
            stop = (uint32_t*)malloc(maxVar * sizeof(uint32_t) * 2);
        } else {
        ...
        for (uint32_t i = 0; i < ts; i++) {
            ...
            const Riss::Lit tmp = data.lits[i];
            data.lits[i] = data.lits[rnd];
            data.lits[rnd] = tmp;
        }
    ...
    inline void BIG::generateImplied(uint32_t nVars, Riss::vec<Riss::Lit>& tmpLits) {
        ...
        for (uint32_t i = 0; i < ts; i++) {
            const uint32_t rnd = rand() % ts;
            const Riss::Lit tmp = tmpLits[i];
            tmpLits[i] = tmpLits[rnd];
            tmpLits[rnd] = tmp;
        }
    ...
    inline void BIG::fillSorted(std::vector<Riss::Lit>& literals, CoprocessorData& data, bool rootsOnly, bool getAll) {
        ...
        for (Riss::Var v = 0; v < maxVar; ++v) {
            if (getSize(Riss::mkLit(v, false)) == 0)
                if (getSize(Riss::mkLit(v, true)) == 0) {
                    continue;
                } else {
        ...
        for (int i = 0; i < literals.size(); ++i) {
            ...
            for (int j = 0; j < s; ++j) {
                ...
                if (data.ma.isCurrentStep(Riss::toInt(l2))) {
                    continue;
                }
        ...
        for (Riss::Var v = 0; v < maxVar; ++v) {
            for (int p = 0; p < 2; ++p) {
                const Riss::Lit l = Riss::mkLit(v, p == 1);
                if (data.ma.isCurrentStep(Riss::toInt(l))) {
                    continue;
                } // literal already in heap
                else {
    ...
    inline uint32_t BIG::stampLiteral(const Riss::Lit& literal, uint32_t stamp, int32_t* index, std::deque<Riss::Lit>& stampQueue) {
        ...
        if (start[Riss::toInt(literal)] != 0) {
            return stamp;
        }

        if (global_debug_out) {
            std::cerr << "c call STAMP for " << literal << std::endl;
        }
        ...
        if (global_debug_out) {
            std::cerr << "c start[" << literal << "] = " << stamp << std::endl;
        }
        ...
        while (!stampQueue.empty()) {
            ...
            if (index[Riss::toInt(current)] == adjSize) {
                ...
                if (global_debug_out) {
                    std::cerr << "c stop[" << current << "] = " << stamp << std::endl;
                }
            } else {
                ...
                ind++;
                if (start[Riss::toInt(impliedLit)] != 0) {
                    continue;
                }
                stamp++;
                ...
                if (global_debug_out) {
                    std::cerr << "c start[" << impliedLit << "] = " << stamp << std::endl;
                }
    ...
    inline bool BIG::implies(const Riss::Lit& from, const Riss::Lit& to) const {
        if (start == 0 || stop == 0 || var(from) >= duringCreationVariables || var(to) >= duringCreationVariables) {
            return false;
        }
    ...
    inline bool BIG::isOneChild(const Riss::Lit& parent, const Riss::Lit& child1, const Riss::Lit& child2) const {
        ...
        const int listSize = getSize(parent);
        for (int j = 0; j < listSize; ++j) {
            if (list[j] == child1 || list[j] == child2) {
                return true;
            }
        }
        return false;
    ...
    inline void Logger::log(int level, const std::string& s) {
        ...
        (useStdErr ? std::cerr : std::cout) << "c [" << level << "] " << s << std::endl;
    }
    ...
    inline void Logger::log(int level, const std::string& s, const Riss::Clause& c) {
        ...
        (useStdErr ? std::cerr : std::cout) << "c [" << level << "] " << s << " : ";
        for (int i = 0; i < c.size(); ++i) {
            const Riss::Lit& l = c[i];
            (useStdErr ? std::cerr : std::cout) << " " << (sign(l) ? "-" : "") << var(l) + 1;
        }
        (useStdErr ? std::cerr : std::cout) << std::endl;
    }
    ...
    inline void Logger::log(int level, const std::string& s, const Riss::Lit& l) {
        ...
        (useStdErr ? std::cerr : std::cout) << "c [" << level << "] " << s << " : " << (sign(l) ? "-" : "") << var(l) + 1 << std::endl;
    }
    ...
    inline void Logger::log(int level, const std::string& s, const int i) {
        ...
        (useStdErr ? std::cerr : std::cout) << "c [" << level << "] " << s << " " << i << std::endl;
    }
    ...
    inline void Logger::log(int level, const std::string& s, const Riss::Clause& c, const Riss::Lit& l) {
        ...
        (useStdErr ? std::cerr : std::cout) << "c [" << level << "] " << s << " : " << (sign(l) ? "-" : "") << var(l) + 1 << " with clause ";
        for (int i = 0; i < c.size(); ++i) {
            const Riss::Lit& l = c[i];
            (useStdErr ? std::cerr : std::cout) << " " << (sign(l) ? "-" : "") << var(l) + 1;
        }
