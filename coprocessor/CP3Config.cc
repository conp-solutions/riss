...
CP3Config::CP3Config(const std::string& presetOptions) // add new options here!
    ...
    pr_lcm_vivi       (_cat_pr, "pr-lcmC",        "Use vivifaction for clauses (1=plain vivi, 2=vivi+analyze, 3=vivi+unionanalyze, 4+reverse, >4 X-4, and-reverse)", 18, IntRange(0, 24), optionListPtr, &pr_lcm),
    pr_lcm_min_size   (_cat_pr, "pr-lcmMinSize",  "Apply LCM only to clauses with at least X literals", 3, IntRange(1, INT32_MAX),                                  optionListPtr, &pr_lcm),
    ...
    pr_lcm_iterations (_cat_pr, "pr-lcmIter",     "number of iterations we should go over the clauses", 2,  IntRange(0, INT32_MAX),                                 optionListPtr, &pr_lcm),
    ...
    opt_lcm_lits      (_cat,    "cp3_lcm_lits",   "total literal limit to enable LCM-VIVIFICATION",   15000000, IntRange(0, INT32_MAX),                             optionListPtr, &pr_vivi),
    #ifndef NDEBUG
