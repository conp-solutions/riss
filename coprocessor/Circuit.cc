...
namespace Coprocessor {
    ...
    void Circuit::extractGates(CoprocessorData& data, vector<Gate>& gates) {
        ...
        DOUT(if (config.circ_debug_out) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                for (int p = 0; p < 2; ++p) {
                    const Lit l = mkLit(v, p == 1);
                    cerr << "c " << l << " -> ";
                    for (int i = 0; i < big->getSize(l); ++i) {
                        cerr << " " << big->getArray(l)[i];
                    }
                    cerr << endl;
                }
            }
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                if (c.size() == 3) {
                    for (int j = 0; j < 3; ++j)
                        ternaries[toInt(c[j])].push_back(ternary(j == 0 ? c[2] : c[0], j == 1 ? c[2] : c[1]));
                } else if (c.size() == 4) {
                    for (int j = 0; j < 4; ++j)
                        quads[toInt(c[j])].push_back(quad(j == 0 ? c[3] : c[0], j == 1 ? c[3] : c[1], j == 2 ? c[3] : c[2]));
                }
            }
        }
    ...
    void Circuit::getGatesWithOutput(const Var v, vector<Circuit::Gate>& gates, CoprocessorData& data) {
        ...
        if (config.circ_ExO) {
            getExOGates(v, gates, data);
        }
        if (config.circ_FASUM) {
            getFASUMGates(v, gates, data);
        }
    ...
    void Circuit::getANDGates(const Var& v, vector<Coprocessor::Circuit::Gate>& gates, Coprocessor::CoprocessorData& data) {
        DOUT(if (config.circ_debug_out) cerr << "c try to find AND gate for variable " << v + 1 << " (found so far:" << gates.size()
                                             << ")[ok?: " << data.ok() << "]" << endl;);
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            for (int i = 0; i < listSize; ++i) {
                if (data.ma.isCurrentStep(toInt(~list[i]))) {
                    DOUT(if (config.circ_debug_out) cerr << "c marked complementary literal of " << list[i] << " already! hence, imply literal "
                                                         << ~pos << " as a unit" << endl;);
                    data.enqueue(~pos);
                    data.lits.clear();
                    break;
            ...
            if (data.lits.size() > 1) { // work on found binary clauses!
                ...
                if (config.circ_genAND) {
                    ...
                    for (int i = 0; i < cList.size(); ++i) {    // there can be multiple full encoded gates per variable
                        ...
                        if (c.can_be_deleted() || c.size() < 3 || c.size() > data.lits.size() + 1) {
                            continue;
                        } // new clauses that provide not too much literals
                        ...
                        for (int j = 0; j < c.size(); ++j)
                            if (data.ma.isCurrentStep(toInt(~c[j]))) {
                                ...
                                //    cerr << "literal " << ~c[j] << " is not marked, and literal " << c[j] << " is not implied by literal " << pos <<
                                //    endl;
                                break;
                            }
                        if (marked == c.size()) {
                    ...
                    for (int i = 0; i < cList.size(); ++i) {              // there can be multiple gates per variable
                        ...
                        if ((data.ma.isCurrentStep(toInt(~c.l1)) ||
                             big->implies(pos, ~c.l1)) // since implied does not always work, also check alternative!
                ...
                if (!foundBlockedCandidate && config.circ_BLOCKED) {
                    if (data.lits.size() > 2 && !config.circ_genAND) {
                        continue;
                    }
                    ...
                    if (presentClauses == data.lits.size()) { // all clauses with ~pos are binary and define one half of the gate!
                        ...
                        if (config.circ_AddBlocked) {
                            ...
                            if (!config.circ_genAND) {
                                assert(learnt_clause.size() == 3 && "only binary and-gates are allowed");
                            }
                            ...
                            DOUT(if (config.circ_debug_out) {
                                ...
                                cerr << "clause [";
                                for (int abc = 0; abc < learnt_clause.size(); ++abc) {
                                    cerr << learnt_clause[abc] << " ";
                                }
                                cerr << "] leads to gate: ";
                                gates[gates.size() - 1].print(cerr);
                            });
                            ...
                            CRef cr = ca.alloc(learnt_clause, false); // create as learnt clause (theses clauses have to be considered for
                                                                      // inprocessing as well, see "inprocessing rules" paper!
                            data.addClause(cr);
            ...
            } else if (config.circ_BLOCKED &&
                       (!config.circ_genAND ||
                        data.lits.size() ==
                            2)) { // handle case where all binary clauses are blocked (and removed) and only the large clause is present
    ...
    void Circuit::getExOGates(const Var& v, vector<Coprocessor::Circuit::Gate>& gates, Coprocessor::CoprocessorData& data) {
        for (int p = 0; p < 2; ++p) {
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                for (int j = 0; j < c.size(); ++j) {
                    if (var(c[j]) < v) {
                        cont = true;
                        break;
                    } // not smallest variable in this clause!
                ...
                if (config.circ_BLOCKED) {
                    ...
                    for (int j = 0; j < c.size(); ++j) {
                        ...
                        assert((!found || count == 1) &&
                               "if for the current literal the situation for blocked has been found, one clause has been found");
                    }
                    if (found) {
                ...
                if (!found) {
                    ...
                    gates.push_back(Gate(c, Gate::ExO, Gate::FULL));
                }
            }
            ...
            for (int j = 0; j < data.lits.size(); ++j)
                if (var(data.lits[j]) < v) {
                    data.lits.clear();
                    break;
                }
            if (data.lits.size() < 3) {
                continue;
            } // only consider clauses with more literals!
            ...
            for (int j = 0; j < data.lits.size(); ++j) {
                ...
                for (int k = 0; k < lList.size(); ++k) {
                    ...
                    if (ck.size() != 2) {
                        found = false;
                        break;
                    } // not the current clause, or more than one clause
                    // all literals of the clause have to appear in the list of excluded literals
                    if (!data.ma.isCurrentStep(toInt(ck[0])) || !data.ma.isCurrentStep(toInt(ck[1]))) {
                        found = false;
                        break;
                    }
                    count++;
                ...
                assert((!found || count + 1 >= data.lits.size()) &&
                       "if for the current literal the situation for blocked has been found, exactly the n binary clauses have to be found!");
            }
            if (found) {
                for (int j = 0; j < data.lits.size(); j++) {
                    data.lits[j] = ~data.lits[j];
                }
    ...
    void Circuit::getFASUMGates(const Var& v, vector<Coprocessor::Circuit::Gate>& gates, Coprocessor::CoprocessorData& data) {
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            bool found[8];
            found[0] = true; // found first clause!
            for (int i = 0; i < cList.size(); ++i) {
                DOUT(if (config.circ_debug_out) cerr << "c check quad " << i << "/" << cList.size() << endl;);
                const quad& current = cList[i];
                // check size occurrence:
                if (var(current.l1) < var(a) || var(current.l2) < var(a) || var(current.l3) < var(a)) {
                    continue;
                } // do not consider this order of this quad!
                // reset
                for (int j = 1; j < 8; ++j) {
                    found[j] = false;
                }
                // scan for the clauses!
                const Lit b = current.l1;
                const Lit c = current.l2;
                const Lit d = current.l3;
                // check all binary clauses!
                if (big->isChild(a, d) || big->implies(a, d)) {
                    found[1] = found[2] = true;
                }
                if (big->isChild(a, ~d) || big->implies(a, ~d)) {
                    found[4] = found[7] = true;
                }
                if (big->isChild(~a, d) || big->implies(~a, d)) {
                    found[3] = true;
                } // part of the initial quadrary!
                if (big->isChild(~a, ~d) || big->implies(~a, ~d)) {
                    found[5] = found[6] = true;
                }
                if (big->isChild(a, c) || big->implies(a, c)) {
                    found[1] = found[4] = true;
                }
                if (big->isChild(a, ~c) || big->implies(a, ~c)) {
                    found[2] = found[7] = true;
                }
                if (big->isChild(~a, c) || big->implies(~a, c)) {
                    found[5] = true;
                } // part of the initial quadrary!
                if (big->isChild(~a, ~c) || big->implies(~a, ~c)) {
                    found[3] = found[6] = true;
                }
                if (big->isChild(a, b) || big->implies(a, b)) {
                    found[2] = found[4] = true;
                }
                if (big->isChild(a, ~b) || big->implies(a, ~b)) {
                    found[1] = found[7] = true;
                }
                if (big->isChild(~a, b) || big->implies(~a, b)) {
                    found[6] = true;
                } // part of the initial quadrary!
                if (big->isChild(~a, ~b) || big->implies(~a, ~b)) {
                    found[3] = found[5] = true;
                }
                if (big->isChild(b, d) || big->implies(b, d)) {
                    found[1] = found[3] = true;
                }
                if (big->isChild(b, ~d) || big->implies(b, ~d)) {
                    found[5] = found[7] = true;
                }
                if (big->isChild(~b, d) || big->implies(~b, d)) {
                    found[2] = true;
                } // part of the initial quadrary!
                if (big->isChild(~b, ~d) || big->implies(~b, ~d)) {
                    found[4] = found[6] = true;
                }
                if (big->isChild(b, c) || big->implies(b, c)) {
                    found[1] = found[5] = true;
                }
                if (big->isChild(b, ~c) || big->implies(b, ~c)) {
                    found[3] = found[7] = true;
                }
                if (big->isChild(~b, c) || big->implies(~b, c)) {
                    found[4] = true;
                } // part of the initial quadrary!
                if (big->isChild(~b, ~c) || big->implies(~b, ~c)) {
                    found[2] = found[6] = true;
                }
                if (big->isChild(c, d) || big->implies(c, d)) {
                    found[2] = found[3] = true;
                }
                if (big->isChild(c, ~d) || big->implies(c, ~d)) {
                    found[6] = found[7] = true;
                }
                if (big->isChild(~c, d) || big->implies(~c, d)) {
                    found[1] = true;
                } // part of the initial quadrary!
                if (big->isChild(~c, ~d) || big->implies(~c, ~d)) {
                    found[4] = found[5] = true;
                }
                ...
                for (int j = 1; j < 8; ++j) {
                    ...
                    DOUT(if (config.circ_debug_out) cerr << "c try to find quad: [" << lits[0] << ", " << lits[1] << ", " << lits[2] << ", "
                                                         << lits[3] << "]" << endl;);
                    for (int ind = 0; ind < 2; ++ind) {                           // index of literal to iterate over
                        ...
                        for (int ti = 0; ti < tList.size(); ++ti) {
                            ...
                            DOUT(if (config.circ_debug_out) cerr << "c check with ternaries of lit " << lits[0] << " : " << ti << "/" << tList.size()
                                                                 << " [" << lits[0] << "," << tern.l1 << "," << tern.l2 << "]" << endl;);
                            if (tern.l1 == lits[1] && (tern.l2 == lits[2] || tern.l2 == lits[3])) {
                                found[j] = true;
                                goto HASUMnextJ;
                            }
                            if (tern.l1 == lits[2] && (tern.l2 == lits[1] || tern.l2 == lits[3])) {
                                found[j] = true;
                                goto HASUMnextJ;
                            }
                            if (tern.l1 == lits[3] && (tern.l2 == lits[2] || tern.l2 == lits[1])) {
                                found[j] = true;
                                goto HASUMnextJ;
                            }
                        }
                    ...
                    // swap back in case the ternary attempt failed
                    {
                        ...
                        for (int qi = 0; qi < qList.size(); ++qi) {
                            ...
                            DOUT(if (config.circ_debug_out) cerr << "c check sub quad " << qi << "/" << qList.size() << ": " << qQuad.l1 << ","
                                                                 << qQuad.l2 << "," << qQuad.l3 << endl;);
                ...
                if (count < 4) {
                    continue;
                }                 // need at least 4 clauses to have a chance for blocked!
                ...
                if (config.circ_BLOCKED) {
                    ...
                    if (found[0] && found[3] && found[5] && found[6]) {
                        blockLit = a;
                    } else if (found[1] && found[2] && found[4] && found[7]) {
                        blockLit = ~a;
                    } else if (found[0] && found[2] && found[4] && found[6]) {
                        blockLit = b;
                    } else if (found[1] && found[3] && found[5] && found[7]) {
                        blockLit = ~b;
                    } else if (found[0] && found[1] && found[4] && found[5]) {
                        blockLit = c;
                    } else if (found[2] && found[3] && found[6] && found[7]) {
                        blockLit = ~c;
                    } else if (found[0] && found[1] && found[2] && found[3]) {
                        blockLit = d;
                    } else if (found[4] && found[5] && found[6] && found[7]) {
                        blockLit = ~d;
                    }
                    DOUT(if (config.circ_debug_out) cerr << "c block lit for gate: " << blockLit << endl;);
                    if (blockLit == lit_Undef) {
                        continue;
                    } // no opportunity for blocking!
        ...
        for (int i = oldGates + 1; i < gates.size(); ++i) {
            for (int j = oldGates; j < i; ++j) {
                ...
                if (var(gates[i].x()) == var(gates[j].x()) && var(gates[i].a()) == var(gates[j].a()) && var(gates[i].b()) == var(gates[j].b()) &&
            ...
            for (int i = 0; i < cList.size(); ++i) {              // there can be multiple gates per variable
                ...
                for (int posS = 0; posS < 2; posS++) {            // which of the other two literals is the selector?
                    ...
                    for (int j = 0; j < cList.size(); ++j) {
                        ...
                        DOUT(if (config.circ_debug_out) cerr << "candidate [" << j << "] : [" << x << "," << cand.l1 << "," << cand.l2 << "]"
                                                             << endl;);
                    ...
                    if (true) { // TODO: binary representations cannot be applied with blocked!
                        ...
                        for (int j = 0; j < listSize; ++j) {
                            if (data.ma.isCurrentStep(toInt(list[j]))) {
                                continue;
                            } // do not add literals twice!
                            data.lits.push_back(~list[j]);
                            DOUT(if (config.circ_debug_out) cerr << "added candidate " << ~list[j] << " by implication " << ~pos << " -> " << list[j]
                                                                 << endl;);
                        }
                    }
                    int countPos = 0;
                    ...
                    if (config.circ_BLOCKED) { // try to extract blocked gates!
                        DOUT(if (config.circ_debug_out) cerr << "c blocked check with candidates: " << data.lits.size()
                                                             << " including ternary cands: " << preBinaryFs << endl;);
                        if (data.lits.size() == 1 && preBinaryFs == 1) { // ITE gate can be blocked only if there is a single f candidate!
                            ...
                            if (!nonTernary && (countPos == 2 || (countPos == 3 && redundantTernary))) {
                                DOUT(if (config.circ_debug_out) cerr << "c current ITE gate is implied with blocked clause! ternaries: " << countPos
                                                                     << " found redundant: " << redundantTernary << endl;);
                                // Lit x, Lit s, Lit t, Lit f, const Coprocessor::Circuit::Gate::Type _type, const Coprocessor::Circuit::Gate::Encoded
                                // e
                                gates.push_back(Gate(x, s, t, data.lits[0], Gate::ITE, Gate::NEG_BLOCKED));
                                continue;
                            }
                            DOUT(if (config.circ_debug_out) cerr << "counted occurrences: " << countPos << " including " << nonTernary
                                                                 << " nonTernary clauses, found redundant: " << redundantTernary << endl;);
                        }
                    }
                    ...
                    for (int j = 0; j < data.lits.size(); ++j) {
                        ...
                        for (int k = 0; k < nList.size(); ++k) {
                            ...
                            DOUT(if (config.circ_debug_out) cerr << "c check ternary " << ~pos << "," << cand.l1 << "," << cand.l2
                                                                 << "  1st: " << foundFirst << " 2nd: " << foundSecond << endl;);
                            if ((cand.l1 == ~s || cand.l2 == ~s) && (cand.l1.x ^ cand.l2.x ^ (~s).x == t.x)) {
                                foundFirst = true;
                            } else if ((cand.l1 == s || cand.l2 == s) && (cand.l1.x ^ cand.l2.x ^ s.x == f.x)) {
                                foundSecond = true;
                            }
                        }
                        if (!foundFirst || !foundSecond) {
                            ...
                            if (!foundFirst) {
                                foundFirst = (big->implies(x, t) || big->implies(x, ~s)) ? true : foundFirst;
                            }
                            ...
                            if (!foundSecond) {
                                foundSecond = (big->implies(x, f) || big->implies(x, s)) ? true : foundSecond;
                            }
                        }
                        if (!foundFirst || !foundSecond) { // f candidate not verified -> remove gate!!
                            DOUT(if (config.circ_debug_out) cerr << "c could not verify " << data.lits[j] << " 1st: " << foundFirst
                                                                 << " 2nd: " << foundSecond << endl;);
        ...
        for (int i = oldGates + 1; i < gates.size(); ++i) {
            for (int j = oldGates; j < i; ++j) {
                if (var(gates[j].s()) == var(gates[i].s())) {
                    if ((gates[j].s() == ~gates[i].s() && gates[j].x() == gates[i].x() && gates[j].f() == gates[i].t() &&
                         gates[j].t() == gates[i].f()) ||
                        (gates[j].s() == gates[i].s() && gates[j].x() == ~gates[i].x() && gates[j].t() == ~gates[i].t() &&
                         gates[j].f() == ~gates[i].f()) ||
                        (gates[j].s() == ~gates[i].s() && gates[j].x() == ~gates[i].x() && gates[j].t() == ~gates[i].f() &&
                         gates[j].f() == ~gates[i].t())) {
    ...
    void Circuit::getXORGates(const Var& v, vector<Coprocessor::Circuit::Gate>& gates, Coprocessor::CoprocessorData& data) {
        ...
        for (int p = 0; p < 2; ++p) {
            ...
            for (int i = 0; i < cList.size(); ++i) {
                ...
                bool binary = false;
                const Lit b = cList[i].l1;
                const Lit c = cList[i].l2;
                DOUT(if (config.circ_debug_out) {
                    cerr << endl
                         << "c check [" << i << "/" << cList.size() << "] for "
                         << "[0](" << a << "," << b << "," << c << ")" << endl
                         << "[1](" << a << "," << ~b << "," << ~c << ")" << endl
                         << "[2](" << ~a << "," << ~b << "," << c << ")" << endl
                         << "[3](" << ~a << "," << b << "," << ~c << ")" << endl;
                });
                if (var(b) == var(c) || var(b) == var(a) || var(a) == var(c)) {
                    continue;
                } // just to make sure!
                ...
                if (!found[1]) {
                    continue;
                } // this clause does not contribute to an XOR!
                ...
                if (false && !binary && config.circ_BLOCKED) {
                    ...
                    for (int j = 0; j < data.list(a).size(); ++j) {
                        ...
                        //                     countPos = countPos + 1;  // the ternary clauses have to fullfill further properties to enable the XOR
                        //                     gate to be blocked!
                    }
                    if (countPos == 2) {
                        DOUT(if (config.circ_debug_out) cerr << "c current XOR gate is implied with blocked clauses! ternaries: " << countPos
                                                             << endl;);
                ...
                if (var(a) < var(c) && var(a) < var(b)) {
                    ...
                    DOUT(if (config.circ_debug_out) cerr << "c current XOR gate is fully encodeds: " << found[0] << "," << found[1] << "," << found[2]
                                                         << "," << found[3] << "," << endl;);
        ...
        for (int i = oldGates + 1; i < gates.size(); ++i) {
            for (int j = oldGates; j < i; ++j) {
                if ( // all variables are the same
                    var(gates[i].a()) == var(gates[j].a()) && ((var(gates[i].b()) == var(gates[j].b()) && var(gates[i].c()) == var(gates[j].c())) ||
    ...
    Circuit::Gate::Gate(const Clause& c, const Coprocessor::Circuit::Gate::Type _type, const Coprocessor::Circuit::Gate::Encoded e, const Lit& output)
        ...
        if (_type == Gate::GenAND || _type == Gate::ExO) {
            ...
            for (int i = 0; i < c.size(); ++i)
                if (c[i] != output) {
                    data.e.externLits[j++] = (type == Gate::ExO) ? c[i] : ~c[i];
                }
    ...
    Circuit::Gate::Gate(const vector<Lit>& c, const Coprocessor::Circuit::Gate::Type _type, const Coprocessor::Circuit::Gate::Encoded e,
                        const Lit& output)
        : inQueue(false)
        , touched(0)
        , type(_type)
    ...
    Circuit::Gate::Gate(const Lit& _x, const Lit& _a, const Lit& _b, const Coprocessor::Circuit::Gate::Type& _type,
                        const Coprocessor::Circuit::Gate::Encoded& e)
        : inQueue(false)
        , touched(0)
        , type(_type)
        , encoded(e) {
