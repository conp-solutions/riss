...
#error clear graph class
        ...
        for (int i = 0; i < communities->getSize(); ++i) {
            ...
            for (int n = 0; n < neigborCommunities.size(); ++n) {
                ...
                if (neigborCommunities[n] < i) {
                    continue;
                } // solver each pair only once!
                ...
                for (int selector = 0; selector < 2; ++selector) {
                    ...
                    for (int j = 0; j < currentCommunity.size();
                         ++j) { // for the given community, collect all clauses with a variable of this community!
                        Var v = currentCommunity[j];
                        if (data.ma.isCurrentStep(v)) {
                            continue;
                        } // saw variable already
                        ...
                        for (int p = 0; p < 2; ++p) {
                            ...
                            for (int k = 0; k < data.list(l).size(); ++k) {
                                ...
                                if (c.can_be_deleted()) {
                                    continue;
                                } // jump over ignored clauses
                                assert(c.size() > 1 && "there should not be unit clauses in the formula");
                                if (l != c[0]) {
                                    continue;
                                }
                                checkSolver.addClause(
                                    ca[data.list(l)[k]]); // only attach clauses where the current variable is the first element of the clause!

                                for (int m = 0; m < c.size();
                                     ++m) { // collect all variables that are added to the solver to clean a subset of the watch lists afterwards
                ...
                if (status == l_False || totalConflictLimit < totalConflicts) {
                    break;
                }

                vec<Lit>& trail = checkSolver.trail;
                for (int t = 0; t < trail.size(); ++t) {
                    if (data.value(trail[t]) == l_False) {
                        data.setFailed();
                    }
                    if (data.value(trail[t]) == l_Undef) {
                        data.enqueue(trail[t]);
                    }
                }
    ...
    bool ModPrep::getRandomCommunities(int randomCommunities, vector<int>& communityPerVariable, SimpleGraph*& communities,
            ...
            for (int j = 0; j < community.size(); ++j) {   // list with all variables of community i
                ...
                for (int k = 0; k < adj.size(); ++k) {
                    if (communityPerVariable[adj[k]] > i) { // neighbors in one directions!
                        ...
                        communityNeighbors->addUndirectedEdge(
                            i, communityPerVariable[adj[k]]); // community i and communityPerVariable[adj[k]] are neighbors
                    }
                    step++;
                    if (step > stepLimit) {
                        delete communityNeighbors;
                        communityNeighbors = nullptr;
                        delete communities;
                        communities = nullptr;
                        delete vigGraph;
                        vigGraph = nullptr;
        ...
        for (int i = 0; i < solver.clauses.size(); ++i) {
            ...
            for (int j = 0; j < c.size(); ++j) {
                ...
                for (int k = j + 1; k < c.size(); ++k) {
                    vigGraph->addDirectedEdgeAndInvertedEdge(v, var(c[k]),
                                                             1); // with undirected edges there would be problems finding features(e.g. diameter)
                }
                step++;
