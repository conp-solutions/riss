...
namespace Coprocessor {
    ...
    bool BoundedVariableAddition::process() {
        ...
        if (data.nVars() < config.opt_bva_VarLimit || !data.unlimited()) {
            ...
            if (data.unlimited() ||
                ...
                } else if (config.opt_Xbva == 2) {
                    modifiedFormula = xorBVAfull() || modifiedFormula;
                }
            }
            if (data.unlimited() ||
                ...
                } else if (config.opt_Ibva == 2) {
                    modifiedFormula = iteBVAfull() || modifiedFormula;
                }
            }
        }
    ...
    bool BoundedVariableAddition::andBVA() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (data[mkLit(v, false)] > 2)
                if (!bvaHeap.inHeap(toInt(mkLit(v, false)))) {
                    bvaHeap.insert(toInt(mkLit(v, false)));
                }
            if (data[mkLit(v, true)] > 2)
                if (!bvaHeap.inHeap(toInt(mkLit(v, true)))) {
                    bvaHeap.insert(toInt(mkLit(v, true)));
                }
        }
        ...
        while (bvaHeap.size() > 0 && (data.unlimited() || bvaALimit > andMatchChecks) && !data.isInterupted()) {
            ...
            DOUT(if (config.bva_debug > 1) cerr << "c next major loop iteration with heapSize " << bvaHeap.size() << endl;);

            DOUT(if (config.bva_debug > 3) if (checkLists("check data structure integrity"))
            ...
            DOUT(if (config.bva_debug > 2 && bvaHeap.size() > 0) cerr << "c [BVA] new first item: " << bvaHeap[0] << " which is " << right << endl;);
            bvaHeap.removeMin();

            // TODO: check for literals that are frozen
            // if( search.eliminated.get( right.toVar() ) ) continue;

            if (data.value(right) != l_Undef) {
                continue;
            }

            bvaMatchingLiterals.clear();
            if (data.list(right).size() < 3) {
                continue;
            }
            ...
            uint32_t index = 0;
            while (bvaMatchingClauses.size() < index + 1) {
                bvaMatchingClauses.push_back(vector<CRef>());
            }
            ...
            DOUT(if (config.bva_debug > 2) cerr << "c BVA test right: " << right << endl;);
            ...
            while (true) {
                ...
                for (uint32_t i = 0; i < bvaMatchingClauses[0].size() && (data.unlimited() || bvaALimit > andMatchChecks) && !data.isInterupted();
                     ++i) {
                    DOUT(if (config.bva_debug > 2) cerr
                             << "c next reduce literal loop iteration with matching clauses: " << bvaMatchingClauses[0].size() << endl;);
                    // reserve space
                    while (bvaMatchingClauses.size() < index + 1) {
                        bvaMatchingClauses.push_back(vector<CRef>());
                    }
                    ...
                    DOUT(if (!rightInFlag && config.bva_debug > 3) {
                        cerr << "literal " << right << " is not part of the clause " << clauseC << " , which is learned(" << clauseC.learnt()
                             << ") and canBeDeleted(" << clauseC.can_be_deleted() << ")" << endl;
                    });
                    ...
                    if (data.list(l1).size() < 2) {
                        continue;
                    } // there has to be something to be joined
                    ...
                    for (uint32_t j = 0; j < data.list(l1).size(); ++j) {
                        ...
                        if (clauseD.can_be_deleted()) {
                            continue;
                        }
                        if (clauseC.size() != clauseD.size()) {
                            continue;
                        } // TODO: relax later
                        ...
                        DOUT(if (config.bva_debug > 3) cerr << "c check count for literal " << match
                                                            << " where countSize is mark: " << bvaCountMark.size()
                                                            << " count:" << bvaCountCount.size() << " index: " << toInt(match) << endl;);
                        // count found match
                ...
                DOUT(if (config.bva_debug > 2) cerr << "c BVA selected left: " << left << " foundRC=" << foundRightComplement
                                                    << ", bvaCmpl=" << bvaComplement << endl;);
                // tackle complement specially, if found
                if (bvaComplement && foundRightComplement) {
                    left = ~right;
                }
                ...
                if (left == ~right && bvaComplement) {
                    didSomething = true;
                    DOUT(if (config.bva_debug > 1) cerr << "c [BVA] found complement " << left << endl;);
                    if (!bvaHandleComplement(right, bvaHeap)) {
                        ...
                        if (l_False == propagation.process(data, true)) {
                            return didSomething;
                        } // this destroys the properties of the BVA heap!
                    }
                    andComplementCount++;
                    if (!bvaHeap.inHeap(toInt(right))) {
                        bvaHeap.insert(toInt(right));
                    } else {
                        bvaHeap.update(toInt(right));
                    }
                    if (!bvaHeap.inHeap(toInt(left))) {
                        bvaHeap.insert(toInt(left));
                    } else {
                        bvaHeap.update(toInt(left));
                    }
                    // if complement has been found, this complement is treated and the next literal is picked
                    DOUT(if (config.bva_debug > 1 && bvaHeap.size() > 0) cerr << "c [BVA] heap(" << bvaHeap.size()
                                                                              << ") with at 0: " << toLit(bvaHeap[0]) << endl;);
                    break;
                }
                ...
                if (index == 1 && max < 3) {
                    DOUT(if (config.bva_debug > 2) cerr << "c [BVA] interrupt because of too few matchings " << right << " @" << index << " with "
                                                        << max << endl;);
                    break;
                }
                DOUT(if (config.bva_debug > 0) cerr << "c [BVA] index=" << index << " max=" << max << " rightList= " << data.list(right).size()
                                                    << " leftList= " << data.list(left).size() << endl;);

                // heuristically remove duplicates!
                if (var(left) >= data.nVars())
                    cerr << "c [BVA] working on too large variable " << var(left) << " vs " << data.nVars() << " literal L: " << left
                         << " int: " << toInt(left) << " literal R: " << right << " int: " << toInt(right) << endl;
                if (max > data.list(left).size() || max > data.list(right).size()) {
                    DOUT(if (config.bva_debug > 1) cerr << "c remove duplicate clauses from left list " << left << " with index= " << index << endl;);
                    ...
                    DOUT(if (config.bva_debug > 1) {
                        ...
                        for (uint32_t j = 0; j < data.list(left).size(); ++j) {
                            cerr << data.list(left)[j] << " : " << ca[data.list(left)[j]] << endl;
                        }
                    });
                    index--;
                    continue; // redo the current literal!
                }
                ...
                while (bvaMatchingClauses.size() < bvaMatchingLiterals.size()) {
                    bvaMatchingClauses.push_back(vector<CRef>());
                }
                ...
                for (uint32_t i = 0; i < rightList.size(); ++i) {
                    ...
                    DOUT(if (config.bva_debug > 2 && clauseC.size() < 2) cerr << "c [BVA] unit clause in right clauses (" << clauseC.size() << " "
                                                                              << endl;);
                    if (clauseC.size() < 2) {
                        continue;
                    }
                    ...
                    for (uint32_t j = foundCurrentMatches; j < leftList.size(); ++j) {
                        ...
                        DOUT(if (config.bva_debug > 2 && clauseD.size() < 1) cerr << "c [BVA] found unit clause in list" << left << endl;);

                        CRef tmp2 = CRef_Undef;
                        if (clauseD.size() != clauseC.size()) {
                            continue;
                        } // TODO: relax later!
                        for (uint32_t k = 0; k < clauseD.size(); ++k) {
                    ...
                    if (foundCurrentMatches == oldFoundCurrentMatches) {
                        bvaMatchingClauses[0][i] = CRef_Undef;
                    }
                }
            ...
            if (bvaMatchingLiterals.size() < 2) {
                if (bvaHeap.size() > 0) {
                    DOUT(if (config.bva_debug > 2) cerr << "c [BVA] continue because not two matching literals (" << right << "), bvaHeap["
                                                        << bvaHeap.size() << "],0=" << toLit(bvaHeap[0]) << endl;);
                }
                continue;
            }
            ...
            if (data.isInprocessing()) {
                int learnRows = 0;
                {
                    ...
                    for (uint32_t j = 0; j < iClauses.size(); ++j) {
                        if (iClauses[j] == CRef_Undef) {
                            continue;
                        } // don't bvaHeap with 0-pointers
                        const Clause& clauseI = ca[iClauses[j]];
                        if (clauseI.can_be_deleted()) {
                            continue;
                        }
                        ...
                        for (uint32_t k = 1; k < bvaMatchingLiterals.size(); ++k) {
                            if (!ca[bvaMatchingClauses[k][j]].learnt()) {
                                learntCol++;
                                if (learntCol >= 2) {
                                    break;
                                }
                            } // count number of nice replacements
                        }
                        if (learntCol >= 2) {
                            learnRows++;
                        }
                ...
                if (learnRows < 3) {
                    continue;
                } // do not rewrite this literal, if the current matching without learned clauses would not work. Note: might end in infinite loop, if
                  // unlimited
            }
            ...
            for (uint32_t i = 0; i < bvaMatchingLiterals.size(); ++i) {
                ...
                if (i == 0) { // clauses that are in the occurrenceList list of right
                    ...
                    for (uint32_t j = 0; j < iClauses.size(); ++j) {
                        ...
                        DOUT(if (config.bva_debug > 2 && clauseI.size() < 2) cerr << "c [BVA] unit clause for replacement (" << clauseI.size() << ")"
                                                                                  << endl;);
                        assert(!clauseI.can_be_deleted() && "matching clause is ignored");

                        DOUT(if (config.bva_debug > 2) cerr << "c rewrite clause[ " << iClauses[j] << " ]= " << clauseI << endl;);
                        if (data.outputsProof()) { // memorize original clause to be able to delete it afterwards
                            clauseLits.clear();
                            for (uint32_t k = 0; k < clauseI.size(); ++k) {
                                clauseLits.push(clauseI[k]);
                            }
                        }
                        ...
                        for (uint32_t k = 0; k < clauseI.size(); ++k) {
                            if (clauseI[k] == right) {
                                DOUT(if (config.bva_debug > 2) cerr << "c replace literal " << clauseI[k] << " with literal " << replaceLit << endl;);
                        ...
                        if (clauseI.size() == 1) {
                            ...
                            if (didIt) {
                                cerr << "c BVA clause with replaced literal is unit" << endl;
                                didIt = false;
                            }
                        ...
                        if (!data.removeClauseFrom(iClauses[j], right)) {
                            cerr << "c was not able to find clause ref " << iClauses[j] << " (" << clauseI << ") in list of literal " << right
                                 << " (was rewritten)" << endl;
                        }
                        ...
                        DOUT(if (config.bva_debug > 2) cerr << "c into pre-sort clause[ " << iClauses[j] << " ]= " << clauseI << endl;);
                        // clauseI.sort();
                        DOUT(if (config.bva_debug > 2) cerr << "c into clause[ " << iClauses[j] << " ]= " << clauseI << endl;);
            ...
            DOUT(if (config.bva_debug > 2) cerr << "c [BVA] current reduction: " << currentReduction << " subOr: " << bvaSubstituteOr << endl;);
            if (bvaSubstituteOr && currentReduction > 1) { // -newX = (a AND b), thus newX = (-a OR -b). replace all occurrences!
                ...
                for (uint32_t i = 0; i < bvaMatchingLiterals.size(); ++i) {
                    ...
                    DOUT(if (config.bva_debug > 3) cerr << "c [BVA] mark literal for subOr:  " << literal << endl;);
                ...
                DOUT(if (config.bva_debug > 2) cerr << "c [BVA] smallest Or literal: " << min << endl;);
                ...
                for (uint32_t i = data.list(min).size(); i > 0; --i) {
                    ...
                    if (clause.can_be_deleted() || clause.size() < bvaMatchingLiterals.size()) {
                        continue;
                    }
                    // check whether all literals are matched!
                    DOUT(if (config.bva_debug > 2) cerr << "c [BVA] SubOr check clause " << clause << endl;);
                    uint32_t count = 0;
                    ...
                    DOUT(if (config.bva_debug > 2) cerr << "c [BVA] SubOr found matching literals " << count << endl;);
                    // reject current clause
                    if (count != bvaMatchingLiterals.size()) {
                        continue;
                    }

                    if (!addedProofAnd &&
                        data.outputsProof()) { // add the clause that is necessary for the proof and. yet, the rewrite literal is pure in the formula.
                        addedProofAnd = true;
                        clauseLits.clear();
                    ...
                    // addedAnd = true;
                    {
                        DOUT(if (config.bva_debug > 2) cerr << "c rewrite clause[ " << clRef << " ]= " << clause << endl;);
                        andReplacedOrs++;
                        ...
                        if (data.outputsProof()) {
                            ...
                            for (int k = 0; k < clause.size(); ++k) {
                                clauseLits.push(clause[k]);
                            } // memorize clause
                        }
                        for (; j < clause.size(); ++j) {
                            const Lit literal = clause[j];
                            if (data.ma.isCurrentStep(toInt(literal))) {
                                DOUT(if (config.bva_debug > 2) cerr << "c replace literal " << clause[j] << " with " << ~replaceLit << endl;);
                        ...
                        DOUT(if (config.bva_debug > 2) cerr << "c found first literal at " << j << "/" << clause.size() << endl;);
                        for (; j < clause.size(); ++j) {
                            const Lit literal = clause[j];
                            DOUT(if (config.bva_debug > 2) cerr << "c check literal " << clause[j] << endl;);
                            if (data.ma.isCurrentStep(toInt(literal))) {
                                DOUT(if (config.bva_debug > 2) cerr << "c remove literal " << clause[j] << endl;);
                                data.removeClauseFrom(clRef, literal);
                                if (doSort) {
                                    clause.removePositionUnsorted(j);
                                } else {
                                    clause.removePositionSorted(j);
                                }
                                data.removedLiteral(literal);
                        ...
                        DOUT(if (config.bva_debug > 2) cerr << "c into clause[ " << clRef << " ]= " << clause << endl;);
                    }
                    ...
                    if (!addedAnd) {
                        ...
                        DOUT(if (config.bva_debug > 2) cerr << "add new ANDclause[ " << tmpRef << " ]= " << ca[tmpRef] << endl;);
                    }
                }
            }
            ...
            for (uint32_t i = 0; i < bvaMatchingLiterals.size(); ++i) {
                ...
                DOUT(if (config.bva_debug > 2) cerr << "add BVA clause " << ca[newClause] << endl;);
            }

            if (bvaPush > 0) {
                DOUT(if (config.bva_debug > 1 && bvaHeap.inHeap(toInt(right))) cerr << "c [BVA] add item that is not removed: " << right << endl;);
                if (bvaHeap.inHeap(toInt(right))) {
                    bvaHeap.update(toInt(right));
                } else {
                    bvaHeap.insert(toInt(right));
            ...
            if (bvaPush > 1) {
                if (bvaMatchingLiterals.size() > 2) {
                    if (bvaHeap.inHeap(toInt(mkLit(newX, false)))) {
                        bvaHeap.update(toInt(mkLit(newX, false)));
                    } else {
                        bvaHeap.insert(toInt(mkLit(newX, false)));
                    }
                }
            }
            DOUT(if (config.bva_debug > 1 && bvaHeap.size() > 0) cerr << "c [BVA] heap(" << bvaHeap.size() << ") with at 0: " << toLit(bvaHeap[0])
                                                                      << endl;);
    ...
    bool BoundedVariableAddition::xorBVAhalf() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            ...
            if (data[mkLit(v, true)] >= replacePairs)
                if (!bvaHeap.inHeap(toInt(mkLit(v, true)))) {
                    bvaHeap.insert(toInt(mkLit(v, true)));
                }
        }
        ...
        while (bvaHeap.size() > 0 && (data.unlimited() || bvaXLimit > xorMatchChecks) && !data.isInterupted()) {
            ...
            for (uint32_t j = 0; j < data.list(right).size() && !data.isInterupted(); ++j) { // iterate over all candidates for C
                ...
                if (c.can_be_deleted() || c.size() < smallestSize) {
                    continue;
                }

                DOUT(if (config.opt_bvaAnalysisDebug > 3) cerr << "c work on clause " << c << endl;);
                data.ma.nextStep();
                ...
                for (int k = 0; k < c.size(); ++k) {
                    const Lit l1 = c[k];
                    if (toInt(l1) <= toInt(right)) {
                        continue;
                    } // only bigger literals!
                    ...
                    for (uint32_t m = 0; m < data.list(~l1).size() && (data.unlimited() || bvaXLimit > xorMatchChecks); ++m) {
                        if (data.list(~l1)[m] == data.list(right)[j]) {
                            continue;
                        } // C != D
                        if (data.list(right)[j] > data.list(~l1)[m]) {
                            continue;
                        } // find each case only once!

                        const Clause& d = ca[data.list(~l1)[m]];
                        xorMatchChecks++;
                        if (d.can_be_deleted() || d.size() != c.size()) {
                            continue;
                        } // |D| == |C|
                        ...
                        if (!doesMatch) {
                            continue;
                        } // check next candidate for D!
                        DOUT(if (config.opt_bvaAnalysisDebug > 3) cerr << "c data.ma with clause " << d << endl;);
                    ...
                    if (doesMatch) {
                        break;
                    } // do not collect all pairs of this clause!
                }
            }
            ...
            for (int i = 0; i < xorPairs.size(); ++i) {
                int j = i;
                while (j < xorPairs.size() && toInt(xorPairs[i].l2) == toInt(xorPairs[j].l2)) {
                    ++j;
                }
                assert(j >= i);
                if (j - i >= replacePairs) {
            ...
            if (maxR >= replacePairs) {
                ...
                DOUT(if (config.opt_bvaAnalysisDebug > 0) {
                    if (config.opt_bvaAnalysisDebug) {
                        cerr << "c found XOR matching with " << maxJ - maxI << " pairs for (" << xorPairs[maxI].l1 << " -- " << xorPairs[maxI].l2
                             << ")" << endl;
                    }
                    if (config.opt_bvaAnalysisDebug > 1) {
                        for (int k = maxI; k < maxJ; ++k) {
                            cerr << "c p " << k - maxI << " : " << ca[xorPairs[k].c1] << " and " << ca[xorPairs[k].c2] << endl;
                        }
                    }
                });
                ...
                DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c introduce new variable " << newX + 1 << endl;);

                didSomething = true;
                for (int k = maxI; k < maxJ; ++k) {
                    ca[xorPairs[k].c2].set_delete(true); // all second clauses will be deleted, all first clauses will be rewritten
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA deleted " << ca[xorPairs[k].c2] << endl;);
                    data.removedClause(xorPairs[k].c2);
                    Clause& c = ca[xorPairs[k].c1];
                    if (!ca[xorPairs[k].c2].learnt() && c.learnt()) {
                        c.set_learnt(false);
                    } // during inprocessing, do not remove other important clauses!
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA rewrite " << c << endl;);
                    for (int ci = 0; ci < c.size(); ++ci) { // rewrite clause
                        if (c[ci] == xorPairs[k].l1) {
                            c[ci] = mkLit(newX, false);
                    ...
                    c.sort();
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA into " << c << endl;);
                    data.removeClauseFrom(xorPairs[k].c1, xorPairs[k].l1);
                    data.removeClauseFrom(xorPairs[k].c1, xorPairs[k].l2);
                    data.removedLiteral(xorPairs[k].l1);
                    data.removedLiteral(xorPairs[k].l2);
                ...
                data.addClause(tmpRef);
                data.getClauses().push(tmpRef);
                DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA added " << ca[tmpRef] << endl;);
                data.lits[1] = ~data.lits[1];
                data.lits[2] = ~data.lits[2];
            ...
            DOUT(if (config.opt_bvaAnalysisDebug > 3 && checkLists("XOR: check data structure integrity")) {
    ...
    bool BoundedVariableAddition::xorBVAfull() {
        ...
        while (bvaHeap.size() > 0 && (data.unlimited() || bvaXLimit > xorMatchChecks) && !data.isInterupted()) {
            ...
            bvaHeap.removeMin();
            if (data.value(right) != l_Undef || data.list(right).size() <= 3) {
                continue;
            } // abort iterations where there are not enough clauses
            // TODO: how to do symmetry breaking; avoid redundant work?

            xorPairs.clear();
            // cerr << "c collect for pos. lit " << right << endl;
            DOUT(if (config.opt_bvaAnalysisDebug > 2) cerr << "c analysis on " << right << endl;);
            for (uint32_t j = 0; j < data.list(right).size() && !data.isInterupted() && (data.unlimited() || bvaXLimit > xorMatchChecks);
                 ++j) {                                    // iterate over all candidates for C
                const Clause& c = ca[data.list(right)[j]]; // clause C that contains the variable right
                if (c.can_be_deleted() || c.size() < smallestSize || c.learnt()) {
                    continue;
                }
                ...
                for (int k = 0; k < c.size(); ++k) {
                    ...
                    for (uint32_t m = 0; m < data.list(~l1).size() && (data.unlimited() || bvaXLimit > xorMatchChecks); ++m) {
                        ...
                        xorMatchChecks++;
                        if (d.can_be_deleted() || d.size() != c.size() || d.learnt()) {
                            continue;
                        } // |D| == |C|
                ...
                if (xorPairs.size() > config.opt_Xbva_maxRed && !data.unlimited()) {
                    break;
                } // do not collect too many matching pairs
            }
            // evaluate matches here!
            // sort based on second literal -- TODO: use improved sort!
            // sort( xorPairs.begin(), xorPairs.end() );
            ...
            for (uint32_t j = 0; j < data.list(nRight).size() && !data.isInterupted() && (data.unlimited() || bvaXLimit > xorMatchChecks);
                 ++j) {                                     // iterate over all candidates for C
                ...
                for (int k = 0; k < c.size(); ++k) {
                    ...
                    bool doesMatch = true;
                    for (uint32_t m = 0; m < data.list(~l1).size() && (data.unlimited() || bvaXLimit > xorMatchChecks); ++m) {
                        if (data.list(~l1)[m] == data.list(nRight)[j]) {
                            continue;
                        } // C != D
                        if (data.list(nRight)[j] > data.list(~l1)[m]) {
                            continue;
                        } // find each case only once!
                        ...
                        for (int r = 0; r < d.size(); ++r) {
                            const Lit dl = d[r];
                            if (var(dl) == var(l1) || dl == ~nRight) {
                                continue;
                            } // has to be complement of nRight!
                            if (!data.ma.isCurrentStep(toInt(dl))) {
                                doesMatch = false;
                                break;
                            }
                        ...
                        DOUT(if (config.opt_bvaAnalysisDebug > 3) cerr << "c data.ma with clause " << d << endl;);
                        DOUT(if (config.opt_bvaAnalysisDebug > 2) cerr << "c match for (" << nRight << "," << l1 << ") -- (" << ~nRight << "," << ~l1
                                                                       << "): " << c << " and " << d << endl;);
            ...
            DOUT(if (config.opt_bvaAnalysisDebug > 2) {
                cerr << "c negative match pairs (presort)[" << nxorPairs.size() << " ]: " << endl;
                for (int i = 0; i < nxorPairs.size(); ++i) {
                    cerr << "[" << i << "]: " << nxorPairs[i].l1 << " -- " << ca[nxorPairs[i].c1] << "  VS " << nxorPairs[i].l2 << " -- "
                         << ca[nxorPairs[i].c2] << endl;
                }
            });
            ...
            for (int i = 0; i < nxorPairs.size(); ++i) {
                int j = i;
                while (j < nxorPairs.size() && toInt(nxorPairs[i].l2) == toInt(nxorPairs[j].l2)) {
                    ++j;
                }
                assert(j >= i);
                DOUT(if (config.opt_bvaAnalysisDebug > 1) cerr << "c for literal " << nxorPairs[i].l2 << " add value " << (j - i) << endl;);
                posLitCount[toInt(nxorPairs[i].l2)] += (j - i); // store the value!
                nmaxR = nmaxR > (j - i) ? nmaxR : j - i;
                i = j - 1; // jump to next matching
            }

            DOUT(if (config.opt_bvaAnalysisDebug > 1) cerr << "c found nagetaive: " << nmaxR << " for " << right << endl;);
            ...
            DOUT(if (config.opt_bvaAnalysisDebug > 1) cerr << "c found positive: " << maxR << " /" << replacePairs << " for " << right << endl;);
            ...
            if (maxR >= replacePairs) {
                ...
                for (; nmaxI < nxorPairs.size(); ++nmaxI)
                    if (nxorPairs[nmaxI].l2 == l2) {
                        break;
                    } // first literal has different polarity, hence, second needs same polarity!
                int nmaxJ = nmaxI > nxorPairs.size() ? nxorPairs.size() : nmaxI; // to find "upper bound"
                for (; nmaxJ < nxorPairs.size(); ++nmaxJ)
                    if (nxorPairs[nmaxJ].l2 != l2) {
                        break;
                    } // upper bound
                if (nmaxJ - nmaxI != posLitCount[toInt(l2)]) {
                    DOUT(if (config.opt_bvaAnalysisDebug > 0) cerr << "c numbers do not match for lit " << l2 << " s=" << nmaxI << " e=" << nmaxJ
                                                                   << " count= " << posLitCount[toInt(l2)]
                                                                   << " complCount= " << posLitCount[toInt(~l2)] << endl;);
                }
                ...
                DOUT(if (config.opt_bvaAnalysisDebug > 0) {
                    cerr << "c found pos XOR matching with " << maxJ - maxI << " pairs for (" << xorPairs[maxI].l1 << " -- " << xorPairs[maxI].l2
                         << ")" << endl;
                    if (config.opt_bvaAnalysisDebug > 1) {
                        for (int k = maxI; k < maxJ; ++k) {
                            cerr << "c p " << k - maxI << " : [" << xorPairs[k].c1 << "]" << ca[xorPairs[k].c1] << " and [" << xorPairs[k].c2 << "] "
                                 << ca[xorPairs[k].c2] << endl;
                        }
                    }
                    cerr << "c found neg XOR matching with " << nmaxJ - nmaxI << "(vs. " << nxorPairs.size() << ") pairs for ";
                    if (nxorPairs.size() > 0) {
                        cerr << "(" << nxorPairs[nmaxI].l1 << " -- " << nxorPairs[nmaxI].l2 << ")";
                    }
                    cerr << endl;
                    if (config.opt_bvaAnalysisDebug > 1) {
                        for (int k = nmaxI; k < nmaxJ; ++k) {
                            cerr << "c p " << k - nmaxI << " : [" << nxorPairs[k].c1 << "]" << ca[nxorPairs[k].c1] << " and [" << nxorPairs[k].c2
                                 << "]" << ca[nxorPairs[k].c2] << endl;
                        }
                    }
                });
                // apply replacing/rewriting here (right,l1) -> (x); add clauses (-x,right,l1),(-x,-right,-l1)
                ...
                didSomething = true;
                DOUT(if (config.opt_bvaAnalysisDebug) cerr << "positive occurrences ... " << endl;);
                for (int k = maxI; k < maxJ; ++k) {
                    ca[xorPairs[k].c2].set_delete(true); // all second clauses will be deleted, all first clauses will be rewritten
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA deleted [" << xorPairs[k].c2 << "] " << ca[xorPairs[k].c2] << endl;);
                    data.removedClause(xorPairs[k].c2);
                    Clause& c = ca[xorPairs[k].c1];
                    if (!ca[xorPairs[k].c2].learnt() && c.learnt()) {
                        c.set_learnt(false);
                    } // during inprocessing, do not remove other important clauses!
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA rewrite [" << xorPairs[k].c1 << "] " << c << endl;);
                    for (int ci = 0; ci < c.size(); ++ci) { // rewrite clause
                        if (c[ci] == xorPairs[k].l1) {
                            c[ci] = mkLit(newX, false);
                        } else if (c[ci] == xorPairs[k].l2) {
                ...
                DOUT(if (config.opt_bvaAnalysisDebug) cerr << "negative occurrences ... " << endl;);
                for (int k = nmaxI; k < nmaxJ; ++k) {
                    ca[nxorPairs[k].c2].set_delete(true); // all second clauses will be deleted, all first clauses will be rewritten
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA deleted [" << nxorPairs[k].c2 << "] " << ca[nxorPairs[k].c2] << endl;);
                    data.removedClause(nxorPairs[k].c2);
                    Clause& c = ca[nxorPairs[k].c1];
                    if (!ca[nxorPairs[k].c2].learnt() && c.learnt()) {
                        c.set_learnt(false);
                    } // during inprocessing, do not remove other important clauses!
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA rewrite [" << nxorPairs[k].c1 << "]" << c << endl;);
                    for (int ci = 0; ci < c.size(); ++ci) { // rewrite clause
                        if (c[ci] == nxorPairs[k].l1) {
                            c[ci] = mkLit(newX, true);
                    ...
                    c.sort();
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c XOR-BVA into [" << nxorPairs[k].c1 << "] " << c << endl;);
                    data.removeClauseFrom(nxorPairs[k].c1, nxorPairs[k].l1);
                    data.removeClauseFrom(nxorPairs[k].c1, nxorPairs[k].l2);
                    data.removedLiteral(nxorPairs[k].l1);
                    data.removedLiteral(nxorPairs[k].l2);
                    data.addedLiteral(mkLit(newX, true));
                ...
                if (maxI < maxJ) { // there are clauses for the positive to be introduced
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c introduce positive XBVA ... " << endl;);
                    data.lits.clear();
                    data.lits.push_back(mkLit(newX, true));
                    data.lits.push_back(xorPairs[maxI].l1);
                    data.lits.push_back(xorPairs[maxI].l2);
                ...
                if (nmaxI < nmaxJ) { // there are clauses for the negative to be introduced
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c introduce negative XBVA ... " << endl;);
                    data.lits.clear();
                ...
                if (config.opt_bva_push > 1 && data[mkLit(newX, true)] > replacePairs) {
                    if (!bvaHeap.inHeap(toInt(mkLit(newX, true)))) {
                        bvaHeap.insert(toInt(mkLit(newX, true)));
                    }
                }
                // stats
                xorfoundMatchings++;
    ...
    bool BoundedVariableAddition::iteBVAhalf() {
        ...
        bvaHeap.addNewElement(data.nVars() * 2);
        for (Var v = 0; v < data.nVars(); ++v) {
            if (data[mkLit(v, false)] >= replacePairs && data[mkLit(v, true)] >= replacePairs // add only, if both polarities occur frequently enough!
            ) {
                if (!bvaHeap.inHeap(toInt(mkLit(v, false)))) {
                    bvaHeap.insert(toInt(mkLit(v, false)));
                }
        ...
        while (bvaHeap.size() > 0 && (data.unlimited() || bvaILimit > iteMatchChecks) && !data.isInterupted()) {
            ...
            for (uint32_t j = 0; j < data.list(right).size() && !data.isInterupted(); ++j) { // iterate over all candidates for C
                ...
                for (int k = 0; k < c.size(); ++k) {
                    ...
                    if (l1 == right) {
                        continue;
                    } // TODO can symmetry breaking be applied?

                    data.ma.reset(toInt(l1)); // should have all literals except right and l1, but at least ~right, and some other literal! remaining
                                              // literals have to be equal!
                    ...
                    for (uint32_t m = 0; m < data.list(~right).size() && (data.unlimited() || bvaILimit > iteMatchChecks); ++m) {
                        if (data.list(~right)[m] == data.list(right)[j]) {
                            continue;
                        } // C != D
                        if (data.list(right)[j] > data.list(~right)[m]) {
                            continue;
                        } // find each case only once!
                        ...
                        for (int r = 0; r < d.size(); ++r) {
                            ...
                            if (dl == ~right) {
                                continue;
                            } // first literal, which does not hit!
                            if (!data.ma.isCurrentStep(toInt(dl))) {
                                if (matchLit == lit_Undef && var(dl) != var(l1)) {
                                    matchLit = dl;
                                }      // ensure that f and t have different variables
                        ...
                        DOUT(if (config.opt_bvaAnalysisDebug > 3) cerr << "c match with clause " << d << endl;);
                        // cerr << "c match for (" << right << "," << l1 << ") -- (" << ~right << "," << ~l1 << "): " << c << " and " << d << endl;
            ...
            for (int i = 0; i < itePairs.size(); ++i) {
                ...
                while (j < itePairs.size() &&
                       ( // if both literals match // TODO: have a more symmetry-breaking one here? there are 4 kinds of ITEs that can be represented
                         // with 4 variables, two work on the same output literal, combine them!
            ...
            if (maxR >= replacePairs) {
                ...
                for (int k = maxI; k < maxJ; ++k) {
                    ...
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c ITE-BVA deleted " << ca[itePairs[k].c2] << endl;);
                    data.removedClause(itePairs[k].c2);
                    Clause& c = ca[itePairs[k].c1];
                    if (!ca[itePairs[k].c2].learnt() && c.learnt()) {
                        c.set_learnt(false);
                    } // during inprocessing, do not remove other important clauses!
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c ITE-BVA rewrite " << c << endl;);
                    for (int ci = 0; ci < c.size(); ++ci) { // rewrite clause
                        if (c[ci] == itePairs[k].l1) {
                            c[ci] = mkLit(newX, false);
                    ...
                    c.sort();
                    DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c ITE-BVA into " << c << endl;);
                    data.removeClauseFrom(itePairs[k].c1, itePairs[k].l1);
                    data.removeClauseFrom(itePairs[k].c1, itePairs[k].l2);
                    data.removedLiteral(itePairs[k].l1);
                    data.removedLiteral(itePairs[k].l2);
                    data.addedLiteral(mkLit(newX, false));
                ...
                data.addClause(tmpRef);
                data.getClauses().push(tmpRef);
                DOUT(if (config.opt_bvaAnalysisDebug) cerr << "c ITE-BVA added " << ca[tmpRef] << endl;);
                data.lits[1] = ~data.lits[1];
                data.lits[2] = itePairs[maxI].l3;
                tmpRef = ca.alloc(data.lits, false); // no learnt clause!
                ca[tmpRef].sort();
                data.addClause(tmpRef);
                data.getClauses().push(tmpRef);
            ...
            DOUT(if (config.opt_bvaAnalysisDebug > 3 && checkLists("ITE: check data structure integrity")) {
                assert(false && "integrity of data structures has to be ensured");
            });
            ...
            for (uint32_t j = 0; j < data.list(right).size() && !data.isInterupted(); ++j) { // iterate over all candidates for C
                ...
                for (int k = 0; k < c.size(); ++k) {
                    ...
                    for (uint32_t m = 0; m < data.list(~right).size() && (data.unlimited() || bvaILimit > iteMatchChecks); ++m) {
                        ...
                        if (data.list(right)[j] > data.list(~right)[m]) {
                            continue;
                        } // symmetry breaking: find each case only once!
                ...
                if (itePairs.size() > config.opt_Ibva_maxRed && !data.unlimited()) {
                    break;
                } // do not collect too many matching pairs
            }
            // evaluate matches here!
            ...
            */
            ...
            for (int i = 0; i < itePairs.size(); ++i) {
                ...
                while (j < itePairs.size() &&
                       ( // if both literals match // TODO: have a more symmetry-breaking one here? there are 4 kinds of ITEs that can be represented
                         // with 4 variables, two work on the same output literal, combine them!
                           (toInt(itePairs[i].l2) == toInt(itePairs[j].l2) && toInt(itePairs[i].l3) == toInt(itePairs[j].l3)))) {
                    ++j;
                } // j exatly points behind the last hitting tuple
                assert(j >= i);

                // if( j + 1 < itePairs.size() ) cerr << "c following ITE s=" << itePairs[j+1].l1 << " f=" << itePairs[j+1].l2 << " t=" <<
                // itePairs[j+1].l3 << endl;
                int k = j;
                if (j + 1 < itePairs.size()) { // check for other half of the gate!
                    while (k < itePairs.size() &&
                           ( // if both literals match // TODO: have a more symmetry-breaking one here? there are 4 kinds of ITEs that can be
                             // represented with 4 variables, two work on the same output literal, combine them!
                ...
                if (k - i >= replacePairs) {
                    multipleMatches = maxR > 0; // set to true, if multiple matchings could be found
                    int thisR = k - i;
                    // cerr << "c found replaceable (" << thisR << ") ITE s=" << itePairs[j-1].l1 << " f=" << itePairs[j-1].l2 << " t=" <<
                    // itePairs[j-1].l3 << endl;
                    if (thisR > maxR) {
                        maxI = i;
                        maxJ = j;
    ...
    bool BoundedVariableAddition::bvaHandleComplement(const Lit& right, Heap<Coprocessor::LitOrderHeapLt>& bvaHeap) {
        ...
        DOUT(if (config.bva_debug > 2) cerr << "c [BVA] handle complements for literal " << right << endl;);
        for (uint32_t i = 0; i < data.list(right).size(); ++i) {
            ...
            DOUT(if (config.bva_debug > 2 && clauseC.size() < 2) cerr << "c [BVA] unit clause in complements" << endl;);
            if (clauseC.size() < 2) {
                continue;
            }
            data.ma.nextStep();
            ...
            for (uint32_t j = 0; j < data.list(left).size(); ++j) {
                ...
                DOUT(if (config.bva_debug > 1) { cerr << "c [CP2-BVAC] check clause " << j << " of literal " << left << ": " << ca[D] << endl; });
                for (uint32_t k = 0; k < clauseD.size(); ++k) {
                    if (!data.ma.isCurrentStep(toInt(clauseD[k]))) {
                        goto nextD2;
                    }
                ...
                if (!addedCalready) { // if C did not yet result in a clause
                    ...
                    if (data.outputsProof()) {
                        ...
                        for (int k = 0; k < clauseC.size(); ++k)
                            if (clauseC[k] != right) {
                                data.getSolver()->oc.push(clauseC[k]);
                            }
        ...
        for (uint32_t i = 0; i < data.clss.size(); ++i) {
            ...
            DOUT(if (config.bva_debug > 1) {
                cerr << "c [CP2-BVAC] remove " << right << " from (" << i << "/" << data.clss.size() << ")" << ca[data.clss[i]] << endl;
            });
    ...
    Var BoundedVariableAddition::nextVariable(char type, Heap<LitOrderHeapLt>& bvaHeap) {
        ...
        DOUT(if (config.bva_debug > 3) cerr << "c enlarge count mark,size and count to " << bvaCountMark.size() << endl;);

        return nextVar;
    }

    void BoundedVariableAddition::printStatistics(ostream& stream) {
        stream << "c [STAT] BVA " << processTime << " s, " << andTotalReduction + xorTotalReduction + iteTotalReduction << " reducedClauses, "
               << andReplacements + xorfoundMatchings + iteFoundMatchings << " freshVariables, " << endl;

        stream << "c [STAT] AND-BVA " << andTime << " seconds, " << andReplacements << " freshVars, " << andTotalReduction << " reducedClauses, "
               << andDuplicates << " duplicateClauses, " << andComplementCount << " complLits, " << andReplacedOrs << " orGSubs, "
               << andReplacedMultipleOrs << " multiOGS, " << andMatchChecks << " checks, " << endl;

        // print only if enabled!
        if (config.opt_Xbva)
            stream << "c [STAT] XOR-BVA " << xorTime << " seconds, " << xorfoundMatchings << " matchings, " << xorMultiMatchings << " multis, "
                   << xorTotalReduction << " reducedClauses, " << xorMatchSize << " matchSize, " << xorMaxPairs << " maxPair, " << xorMatchChecks
                   << " checks, " << endl;

        if (config.opt_Ibva)
            stream << "c [STAT] ITE-BVA " << iteTime << " seconds, " << iteFoundMatchings << " matchings, " << iteMultiMatchings << " multis, "
                   << iteTotalReduction << " reducedClauses, " << iteMatchSize << " matchSize, " << iteMaxPairs << " maxPair, " << iteMatchChecks
                   << " checks, " << endl;
    }
    ...
    void BoundedVariableAddition::removeDuplicateClauses(const Lit& literal) {
        ...
        for (uint32_t i = 0; i < data.list(literal).size(); ++i) {
            ...
            for (uint32_t j = i + 1; j < data.list(literal).size(); ++j) {
                ...
                if (clauseI.size() != clauseJ.size()) {
                    continue;
                }
                for (uint32_t k = 0; k < clauseI.size(); ++k) {
                    if (clauseI[k] != clauseJ[k]) {
                        goto duplicateNextJ;
                    }
                }
                ca[removeCandidate].set_delete(true); // set removed flag
                if (!ca[removeCandidate].learnt()) {
                    clauseI.set_learnt(false);
                } // take care of the learned flag
