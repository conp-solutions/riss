...
namespace Coprocessor {
    ...
    bool Symmetry::process() {
        ...
        if (!data.unlimited() && (data.nVars() > config.opt_symm_vars && data.getClauses().size() + data.getLearnts().size() > config.opt_symm_cls &&
                                  data.nTotLits() > config.opt_symm_lits)) {
            return true;
        }
        ...
        for (int i = 0; i < solver.clauses.size(); ++i) {
            ...
            for (int j = 0; j < c.size(); ++j) {
                Var v = var(c[j]);
                if (false && sign(c[j]) && config.sym_opt_hpol) {
                    varSymm[v].sub(c.size());
                } // take care of signs, is enabled
                else {
                    varSymm[v].add(c.size());
                }
                freq[v]++;
            }
        }
        ...
        if (false) {
            for (Var v = 0; v < solver.nVars(); ++v) {
                printf("c %4d[%4d] : %3d -- %3ld %3ld %3ld %3ld %3ld %3ld %3ld \n", v + 1, freq[v], varSymm[v].v + 1, varSymm[v].c2, varSymm[v].c3,
                       varSymm[v].c4, varSymm[v].c5, varSymm[v].c6, varSymm[v].c7, varSymm[v].cl);
            }
        }
        ...
        for (int iter = 0; iter < iteration; ++iter) {
            ...
            for (int i = 0; i < solver.clauses.size(); ++i) {
                ...
                for (int j = 0; j < c.size(); ++j) {
                    ...
                    thisClause = (sign(c[j]) && config.sym_opt_hpol ? lastIter[v].inv() : lastIter[v]) * factor +
                                 thisClause; // substract, if necessary! // be careful with order here, otherwise, variable will be overwritten!
                }
                for (int j = 0; j < c.size(); ++j) {
                    Var v = var(c[j]);
            ...
            if (false) {
                ...
                for (Var v = 0; v < solver.nVars(); ++v) {
                    printf("c %4d[%4d] : %3d -- %3ld %3ld %3ld %3ld %3ld %3ld %3ld \n", v + 1, freq[v], varSymm[v].v + 1, varSymm[v].c2,
                           varSymm[v].c3, varSymm[v].c4, varSymm[v].c5, varSymm[v].c6, varSymm[v].c7, varSymm[v].cl);
                }
            }
        }
        ...
        if (false) {
            for (Var v = 0; v < solver.nVars(); ++v) {
                printf("c %d : %3d -- %3ld %3ld %3ld %3ld %3ld %3ld %3ld \n", v + 1, thisIter[v].v + 1, thisIter[v].c2, thisIter[v].c3,
                       thisIter[v].c4, thisIter[v].c5, thisIter[v].c6, thisIter[v].c7, thisIter[v].cl);
            }
        }
        ...
        for (int i = 0; i < solver.nVars(); ++i) {
            ...
            while (j < solver.nVars() && thisIter[i] == thisIter[j]) {
                ++j;
            }
            if (j > i + 1) {
                int thisR = j - i;
                ...
                if (config.sym_opt_print) {
                    for (int k = 0; k < thisR; ++k) {
                        if (freq[thisIter[i + k].v] > 0) {
                            printf("c %4d[%4d] : %3d -- %3ld %3ld %3ld %3ld %3ld %3ld %3ld \n", thisIter[i + k].v + 1, freq[thisIter[i + k].v],
                                   thisIter[i + k].v + 1, thisIter[i + k].c2, thisIter[i + k].c3, thisIter[i + k].c4, thisIter[i + k].c5,
                                   thisIter[i + k].c6, thisIter[i + k].c7, thisIter[i + k].cl);
                        }
                    }
                }
                ...
                for (int k = 0; k < thisR; ++k) {
                    if (freq[thisIter[i + k].v] <= config.sym_opt_hmin || freq[thisIter[i + k].v] < (avgFreq * config.sym_opt_hratio)) {
                        continue;
                    }
                    ...
                    if (config.sym_opt_hprop) { // try to add clauses by checking propagation
                        ...
                        for (int m = thisR - 1; m > k; --m) {
                            if (freq[thisIter[i + m].v] <= config.sym_opt_hmin || freq[thisIter[i + m].v] < (avgFreq * config.sym_opt_hratio)) {
                                continue;
                            }                             // do not consider too small literals!
                            for (int o = 0; o < 4; ++o) { // check all 4 polarity combinations?
                                if (!config.sym_opt_hpropA && (o == 1 || o == 2)) {
                                    continue;
                                } // just check equivalence
                                const Lit l1 = mkLit(thisIter[i + k].v, o == 0 || o == 1);
                                const Lit l2 = mkLit(thisIter[i + m].v, o == 0 || o == 2);

                                int oldHead = solver.realHead;
                                if (solver.value(l1) != l_Undef) {
                                    continue;
                                } // already top level unit?
                                if (solver.value(l2) != l_Undef) {
                                    continue;
                                } // already top level unit?

                                solver.newDecisionLevel();
                                assert(solver.decisionLevel() == 1 && "can do this only on level 1");
                                solver.uncheckedEnqueue(l1);
                                solver.uncheckedEnqueue(l2); // double lookahead
                                bool entailed = false;
                                if (CRef_Undef != solver.propagate()) { // check if conflict, if yes, add new clause!
                                    if (solver.realHead > oldHead + 1) {
                                        entailed = true;
                                    } // only add clause, if ont already present!
                                }
                                ...
                                if (entailed) {
                                    ...
                                    assumptions.push(l1);
                                    assumptions.push(l2);
                                    DOUT(if (config.sym_debug_out > 1) cerr << "c search with assumptions [" << l1 << ", " << l2
                                                                            << "] starting at level " << solver.decisionLevel() << endl;);
                            ...
                            if (!config.sym_opt_hpropF) {
                                break;
                            } // stop after first iteration!
                        }
                    }

                    while (b > i + k && (freq[thisIter[b].v] <= config.sym_opt_hmin || freq[thisIter[b].v] < (avgFreq * config.sym_opt_hratio))) {
                        --b;
                    }
    ...
    void Coprocessor::Symmetry::printStatistics(ostream& stream) {
        stream << "c [STAT] SYMM " << processTime << " s, " << eqs << " symm-cands, " << maxEq << " maxSize, " << addPairs << " addedPairs "
               << symmAddClause << " entailedClauses, " << totalConflicts << " totCons, " << endl;
    }
