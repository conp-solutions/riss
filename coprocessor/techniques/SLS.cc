...
namespace Coprocessor {
    ...
    bool SLS::solve(const vec<CRef>& formula, uint64_t stepLimit) {
        ...
        if (!data.unlimited() && (data.nVars() > config.opt_sls_vars && data.getClauses().size() + data.getLearnts().size() > config.opt_sls_cls) &&
            data.nTotLits() > config.opt_sls_lits) {
            return false;
        }
        ...
        for (int i = 0; i < formula.size(); ++i) {
            ...
            for (int j = 0; j < c.size(); j++) {
                ...
                if (isSat(l)) {
                    if (satLits == 0) {
                        clsData[index].watch1 = var(l);
                    } else {
                        clsData[index].watch2 = var(l);
                    }
                    satLit = l;
                    satLits++;
                }
        ...
        if (minSize == maxSize && minSize > 2 && maxSize < 10) {
            stepLimit = config.opt_sls_ksat_flips == -1 ? 0 : config.opt_sls_ksat_flips;
        }

        DOUT(if (config.opt_sls_debug) {
            for (int i = 0; i < formula.size(); ++i) {
                const Clause& c = ca[formula[i]];
                if (c.can_be_deleted() || c.size() == 1) {
                    continue;
                }
                int satLits = 0;
                for (int j = 0; j < c.size(); ++j) {
                ...
                cerr << "c clause[" << i << "] " << c << " has " << clsData[i].satLiterals
                     << " satisfied literals, watches 1=" << clsData[i].watch1 + 1 << " 2=" << clsData[i].watch2 + 1 << endl;
            }
            ...
            for (int i = 0; i < unsatClauses.size(); ++i) {
                ...
                cerr << "c UNSAT clause[" << unsatClauses[i] << "] " << c << " has " << clsData[unsatClauses[i]].satLiterals
                     << " satisfied literals, watches 1=" << clsData[unsatClauses[i]].watch1 + 1 << " 2=" << clsData[unsatClauses[i]].watch2 + 1
                     << endl;
            }
        });
        ...
        for (flips = 0; (flips < stepLimit || stepLimit == 0) && !data.isInterupted() && unsatClauses.size() > 0; ++flips) {
            DOUT(if (config.opt_sls_debug) {
                for (int i = 0; i < unsatClauses.size(); ++i) {
                    if (clsData[unsatClauses[i]].satLiterals != 0) {
                        cerr << "c unsat clause has sat lits (according to counter) : [" << unsatClauses[i] << "]: " << ca[formula[unsatClauses[i]]]
                             << endl;
                    }
                }
            });
            ...
            DOUT(if (config.opt_sls_debug) {
                cerr << "c sat lits: ";
                for (Var v = 0; v < data.nVars(); ++v)
                    if (isSat(mkLit(v, false))) {
                        cerr << " " << v + 1;
                    }
                cerr << endl;
                cerr << "c unsat lits: ";
                for (Var v = 0; v < data.nVars(); ++v)
                    if (isUnsat(mkLit(v, false))) {
                        cerr << " " << v + 1;
                    }
                cerr << endl;
            });
            ...
            DOUT(if (config.opt_sls_debug) cerr << "c flip literal " << ~unsatLit << " to " << unsatLit << " , taken from clause " << c
                                                << " [left:" << unsatClauses.size() << "]" << endl;);
            ...
            for (int i = 0; i < negClauses.size(); ++i) {
                ...
                DOUT(if (config.opt_sls_debug) cerr << "c give this clause one less sat literal: [" << nIndex << "]: " << ca[formula[nIndex]]
                                                    << ", current satLits: " << clsData[nIndex].satLiterals << endl;);
                ...
                if (clsData[nIndex].satLiterals == 1) { // cls became sat
                    ...
                    if (v == clsData[nIndex].watch1) {
                        clsData[nIndex].watch1 = clsData[nIndex].watch2;
                    } // set other sat lit to watcher
                    ...
                    if (clsData[nIndex].watch1 == v || clsData[nIndex].watch2 == v) {
                        ...
                        for (int j = 0; j < c.size(); j++) {
                            ...
                            if (isSat(l)) {
                                if (satLits == 0) {
                                    clsData[nIndex].watch1 = var(l);
                                    satLits++;
                                } else {
                                    clsData[nIndex].watch2 = var(l);
                                    satLits++;
                                    break;
                                }
                            }
                        ...
                        DOUT(if (config.opt_sls_debug && satLits < 2) {
                            cerr << "c not enough sat lits for [" << nIndex << "]: " << c << " with set watches 1=" << clsData[nIndex].watch1 + 1
                                 << " 2=" << clsData[nIndex].watch2 + 1 << endl;
                            cerr << "c sat lits: ";
                            for (int j = 0; j < c.size(); j++) {
                                const Lit& l = c[j];
                                if (var(l) == v) {
