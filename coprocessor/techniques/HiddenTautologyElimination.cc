...
namespace Coprocessor {
    ...
    HiddenTautologyElimination::HiddenTautologyElimination(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller,
    ...
    void HiddenTautologyElimination::process(CoprocessorData& data) {
        ...
        if (lastModTime() == 0)
            ...
            data.getActiveVariables(lastModTime(), activeVariables);
        }
    ...
    void HiddenTautologyElimination::elimination_worker(CoprocessorData& data, uint32_t start, uint32_t end, BIG& big, bool doStatistics,
                                                        bool doLock) {
        if (data.randomized()) { // shake order, if randomized is wanted
            const uint32_t diff = end - start;
            for (uint32_t v = start; v < end; ++v) {
                int newPos = start + rand() % diff;
                const uint32_t tmp = activeVariables[v];
                activeVariables[v] = activeVariables[newPos];
                activeVariables[newPos] = tmp;
            }
        }
        ...
        DOUT(if (config.hte_debug_out > 3) cerr << "c allocate litQueue for " << data.nVars() * 2 << " elements at " << std::hex << litQueue
                                                << std::hex << endl;);
        MethodFree litQueueFree((void*&)litQueue); // will automatically free the resources at a return!

        if (config.opt_hteTalk) {
            fprintf(stderr, "c HTE from %d to %d out of %lud\n", start, end, activeVariables.size());
        }

        for (uint32_t index = start; index < end && !data.isInterupted(); ++index) {
            if (steps == 0 && !data.unlimited()) {
                break;
            } // stop if number of iterations has been reached
            const Var v = activeVariables[index];
            if (config.opt_hteTalk) {
                fprintf(stderr, "c HTE on variable %d\n", v + 1);
            }
    ...
    void HiddenTautologyElimination::parallelElimination(CoprocessorData& data, BIG& big) {
        ...
        if (!didIt) {
            cerr << "c parallel HTE can result in unsound formulas!" << endl;
            didIt = true;
        }
    ...
    bool HiddenTautologyElimination::hiddenTautologyElimination(Var v, CoprocessorData& data, BIG& big, MarkArray& hlaPositive,
                                                                MarkArray& hlaNegative, bool statistic, bool doLock) {
        ...
        for (uint32_t pol = 0; pol < 2; ++pol) {
            ...
            DOUT(if (config.hte_debug_out > 2) {
                ...
                for (Var v = 0; v < data.nVars(); ++v)
                    for (int p = 0; p < 2; ++p)
                        if (hlaArray.isCurrentStep(toInt(mkLit(v, p)))) {
                            printLit(mkLit(v, p));
                            fprintf(stderr, " ");
                        }
                fprintf(stderr, "\n");
            ...
            for (uint32_t k = 0; k < iList.size(); k++) {
                CRef clsidx = iList[k];
                Clause& cl = ca[clsidx];
                if (cl.can_be_deleted()) {
                    continue;
                } // there should not be any clauses left! still, make sure!
    ...
    Lit HiddenTautologyElimination::fillHlaArrays(Var v, BIG& big, MarkArray& hlaPositive, MarkArray& hlaNegative, Lit* litQueue, bool doLock) {
        ...
        for (uint32_t pol = 0; pol < 2; ++pol) {
            ...
            hlaArray.nextStep();
            if (config.opt_hteTalk) {
                cerr << "c [HTE] fill hla for " << i << endl;
            }
            ...
            for (uint32_t j = 0; j < posSize; ++j) {
                ...
                if (config.opt_hteTalk) {
                    cerr << "c [HTE] look at literal " << imp << endl;
                }
                if (hlaArray.isCurrentStep(toInt(imp))) {
                    continue;
                }
                ...
                *(head++) = imp;
                ...
                if (config.opt_hteTalk) {
                    cerr << "c [HTE] add to array: " << imp << endl;
                }
                // process queue
                while (tail < head) {
                    const Lit lit = *(tail++);
                    ...
                    for (uint32_t k = 0; k < kListSize; ++k) {
                        ...
                        if (!hlaArray.isCurrentStep(toInt(kLit))) {
                            if (hlaArray.isCurrentStep(toInt(~kLit))) {
                                if (config.opt_hteTalk) {
                                    cerr << "c [HTE] failed literal: " << i << endl;
                                }
                            ...
                            DOUT(if (config.hte_debug_out > 3) cerr << "c [HTE] put an element to the queue at position " << (int)(head - litQueue)
                                                                    << " with ptr " << std::hex << head << std::dec << endl;);
                            *(head++) = kLit;
                        }
                    }
                }
                if (config.opt_hteTalk) {
                    cerr << "c [HTE] remove from array: " << imp << endl;
                }
    ...
    bool HiddenTautologyElimination::hlaMarkClause(const Riss::CRef& cr, Coprocessor::BIG& big, MarkArray& markArray, Lit* litQueue) {
        ...
        if (clause.size() < 3) {
            return false;
        } // do not work on binary and smaller clauses!
        ...
        while (tail < head) {
            ...
            for (uint32_t j = 0; j < jListSize; ++j) {
                ...
                if (clause.size() == 2) {
                    ...
                    if (lit == clause[0] && ~jLit == clause[1]) {
                        continue;
                    }
                    if (lit == clause[1] && ~jLit == clause[0]) {
                        continue;
                    }
                }
    ...
    bool HiddenTautologyElimination::alaMarkClause(const Riss::CRef& cr, Coprocessor::CoprocessorData& data, MarkArray& markArray,
                                                   MarkArray& helpArray) {
        vec<Lit> lits;
        const Clause& c = ca[cr];
        for (int i = 0; i < c.size(); ++i) {
    ...
    bool HiddenTautologyElimination::alaMarkClause(vec<Lit>& clause, CoprocessorData& data, MarkArray& markArray, MarkArray& helpArray,
                                                   bool addLits) {
        helpArray.nextStep();
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                for (int j = 0; j < c.size(); ++j) {
                    ...
                    if (!markArray.isCurrentStep(toInt(l2))) {
                        if (l1 == lit_Undef) {
                            l1 = l2;
                        } // remember missing literal!
                        else {
                            l1 = lit_Error;
                            break;
                        } // there is more than one literal that does not fit!
                    }
                }
                if (l1 == lit_Undef) {
                    return true;
                } // ATE, since all literals are inside of the analyzed clause clause (is subsumed!)
                if (l1 != lit_Undef && l1 != lit_Error) {
                    if (markArray.isCurrentStep(toInt(l1))) {
                        return true;
                    } // found ATE, the clause can be removed from the formula!
                    if (!markArray.isCurrentStep(toInt(~l1))) {
                        if (addLits) {
                            clause.push(~l1);
                        } // add literal to array and to vector
    ...
    void HiddenTautologyElimination::printStatistics(ostream& stream) {
        stream << "c [STAT] HTE " << processTime << " s, " << removedClauses << " cls, " << removedLits << " lits, " << steps << " steps left"
               << endl;
    }
