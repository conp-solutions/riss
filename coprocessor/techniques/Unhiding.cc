...
namespace Coprocessor {
    ...
    Unhiding::Unhiding(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data, Propagation& _propagation,
                       Subsumption& _subsumption, EquivalenceElimination& _ee)
    ...
    uint32_t Unhiding::linStamp(const Lit& literal, uint32_t stamp, bool& detectedEE) {
        ...
        if (!uhdNoShuffle) {
            shuffleVector(adj, big.getSize(literal));
        }
        stampEE.push_back(literal);
        stampQueue.push_back(literal);
        level++;
        while (level >= unhideEEflag.size()) {
        ...
        while (!stampQueue.empty()) {
            ...
            if (adjSize > 0 && stampInfo[toInt(l)].lastSeen != lit_Undef && stampInfo[toInt(l)].index > 0 &&
                adj[stampInfo[toInt(l)].index - 1] != stampInfo[toInt(l)].lastSeen) {
                for (stampInfo[toInt(l)].index = 0; stampInfo[toInt(l)].index < adjSize; stampInfo[toInt(l)].index++) {
                    if (adj[stampInfo[toInt(l)].index] == stampInfo[toInt(l)].lastSeen) {
                        break;
                    }
                }
            ...
            for (; stampInfo[toInt(l)].index < big.getSize(l);) { // size might change by removing transitive edges!
                ...
                if (uhdTransitive && stampInfo[toInt(l)].dsc < stampInfo[toInt(l1)].obs) {
                    DOUT(if (config.opt_uhd_Debug > 4) cerr << "c [UHD-A] l.dsc=" << stampInfo[toInt(l)].dsc << " l1.obs=" << stampInfo[toInt(l1)].obs
                                                            << endl;);
                    data.removedClause(~l, l1);
                    modifiedFormula = true;
                    big.removeEdge(~l, l1);
                    DOUT(if (config.opt_uhd_Debug > 4) {
                        cerr << "c [UHD-A] remove transitive edge " << ~l << "," << l1 << " and reduce index to " << stampInfo[toInt(l)].index - 1
                             << endl;
                    });
                    --stampInfo[toInt(l)].index;
                    continue;
                ...
                if (stampInfo[toInt(stampInfo[toInt(l)].root)].dsc <= stampInfo[toInt(~l1)].obs) {
                    ...
                    if (data.value(~lfailed) == l_Undef) {
                        DOUT(if (config.opt_uhd_Debug > 1) cerr << "c derived unit " << ~lfailed << " with stamps pos(" << ~lfailed
                                                                ...
                                                                << " and neg (" << lfailed << "): " << stampInfo[toInt(lfailed)].dsc << " -- "
                                                                << stampInfo[toInt(lfailed)].fin << endl;);
                    }
                    if (data.value(~lfailed) != l_True) {
                        data.addCommentToProof("found during stamping in unhiding");
                        data.addUnitToProof(~lfailed);
                    } // add the unit twice?
                    if (l_False == data.enqueue(~lfailed, data.currentDependencyLevel())) {
                        return stamp;
                    }

                    if (stampInfo[toInt(~l1)].dsc != 0 && stampInfo[toInt(~l1)].fin == 0) {
                        continue;
                    }
                }

                DOUT(if (config.opt_uhd_Debug > 4) cerr << "c [UHD-A] enqueue " << l1 << " with dsc=" << stampInfo[toInt(l1)].dsc
                                                        << " index=" << stampInfo[toInt(l1)].index << " state ok? " << data.ok() << endl;);
                if (stampInfo[toInt(l1)].dsc == 0) {
                    ...
                    assert(stampInfo[toInt(l1)].obs == 0 && stampInfo[toInt(l1)].fin == 0 &&
                           "cannot have accessed literal already, if it is considered fresh");
                    /*
                    ...
                    */
                    ...
                    if (!uhdNoShuffle) {
                        shuffleVector(adj, big.getSize(l1));
                    }
                ...
                if (level > 0) { // if there are still literals on the queue
                    ...
                    if (uhdEE && stampInfo[toInt(l1)].fin == 0 && stampInfo[toInt(l1)].dsc < stampInfo[toInt(l)].dsc && !data.doNotTouch(var(l)) &&
                        !data.doNotTouch(var(l1))) {
                        stampInfo[toInt(l)].dsc = stampInfo[toInt(l1)].dsc;
                        DOUT(if (config.opt_uhd_Debug > 4) cerr << "c found equivalent literals " << l << " and " << l1 << ", set flag at level "
                                                                << level << " to 0!" << endl;);
            ...
            if (stampInfo[toInt(l)].index >= big.getSize(l)) {
                ...
                if (unhideEEflag[level] == 1 || level == 1) {
                    ...
                    do {
                        ...
                        DOUT(if (config.opt_uhd_Debug > 1 && l1 != l) cerr << "c collect EE literals " << l << " and " << l1 << " at level " << level
                                                                           << endl;);
                ...
                level--;
                if (level > 0) { // if there are still literals on the queue
                    DOUT(if (config.opt_uhd_Debug > 4) cerr << "c [UHD-A] return to level " << level << " and literal " << stampQueue.back()
                                                            << endl;);
                    ...
                    if (uhdEE && stampInfo[toInt(l)].fin == 0 && stampInfo[toInt(l)].dsc < stampInfo[toInt(parent)].dsc && !data.doNotTouch(var(l)) &&
                        !data.doNotTouch(var(parent))) {
                        stampInfo[toInt(parent)].dsc = stampInfo[toInt(l)].dsc;
                        DOUT(if (config.opt_uhd_Debug > 1) cerr << "c found equivalent literals " << parent << " and " << l << ", set flag at level "
                                                                << level << " to 0!" << endl;);
                        unhideEEflag[level] = 0;
                        detectedEE = true;
                    }
    ...
    uint32_t Unhiding::stampLiteral(const Lit& literal, uint32_t stamp, bool& detectedEE) {
        ...
        if (stampInfo[toInt(literal)].dsc != 0) {
            return stamp;
        }
    ...
    bool Unhiding::unhideSimplify(bool borderIteration, bool& foundEE) {
        ...
        DOUT(if (config.opt_uhd_Debug > 4) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                ...
                if (show) {
                    lit = ~lit;
                    cerr << "c " << lit << " : \tdsc=" << stampInfo[toInt(lit)].dsc << " \tfin=" << stampInfo[toInt(lit)].fin
                         << " \tobs=" << stampInfo[toInt(lit)].obs << " \troot=" << stampInfo[toInt(lit)].root
                         << " \tprt=" << stampInfo[toInt(lit)].parent << endl;
                    lit = ~lit;
                    cerr << "c " << lit << " : \tdsc=" << stampInfo[toInt(lit)].dsc << " \tfin=" << stampInfo[toInt(lit)].fin
                         << " \tobs=" << stampInfo[toInt(lit)].obs << " \troot=" << stampInfo[toInt(lit)].root
                         << " \tprt=" << stampInfo[toInt(lit)].parent << endl;
        ...
        for (uint32_t i = 0; i < data.getClauses().size() && !data.isInterupted(); ++i) {
            ...
            DOUT(if (config.opt_uhd_Debug > 4) {
                if (config.opt_uhd_Debug > 4) {
                    ...
                    for (uint32_t p = 0; p < cs; ++p) {
                        cerr << p << " " << Splus[p] << " dsc=" << stampInfo[toInt(Splus[p])].dsc << " fin=" << stampInfo[toInt(Splus[p])].fin
                             << endl;
                    };
                    cerr << "c Sminus: " << endl;
                    for (uint32_t n = 0; n < cs; ++n) {
                        cerr << n << " " << Sminus[n] << " dsc=" << stampInfo[toInt(Sminus[n])].dsc << " fin=" << stampInfo[toInt(Sminus[n])].fin
                             << endl;
                    };
                }
            ...
            if (doUHTE && clause.size() != 2) { // could check whether the direct edge is used
                ...
                while (true) {
                    ...
                    if (stampInfo[toInt(lneg)].dsc > stampInfo[toInt(lpos)].dsc) {
                        ...
                        lpos = Splus[++pp];
                        // line 7
                    } else if ((stampInfo[toInt(lneg)].fin < stampInfo[toInt(lpos)].fin) ||
                               (cs == 2 && ((lpos == ~lneg || stampInfo[toInt(lpos)].parent == lneg) || (stampInfo[toInt(~lneg)].parent == ~lpos)))) {
                        if (pn + 1 == cs) {
                            break;
                        }
                        lneg = Sminus[++pn];
                    } else {
                        DOUT(if (config.opt_uhd_Debug > 4) cerr
                                 << "c UHTE: lneg=" << lneg << " lpos=" << lpos << " finStamps (neg<pos): " << stampInfo[toInt(lneg)].fin << " < "
                                 << stampInfo[toInt(lpos)].fin << "  pos=neg.complement: " << lpos << " == " << ~lneg
                                 << " or lpos.par == lneg: " << stampInfo[toInt(lpos)].parent << " == " << lneg << endl;);
                        UHTE = true;
                        break;
                    }
                }
                ...
                if (UHTE) {
                    ...
                    DOUT(if (config.opt_uhd_Debug > 1) {
                        ...
                        for (int j = 0; j < clause.size(); ++j) {
                            cerr << "c " << clause[j] << " s: " << stampInfo[toInt(clause[j])].dsc << " e: " << stampInfo[toInt(clause[j])].fin
                                 << " -- and -- " << ~clause[j] << " s: " << stampInfo[toInt(~clause[j])].dsc
                                 << " e: " << stampInfo[toInt(~clause[j])].fin << endl;
                        }
                    });
            ...
            if (doUHLE != 0) {
                ...
                if (doUHLE == 1 || doUHLE == 3) {
                    ...
                    for (pp = cs - 1; pp > 0; --pp) {
                        ...
                        if (fin > finished) {
                            DOUT(if (config.opt_uhd_Debug > 3) {
                                cerr << "c [UHLE-P] remove " << l << " because finish time of " << finLit << " from " << clause << endl;
                            });
                            ...
                            // if( !UHLE && data.outputsProof() ) { for( int j = 0 ; j < clause.size(); ++ j ) data.lits.push_back( clause[j] ); } //
                            // copy the real clause, so that it can be deleted
                            clause.remove_lit(l);
                            data.addCommentToProof("remove literal by first UHLE");
                            data.addToProof(clause);
                            data.addToProof(clause, true, l); // tell proof that a literal has been removed!
                ...
                if (doUHLE == 2 || doUHLE == 3) {
                    ...
                    DOUT(if (config.opt_uhd_Debug > 4) {
                        for (pn = 0; pn < csn; ++pn) {
                            cerr << pn << " " << Sminus[pn] << " " << stampInfo[toInt(Sminus[pn])].dsc << " - " << stampInfo[toInt(Sminus[pn])].fin
                                 << endl;
                        }
                    });
                    ...
                    for (pn = 1; pn < csn; ++pn) {
                        ...
                        if (fin < finished) {
                            DOUT(if (config.opt_uhd_Debug > 4) {
                                cerr << "c [UHLE-N] remove " << ~l << " because of fin time " << fin << " of " << l << " and finLit " << finLit
                                     << " [" << finished << "] from " << clause << endl;
                            });
                            data.removeClauseFrom(clRef, ~l);
                            data.removedLiteral(~l);
                            // if( !UHLE && data.outputsProof() ) { for( int j = 0 ; j < clause.size(); ++ j ) data.lits.push_back( clause[j] ); } //
            ...
            if (config.opt_uhd_probe > 0 && clause.size() == 2 && !clause.can_be_deleted()) {
                    ...
                    for (int j = 0; j < aSize; ++j) {
                        ...
                        for (int k = 0; k < ((config.opt_uhd_probe > 2 || j == 0) ? bSize : 1); ++k) { // even more expensive method
                            ...
                            if ((stampInfo[toInt(aLit)].dsc < stampInfo[toInt(bLit)].dsc && stampInfo[toInt(bLit)].fin < stampInfo[toInt(aLit)].fin)
                                ...
                                || (stampInfo[toInt(~bLit)].dsc < stampInfo[toInt(~aLit)].dsc &&
                                    stampInfo[toInt(~aLit)].fin < stampInfo[toInt(~bLit)].fin)) {
                                ...
                                if ((stampInfo[toInt(bLit)].dsc < stampInfo[toInt(aLit)].dsc &&
                                     stampInfo[toInt(aLit)].fin < stampInfo[toInt(bLit)].fin) ||
                                    (stampInfo[toInt(~aLit)].dsc < stampInfo[toInt(~bLit)].dsc &&
                                     stampInfo[toInt(~bLit)].fin < stampInfo[toInt(~aLit)].fin)) {
                            ...
                            // handle the case  (a \lor b) and that literal a \to l, and b \to ~l, then a \to ~b, hence, have another clause (~a \lor
                            // ~b) if not already present
                            if (config.opt_uhd_probeEE) {
                                uhdProbeSteps++;
                                const Lit bLit = k == 0
                                                     ? ~clause[1]
                                                     : ~bList[k - 1]; // pick the complement here, so that complementary implied literals can be found
                                uhdProbeEEChecks++;                   // tested another pair
                                // a -> aLit -> bLit, and b -> bLit ; thus F \land (a \lor b) -> bLit, and bLit is a backbone!
                                if ((stampInfo[toInt(aLit)].dsc < stampInfo[toInt(bLit)].dsc &&
                                     stampInfo[toInt(bLit)].fin < stampInfo[toInt(aLit)].fin)
                                    // a -> aLit, b -> bLit, -bLit -> -aLit = aLit -> bLit -> F -> bLit
                                    || (stampInfo[toInt(~bLit)].dsc < stampInfo[toInt(~aLit)].dsc &&
                                        stampInfo[toInt(~aLit)].fin < stampInfo[toInt(~bLit)].fin)) {
                                    // found the clause (~a \lor ~b) do something with it
                                    DOUT(if (config.opt_uhd_Debug > 1) cerr << "c formula implies by " << clause << " with lits " << aLit << " -> "
                                                                            << bLit << " clause: [" << ~clause[0] << ", " << ~clause[1] << "]"
                                                                            << endl;);
                                    uhdProbeEECandss++;
                                    ...
                                    if ((stampInfo[toInt(~bLit)].dsc < stampInfo[toInt(~aLit)].dsc &&
                                        ...
                                        (stampInfo[toInt(aLit)].dsc < stampInfo[toInt(bLit)].dsc &&
                                         stampInfo[toInt(bLit)].fin < stampInfo[toInt(aLit)].fin)) {
                                        // found the clause (~a \lor ~b) do something with it!
                                        DOUT(if (config.opt_uhd_Debug > 1) cerr << "c formula implies by " << clause << " with lits " << bLit
                                                                                << " -> " << aLit << " clause: [" << ~clause[0] << ", " << ~clause[1]
                                                                                << "]" << endl;);
                                        uhdProbeEECandss++;
                                        // check in BIG whether clause already exists
                                        if (!big.implies(clause[0], ~clause[1])) {
            ...
            } else if ((borderIteration || config.opt_uhd_fullBorder) && clause.size() > 2 &&
                       clause.size() < config.opt_uhd_fullProbe) { // approximate probing for larger clauses
                ...
                if (!oneDoesNotImply) {
                    ...
                    DOUT(if (config.opt_uhd_Debug > 1) {
                        ...
                        for (int j = 0; j < clause.size(); ++j) {
                            cerr << "c " << clause[j] << " ->";
                            for (int k = 0; k < big.getSize(clause[j]); ++k) {
                                cerr << " " << big.getArray(clause[j])[k];
                            }
                            cerr << endl;
                        }
                    });
                    ...
                    while (allInLimit) {
                        ...
                        for (int j = 1; j < clause.size(); ++j) {
                            ...
                            if (big.getArray(clause[j])[currentPosition[j]] != big.getArray(clause[j - 1])[currentPosition[j - 1]]) {
                                allEqual = false;
                            } else {
                                DOUT(if (config.opt_uhd_Debug > 2) {
                                    cerr << "c [" << whileIteration << "] the two implied literals are equal (" << j << " vs " << j - 1 << ") ["
                                         << clause[j] << " vs " << clause[j - 1] << "]: " << big.getArray(clause[j])[currentPosition[j]] << " vs-1 "
                                         << big.getArray(clause[j - 1])[currentPosition[j - 1]] << endl;
                                });
                            }
                        }
    ...
    bool Unhiding::process() {
        if (!performSimplification()) {
            return false;
        } // do not execute due to previous errors?
        ...
        for (uint32_t iteration = 0; iteration < unhideIter; ++iteration) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                ...
                if (stampInfo[toInt(pos)].dsc == 0) {
                    data.lits.push_back(pos);
                }
                const Lit neg = ~pos;
                if (stampInfo[toInt(neg)].dsc == 0) {
                    data.lits.push_back(neg);
                }
            }
            ...
            for (uint32_t i = 0; i < ts2; ++i) {
                DOUT(if (config.opt_uhd_Debug > 4) cerr << "c [UHD-A] call stamping for literal " << data.lits[i]
                                                        << ", dsc=" << stampInfo[toInt(data.lits[i])].dsc << endl;);
                stamp = stampLiteral(data.lits[i], stamp, foundEE);
            }
            DOUT(if (config.opt_uhd_Debug > 1) cerr << "c stamped " << ts << " roots, and " << ts2 << " remaining lits HTP:" << data.hasToPropagate()
                                                    << "" << endl;);
...
// expensive cross check that each stamp is unique, and there are no 0 stamps
    ...
    void Unhiding::printStatistics(ostream& stream) {
        stream << "c [STAT] UNHIDE " << unhideTime << " s, " << removedClauses << " cls, " << removedLiterals << " totalLits, " << removedLits
               << " lits " << endl;
        stream << "c [STAT] UNHIDE(2) " << unhideProbeTime << " prSecs, " << uhdProbeSteps << " prSteps, " << uhdProbeL1Units << " L1units, "
               << uhdProbeL2Units << " L2units, " << uhdProbeL3Units << " L3units, " << uhdProbeL4Units << " L4units, " << uhdProbeL5Units
               << " L5units, " << endl;
        stream << "c [STAT] UNHIDE(3) " << uhdProbeEEChecks << " EE-checks, " << uhdProbeEECandss << " EE-cands, " << uhdProbeEE << " EEs " << endl;
    }
