...
namespace Coprocessor {
     ...
     */
    void BoundedVariableElimination::par_bve_worker(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, deque<CRef>& strengthQueue,
                                                    deque<CRef>& sharedStrengthQueue, deque<PostponeReason>& postponed, vector<SpinLock>& var_lock,
                                                    ReadersWriterLock& rwlock, ParBVEStats& stats, MarkArray* gateMarkArray, int& rwlock_count,
                                                    int& garbageCounter, int64_t& parBVEchecks, const bool force, const bool doStatistics) {
        if (doStatistics) {
        ...
        int32_t timeStamp;
        while (data.ok() && (!data.isInterupted()) && (stepper.inLimit(parBVEchecks) || data.unlimited())) { // if solver state = false => abort
            // Propagate (rw-locks)
            if (data.hasToPropagate()) {
                if (doStatistics) {
                    stats.upTime = wallClockTime() - stats.upTime;
                }
                par_bve_propagate(data, heap, var_Undef, var_lock, rwlock, dirtyOccs, sharedStrengthQueue, stats, rwlock_count, garbageCounter,
                                  doStatistics);
                if (doStatistics) {
                    stats.upTime = wallClockTime() - stats.upTime;
                }
            ...
            if (sharedStrengthQueue.size() > 0) {
                if (doStatistics) {
                    stats.subsimpTime = wallClockTime() - stats.subsimpTime;
                }
                par_bve_strengthening_worker(data, heap, var_Undef, var_lock, rwlock, sharedStrengthQueue, strengthQueue, dirtyOccs, stats,
                                             rwlock_count, garbageCounter, false, doStatistics);
                if (doStatistics) {
                    stats.subsimpTime = wallClockTime() - stats.subsimpTime;
                }
                continue;
        ...
        calculate_neighbors:
            ...
            if (doStatistics) {
                stats.lockNeighborTime = wallClockTime() - stats.lockNeighborTime;
            }
            ...
            if (doStatistics) {
                stats.mereLockingTime = wallClockTime() - stats.mereLockingTime;
            }
            ...
            // or UNSAT
            //  or var enqueued
            if (lastTouched.getIndex(v) != timeStamp || !data.ok() || data.value(mkLit(v, true)) != l_Undef ||
            ...
            if (pos_count != 0 && neg_count != 0) {
                ...
                if (anticipateEliminationThreadsafe(data, pos, neg, v, p_limit, n_limit, ps, pos_stats, neg_stats, lit_clauses, lit_learnts,
                                                    new_clauses, new_learnts, data_lock, stats, parBVEchecks, doStatistics) == l_False) {
            ...
            if (config.opt_bve_bc) {
                ...
                if (config.opt_bve_verbose > 1) {
                    cerr << "c removing blocked clauses from F_" << v + 1 << endl;
                }
                removeBlockedClausesThreadSafe(data, heap, pos, pos_stats, mkLit(v, false), p_limit, data_lock, heap_lock, stats, garbageCounter,
                                               doStatistics);
                if (config.opt_bve_verbose > 1) {
                    cerr << "c removing blocked clauses from F_¬" << v + 1 << endl;
                }
                removeBlockedClausesThreadSafe(data, heap, neg, neg_stats, mkLit(v, true), n_limit, data_lock, heap_lock, stats, garbageCounter,
                                               doStatistics);
            }
            assert(rwlock_count == 1);
            --rwlock_count;
            ...
            if (!config.opt_bce_only && new_clauses > 0 && (force || doResolve)) {
                ...
                ca_lock.lock();
                if (false) {
                    cerr << "c want resolve " << new_clauses + new_learnts << " clauses on variable " << v + 1 << " with pos size: " << pos.size()
                         << " negs: " << neg.size() << endl;
                ...
                if (doStatistics) {
                    stats.usedGates = (foundGate ? stats.usedGates + 1 : stats.usedGates);
                } // statistics
                if (config.opt_bve_verbose > 1) {
                    cerr << "c resolveSet" << endl;
                }

                if (resolveSetThreadSafe(data, heap, pos, neg, v, p_limit, n_limit, ps, memoryReservation, strengthQueue, stats, data_lock, heap_lock,
                                         new_clauses + new_learnts, parBVEchecks, doStatistics) == l_False) {
                    // UNSAT case -> end thread, but first release all locks
                    assert(rwlock_count == 1);
                    --rwlock_count;
                    rwlock.readUnlock();
                ...
                if (config.opt_bve_verbose > 0) {
                    cerr << "c Resolved " << v + 1 << endl;
                }
                if (doStatistics) {
                    ++stats.eliminatedVars;
                }
                ...
                par_bve_strengthening_worker(data, heap, v, var_lock, rwlock, sharedStrengthQueue, strengthQueue, dirtyOccs, stats, rwlock_count,
                                             garbageCounter, true, doStatistics);
                if (doStatistics) {
                    stats.subsimpTime = wallClockTime() - stats.subsimpTime;
                }
                assert(rwlock_count == 1);
     ...
     */
    inline void BoundedVariableElimination::removeClausesThreadSafe(Coprocessor::CoprocessorData& data, Heap<Coprocessor::VarOrderBVEHeapLt>& heap,
                                                                    const vector<Riss::CRef>& list, const Lit& l, const int limit,
                                                                    SpinLock& data_lock, SpinLock& heap_lock,
                                                                    Coprocessor::BoundedVariableElimination::ParBVEStats& stats, int& garbageCounter,
     ...
     *
     */
    inline lbool BoundedVariableElimination::anticipateEliminationThreadsafe(
        CoprocessorData& data, vector<Riss::CRef>& positive, vector<Riss::CRef>& negative, const int v, const int p_limit, const int n_limit,
        vec<Lit>& resolvent, vec<int32_t>& pos_stats, vec<int32_t>& neg_stats, int& lit_clauses, int& lit_learnts, int& new_clauses, int& new_learnts,
        SpinLock& data_lock, BoundedVariableElimination::ParBVEStats& stats, int64_t& bveChecks, const bool doStatistics) {
        if (config.opt_bve_verbose > 2) {
            cerr << "c starting anticipate BVE" << endl;
        }
        // Clean the stats
        lit_clauses = 0;
        lit_learnts = 0;
            ...
            for (int cr_n = 0; cr_n < negative.size(); ++cr_n) {
                ...
                if (newLits > 1) {
                    ...
                    if (p.learnt() || n.learnt()) {
                        if (config.opt_resolve_learnts > 1 || (config.opt_resolve_learnts > 0 && !(p.learnt() && n.learnt()) &&
                ...
                // unit Clause
                    ...
                    if (status == l_False) {
                        if (config.opt_bve_verbose > 2) {
                            cerr << "c finished anticipate_bve with conflict" << endl;
                        }
                        return l_False;
     ...
     */
    lbool BoundedVariableElimination::resolveSetThreadSafe(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, vector<CRef>& positive,
                                                           vector<CRef>& negative, const int v, const int p_limit, const int n_limit, vec<Lit>& ps,
                                                           AllocatorReservation& memoryReservation, deque<CRef>& strengthQueue, ParBVEStats& stats,
                                                           SpinLock& data_lock, SpinLock& heap_lock, int expectedResolvents, int64_t& bveChecks,
                                                           const bool doStatistics, const bool keepLearntResolvents) {
        int resolvents = 0;
        const bool hasDefinition = (p_limit < positive.size() || n_limit < negative.size());
        for (int cr_p = 0; cr_p < positive.size(); ++cr_p) {
            const CRef crPos = positive[cr_p];
                ...
                if (!resolve(p, n, v, ps)) {
                    ...
                    if (ps.size() == 1) {
                        ...
                        if (false) {
                            ...
                            cerr << "c resolving " << resolvents << " of " << expectedResolvents << " at adress " << memoryReservation.getCurrent()
                                 << " with limit " << memoryReservation.getUpperLimit() << endl;
                        }
     ...
     *
     */
    inline void BoundedVariableElimination::removeBlockedClausesThreadSafe(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap,
                                                                           const vector<CRef>& list, const int32_t _stats[], const Lit& l,
                                                                           const int limit, SpinLock& data_lock, SpinLock& heap_lock,
    ...
    void* BoundedVariableElimination::runParallelBVE(void* arg) {
        ...
        workData->bve->par_bve_worker(*(workData->data), *(workData->heap), *(workData->strengthQueue), *(workData->sharedStrengthQueue),
                                      *(workData->postponed), *(workData->var_locks), *(workData->rw_lock), *(workData->bveStats),
                                      workData->gateMarkArray, workData->rwlock_count, workData->garbageCounter, workData->bveStats->parBveChecks);
        return 0;
    }
    ...
    void BoundedVariableElimination::parallelBVE(CoprocessorData& data, const bool doStatistics) {
        ...
        while ( // variable queue / heap is not empty
            ((config.opt_bve_heap != 2 && newheap.size() > 0) || (config.opt_bve_heap == 2 && variable_queue.size() > 0)) && !data.isInterupted() &&
            !reachedLimit) {
            ...
            if (config.opt_par_bve == 2                  // always parallel (cp3_par_bve: 2)
                ...
                if (config.opt_minimal_updates) {
                    for (int i = 0; i < controller.size(); ++i) {
                        if (workData[i].garbageCounter > 0) {
                            ca.freeLit(workData[i].garbageCounter);
                        }
                    }
                }
            }
            // we run a sequential BVE - its likly that we have performed parallel runs before
                ...
                if (config.opt_bve_verbose > 0) {
                    cerr << "c sequentiel bve on " << QSize << " variables" << endl;
                }
            ...
            if (doStatistics) {
                subsimpTime = wallClockTime() - subsimpTime;
            }
            subsumption.process();
     ...
     *      -> lock other
     */
    void BoundedVariableElimination::par_bve_strengthening_worker(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, const Var ignore,
                                                                  vector<SpinLock>& var_lock, ReadersWriterLock& rwlock,
                                                                  deque<CRef>& sharedStrengthQueue, deque<CRef>& localQueue, MarkArray& dirtyOccs,
                                                                  ParBVEStats& stats, int& rwlock_count, int& garbageCounter,
        ...
        while (data.ok() && ((!strength_resolvents && sharedStrengthQueue.size() > 0) || localQueue.size() > 0)) {
            ...
            lbool state = strength_check_pos(data, heap, ignore, list, sharedStrengthQueue, localQueue, strengthener, cr, fst, var_lock, dirtyOccs,
                                             stats, garbageCounter, strength_resolvents, doStatistics);
            if (l_False != state)
                strength_check_neg(data, heap, ignore, list_neg, sharedStrengthQueue, localQueue, strengthener, cr, min, fst, var_lock, dirtyOccs,
                                   stats, garbageCounter, strength_resolvents, doStatistics);
     ...
     *
     */
    inline lbool BoundedVariableElimination::strength_check_pos(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, const Var ignore,
                                                                vector<CRef>& list, deque<CRef>& sharedStrengthQueue, deque<CRef>& localQueue,
                                                                Clause& strengthener, CRef cr, Var fst, vector<SpinLock>& var_lock,
                                                                MarkArray& dirtyOccs, ParBVEStats& stats, int& garbageCounter,
                                                                const bool strength_resolvents, const bool doStatistics) {
        ...
        lock_to_strengthen_nn_pos:
            ...
            if (negated_lit_pos == -1 && si == strengthener.size()) {
                ...
                if (config.opt_bve_verbose > 2) {
                    cerr << "c " << strengthener << " subsumed " << other << endl;
                }
                if (doStatistics) {
                    if (!other.learnt()) {
            ...
            // if subsumption successful, strengthen
                ...
                if (doStatistics && other.learnt()) {
                    ++stats.strengthtLearntLits;
     ...
     *
     */
    inline lbool BoundedVariableElimination::strength_check_neg(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, const Var ignore,
                                                                vector<CRef>& list, deque<CRef>& sharedStrengthQueue, deque<CRef>& localQueue,
                                                                Clause& strengthener, CRef cr, Lit min, Var fst, vector<SpinLock>& var_lock,
                                                                MarkArray& dirtyOccs, ParBVEStats& stats, int& garbageCounter,
                                                                const bool strength_resolvents, const bool doStatistics) {
        int si, so;          // indices used for "can be strengthened"-testing
        int negated_lit_pos; // index of negative lit, if we find one
    ...
    lbool BoundedVariableElimination::par_bve_propagate(CoprocessorData& data, Heap<VarOrderBVEHeapLt>& heap, const Var ignore,
                                                        vector<SpinLock>& var_lock, ReadersWriterLock& rwlock, MarkArray& dirtyOccs,
                                                        deque<CRef>& sharedSubsimpQueue, ParBVEStats& stats, int& rwlock_count, int& garbageCounter,
                                                        const bool doStatistics) {
        ...
        while (data.ok() && data.hasToPropagate()) {
            ...
            for (int i = 0; i < positive.size(); ++i) {
                ...
                if (config.opt_bve_verbose > 2 || global_debug_out) {
                    cerr << "c UP remove " << satisfied << endl;
                }
                if (doStatistics) {
                    if (satisfied.learnt()) {
                        ++stats.removedLearnts;
