...
namespace Coprocessor {
    ...
    EquivalenceElimination::EquivalenceElimination(CoprocessorData& _data, CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller,
                                                   Propagation& _propagation, Coprocessor::Subsumption& _subsumption)
    ...
    bool EquivalenceElimination::process(CoprocessorData& data) {
        if (!performSimplification()) {
            return false;
        } // do not perform simplification because of presiously failed runs?
        ...
        if (config.opt_ee_level > 1 && data.ok()) {
            ...
            while (moreEquivalences) {
                ...
                iter++;
                if (config.opt_ee_circuit_iters != -1 && iter > config.opt_ee_circuit_iters) {
                    break;
                }
...

                {
                    ...
                    DOUT(if (moreEquivalences) if (config.ee_debug_out > 2) cerr
                             << "c found new equivalences with the gate method! [ok?: " << data.ok() << "]" << endl;);
                ...
                do {                             // will set literals that have to be analyzed again!
                    ...
                    doRepeat =
                        applyEquivalencesToFormula(data, (iter == 1 && eeIter == 0)); // in the first iteration, run subsumption/strengthening and UP!
                    moreEquivalences = doRepeat || moreEquivalences;
                    eeIter++;
                    if (eeIter >= config.opt_ee_bigIters) {
                        break;
                    }
        ...
        if (data.ok()) {
            ...
            do {
                findEquivalencesOnBig(data); // finds SCC based on all literals in the eqDoAnalyze array!
                eeIter++;
                if (!applyEquivalencesToFormula(data)) {
                    break;
                } // definitely apply all found eq's
                if (eeIter >= config.opt_ee_bigIters) {
                    break;
                }
            } while (data.ok() && !data.isInterupted() &&
                     (data.unlimited() || steps < config.opt_ee_limit)); // will set literals that have to be analyzed again!

            // if( config.ee_debug_out > 1 && )
            if (steps > config.opt_ee_limit) {
                cerr << "c abort global EE round, because step limit has been reached " << endl;
            }
        ...
        } // notify system that nothing has been done here!
        return modifiedFormula;
    }
    ...
    bool EquivalenceElimination::findGateEquivalencesNew(Coprocessor::CoprocessorData& data, vector<Circuit::Gate>& gates) {
        ...
        vector<unsigned int> bitType(data.nVars(),
                                     0); // upper 4 bits are a counter to count how often this variable has been considered already as output
        ...
        for (int i = 0; i < gates.size(); ++i) {
            ...
            case Circuit::Gate::AND:
                ...
                if (g.isFull()) {
                    bitType[var(g.x())] = bitType[var(g.x())] | 1;
                } // set output bit
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (!putAllAlways && bitType[v] != 0) {
                continue;
            }
            currentPtr->push_back(v);
            if (!putAllAlways) {
                active.setCurrentStep(v);
            }
            // Assumption: inside a miter, each pure input variable has to have an even number of gates!
            if ((varTable[v].size() & 1) != 0) {
                isMiter = false;
                DOUT(if (config.ee_debug_out > 2) cerr << "c the given gate structure cannot be a miter, because variable " << v + 1 << " has "
                                                       << varTable[v].size() << " gates" << endl;);
            }
        }
        ...
        while (currentPtr->size() > 0) {
            ...
            DOUT(if (config.ee_debug_out > 2) cerr << "c check variable " << v + 1 << " with " << varTable[v].size() << " gates and replace literal "
                                                   << getReplacement(mkLit(v, false)) << " [ok?: " << data.ok() << "]" << endl;);
            ...
            for (int i = 0; i < varTable[v].size(); ++i) {
                ...
                if (var(x) == var(a) || var(x) == var(b) || var(a) == var(b)) {
                    continue;
                } // such a gate would not be found during analysis ...
                ...
                if (data.value(a) != l_Undef || data.value(b) != l_Undef || data.value(x) != l_Undef) {
                    ...
                    if (data.value(a) == l_True) {
                        if (config.opt_ee_eagerEquivalence) {
                            setEquivalent(b, x);
                        }
                ...
                // somehow same inputs
                if (a == b) {
                    DOUT(if (config.ee_debug_out > 2) cerr << "c found equivalence based on equivalent inputs " << x << " <-> AND(" << a << "," << b
                                                           << ")" << endl;);
                    if (config.opt_ee_eagerEquivalence) {
                        setEquivalent(a, x);
                    }
                ...
                //  else if ( a == ~b ) {
                //    if( config.ee_debug_out > 2 ) cerr << "c find an unsatisfiable G-gate based on complementary inputs " << x << " <-> AND(" << a
                //    << "," << b << ")" << endl; if( enqOut ) {
                ...
                for (int j = i + 1; j < varTable[v].size(); ++j) {
                    ...
                    if (other.getType() != Circuit::Gate::AND) {
                        continue;
                    }
                    ...
                    if (var(ox) == var(oa) || var(ox) == var(ob) || var(oa) == var(ob)) {
                        continue;
                    } // such a gate would not be found during analysis ...
                    // assigned value
                    if (data.value(oa) != l_Undef || data.value(ob) != l_Undef || data.value(ox) != l_Undef) {
                        DOUT(if (config.ee_debug_out > 2) {
                            ...
                            other.print(cerr);
                            cerr << "trail: ";
                            for (int k = 0; k < data.getTrail().size(); ++k) {
                                cerr << " " << data.getTrail()[k];
                            }
                            cerr << endl;
                        });
                        ...
                        if (data.value(ob) == l_True) {
                            ...
                            if (enqInp) {
                                data.enqueue(oa);
                                data.addUnitToProof(oa);
                            }
                            if (enqInp) {
                                data.enqueue(ob);
                                data.addUnitToProof(ob);
                            }
                        }
                        // do not reason with assigned gates!
                        continue;
                    ...
                    if ((oa == a && ob == b) || (oa == b && ob == a)) {
                        ...
                        if ((oa == a && ob == x) // x<->AND(a,c) , and c <-> AND(a,x) => c == x !!!
                            || (ob == b && oa == x) || (b == ob && a == ox) || (a == oa && b == ox)) {
                            DOUT(if (config.ee_debug_out > 2) cerr << "[  11] match one input, and output is another input: " << x << " <-> AND(" << a
                                                                   << "," << b << ")  vs " << ox << " <-> AND(" << oa << "," << ob << ")" << endl;);
                            eeLits.push_back(x);
                            eeLits.push_back(ox);
                        } else if (ox == ~x && ((oa == ~a && ob == ~b) || (oa == ~b && ob == ~a))) {
                            DOUT(if (config.ee_debug_out > 2) cerr << "[  12]" << endl;);
                            // x <-> AND(a,b) and -x <-> AND(-a,-b) => x=a=b!
                            eeLits.push_back(x);
                            eeLits.push_back(a); // every two literals represent an equivalent pair
                            eeLits.push_back(x);
                            eeLits.push_back(b);
                        }
                        ...
                        //      }
                            ...
                            if (oa == a) {
                                ...
                                if (x == ~ox) {
                                    cerr << "c complementary outputs, one complementary input, other input equal " << ox << " <-> AND(" << ob << ","
                                         << oa << ")" << endl;
                                    data.addUnitToProof(a);
                                    data.enqueue(a); // handle gates where the input would be complementary
                                } else {
                                    gates.push_back(Circuit::Gate(~a, x, ox, Circuit::Gate::AND, Circuit::Gate::FULL));
                                }
                            } else if (ob == b) {
                                ...
                                if (x == ~ox) {
                                    ...
                                    gates.push_back(Circuit::Gate(~b, x, ox, Circuit::Gate::AND, Circuit::Gate::FULL));
                                }
                            } else if (ob == a) {
                        ...
                        } else if ((x == oa && ob == ~a) || (x == oa && oa == ~b) || (x == ob && oa == ~a) || (x == ob && oa == ~b)
                    ...
                    while (eeLits.size() >= 2) {
                        const Lit x = eeLits[eeLits.size() - 1];
                        const Lit ox = eeLits[eeLits.size() - 2];
                        eeLits.pop_back();
                        eeLits.pop_back();
                        if (var(x) != var(ox)) {
                            if (config.opt_ee_eagerEquivalence) {
                            ...
                            DOUT(if (config.ee_debug_out > 2) cerr << "c equi: " << x << " == " << ox << " min=" << minV + 1 << " max=" << maxV + 1
                                                                   << endl;);
        ...
        DOUT(if (config.ee_debug_out > 2) cerr << "c OLD equis: " << oldEquivalences << "  current equis: " << data.getEquivalences().size()
                                               << " hasToPropagate" << data.hasToPropagate() << endl;);
        gateTime = cpuTime() - gateTime;
    ...
    bool EquivalenceElimination::findGateEquivalences(Coprocessor::CoprocessorData& data, vector<Circuit::Gate> gates) {
        ...
        if (!config.opt_ee_wip)
            return false; // do not execute this algorithm, unless you are willing to fix it
        ...
        */
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (0 == bitType[v]) {
                if (varTable[v].size() == 0) {
                    continue;
                } // use only if it helps enqueuing gates!
        ...
        if (queue.empty()) {
            cerr << "c there are no gates to start from, with " << gates.size() << " initial gates!" << endl;
        }
    ...
    bool EquivalenceElimination::allInputsStamped(Circuit::Gate& g, std::vector<unsigned int>& bitType) {
        ...
        case Circuit::Gate::GenAND:
            ...
            return true;
        case Circuit::Gate::ITE:
            if ((bitType[var(g.s())] & 4) == 0 || (bitType[var(g.t())] & 4) == 0 || (bitType[var(g.f())] & 4) == 0) {
                return false;
            }
            return true;
        case Circuit::Gate::XOR:
            ...
            if ((bitType[var(g.b())] & 4) == 0) {
                count++;
            }
            if ((bitType[var(g.c())] & 4) == 0) {
                count++;
            }
            if (count > 1) {
                return false;
    ...
    void EquivalenceElimination::enqueueSucessorGates(Circuit::Gate& g, std::deque<int> queue, std::vector<Circuit::Gate>& gates,
                                                      std::vector<unsigned int>& bitType, vector<vector<int32_t>>& varTable) {
        switch (g.getType()) {
        case Circuit::Gate::AND: {
            const Var v = var(g.x());
    ...
    void EquivalenceElimination::processANDgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
                                                std::vector<unsigned int>& bitType, vector<vector<int32_t>>& varTable, MarkArray* active,
                                                std::deque<Var>* activeVariables) {
    ...
    void EquivalenceElimination::processExOgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
        ...
        for (int i = 0; i < queue.size(); ++i) {
            const Circuit::Gate& other = gates[queue[i]];
            if (other.getType() != Circuit::Gate::ExO) {
                continue;
            }
            if (other.size() != g.size()) {
                continue;
            } // cannot say anything about different sizes
            ...
            for (int j = 0; j < other.size(); ++j) {
                ...
                if (hit < j) {
                    break;
                } else if (hit == j && freeLit == lit_Undef) {
                    freeLit = other.get(j);
                }
            }
            if (hit + 1 < other.size()) {
    ...
    void EquivalenceElimination::processGenANDgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
        ...
        for (int i = 0; i < queue.size(); ++i) {
            const Circuit::Gate& other = gates[queue[i]];
            if (other.getType() != Circuit::Gate::GenAND) {
                continue;
            }
            if (other.size() != g.size()) {
                continue;
    ...
    void EquivalenceElimination::processITEgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
        ...
        for (int i = 0; i < queue.size(); ++i) {
            const Circuit::Gate& other = gates[queue[i]];
            if (other.getType() != Circuit::Gate::ITE) {
                continue;
            }
            ...
            if (var(s) != var(os)) {
                continue;
            } // the selector variable has to be the same!!
            const Lit ot = getReplacement(other.t());
            if (var(t) != var(ot) && var(ot) != var(f)) {
                continue;
            } // has to match one of the inputs!
            const Lit of = getReplacement(other.f());
            if ((var(t) == var(ot) && var(of) != var(f)) || (var(f) == var(ot) && var(of) != var(t))) {
                continue;
            } // inputs have to match inputs!
            Lit ox = getReplacement(other.x());
            if (x == ox) {
                continue;
            } // can only define output, but no other bits of the gates!
            // equivalence could be either positive or negative
            enum Match { POS, NONE, NEG };
            Match match = NONE;
            if (s == os) {
                if (t == ot && f == of) {
                    match = POS;
    ...
    void EquivalenceElimination::processXORgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
        ...
        for (int i = 0; i < queue.size(); ++i) {
            ...
            if (other.getType() != Circuit::Gate::XOR // xor is either equivalent to another xor
                && other.getType() != Circuit::Gate::FA_SUM) {
                continue;
            } // or subsumes equivalent FA_SUM
            ...
            if (other.getType() == Circuit::Gate::XOR) {
                ...
                for (int j = 0; j < 3; ++j) {
                    if (var(lits[j]) == var(oa)) {
                        const Lit tmp = lits[0];
                        lits[0] = lits[j];
                        lits[j] = tmp;
                        hit++;
                        break;
                ...
                for (int j = hit; j < 3; ++j) {
                    if (var(lits[j]) == var(ob)) {
                        const Lit tmp = lits[hit];
                        lits[hit] = lits[j];
                        lits[j] = tmp;
                        hit++;
                        break;
                ...
                if (hit < 2) {      // check for the free literal
                    if (hit == 0) { /*cerr << "failed with one of literal " << oa << " ," << ob << endl;*/
                        continue;
                    } // this xor gate does not match 2 variables!
                    else if (freeLit == lit_Undef) {
                        freeLit = ob;
                    } // this literal is the literal that does not match!
                }

                //       if( freeLit != lit_Undef ) cerr << "c freelit = " << freeLit << endl;
                const Lit oc = getReplacement(other.c());
                for (int j = hit; j < 3; ++j) {
                    if (var(lits[j]) == var(oc)) {
                        const Lit tmp = lits[hit];
                        lits[hit] = lits[j];
                        lits[j] = tmp;
                        hit++;
                ...
                if (hit < 3)       // check for the free literal
                    if (hit < 2) { /*cerr << "failed with one of literal " << oa << " ," << ob << ", " << oc << endl; */
                        continue;
                    } // this xor gate does not match 2 variables!
                // might be the case that all variables match!
                if (freeLit == lit_Undef) {
                    freeLit = oc;
                } // this literal is the literal that does not match!
                ...
                if ((pol ^ sign(lits[2])) != (sign(oa) ^ sign(ob) ^ sign(oc) ^ sign(freeLit))) {
                    freeLit = ~freeLit;
                }
                ...
                if (freeLit == lit_Error) {
                    continue;
                } // these gates do not match!

                if (pol == qPol) {
                    freeLit = ~freeLit;
                }
                data.addUnitToProof(freeLit);
                if (l_False == data.enqueue(freeLit)) {
                    return;
                }
    ...
    void EquivalenceElimination::processFASUMgate(CoprocessorData& data, Circuit::Gate& g, vector<Circuit::Gate>& gates, std::deque<int>& queue,
                                                  std::vector<unsigned int>& bitType, vector<vector<int32_t>>& varTable, MarkArray* active,
                                                  std::deque<Var>* activeVariables) {
        ...
        for (int i = 0; i < queue.size(); ++i) {
            ...
            if (other.getType() == Circuit::Gate::FA_SUM) {
                ...
                if (hit < 2) {      // check for the free literal
                    if (hit == 0) { /*cerr << "failed with one of literal " << oa << " ," << ob << endl; */
                        continue;
                    } // this xor gate does not match 3 variables!
                    else if (freeLit == lit_Undef) {
                        freeLit = ob;
                    } // this literal is the literal that does not match!
                }
                ...
                if (hit < 3) { /*cerr << "failed with one of literal " << oa << " ," << ob << ", " << oc << " ," << od << endl;*/
                    continue;
                } // this xor gate does not match 3 variables!
                // might be the case that all variables match!
                if (freeLit == lit_Undef) {
                ...
                if ((pol ^ sign(lits[3])) != (sign(oa) ^ sign(ob) ^ sign(oc) ^ sign(od) ^ sign(freeLit))) {
                    freeLit = ~freeLit;
                }
    ...
    void EquivalenceElimination::findEquivalencesOnBigFast(CoprocessorData& data, vector<vector<Lit>>* externBig) {
        ...
        while (!eqDoAnalyze.empty() && !data.isInterupted() && (data.unlimited() || steps < config.opt_ee_limit)) {
            ...
            if (false && size == 0) {
                ...
                vertexes[toInt(l)].seen = 0;
                vertexes[toInt(l)].min = -2; // set min to invalid!
                continue;
            } // do not process literals without child literals!

            // init literal
            vertexes[toInt(l)].start = dfs++;
            vertexes[toInt(l)].min = vertexes[toInt(l)].start;
            ...
            while (todo.size() > 0) {
                ...
                DOUT(if (config.ee_debug_out > 1) cerr << "c continue with " << V << "[" << vertexes[toInt(V)].start << " , "
                                                       << vertexes[toInt(V)].min << " , " << vertexes[toInt(V)].seen << " / "
                                                       << (externBig == 0 ? big.getSize(V) : (*externBig)[toInt(V)].size()) << "]" << endl;);
                ...
                if (vertexes[toInt(V)].seen != -1) { // have been working on this literal before already!
                    ...
                    vertexes[toInt(V)].min = vertexes[toInt(V)].min < vertexes[toInt(lastSeen)].min
                                                 ? vertexes[toInt(V)].min
                                                 : vertexes[toInt(lastSeen)].min; // set min of "parent node"
                ...
                for (; vertexes[toInt(V)].seen < size;) {
                    ...
                    DOUT(if (config.ee_debug_out > 1) cerr << "c check child " << v << " [ seen(" << v << ") =" << vertexes[toInt(v)].seen << " , "
                                                           << vertexes[toInt(v)].start << " , " << vertexes[toInt(V)].min << "]" << endl;);
                    if (vertexes[toInt(v)].seen == -1) { // have not seen this node so far, apply recursion on it
                        DOUT(if (config.ee_debug_out > 1) cerr << "c found [" << vertexes[toInt(V)].seen << "]:  fresh " << v << "["
                                                               << vertexes[toInt(v)].start << " <= " << dfs + 1 << "]" << endl;);
                        todo.push_back(v);
                        path.push_back(v);
                        vertexes[toInt(v)].start = dfs++;
                        vertexes[toInt(v)].min = vertexes[toInt(v)].start; // init the new node!
                        goto nextNode;
                    } else {
                        DOUT(if (config.ee_debug_out > 1) cerr << "c found [" << vertexes[toInt(V)].seen << "]: already analyzed " << v << " ["
                                                               << vertexes[toInt(v)].start << " , " << vertexes[toInt(V)].min << "]" << endl;);
                        ...
                        if (!data.ma.isCurrentStep(toInt(v))) { // this literal does not give a bigger SCC! -> has been removed from path already!
                            vertexes[toInt(V)].min =
                                vertexes[toInt(V)].min <= vertexes[toInt(v)].start
                                    ? vertexes[toInt(V)].min
                                    : vertexes[toInt(v)]
                                          .start; // set min of "parent node" // TODO: according to recursive algorithm: use start instead of min!
                        }
                ...
                if (vertexes[toInt(V)].start == vertexes[toInt(V)].min) {
                    ...
                    do { // pop all elements behind V!
                        ...
                        if (!config.opt_ee_eager_frozen || !data.doNotTouch(var(w))) {
                            eqCurrentComponent.push_back(w);
                        } // add variable only, if it is not frozen, or if frozen variables should not be treated eagerly
                        data.ma.setCurrentStep(toInt(w)); // ensure that the same SCC will not be found twice!
                        DOUT(if (config.ee_debug_out > 1) cerr << "c put " << w << "[" << vertexes[toInt(V)].start << "] into the same component!"
                                                               << endl;);
                    } while (w != V);
                    if (eqCurrentComponent.size() > 1 && !sign(minLit)) { // only add one of the two symmetric SCCs!
                        DOUT(if (config.ee_debug_out > 1) cerr << "c add SCC of size " << eqCurrentComponent.size() << " with smallest lit " << minLit
                                                               << " : " << eqCurrentComponent << endl;);
                        if (steps < config.opt_ee_limit) {
                            data.addEquivalences(eqCurrentComponent);
                        } // do only add the current component, if its guaranteed to be correct
                    }
                }
                ...
                DOUT(if (config.ee_debug_out > 1) cerr << "c finish working on " << V << "[" << vertexes[toInt(V)].start << " , "
                                                       << vertexes[toInt(V)].min << " , " << vertexes[toInt(V)].seen << " / "
                                                       << (externBig == 0 ? big.getSize(V) : (*externBig)[toInt(V)].size()) << "]" << endl;);
    ...
    void EquivalenceElimination::findEquivalencesOnBig(CoprocessorData& data, vector<vector<Lit>>* externBig) {
        ...
        if (config.opt_ee_iterative) {
            return findEquivalencesOnBigFast(data, externBig);
        } else {
            return findEquivalencesOnBigRec(data, externBig);
        }
    }
    ...
    void EquivalenceElimination::findEquivalencesOnBigRec(CoprocessorData& data, vector<vector<Lit>>* externBig) {
        ...
        if (eqLitInStack == 0) {
            eqLitInStack = (char*)malloc(sizeof(char) * data.nVars() * 2);
        } else {
            eqLitInStack = (char*)realloc(eqLitInStack, sizeof(char) * data.nVars() * 2);
        }
        if (eqInSCC == 0) {
            eqInSCC = (char*)malloc(sizeof(char) * data.nVars());
        } else {
            eqInSCC = (char*)realloc(eqInSCC, sizeof(char) * data.nVars());
        }
...
#define MININ(x, y) (x) < (y) ? (x) : (y)
    ...
    void EquivalenceElimination::eqTarjan(int depth, Lit l, Lit list, CoprocessorData& data, BIG& big, vector<vector<Lit>>* externBig) {
        ...
        if (depth > 32000) {
            ...
            if (!didit) {
                cerr << "c recursive EE algorithm reached depth 32K, use the iterative version!" << endl;
                didit = true;
            }
        ...
        if (externBig != 0) {
            ...
            for (uint32_t i = 0; i < impliedLiteralsSize; ++i) {
                const Lit n = impliedLiterals[i];
                DOUT(if (config.ee_debug_out > 2) cerr << "c next implied lit from " << l << " is " << n << " [" << i << "/" << impliedLiteralsSize
                                                       << "]" << endl;);
                if (eqNodeIndex[toInt(n)] == -1) {
                    eqTarjan(depth + 1, n, n, data, big, externBig);
                    eqNodeLowLinks[toInt(l)] = MININ(eqNodeLowLinks[toInt(l)], eqNodeLowLinks[toInt(n)]);
        ...
        if (eqNodeLowLinks[toInt(l)] == eqNodeIndex[toInt(l)]) {
            ...
            do {
                ...
                eqInSCC[var(n)] = 1;
                if (!config.opt_ee_eager_frozen || !data.doNotTouch(var(n))) {
            ...
            if (eqCurrentComponent.size() > 1) {
                ...
                if (steps < config.opt_ee_limit) {
                    data.addEquivalences(eqCurrentComponent);
                } // do add elements to the equivalence class only, if scanning completed!
            }
        }
    }
    ...
    bool EquivalenceElimination::setEquivalent(Lit representative, Lit toReplace) {
        ...
        DOUT(if (config.ee_debug_out > 2) cerr << "c ee literals: " << representative << " ( -> " << r << ") is representative for " << toReplace
                                               << " ( -> " << s << ")" << endl;);
        if (r < s) {
    ...
    bool EquivalenceElimination::applyEquivalencesToFormula(CoprocessorData& data, bool force) {
        ...
        if (data.getEquivalences().size() > 0 || force) {
            ...
            if (config.opt_ee_eager_frozen) {
                ...
                int keep = 0;
                for (int i = 0; i < ee.size(); ++i) {
                    if (ee[i] == lit_Undef || !data.doNotTouch(var(ee[i]))) {
                        ee[keep++] = ee[i];
                    } // keep all literals that are not frozen, and keep the "lit_Undef" separators!
                }
                ee.shrink(ee.size() - keep);
            }

            while (start < ee.size() && ee[start] == lit_Undef)
                start++; // handle removed classes from white-list
                ...
                if (ee[i] == lit_Undef) {
                    ...
                    for (int j = start; j < i; ++j) { // set all equivalent literals
                        ...
                        if (data.outputsProof() && repr != ee[j]) { // do not add trivial clauses to the proof!
                            ...
                            if (ee[j] != ~repr) { // added equivalences should have different literals, and not complementarly literals (although this
                                                  // case exists)
                                proofClause.clear();
                                proofClause.push_back(~repr);
                                proofClause.push_back(ee[j]);
                                data.addToProof(proofClause, false, ee[j]); // the literal that is not kept should be the first literal, because this
                                                                            // way the same equivalence can be added multiple times
                                proofClause[0] = ~proofClause[0];
                                if (ee[j] != ~repr) {
                                    proofClause[1] = ~proofClause[1];
                                }
                                data.addToProof(proofClause, false, ~ee[j]); // the literal that is not kept should be the first literal, because this
                                                                             // way the same equivalence can be added multiple times
                            }
                        ...
                        if (!setEquivalent(repr, ee[j])) {
                            ...
                            DOUT(if (config.ee_debug_out > 2) cerr << "c applying EE failed due to setting " << repr << " and " << ee[j]
                                                                   << " equivalent -> UNSAT" << endl;);
                            data.setFailed();
                            return newBinary;
                        }
                    }

                    DOUT(if (config.ee_debug_out > 2) for (int j = start; j < i; ++j) { // set all equivalent literals
                        cerr << "c replace " << (sign(ee[j]) ? "-" : "") << var(ee[j]) + 1 << " by " << (sign(getReplacement(ee[j])) ? "-" : "")
                             << var(getReplacement(ee[j])) + 1 << endl;
                    });
                    ...
                    if (setValue == l_Undef) {
                        ...
                        if (setValue == l_Undef) {
                            for (int j = start; j < i; ++j) {
                                if (data.value(ee[j]) != l_Undef) {
                                    setValue = data.value(ee[j]);
                                    break;
                                } // found one assigned literal
                                if (data.value(getReplacement(ee[j])) != l_Undef) {
                                    setValue = data.value(getReplacement(ee[j]));
                                    break;
                                } // found one assigned literal
                            }
                        }
                    ...
                    if (setValue == l_Undef) {
                        for (int j = start; j < i; ++j) { // nothing set, simply add back the clauses as before!
                            ...
                            if (!data.doNotTouch(var(ee[j]))) {
                                continue;
                            } // only create these clauses for the "do not touch" literals
                        ...
                        if (setValue == l_True) {
                            for (int j = start; j < i; ++j) {
                                if (data.value(ee[j]) == l_True) {
                                    continue;
                                } else if (data.value(ee[j]) == l_False) {
                                    data.setFailed();
                                    break;
                                } else {
                            ...
                            for (int j = start; j < i; ++j) {
                                if (data.value(ee[j]) == l_False) {
                                    continue;
                                } else if (data.value(ee[j]) == l_True) {
                                    data.setFailed();
                                    break;
                                } else {
                        ...
                        for (int j = start; j < i; ++j) { // process each element of the class!
                            ...
                            for (int pol = 0; pol < 2; ++pol) { // do for both polarities!
                                ...
                                for (int k = 0; k < list.size(); ++k) {
                                    ...
                                    for (int m = 0; m < c.size(); ++m) {
                                        if (c[m] == repr || c[m] == ~repr) {
                                            duplicate = true;
                                            continue;
                                        } // manage that this clause is not pushed into the list of clauses again!
                                        const Lit tr = getReplacement(c[m]);
                                        if (tr != c[m]) {
                                            if (var(tr) != var(repr)) {
                                                removedFrom[removedFromSize++] = c[m];
                                            } // memorize literal that has been removed from the clause (not repr)
...
#ifdef PCASSO
                                    c.setPTLevel(data.currentDependencyLevel()); // since it cannot be traced which clauses participated, we assume
                                                                                 // the worst case to be on the safe side!
#endif
                                    ...
                                    if (c.size() == 2) { // take care of newly created binary clause for further analysis!
                                        ...
                                        if (data.enqueue(c[0], data.currentDependencyLevel()) == l_False) {
                                            return newBinary;
                                        }
                                    } else if (c.size() == 0) {
                                        data.setFailed();
                                    ...
                                    if (c.size() > 1 && !duplicate) {
                                        ...
                                        if (!hasDuplicate(data, data.list((pol == 0 ? repr : ~repr)), c)) {
                                            ...
                                            if (getsNewLiterals) {
                                                if (data.addSubStrengthClause(list[k]) && config.opt_eeSub) {
                                                    resetVariables = true;
                                                }
                                            }
                                        } else {
                    ...
                    if (data.hasToPropagate()) { // after each application of equivalent literals perform unit propagation!
                        if (propagation.process(data, true) == l_False) {
                            assert(!data.ok() && "had to fail already");
                            return newBinary;
                        }
                    }
                    if (config.opt_eeSub) {
                ...
                if (d.size() < c.size() && (!d.learnt() || c.learnt())) { // do not remove a non-learnt clause by a learnt clause!
                    if (ordered_subsumes(d, c)) {
                        ...
                        return true; // the other clause subsumes the current clause!
                           ...
                           (!c.learnt() || d.learnt())) { // if size is equal, then either removed before, or not removed at all!
    ...
    void EquivalenceElimination::writeAAGfile(CoprocessorData& data) {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (type[v] == 1) {
                pureOutputs.push_back(v);
            }
            if (type[v] == 2) {
                pureInputs.push_back(v);
            }
        }
        cerr << "AAG:" << endl
             << " maxV=" << data.nVars() << " inputs=" << pureInputs.size() << " outputs=" << pureOutputs.size() << " gates=" << gates.size() << endl;

        ofstream AAG(config.opt_ee_aagFile);
        AAG << "aag " << data.nVars() << " " << pureInputs.size() << " "
            << "0 " << pureOutputs.size() << " " << gates.size() << endl;
        // input
    ...
    void EquivalenceElimination::printStatistics(ostream& stream) {
        stream << "c [STAT] EE " << eeTime << " s, " << steps << " steps " << equivalentLits << " ee-lits " << removedCls << " removedCls, "
               << removedViaSubsubption << " removedSubCls," << endl;
        if (config.opt_ee_level > 0) {
