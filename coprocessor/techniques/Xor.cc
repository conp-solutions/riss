...
namespace Coprocessor {
    ...
    XorReasoning::XorReasoning(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
                               Propagation& _propagation, EquivalenceElimination& _ee)
    ...
    bool XorReasoning::process() {
        ...
        if (data.outputsProof()) {
            printDRUPwarning(cerr, "XOR cannot produce DRUP/DRAT proofs yet");
        }
        ...
        DOUT(if (config.opt_xor_debug > 2) {
            for (int i = 0; i < xorList.size(); ++i) {
                cerr << "c [" << i << "]: "
                     << " + ";
                for (int j = 0; j < xorList[i].vars.size(); ++j) {
                    cerr << xorList[i].vars[j] + 1 << " + ";
                }
                cerr << " == " << (xorList[i].k ? 1 : 0) << endl;
            }
        });
        parseTime = cpuTime() - parseTime;
        ...
        for (int i = 0; i < xorList.size(); ++i) {
            for (int j = 0; j < xorList[i].vars.size(); ++j) {
                occs[xorList[i].vars[j]].push_back(i);
            }
        }
        ...
        while (heap.size() > 0 && (data.unlimited() || xorLimit > xorSteps) && !data.isInterupted()) {
            ...
            if (occs[v].size() == 1) {
                DOUT(if (config.opt_xor_debug > 4) cerr << "c there is a pure XOR on " << v + 1 << " of size " << xorList[occs[v][0]].size()
                                                        << endl;);
            ...
            while (selectIndex < occs[v].size() && xorList[occs[v][selectIndex]].used == true) {
                selectIndex++;
            } // ignore this XOR
            ...
            if (config.opt_xor_selectX == 1) { // select smallest xor
                for (int i = selectIndex + 1; i < occs[v].size(); ++i) {
                    xorSteps++;
                    if (xorList[occs[v][i]].used == true) {
                        continue;
                    }
                    if (xorList[occs[v][selectIndex]].size() > xorList[occs[v][i]].size()) {
                        selectIndex = i;
                    }
                }
            ...
            DOUT(if (config.opt_xor_debug > 1) {
                cerr << "c eliminate " << v + 1 << " with XOR "
                     << " + ";
                for (int j = 0; j < selectedX.vars.size(); ++j) {
                    cerr << selectedX.vars[j] + 1 << " + ";
                }
                cerr << " == " << (selectedX.k ? 1 : 0) << endl;
            });
            ...
            while (occs[v].size() > 0) {
                ...
                if (occs[v][0] == xorIndex) {
                    occs[v][0] = occs[v][occs[v].size() - 1];
                    occs[v].pop_back(); // drop it from this list!
                ...
                DOUT(if (config.opt_xor_debug > 1) {
                    cerr << "c change XOR "
                         << " + ";
                    for (int j = 0; j < simpX.vars.size(); ++j) {
                        cerr << simpX.vars[j] + 1 << " + ";
                    }
                    cerr << " == " << (simpX.k ? 1 : 0) << endl;
                });
                toRemoveVars.clear();
                tmpVars.clear();
                newlyAddedVariables.clear();
                ...
                DOUT(if (config.opt_xor_debug > 1) {
                    ...
                    cerr << " == " << (simpX.k ? 1 : 0) << endl;
                    cerr << "c and remove from ";
                    for (int j = 0; j < toRemoveVars.size(); ++j) {
                        cerr << " " << toRemoveVars[j] + 1;
                    }
                    cerr << " " << endl;
                });
                ...
                if (simpX.unit()) {
                    if (!data.ma.isCurrentStep(toInt(simpX.getUnitLit()))) {
                        ...
                        DOUT(if (config.opt_xor_debug > 1) {
                            cerr << "c created unit " << simpX.getUnitLit() << " from  XOR "
                                 << " + ";
                            for (int j = 0; j < simpX.vars.size(); ++j) {
                                cerr << simpX.vars[j] + 1 << " + ";
                            }
                    ...
                    DOUT(if (config.opt_xor_debug > 2) cerr << "c eq lits: " << mkLit(simpX.vars[0], false) << " == " << mkLit(simpX.vars[1], simpX.k)
                                                            << endl;);
        ...
        if (config.opt_xor_setPolarity != 0) {
            ...
            for (int i = 0; i < xorOrder.size(); ++i) {
                ...
                if (data.value(mkLit(v)) != l_Undef) {
                    continue;
                }                                                 // this variable is set already
    ...
    bool XorReasoning::simulatePropagate(vector<Lit>& unitQueue, MarkArray& ma, vector<vector<int>>& occs, vector<XorReasoning::GaussXor>& xorList,
    ...
    bool XorReasoning::propagate(vector<Lit>& unitQueue, MarkArray& ma, vector<std::vector<int>>& occs, vector<XorReasoning::GaussXor>& xorList) {
        while (unitQueue.size() > 0) {
            const Lit p = unitQueue.back();
            unitQueue.pop_back();
            if (l_False == data.enqueue(p)) {
                return false;
            } // enqueing next literal failed!
            // propagate in all xors that contain var(p)
            vector<int>& indexes = occs[var(p)];
            for (int i = 0; i < indexes.size(); ++i) {
                GaussXor& x = xorList[indexes[i]];
                ...
                if (x.unit()) { // add the newly created unit
                    if (!data.ma.isCurrentStep(toInt(x.getUnitLit()))) {
                        ...
                        unitQueue.push_back(x.getUnitLit());
                        DOUT(if (config.opt_xor_debug > 1) {
                            cerr << "c created unit " << x.getUnitLit() << " from  XOR "
                                 << " + ";
    ...
    void XorReasoning::dropXor(int index, vector<Var>& x, vector<std::vector<int>>& occs) {
        for (int i = 0; i < x.size(); ++i) {
            const Var& v = x[i];
            for (int j = 0; j < occs[v].size(); ++j)
                if (occs[v][j] == index) {
                    occs[v][j] = occs[v][occs[v].size() - 1];
                    occs[v].pop_back();
                    break;
                }
        }
    ...
    bool XorReasoning::findXor(vector<GaussXor>& xorList) {
        ...
        while (cP < table.size() && cL < config.opt_xorMatchLimit) {
            ...
            for (; start < cP - 1; ++start) {
                ...
                if (cl.size() > config.opt_xorMatchLimit || cl.can_be_deleted()) {
                    break;
                } // interrupt before too large size is recognized, or if the first ignored clause pops up
                ...
                for (uint32_t j = start; j < stop; ++j) {
                    ...
                    for (uint32_t k = 0; k < cl2.size(); k++)
                        if (!sign(cl2[k])) {
                            o = o ^ 1;
                        } // count literals that are set to true!
                    if (o == 1) {
                ...
                DOUT(if (config.opt_xor_debug > 4) {
                    ...
                    int k = 0;
                    for (; k < count[0]; ++k) {
                        cerr << "c " << ca[data.clss[k]] << endl;
                    }
                    cerr << "c o=0: " << endl;
                    for (; k < data.clss.size(); ++k) {
                        cerr << "c " << ca[data.clss[k]] << endl;
                    }
                });
                ...
                for (uint32_t o = 0; o < 2; ++o) {
                    ...
                    for (uint32_t j = 0; j < count[1 - o]; ++j) {
                        ...
                        DOUT(if (config.opt_xor_debug > 3) {
                            cerr << "c found " << ca[data.clss[j + offset]] << " matchIndex=" << index << "(" << nrByPol
                                 << ") original number: " << numberByPolarity(cl2) << endl;
                        });
                    ...
                    if (foundCount == 0) {
                        DOUT(if (config.opt_xor_debug > 2) cerr << "c reject this XOR, since it cannot produce an XOR (there is no large clause! o="
                                                                << o << ", found large clauses: " << foundCount << ")" << endl;);
                        continue;
                    }
                    ...
                    if (foundCount < shift && config.opt_xor_findSubsumed) {
                            ...
                            for (uint32_t match = 0; match < 2 * shift; ++match) {
                                if (foundByIndex[match / 2] == 1) {
                                    if (config.opt_xor_debug > 3) {
                                        cerr << "c match " << match / 2 << " is already covered!"
                                             << "(" << match << ") [not yet subsumption checked] " << endl;
                                    }
                                    continue;
                                }
                            }
                        ...
                        // check all smaller clauses (with limit!) whether they contain all the variables, if so, use them only, if the selected
                        // variable is the smallest one!
                        for (uint32_t j = 0; j < cL; ++j) {
                            Var cv = variables[j];
                            ...
                            for (uint32_t p = 0; p < 2; ++p) {          // polarity!
                                ...
                                for (uint32_t k = 0; k < cls.size(); ++k) {
                                    ...
                                    if (cls[k] == data.clss[offset]) {
                                        continue;
                                    } // do not consider the same clause twice!
                                    if (cclause.can_be_deleted()) {
                                        continue;
                                    }
                                    if (cclause.size() > config.opt_xorMatchLimit // ignore too large clauses
                                        ...
                                        continue;
                                    } // for performance, and it does not make sense to check clauses that are bigger than the current ones
                                    DOUT(if (config.opt_xor_debug > 4) cerr << "c consider clause " << cclause << endl;);

                                    Lit resolveLit = lit_Undef,
                                        foundLit = lit_Error; // if only one variable is wrong, the other literal could be resovled in?
                                    for (uint32_t m = 0; m < cclause.size(); ++m) {
                                        const Var ccv = var(cclause[m]);
                                        if (ccv < cv) {
                                            DOUT(if (config.opt_xor_debug > 4) cerr << "c reject " << cclause << " ,because currenct variable "
                                                                                    << ccv + 1 << " is smaller than " << cv + 1 << endl;);
                                        ...
                                        if (!data.ma.isCurrentStep(ccv)) {
                                            if (resolveLit == lit_Undef && config.opt_xor_findResolved) {
                                                ...
                                                // the literal that should be found to be resolved in has to be in the matching -> check all childs in
                                                // BIG
                                                const int iSize = big.getSize(resolveLit);
                                                for (int q = 0; q < iSize; ++q) {
                                                    if (data.ma.isCurrentStep(
                                                            var(big.getArray(resolveLit)[q]))) { // found variable that can be resolved in, and which
                                                                                                 // is part of the XOR!
                                                        foundLit = big.getArray(resolveLit)[q];
                                                        for (int m = 0; m < cclause.size();
                                                             ++m) {                       // check whether this literal already appears in the clause
                                                        ...
                                                        if (foundLit != lit_Error) {
                                                            break;
                                                        } // stop only, if the resolvent is not a tautology!
                                                    }
                                                }
                                                if (foundLit == lit_Error) { // not found a candidate!
                                                    DOUT(if (config.opt_xor_debug > 4) cerr << "c did not find a literal to pull in for resolveLit "
                                                                                            << resolveLit << endl;);
                                                    resolveLit = lit_Error;
                                                    break;
                                                }
                                                DOUT(if (config.opt_xor_debug > 3) cerr << "c found resolveLit " << resolveLit << ", which can pull "
                                                                                        << foundLit << " into the clause!" << endl;);
                                            } else {
                                                DOUT(if (config.opt_xor_debug > 4) cerr << "c clause contains non-matching literal " << resolveLit
                                                                                        << endl;);
                                                resolveLit = lit_Error;
                                                break;
                                            }
                                    ...
                                    if (subsumeSize == firstClause.size()) {
                                        ...
                                        for (int m = 0; m < cclause.size(); ++m) {
                                            const Lit ccl = cclause[m] != resolveLit
                                                                ? cclause[m]
                                                                : foundLit; // use the resolvedIn literal, instead of the literal that is resolved
                                        ...
                                        if (o0 != o) {
                                            DOUT(if (config.opt_xor_debug > 4) cerr << "c clause " << cclause
                                                                                    << " has all variables, but the odd value is wrong!" << endl;);
                                            continue; // can consider only smaller clauses, or their odd value is the same as for for the first
                                                      // clause!
                                        }
                                    }
                                    ...
                                    for (uint32_t m = 0; m < cclause.size(); ++m) {
                                        ...
                                        if (ccl == lit_Undef) {
                                            continue;
                                        } // nothing to do here (do not use same literal twice!)
                                        const Var ccv = var(ccl);
                                        bool hitVariable = false;
                                        for (uint32_t mv = 0; mv < cL; ++mv) {
                                            if (variables[mv] == ccv) {
                                                hitVariable = true;
                                                if (ccl != firstClause[mv]) {
                                                    subsumeOriginalClause = false;
                                                }
                                                DOUT(if (config.opt_xor_debug > 3) cerr << "c var " << ccv + 1 << " hits with literal " << ccl
                                                                                        << " -> add " << (1 << (cL - mv - 1)) << endl;);
                                    ...
                                    DOUT(if (config.opt_xor_debug > 3) {
                                        cerr << "c found subsume clause " << ca[cls[k]] << " with number=" << myNumber << " match=" << myMatch
                                             << endl;
                                    });
                                    ...
                                    for (uint32_t match = 0; match < 2 * shift; ++match) {
                                        ...
                                        if (foundByIndex[match / 2] == 1) {
                                            DOUT(if (config.opt_xor_debug > 3) cerr << "c match " << match / 2 << " is already covered!"
                                                                                    << "(" << match << ") [not yet subsumption checked] " << endl;);
                                        ...
                                        if ((match & myMatch) == myNumber) {
                                            ...
                                            DOUT(if (config.opt_xor_debug > 4) cerr << "c initial odd: " << o1 << ", cL: " << cL
                                                                                    << ", match: " << match << endl;);
                                            uint32_t matchBits = match;
                                            for (uint32_t km = 0; km < cL; km++) {
                                                if ((matchBits & 1) == 1) {
                                                    DOUT(if (config.opt_xor_debug > 4) cerr << "c bit " << km << " match, this odd=" << (int)(o1 ^ 1)
                                                                                            << endl;);
                                                    o1 = (o1 ^ 1);
                                                } else {
                                                    DOUT(if (config.opt_xor_debug > 4) cerr
                                                             << "c bit " << km << " does not match, this odd=" << (int)(o1 ^ 1) << endl;);
                                                }
                                                matchBits = matchBits >> 1;
                                            }
                                            // wrong polarity -> reject
                                            if (o1 != o) {
                                                DOUT(if (config.opt_xor_debug > 4) cerr << "c current odd " << o1 << " does not match XOR odd " << o
                                                                                        << endl;);
                                                continue;
                                            }

                                            DOUT(if (config.opt_xor_debug > 3) cerr << "c current clause subsumes match=" << match / 2 << "(" << match
                                                                                    << ") - match&myMatch=" << (int)(match & myMatch) << " vs "
                                                                                    << myNumber << " with odd=" << o1 << endl;);
                                            if (foundByIndex[match / 2] == 0) {
                                                foundCount++;
                                            } // only if this clause sets the value from 0 to 1!
        ...
        // calculate ratio
        clauseRatio = data.getClauses().size() - ignoredRatioClauses == 0
                          ? 1
                          : (double)participatingXorClauses / (double)(data.getClauses().size() - ignoredRatioClauses);
    ...
    CRef XorReasoning::addClause(const Lit* lits, int size, bool learnt) {
        ...
        if (config.opt_xor_checkNewSubsume) {
            reAddedClauses.push(cr);
        }
        return cr;
    }

    void XorReasoning::printStatistics(ostream& stream) {
        stream << "c [STAT] XOR " << processTime << "s, " << parseTime << " parse-s, " << reasonTime << " reason-s, " << endl
               << "c [STAT] XOR(2) " << xors << " xors, " << subsFound << " subXors, " << resFound << " resXors, " << xorClauses << " clauses, "
               << resolvedInCls << " resClauses, " << resTauts << " resTauts, " << resStrength << " resStrength, " << findChecks << " findChecks, "
               << endl
               << "c [STAT] XOR(3) " << xorSteps << " steps, " << foundEmptyLists << " empties, " << xorUnits << " listUnits, " << allUsed
               << " useLists, " << xorDeducedUnits << " xorUnits, " << eqs << " eqs, " << endl
               << "c [STAT] XOR(4) " << participatingXorClauses << " participating, " << clauseRatio << " ratio, " << simDecisions
               << " simDecisions, " << xorProps << " xorProps, " << clsProps << " clsProps, " << endl;
    }
