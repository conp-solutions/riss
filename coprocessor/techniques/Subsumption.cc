...
namespace Coprocessor {
    ...
    Subsumption::Subsumption(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
    ...
    void Subsumption::printStatistics(ostream& stream) {
        stream << "c [STAT] SuSi(1) " << processTime << " s, " << subsumedClauses << " cls, "
               << " with " << subsumedLiterals << " lits, " << removedLiterals << " strengthed " << endl;
        stream << "c [STAT] SuSi(2) " << subsumptionStepper.getCurrentSteps() << " subs-steps, " << strengtheningStepper.getCurrentSteps()
               << " strenght-steps, " << limitIncreases << " increases, " << strengthTime << "s strengthening " << endl;
        for (int i = 0; i < controller.size(); ++i) {
            stream << "c [STAT] SuSi(1)-Thread " << i << " " << localStats[i].processTime << " s, " << localStats[i].subsumedClauses << " cls, "
                   << " with " << localStats[i].subsumedLiterals << " lits, " << localStats[i].removedLiterals << " strengthed " << endl;
            stream << "c [STAT] SuSi(2)-Thread " << i << " " << localStats[i].subsumeSteps << " subs-steps, " << localStats[i].strengthSteps
                   << " strenght-steps, " << localStats[i].strengthTime << "s strengthening " << localStats[i].lockTime << "s locking " << endl;
        }
    }
    ...
    bool Subsumption::process(bool doStrengthen, Heap<VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        ...
        DOUT(if (config.opt_sub_debug > 1) cerr << "c call susi process with subs: " << data.getSubsumeClauses()
                                                << " and str: " << data.getStrengthClauses() << endl;);
        ...
        if (!performSimplification()) { // if nothing to be done, at least clean all the lists!
            ...
            for (int i = 0; i < data.getSubsumeClauses().size(); ++i) {
                ca[data.getSubsumeClauses()[i]].set_subsume(false);
            }
            data.getSubsumeClauses().clear();
            for (int i = 0; i < data.getStrengthClauses().size(); ++i) {
                ca[data.getStrengthClauses()[i]].set_strengthen(false);
            }
        ...
        DOUT(if (config.opt_sub_debug > 0) {
            ...
            for (int i = 0; i < data.getSubsumeClauses().size(); ++i) {
                cerr << "c [" << i << "](" << data.getSubsumeClauses()[i] << ") : " << ca[data.getSubsumeClauses()[i]] << endl;
            }
            cerr << "c check for strengthening: " << endl;
            for (int i = 0; i < data.getStrengthClauses().size(); ++i) {
                cerr << "c [" << i << "](" << data.getStrengthClauses()[i] << ") : " << ca[data.getStrengthClauses()[i]] << endl;
            }
        });
        ...
        while (data.ok() && (hasToSubsume() || hasToStrengthen())) {
            ...
            if (hasToStrengthen()) {
                ...
                if (controller.size() > 0 && (config.opt_sub_par_strength == 2 || (data.getStrengthClauses().size() > config.opt_sub_par_str_minCls &&
                                                                                   config.opt_sub_par_strength == 1))) {
                    ...
                    if (controller.size() > 0) {
                        strengtheningStepper.increaseSteps(avgParStrengthSteps / controller.size());
                    }
    ...
    lbool Subsumption::fullSubsumption(Heap<VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        ...
        if (heap == 0 && controller.size() > 0 &&
            (config.opt_sub_par_subs == 2 ||
             (config.opt_sub_par_subs == 1 &&
              (data.getSubsumeClauses().size() > 100000 || (data.getSubsumeClauses().size() > 50000 && 10 * data.nCls() > 22 * data.nVars()))))) {
            ...
            // increase seq StrengthSteps by parallel Avarage
            if (controller.size() > 0) {
                subsumptionStepper.increaseSteps(avgParSubsSteps / controller.size());
            }
    ...
    void Subsumption ::subsumption_worker(unsigned int start, unsigned int end, Heap<VarOrderBVEHeapLt>* heap, const Var ignore,
                                          const bool doStatistics) {
        vector<CRef>& occ_updates = subs_occ_updates;
        if (doStatistics) {
            processTime = cpuTime() - processTime;
        }
        DOUT(if (config.opt_sub_debug > 1) cerr << "subsume from " << start << " to " << end << " with size " << data.getSubsumeClauses().size()
                                                << endl;);
        ...
        for (; end > start && !data.isInterupted() && (data.unlimited() || (doStatistics && subsumptionStepper.inLimit()));) {
            --end;
            ...
            for (unsigned i = 0; i < list.size() && (data.unlimited() || (doStatistics && subsumptionStepper.inLimit())); ++i) {

                if (list[i] == cr) {
                    continue;
                }
                if (doStatistics) {
                    subsumptionStepper.increaseSteps(1);
                }
                ...
                if (ca[list[i]].can_be_deleted()) { // do not use this clause, but remove it from the list!
                    ...
                    DOUT(if (config.opt_sub_debug > 1) cerr << "c subsumption removed " << (ca[list[i]].learnt() ? "learned" : "") << " clause "
                                                            << ca[list[i]] << " by " << (ca[list[i]].learnt() ? "learned" : "") << c << endl;);
     ...
     */
    void Subsumption ::par_subsumption_worker(unsigned int& next_start, unsigned int global_end, SpinLock& balancerLock, vector<CRef>& to_delete,
                                              vector<CRef>& set_non_learnt, struct SubsumeStatsData& stats, const bool doStatistics) {
        ...
        while (global_end > next_start && !data.isInterupted() &&
            ...
            balancerLock.unlock();
            while (end > start && !data.isInterupted() && (data.unlimited() || (doStatistics && subsumptionStepper.inLimit(stats.subsumeSteps)))) {
                --end;
                const CRef cr = data.getSubsumeClauses()[end];
                Clause& c = ca[cr];
                if (doStatistics) {
     ...
     *
     */
    void Subsumption::par_strengthening_worker(unsigned int& next_start, unsigned int global_stop, SpinLock& balancerLock, vector<SpinLock>& var_lock,
                                               struct SubsumeStatsData& stats, vector<OccUpdate>& occ_updates, Heap<VarOrderBVEHeapLt>* heap,
                                               const Var ignore, const bool doStatistics) {
        ...
        while (global_stop > next_start && data.ok() && !data.isInterupted()) {
            ...
            lock_strengthener:
                ...
                // lock 1st var
                if (config.opt_sub_lock_stats) {
                    lock_time = cpuTime() - lock_time;
                }
                var_lock[fst].lock();
                if (config.opt_sub_lock_stats) {
                    lock_time = cpuTime() - lock_time;
                ...
                for (int l = 0; l < c.size(); ++l) {
                    ...
                    lock_to_strengthen:
                        ...
                        if (l1 == c.size()) {
                            assert(pos != -1 && "Position invalid"); // TODO -> if this happens, we found normel a subsumption case?, so why not deal
                                                                     // with it? this is no error
                            if (doStatistics) {
                                ++stats.removedLiterals;
                            }
                            modifiedFormula = true;
                            // unit found
                            if (d.size() == 2) {
                                ...
                                data.addCommentToProof("found during strengthening");
                                data.addUnitToProof(unitLit); // add the unit
     ...
     * @param var_lock vector of locks for each variable
     */
    void Subsumption::par_nn_strengthening_worker(unsigned int& next_start, unsigned int global_end, SpinLock& balancerLock,
                                                  vector<SpinLock>& var_lock, struct SubsumeStatsData& stats, vector<OccUpdate>& occ_updates,
                                                  Heap<VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        ...
        while (global_end > next_start && !data.isInterupted() &&
            ...
            balancerLock.unlock();
            while (end > start && !data.isInterupted() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit(stats.strengthSteps)))) {
                if (!data.ok()) {
                    stats.strengthTime = wallClockTime() - stats.strengthTime;
                    return;
                }
            ...
            lock_strengthener_nn:
                ...
                if (strengthener.size() < 2) {
                    if (strengthener.size() == 1) {
                        data_lock.lock();
                        data.addCommentToProof("found during strengthening");
                        data.addUnitToProof(strengthener[0]); // add the unit
                ...
                if (l_False ==
                    par_nn_strength_check(data, list, localQueue, strengthener, cr, fst, var_lock, stats, occ_updates, heap, ignore, doStatistics)) {
                    var_lock[fst].unlock();
                    if (doStatistics) {
                        stats.strengthTime = wallClockTime() - stats.strengthTime;
                    }
                ...
                if (l_False == par_nn_negated_strength_check(data, list_neg, localQueue, strengthener, cr, min, fst, var_lock, stats, occ_updates,
                                                             heap, ignore, doStatistics)) {
                    var_lock[fst].unlock();
                    if (doStatistics) {
                        stats.strengthTime = wallClockTime() - stats.strengthTime;
                    }
     ...
     *
     */
    inline lbool Subsumption::par_nn_strength_check(CoprocessorData& data, vector<CRef>& list, deque<CRef>& localQueue, Clause& strengthener, CRef cr,
                                                    Var fst, vector<SpinLock>& var_lock, struct SubsumeStatsData& stats,
                                                    vector<OccUpdate>& occ_updates, Heap<VarOrderBVEHeapLt>* heap, const Var ignore,
                                                    const bool doStatistics) {
        ...
        for (unsigned int j = 0;
             j < list.size() && !data.isInterupted() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit(stats.strengthSteps)));
             ++j) {
        ...
        lock_to_strengthen_nn:
            ...
            if (negated_lit_pos != -1 && si == strengthener.size()) {
                ...
                // TODO optimize out
                    ...
                    DOUT(if (config.opt_sub_debug > 1) cerr << "c remove (@" << __LINE__ << ")" << negated_lit_pos << " from clause " << other
                                                            << endl;);
                    // keep track of this clause for further strengthening!
                    ...
                    DOUT(if (config.opt_sub_debug > 1) cerr << "c add remove info (@" << __LINE__ << ")  [" << occ_updates.size() << "] : " << list[j]
                                                            << " with clause " << neg << " and ref " << list[j] << endl;);
     ...
     *
     */
    inline lbool Subsumption::par_nn_negated_strength_check(CoprocessorData& data, vector<CRef>& list, deque<CRef>& localQueue, Clause& strengthener,
                                                            CRef cr, Lit min, Var fst, vector<SpinLock>& var_lock, struct SubsumeStatsData& stats,
                                                            vector<OccUpdate>& occ_updates, Heap<VarOrderBVEHeapLt>* heap, const Var ignore,
                                                            const bool doStatistics) {
        int si, so;          // indices used for "can be strengthened"-testing
        int negated_lit_pos; // index of negative lit, if we find one
    ...
    /** runs a fullstrengthening on data.getStrengthClauses(), needs occurrencelists (naive strengthening seems to be faster, TODO: strengthening on
     * literals with minimal occurrences)
    ...
    lbool Subsumption::fullStrengthening(Heap<VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        ...
        for (int i = 0; i < data.getStrengthClauses().size() && !data.isInterupted();) {
            ...
            for (int j = 0; j < c.size() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit())); ++j) {
                ...
                for (unsigned int k = 0; k < list.size() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit())); ++k) {
                    if (list[k] == cr) {
                        continue;
                    }

                    if (doStatistics) {
                        strengtheningStepper.increaseSteps(1);
                    }
                    ...
                    if (l1 == c.size() && pos != -1) {
                        ...
                        data.addToProof(other);
                        data.addToProof(other, true, remLit);               // add information to proof!
                        ...
                        if (other.size() == 1) {
                            ...
                            if (!other.can_subsume()) {
                                // if the flag was set, this clause is allready in the subsumptionqueue, if not, we need to add this clause as it
                                // could subsume again
                                other.set_subsume(true);
                            ...
                            DOUT(if (config.opt_sub_debug > 1) cerr << "c add remove info (@" << __LINE__ << ")  [" << occ_updates.size() << "] : "
                                                                    << list[k] << " with clause " << neg_lit << " and ref " << list[k] << endl;);
     ...
     *
     */
    lbool Subsumption::createResolvent(const Riss::CRef& cr, Riss::CRef& resolvent, const int negated_lit_pos,
                                       Heap<Coprocessor::VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        Clause& origin = ca[cr];
        ...
        if (origin.size() == 2) {
            ...
            data.addCommentToProof("found by strengthening");
            data.addUnitToProof(unitLit);
            lbool state = data.enqueue(unitLit);
            if (l_False == state) {
                return l_False;
            } else {
    ...
    /** the strengthening-methode, which runs through the strengtheningqueue from start to end and tries to strengthen with the clause from str-queue
     * on the clauses from data
     ...
     *
     */
    lbool Subsumption::strengthening_worker(unsigned int start, unsigned int end, Heap<VarOrderBVEHeapLt>* heap, const Var ignore,
                                            const bool doStatistics) {
        if (doStatistics) {
            strengthTime = cpuTime() - strengthTime;
        }
        ...
        for (; end > start && !data.isInterupted() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit()));) {
            CRef cr = CRef_Undef;
            if (localQueue.size() == 0) {
                --end;
                cr = data.getStrengthClauses()[end];
            ...
            if (strengthener.size() < 2) {
                if (strengthener.size() == 1) {
                    if (l_False == data.enqueue(strengthener[0])) {
                        break;
                    } else {
                        continue;
                    }
            ...
            // test every clause, where the minimum is, if it can be strenghtened
            for (unsigned int j = 0;
                 j < list.size() && !data.isInterupted() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit())); ++j) {
                if (list[j] == cr) {
                    continue;
                }
                Clause& other = ca[list[j]];
                ...
                if (other.size() == 0) {
                    assert(!data.ok() && "unsat should be found already!");
                    break;
                }                             // here, data.ok should be false already -> nothing to be done!
                if (other.can_be_deleted()) { // skip, but remove from list!
                    DOUT(if (config.opt_sub_debug > 1) cerr << "c overwrite for literal " << min << " : list[" << j << "]=" << list[j]
                                                            << " with list[" << list.size() - 1 << "] = " << list.back() << endl;);
                    list[j] = list.back();
                    list.pop_back();
                    --j;
                    continue;
                ...
                if (negated_lit_pos != -1 &&
                    si == strengthener.size()) { // TODO if negated_lit_pos == -1 -> normal subsumption case, why not apply  it?
                    if (other.size() <= config.opt_sub_allStrengthRes) { // check this only for relevant clauses
                        ...
                        if (l_Undef == status) {
                            ...
                            ca[newCRef].setExtraInformation(strengthener.extraInformation());
                            ca[newCRef].updateExtraInformation(other.extraInformation()); // setup extra information
                        ...
                        if (!other.can_subsume()) {
                            // if the flag was set, this clause is allready in the subsumptionqueue, if not, we need to add this clause as it could
                            // subsume again
                            other.set_subsume(true);
                            data.getSubsumeClauses().push_back(list[j]);
                        }
                        ...
                        successfulSimplification();
                        DOUT(if (config.opt_sub_debug > 1) cerr << "c add remove info (@" << __LINE__ << ")  [" << occ_updates.size()
                                                                << "] : " << list[j] << " with clause " << other[negated_lit_pos] << " and ref "
                                                                << list[j] << endl;);
                        occ_updates.push_back(OccUpdate(list[j], other[negated_lit_pos]));
                        DOUT(if (config.opt_sub_debug > 1) cerr << "c remove (@" << __LINE__ << ")" << other[negated_lit_pos] << " from clause "
                                                                << other << " with " << strengthener << endl;);
                        ...
                        data.addToProof(other);
                        data.addToProof(other, true, remLit);                          // add clause to proof
            ...
            for (unsigned int j = 0;
                 j < list_neg.size() && !data.isInterupted() && (data.unlimited() || (doStatistics && strengtheningStepper.inLimit())); ++j) {
                Clause& other = ca[list_neg[j]];
                if (doStatistics) {
                    strengtheningStepper.increaseSteps(1);
                }
                if (other.can_be_deleted()) {
                    DOUT(if (config.opt_sub_debug > 1) cerr << "c overwrite for literal " << nmin << " : list_neg[" << j << "]=" << list_neg[j]
                                                            << " with list_neg[" << list_neg.size() - 1 << "] = " << list_neg.back() << endl;);
                    list_neg[j] = list_neg.back();
                    list_neg.pop_back();
                    --j;
                    continue;
                }
                ...
                if (si == strengthener.size() && negated_lit_pos != -1) {
                    if (other.size() <= config.opt_sub_allStrengthRes) {
                        ...
                        DOUT(if (config.opt_sub_debug > 1) {
                            cerr << "c add remove info (@" << __LINE__ << ")  [" << occ_updates.size() << "] : " << list_neg[j] << " with clause "
                                 << other[negated_lit_pos] << " and ref " << list_neg[j] << endl;
                            cerr << "c lit " << nmin << " list: " << data.list(nmin) << endl;
                        });
                        occ_updates.push_back(OccUpdate(list_neg[j], other[negated_lit_pos]));
                        DOUT(if (config.opt_sub_debug > 1) cerr << "c remove (@" << __LINE__ << ")" << other[negated_lit_pos] << " from clause "
                                                                << other << " with strengthener " << strengthener << " ( via index: " << list_neg[j]
                                                                << " cls: " << ca[list_neg[j]] << ")" << endl;);
                        successfulSimplification();
                        const Lit remLit = other[negated_lit_pos];
                        other.removePositionSorted(negated_lit_pos);
                        data.addCommentToProof("removed literal by strengthening");
                        data.addToProof(other);
                        data.addToProof(other, true, remLit);                          // add to proof
                        other.updateExtraInformation(strengthener.extraInformation()); // update extra info
    ...
    inline void Subsumption::updateOccurrences(vector<OccUpdate>& updates, Heap<VarOrderBVEHeapLt>* heap, const Var ignore) {
        for (int i = 0; i < updates.size(); ++i) {
            ...
            if (data.removeClauseFrom(updates[i].cr, updates[i].l)) {
                DOUT(if (config.opt_sub_debug > 1) cerr << "c successfully removed clause [" << i << "/" << updates.size()
                                                        << "] from list for literal " << updates[i].l << " from clause " << ca[updates[i].cr] << " ["
                                                        << updates[i].cr << "]" << endl;);
                data.removedLiteral(updates[i].l, 1, heap, ignore);
            } else {
                DOUT(if (config.opt_sub_debug > 1) cerr << "c DID NOT remove clause [" << i << "/" << updates.size() << "] from list for literal "
                                                        << updates[i].l << " from clause " << ca[updates[i].cr] << " [" << updates[i].cr << "]"
                                                        << endl;);
            }
        }
        updates.clear();
    ...
    void Subsumption::parallelSubsumption(const bool doStatistics) {
        if (doStatistics) {
            processTime = wallClockTime() - processTime;
        }
        if (false) {
    ...
    void* Subsumption::runParallelSubsume(void* arg) {
        ...
        workData->subsumption->par_subsumption_worker(*(workData->start), workData->end, *(workData->balancerLock), *(workData->to_delete),
                                                      *(workData->set_non_learnt), *(workData->stats));
        return 0;
    }
    ...
    void Subsumption::parallelStrengthening(Heap<VarOrderBVEHeapLt>* heap, const Var ignore, const bool doStatistics) {
        ...
        if (doStatistics) {
            strengthTime = wallClockTime() - strengthTime;
        }
        SubsumeWorkData workData[controller.size()];
    ...
    void* Subsumption::runParallelStrengthening(void* arg) {
        SubsumeWorkData* workData = (SubsumeWorkData*)arg;
        if (workData->config->opt_sub_naivStrength) {
            workData->subsumption->par_strengthening_worker(*(workData->start), workData->end, *(workData->balancerLock), *(workData->var_locks),
                                                            *(workData->stats), *(workData->occ_updates), workData->heap, workData->ignore);
        } else {
            workData->subsumption->par_nn_strengthening_worker(*(workData->start), workData->end, *(workData->balancerLock), *(workData->var_locks),
                                                               *(workData->stats), *(workData->occ_updates), workData->heap, workData->ignore);
        }
        return 0;
    }
