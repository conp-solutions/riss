...
namespace Coprocessor {
    ...
    class FourierMotzkin : public Technique<FourierMotzkin> {
        ...
        int semExtendedCards, semFailedExtendTries, semExtendLits, semReducedDegrees, semTotalProbes, semTotalFailedProbes, semNrDisabledClauses,
            semNrPreDisabledClauses, semUnits;
        uint64_t semSteps;
        ...
        struct LitOrderHeapLt {
            ...
            LitOrderHeapLt(CoprocessorData& _data)
                : data(_data) {
            }
        };
        ...
        struct Ternary {
            ...
            Ternary(const Riss::Lit& a, const Riss::Lit& b, const Riss::Lit& c) {
                ...
                lit[1] = Riss::toLit(toInt(a) ^ toInt(b) ^ toInt(c) ^ toInt(lit[0]) ^
                                     toInt(lit[2])); // xor all three lits and min and max (the middle remains)
            }
    ...
    public:
        ...
        public:
            ...
            CardC(std::vector<Riss::Lit>& amo)
                : ll(amo)
                , k(1)
                , id(getNextID())
                , parentL(-1)
                , parentR(-1){}; // constructor to add amo constraint
            CardC(std::vector<Riss::Lit>& amk, int _k)
                : ll(amk)
                , k(_k)
                , id(getNextID())
                , parentL(-1)
                , parentR(-1){}; // constructor to add amk constraint
            CardC(const Riss::Clause& c)
                : k(-1)
                , id(getNextID())
                , parentL(-1)
                , parentR(-1) {
                lr.resize(c.size(), Riss::lit_Undef);
                for (int i = 0; i < c.size(); ++i) {
                    lr[i] = c[i];
                }
            } // constructor for usual clauses

            bool amo() const {
                return k == 1 && lr.size() == 0;
            }
            bool amt() const {
                return k == 2 && lr.size() == 0;
            }
            bool amk() const {
                return k >= 0 && lr.size() == 0;
            }
            bool alo() const {
                return k == -1 && ll.size() == 0;
            }
            bool alk() const {
                return k < 0 && ll.size() == 0;
            }
            bool isUnit() const {
                return (k + (int)lr.size()) == 0;
            } // all literals in ll have to be false, and all literals in lr have to be true
            bool failed() const {
                return (((int)lr.size() + k) < 0);
            }
            bool taut() const {
                return k >= (int)ll.size();
            } // assume no literal appears both in ll and lr
            bool invalid() const {
                return k == 0 && ll.size() == 0 && lr.size() == 0;
            } // nothing stored in the constraint any more
            void invalidate() {
                if (!invalid()) {
                    k = 0;
                    std::vector<Riss::Lit>().swap(ll);
                    std::vector<Riss::Lit>().swap(lr);
                }
            }
            ...
            void setParents(int idL, int idR) {
                assert(idL != idR && "cannot have the same parent twice");
                assert(parentL == -1 && parentR == -1);
                parentL = idL;
                parentR = idR;
            }
            int getID() const {
            ...
            int getParentR() const {
                return parentR;
            }
            int getParentL() const {
                return parentL;
            }
        };

    public:
        FourierMotzkin(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller, CoprocessorData& _data,
                       Propagation& _propagation, Riss::Solver& _solver);
    ...
    protected:
        ...
        bool propagateCards(Riss::vec<Riss::Lit>& unitQueue, std::vector<std::vector<int>>& leftHands, std::vector<std::vector<int>>& rightHands,
                            std::vector<CardC>& cards, Riss::MarkArray& inAmo);
