...
namespace Coprocessor {
    ...
    lbool Propagation::process(CoprocessorData& data, bool sort, Heap<VarOrderBVEHeapLt>* heap, const Var ignore) {
        ...
        DOUT(if (config.up_debug_out > 0) cerr << "c call propagate. already propagated: " << lastPropagatedLiteral
                                               << ", total to proapgate: " << solver->trail.size() << endl;);
        ...
        for (; lastPropagatedLiteral < solver->trail.size(); lastPropagatedLiteral++) {
            ...
            for (int i = 0; i < positive.size(); ++i) {
                ...
                DOUT(if (config.up_debug_out > 0) cerr << "c UP remove " << ca[positive[i]] << endl;);
            ...
            for (int i = 0; i < negative.size(); ++i) {
                ...
                for (int j = 0; j < c.size(); ++j) {
                    if (c[j] == nl) {
                        DOUT(if (config.up_debug_out > 0) cerr << "c UP remove " << nl << " from " << c << endl;);
                ...
                if (c.size() == 0) {
                    ...
                    DOUT(if (config.up_debug_out > 0) cerr << "c UNSAT by UP" << endl;);
                    processTime = cpuTime() - processTime;
                    return l_False;
                } else if (c.size() == 1) {
                    c.set_delete(true); // drop this clause from checks!
                    DOUT(if (solver->value(c[0]) == l_Undef && config.up_debug_out > 0) cerr
                             << "c UP enqueue " << c[0] << " with previous value "
        ...
        } // this call did not change anything
        processTime = cpuTime() - processTime;
    ...
    void Propagation::printStatistics(ostream& stream) {
        stream << "c [STAT] UP " << processTime << " s, " << lastPropagatedLiteral << " units, " << removedClauses << " cls, " << removedLiterals
               << " lits" << endl;
    }
