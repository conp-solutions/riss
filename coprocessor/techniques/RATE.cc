...
namespace Coprocessor {
    ...
    RATElimination::RATElimination(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data, Solver& _solver,
    ...
    bool RATElimination::process() {
        if (!config.opt_rate_rate && !config.opt_rate_bcs && !config.opt_rate_ratm && !config.opt_rate_ratm_extended) {
            return false;
        }
    ...
    bool RATElimination::eliminateRAT() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (data.doNotTouch(v)) {
                continue;
            } // do not consider variables that have to stay fixed!
            if (data[mkLit(v, false)] > 0)
                if (!rateHeap.inHeap(toInt(mkLit(v, false)))) {
                    nextRoundLits.push_back(mkLit(v, false));
                }
            if (data[mkLit(v, true)] > 0)
                if (!rateHeap.inHeap(toInt(mkLit(v, true)))) {
        ...
        do {
            ...
            for (int i = 0; i < nextRoundLits.size(); ++i) {
                ...
                if (!nextRound.isCurrentStep(toInt(l))) {
                    continue;
                } // has been processed before already
            ...
            while (rateHeap.size() > 0 && (data.unlimited() || config.rate_Limit > rateSteps) && !data.isInterupted()) {
                ...
                DOUT(if (config.opt_rate_debug > 0) cerr << endl
                                                         << "c RATE work on literal " << right << " with " << data.list(right).size() << " clauses "
                                                         << endl;);
                ...
                for (int i = 0; i < data.list(right).size(); ++i) {
                    ...
                    if (c.size() < config.rate_minSize) {
                        DOUT(if (config.opt_rate_debug > 3) cerr << "c RATE reject clause due to size: " << c << " (vs. " << config.rate_minSize
                                                                 << ")" << endl;);
                        continue;
                    ...
                    } // enqueue all complements

                    bool confl =
                        CRef_Undef !=
                        solver.propagate(); // check whether unit propagation finds a conflict for (F \ C) \land \ngt{C}, and hence C would be AT
                    ...
                    if (confl == true) { // found a conflict during propagating ~C
                        remAT++;
                        // ca[ data.list(right)[i]].sort();        // necessary to ensure a general result -->  RATE is not confluent! (only for
                        // benchnmarking!)
                        solver.cancelUntil(0); // backtrack

                        // reference c might not be valid any more (because propagate could have created new clauses via LHBR), hence, use ca[
                        // data.list(right)[i]]
                        ...
                        DOUT(if (config.opt_rate_debug > 1) cerr << "c RATE eliminate AT clause [" << data.list(right)[i] << "] "
                    ...
                    for (int j = 0; allResolventsRedundant && j < data.list(left).size(); ++j) { // for all clauses D \in F_{\ngt{l}}
                        ...
                        if (isTaut) { // if the resolvent is a tautology, then the resolvent is redundant wrt. formula, and we do not need to perform
                                      // propagation
                        ...
                        for (int k = defaultLits; k < data.lits.size(); ++k) {
                            ...
                            }                                                // check whether the literal is not propagated
                        ...
                        DOUT(if (config.opt_rate_debug > 2) {
                            ...
                            if (ppConfl != CRef_Undef) {
                                cerr << "c conflict: " << ca[ppConfl] << endl;
                            }
                        });
                        ...
                        if (confl == false) {
                            ...
                            if (config.opt_rate_brat) {
                                ...
                                //        for( int k = 0 ; k < data.lits.size(); ++k ) bratResolve.setCurrentStep( toInt(data.lits[k] ) ); // create
                                //        markArray for redundant clause
                                // for (int k = 0 ; k < data.lits.size(); ++k) {
                                const Lit resL = right; // currently not sure how to restore resolvents, that are not blocked on the literal we
                                                        // currently process ... data.lits[k]
                                if (data.doNotTouch(var(resL))) {
                                    isblocked = true;
                                    break;
                                } // do not perform blocked clause addition on doNotTouch variables
                                isblocked = true;
                                ...
                                for (int m = 0; m < data.list(~resL).size(); ++m) { // resolve with all candidates
                                    ...
                                    if (data.list(right)[i] == data.list(~resL)[m]) {
                                        blockCheckOnSameClause++;
                                    }
                                    ...
                                    for (int n = 0; n < e.size(); ++n) {
                                        ...
                                        if (data.ma.isCurrentStep(toInt(~e[n]))) {
                                            hasComplement = true;
                                            break;
                                        }
                                    }
        ...
        } while (nextRoundLits.size() > 0 && (data.unlimited() || config.rate_Limit > rateSteps) &&
            ...
            for (int g = trailPosition; g < solver.trail.size(); ++g) {
                ...
                for (int p = 0; p < data.list(negLit).size(); ++p) {
                    ...
                    if (!gpclause.can_be_deleted()) {
                        ...
                        if (gpclause.size() > 2) {
                            // hard to be checked as we do not know the blocking literal...           assert( ( (gpclause[0] != negLit && gpclause[1]
                            // != negLit) || data.value( gpclause[0] ) == l_True || data.value( gpclause[1] ) == l_True ) && "the literal can be in
                            // the first two places only if the clause is satisfied (or if the blocking literal is satisfied)" );
                            ...
                            assert((data.value(gpclause[0]) == l_True || data.value(gpclause[1]) == l_True) &&
                                   "in larger clauses, the clause is satisfied, or the literal is not on the first two positions");
                            gpclause.set_delete(true);
                            data.addToProof(gpclause, true); // remove the clause from the proof (its subsumed by a unit clause)
    ...
    void RATElimination::checkedAttach(const Riss::CRef& clause, const int& detachTrailSize) {
        ...
        if (solver.value(ca[clause][0]) == l_Undef && solver.value(ca[clause][1]) == l_Undef) {
            ...
            if (solver.value(ca[clause][0]) == l_Undef && solver.value(ca[clause][1]) == l_Undef) {
                ...
                assert(solver.value(ca[clause][0]) != l_True && solver.value(ca[clause][1]) != l_True &&
                       "The clause is satified and shouldn't be attached!");
                if (solver.value(ca[clause][0]) != l_Undef) {
        ...
        if (!config.opt_rate_ratm_extended) {
            ...
            for (int j = 0; j < atlits.size(); ++j) {
                ...
                if (solver.value(atlits[j]) == l_Undef) {
                    ...
                    confl = (solver.value(atlits[j]) == l_True);
                }
            ...
            if (confl) {
                ...
                if (delPrefix > 0 ||
                    lastKeptLiteral < atlits.size() - 1) { // skip clauses, where all negated literals are needed to cause the conflict
                    ...
                    assert(solver.value(ca[clause][0]) == l_Undef && solver.value(ca[clause][1]) == l_Undef &&
                           "Can't detach a clause where one of the watched literals isn't l_Undef");
                    ...
                    cl.shrink(cl.size() - (k));
                    assert(a != cl.size() &&
                           "Clause is not shrinked! Infinite loop may cause!"); // FIXME int a needed to check this, delete/comment after fuzzing
                    data.addCommentToProof("remove literals by ATM");
                    data.addToProof(cl); // add the shrinked clause to the proof
                    data.addToProof(atlits, true,
                                    left); // remove the original clause from the proof (all literals from atlits as well as the literal left)
    ...
    bool RATElimination::minimizeRAT() {
        ...
        do {
            ...
            while (rateHeap.size() > 0 && (data.unlimited() || config.ratm_Limit > ratmSteps) && !data.isInterupted() && data.ok()) {
                ...
                if (data.doNotTouch(var(right))) {
                    continue;
                } // do not consider variables that have to stay fixed or which are already propagated!
                if (solver.value(right) != l_Undef) {
                    continue;
                }
                const Lit left = ~right; // complement

                DOUT(if (config.opt_rate_debug > 0) cerr << endl
                                                         << "c RATM work on literal " << right << " with " << data.list(right).size() << " clauses "
                                                         << endl;);
                DOUT(if (config.opt_rate_debug > 3) cerr << "current trail: " << data.getTrail() << endl;);
                ...
                for (int i = 0; i < data.list(right).size() && data.ok(); ++i) {
                    ...
                    for (int j = 0; j < data.list(left).size() && data.ok() && confl; ++j) {
                        ...
                        if (shortATM(clause, left, trailPosition, atlits)) { // calls the ATM routine, returns true, if clause has AT
                            ...
                            if (solver.value(right) !=
                                l_Undef) { // if left got a value in a Unit-Propagation it is possible a clause will be shrinked wrongly
                                confl = false;
                                break;
                            }
                            ...
                            // should be possible, if clause is already satified ( at this point we are on level 1 ). In this case, a conflict will
                            // follow!
                            //     assert( solver.value(ca[cr][0]) == l_Undef && solver.value(ca[cr][1]) == l_Undef && "Can't detach a clause where
                            //     one of the watched literals isn't l_Undef");
                        ...
                        for (int k = 0; k < data.lits.size(); ++k) {
                            if (solver.value(data.lits[k]) == l_Undef) {
                                ...
                                confl = (solver.value(data.lits[k]) == l_True);
                            }

                            if (confl) {
                                if (k >= lastKeptLiteral) {
                                    lastKeptLiteral = k + 1;
                                }
                                // ratmSteps += k+1;
                                break;
                    ...
                    if (!confl) {
                        continue;
                    } // resolvent didn't fulfill the RAT requirement

                    if (lastKeptLiteral == 0) { // all clauses in data.lits(left) have AT -- kept literals have never been seen

                        solver.detachClause(cr, true);
                        assert(solver.value(ca[cr][0]) == l_Undef && solver.value(ca[cr][1]) == l_Undef &&
                               "Can't detach a clause where one of the watched literals isn't l_Undef");
                        const int detachTrailSize = solver.trail.size();
                    ...
                    if (lastKeptLiteral > 0) {
                        ...
                        for (int k = 0; k < c.size(); ++k) {
                            if (reduct.isCurrentStep(toInt(c[k]))) {
                                c[n++] = c[k];
                            } else {
                        ...
                        data.addToProof(data.lits, true, right); // delete all literals from data.lits and additionally right (right will be printed
                                                                 // first, because its the RAT literal)
        ...
        } while (nextRoundLits.size() > 0 && (data.unlimited() || config.ratm_Limit > ratmSteps) &&
                 !data.isInterupted()); // repeat until all literals have been seen until a fixed point is reached!
        cout << "c  rounds: " << roundsRATM << endl;
        if (minRATM != 0 || minATM != 0 || unitRATM != 0) {
            didSomething = true;
        }
        return didSomething;
    }
    ...
    bool RATElimination::minimizeAT() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            ...
            if (data[mkLit(v, false)] > 0)
                if (!rateHeap.inHeap(toInt(mkLit(v, false)))) {
                    rateHeap.insert(toInt(mkLit(v, false)));
                }
            if (data[mkLit(v, true)] > 0)
                if (!rateHeap.inHeap(toInt(mkLit(v, true)))) {
                    rateHeap.insert(toInt(mkLit(v, true)));
                }
        }
            ...
            for (int i = 0; i < clauses.size() && (data.unlimited() || config.rate_Limit > ratmSteps); ++i) {
                ...
                for (int j = 0; j < data.lits.size(); ++j) {
                    if (solver.value(data.lits[j]) == l_Undef) {
                        ...
                        confl = solver.value(data.lits[j]) == l_True;
                    }
                ...
                if (confl && (delPrefix > 0 ||
                              lastKeptLiteral < data.lits.size())) { // skip clauses, where all negated literals are needed to cause the conflict
    ...
    bool RATElimination::blockedSubstitution() {
        ...
        do {
            ...
            while (rateHeap.size() > 0 && (data.unlimited() || config.bceLimit > rateSteps) && !data.isInterupted()) {
                ...
                data.lits.clear(); // used for covered literal elimination
                for (int i = 0; i < data.list(right).size(); ++i) {
                    if (ca[data.list(right)[i]].can_be_deleted() || ca[data.list(right)[i]].size() == 2) {
                        continue;
                    } // do not use binary clauses and unit clauses
                    assert((ca[data.list(right)[i]].size() != 1 || ca[data.list(right)[i]].can_be_deleted()) &&
                           "unit clauses in lists should always be marked as being deleted");
                    const int sizeC = ca[data.list(right)[i]].size();

                    for (int j = 0; j < data.list(right).size(); ++j) {
                        if (i == j) {
                            continue;
                        } // do not replace the clause with itself!

                        Clause& c = ca[data.list(right)[i]];
                        assert(c.size() == sizeC && "clause c should not change within one iteration");
                        Clause& d = ca[data.list(right)[j]];
                        if (d.can_be_deleted() || c.size() > d.size()) {
                            continue;
                        ...
                        data.lits.clear();
                        for (int k = 0; k < d.size(); ++k) {
                            if (data.ma.isCurrentStep(toInt(d[k]))) {
                                data.lits.push_back(d[k]);
                            }
                        }
                        ...
                        if (extraClit == lit_Undef) { // found duplicate clauses
                            didSomething = true;
                            bcaSubstitue++;
                            bcaSubstitueLits += c.size();
                            // delete the old clause
                            solver.detachClause(data.list(right)[j]); // remove clause from unit propagation
                            ca[data.list(right)[j]].set_delete(
                                true); // d can be deleted, because it can be produced by reslution with c and the new clause!
                        ...
                        if (!isRedundant) { // is clause AT?
                            ...
                            for (int k = 0; k < data.lits.size(); ++k) {
                                if (solver.value(~data.lits[k]) == l_False) {
                                    isRedundant = true;
                                    break;
                                } else if (solver.value(~data.lits[k]) == l_Undef) {
                                    solver.uncheckedEnqueue(~data.lits[k]);
                                } // enqueue all complements
                            }
                            if (!isRedundant) {
                                CRef confl = solver.propagate(); // check whether unit propagation finds a conflict for (F \ C) \land \ngt{C}, and
                                                                 // hence C would be AT
                                DOUT(if (config.opt_rate_debug > 2) cerr << "c propagate with conflict " << (confl != CRef_Undef ? "yes" : " no")
                                                                         << endl;);
                                solver.cancelUntil(0); // backtrack
                                if (confl != CRef_Undef) {
                                    isRedundant = true;
        ...
        } while (nextRoundLits.size() > 0 && (data.unlimited() || config.bceLimit > rateSteps) &&
                 !data.isInterupted()); // repeat until all literals have been seen until a fixed point is reached!
        return didSomething;
    }
    ...
    bool RATElimination::resolveUnsortedStamped(const Lit& l, const Clause& d, MarkArray& ma, vector<Lit>& resolvent) {
        for (int i = 0; i < d.size(); ++i) {
            ...
            if (dl == ~l) {
                continue;
            } // on this literal we currently resolve
            if (ma.isCurrentStep(toInt(~dl))) {
                return true;
            } // complementary literals in the resolvent
            if (!ma.isCurrentStep(toInt(dl))) {
    ...
    void RATElimination::printStatistics(ostream& stream) {
        cerr << "c [STAT] RATM " << ratmTime.getCpuTime() << " seconds, " << ratmSteps << " steps, " << ratmSteps * approxFacAB << " aproxStepsAB, "
             << ratmSteps * approxFacA << " aproxStepsA, " << approxRatm << " realApprox, " << minRATM << " min-RATM, " << minATM << " min-ATM, "
             << unitRATM << " unit-ATM/RATM, " << conflRATM << " confl-RATM, " << conflATM << " confl-ATM," << endl;
        cerr << "c [STAT] RATE " << rateTime.getCpuTime() << " seconds, " << rateSteps << " steps, " << rateCandidates << " checked, " << remRAT
             << " rem-RAT, " << remBCE << " rem-BCE, " << remAT << " rem-AT, " << remHRAT << " rem-HRAT," << endl;
        cerr << "c [STAT] RATE-BCS " << bcaTime.getCpuTime() << " seconds, " << bcaCandidates << " cands, " << bcaResolutionChecks << " resChecks, "
             << bcaSubstitue << " substituted, " << bcaATs << " addedATs, " << bcaSubstitueLits << " subLits, " << bcaFullMatch << " fullMatches, "
             << bcaStrenghening << " strengthenings, " << endl;
        cerr << "c [STAT] RATE-BRAT " << bratTime.getCpuTime() << " seconds, " << remBRAT << " remBRAT, " << blockCheckOnSameClause
             << " sameClauseChecks, " << endl;
    }
