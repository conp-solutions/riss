...
namespace Coprocessor {
    ...
    public:
        EquivalenceElimination(CoprocessorData& _data, CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller,
                               Propagation& _propagation, Subsumption& _subsumption);
    ...
    protected:
        ...
        /** check whether the clause c has duplicates in the list of literal l (irredundant clause is no duplicate for learned clause! -> deletes
         * learned clause!) Note: assumes that all clauses are sorted!
        ...
        void processGate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                         std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable);

        void processANDgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                            std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                            std::deque<Riss::Var>* activeVariables = 0);
        void processGenANDgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                               std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                               std::deque<Riss::Var>* activeVariables = 0);
        void processExOgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                            std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                            std::deque<Riss::Var>* activeVariables = 0);
        void processITEgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                            std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                            std::deque<Riss::Var>* activeVariables = 0);
        void processXORgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                            std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                            std::deque<Riss::Var>* activeVariables = 0);
        void processFASUMgate(CoprocessorData& data, Circuit::Gate& g, std::vector<Circuit::Gate>& gates, std::deque<int>& queue,
                              std::vector<unsigned int>& bitType, std::vector<std::vector<int32_t>>& varTable, Riss::MarkArray* active = 0,
                              std::deque<Riss::Var>* activeVariables = 0);

        /** enqueue all successor gates of the given gate g into the queue, stamp output variables, have a limit when to stop?! */
        void enqueueSucessorGates(Circuit::Gate& g, std::deque<int> queue, std::vector<Circuit::Gate>& gates, std::vector<unsigned int>& bitType,
                                  std::vector<std::vector<int32_t>>& varTable);
