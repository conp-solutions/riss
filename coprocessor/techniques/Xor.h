...
namespace Coprocessor {
    ...
    class XorReasoning : public Technique<XorReasoning> {
        ...
        struct VarLt {
            ...
            VarLt(std::vector<std::vector<int>>& _data)
                : data(_data) {
            }
        };
        ...
        public:
            ...
            Riss::Lit getUnitLit() const {
                return unit() ? Riss::mkLit(vars[0], !k) : Riss::lit_Undef;
            }
            bool failed() const {
                return vars.size() == 0 && k;
            }
            GaussXor(const Riss::Clause& c)
                : k(true)
                , used(false) // build xor from clause
            {
                vars.resize(c.size());
    ...
    public:
        XorReasoning(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller, CoprocessorData& _data,
    ...
    protected:
         ...
         * @return true if no conflict was found, false if a conflict was found
         */
        bool simulatePropagate(std::vector<Riss::Lit>& unitQueue, Riss::MarkArray& ma, std::vector<std::vector<int>>& occs,
                               std::vector<GaussXor>& xorList, bool ignoreConflicts = true);
    };
