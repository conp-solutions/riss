...
namespace Coprocessor {
    ...
    HyperBinaryResolution::HyperBinaryResolution(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
    ...
    bool HyperBinaryResolution::hyperBinaryResolution() {
        ...
        for (int i = 0; i < data.getClauses().size(); ++i) {
            ...
            if (c.size() >= 3 && !c.can_be_deleted()) {
                data.clss.push_back(data.getClauses()[i]);
            }
        }
        ...
        data.lits.clear();
        for (int i = 0; i < 2 * data.nVars(); ++i) {
            nextRound.setCurrentStep(i);
            data.lits.push_back(toLit(i));
        }
        ...
        do {
            ...
            for (int i = 0; i < data.clss.size() && (data.unlimited() || config.hbrLimit > hbrSteps) && !data.isInterupted(); ++i) {
                ...
                if (c.size() < 3 || c.can_be_deleted()) {
                    continue;
                } // delete this pointer from the current version of the clss vector, if the clause is binary now, of can be deleted
                data.clss[keptClauses++] = data.clss[i]; // move the current pointer forward, if there is a gap
                if (c.size() > config.opt_hbr_maxCsize) {
                    continue;
                }
                ...
                for (int j = 0; j < 2; ++j) {
                    ...
                    for (int k = 0; k < xSize; ++k) {
                        ...
                        if (useThisRound.isCurrentStep(toInt(l))) {
                            continue;
                        }                                      // already tested for this clause?
                        ...
                        for (int m = 0; m < c.size(); ++m) {
                            if (m == pos[j]) {
                                continue;
                            } // do not process same literal with itself!
                            const Lit lm = c[m];
                            if (var(lm) == var(l)) { // will not find tautologic binary clause, or binary clause with duplicate literals
                                DOUT(if (config.opt_hbr_debug) cerr << "c HBR literal " << c[m] << " and " << l
                                                                    << " are the same variable - abort scan for " << l << endl;);
                                otherLit = lit_Error;
                                break;
                            }
                            if (!big.implies(lm, l) && !big.isChild(lm, l)) { // check whether the clause (l \lor c[m]) is present in the formula
                                DOUT(if (config.opt_hbr_debug) cerr << "c HBR literal " << c[m] << " does not match " << l << " in a binary clause ("
                                                                    << ~c[m] << " or " << l << ")" << endl;);
                                ...
                                if (otherLit == lit_Undef) {
                                    otherLit = c[m];
                                } // this literal is not present as binary clause, hence it might be left after HBR
                                else {
                                    otherLit = lit_Error;
                                    break;
                                } // there are multiple literals that fail, hence the hyper resolvent will not be binary
                            } else {
                                DOUT(if (config.opt_hbr_debug) cerr << "c HBR hit (" << ~c[m] << " or " << l << ") with imply: " << big.implies(lm, l)
                                                                    << " child: " << big.isChild(lm, l) << endl;);
                            }
                        }
            ...
            if (toBeAdded.size() > 0) {
                ...
                for (int i = 0; i < toBeAdded.size(); ++i) {
                    ...
                    if (i == 0 || lastPair != toBeAdded[i]) { // only add the clause if it does not match another clause in the list
                        ...
                        if (data.value(lits[0]) != l_Undef || data.value(lits[1]) != l_Undef) {
                            continue;
                        }                                   // propagation already set the values accordingly, no need to add the clause
        ...
        // TODO: nextRound mark array might survive multiple calls with original variable names, but then also store the literals in the list
        // data.lits!
        data.lits.clear();
        nextRound.destroy();
    ...
    bool HyperBinaryResolution::addHBR(const Lit& otherLit, const Lit& clauseLit) {
        if (big.isChild(~otherLit, clauseLit)) {
            return false;
        } // sufficient to scan one direction, as BIG is directed graph and each binary clause adds both edges
    ...
    void HyperBinaryResolution::printStatistics(ostream& stream) {
        cerr << "c [STAT] HBR " << hbrTime.getCpuTime() << " seconds, " << hbrSteps << " steps, " << foundHBR << " addedBinaries, " << foundMultiHBR
             << " multiHBR-units, " << endl;
    }
