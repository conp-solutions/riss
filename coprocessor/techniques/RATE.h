...
namespace Coprocessor {
    ...
    class RATElimination : public Technique<RATElimination> {
        ...
        struct LitOrderRATEHeapLt { // sort according to number of occurrences of complement!
            ...
            LitOrderRATEHeapLt(CoprocessorData& _data, bool _useComplements)
                : data(_data)
                , useComplements(_useComplements) {
            }
        };
    ...
    public:
        RATElimination(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller, CoprocessorData& _data,
                       Riss::Solver& _solver, Coprocessor::Propagation& _propagation);
    ...
    protected:
         ...
         * @param resolvent initially contains the literals of the other clause (except literal l), afterwards, stores the resolvent, if the resolvent
         * is not a  tautology
