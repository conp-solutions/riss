...
namespace Coprocessor {
    ...
    class Subsumption : public Technique<Subsumption> {
        ...
        struct OccUpdate {
            Riss::CRef cr;
            Riss::Lit l;
            OccUpdate(const Riss::CRef& _cr, const Riss::Lit& _l)
                : cr(_cr)
                , l(_l) {
            }
        };
    ...
    public:
        Subsumption(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller, CoprocessorData& _data,
         ...
         */
        bool process(bool doStrengthen = true, Riss::Heap<VarOrderBVEHeapLt>* heap = nullptr, const Riss::Var ignore = var_Undef,
    ...
    protected:
        inline void updateOccurrences(std::vector<Coprocessor::Subsumption::OccUpdate>& updates, Riss::Heap<Coprocessor::VarOrderBVEHeapLt>* heap,
                                      const Riss::Var ignore = (-1));

        bool hasToSubsume() const; // return whether there is something in the subsume queue
        Riss::lbool fullSubsumption(Riss::Heap<VarOrderBVEHeapLt>* heap, const Riss::Var ignore = var_Undef,
                                    const bool doStatistics = true); // performs subsumtion until completion
        void
        subsumption_worker(unsigned int start, unsigned int end, Riss::Heap<VarOrderBVEHeapLt>* heap, const Riss::Var ignore = var_Undef,
                           const bool doStatistics = true); // subsume certain set of elements of the processing queue, does not write to the queue
        void par_subsumption_worker(unsigned int& next_start, unsigned int global_end, SpinLock& balancerLock, std::vector<Riss::CRef>& to_delete,
                                    std::vector<Riss::CRef>& set_non_learnt, struct SubsumeStatsData& stats, const bool doStatistics = true);

        bool hasToStrengthen() const; // return whether there is something in the strengthening queue

        Riss::lbool fullStrengthening(Riss::Heap<VarOrderBVEHeapLt>* heap, const Riss::Var ignore = var_Undef,
                                      const bool doStatistics = true); // performs strengthening until completion, puts clauses into subsumption queue
        Riss::lbool strengthening_worker(unsigned int start, unsigned int end, Riss::Heap<VarOrderBVEHeapLt>* heap,
                                         const Riss::Var ignore = var_Undef, bool doStatistics = true);
        Riss::lbool createResolvent(const Riss::CRef& cr, Riss::CRef& resolvent, const int negated_lit_pos, Riss::Heap<VarOrderBVEHeapLt>* heap,
                                    const Riss::Var ignore = var_Undef, const bool doStatistics = true);
        void par_strengthening_worker(unsigned int& next_start, unsigned int global_stop, SpinLock& balancerLock, std::vector<SpinLock>& var_lock,
                                      struct SubsumeStatsData& stats, std::vector<OccUpdate>& occ_updates, Riss::Heap<VarOrderBVEHeapLt>* heap,
                                      const Riss::Var ignore = var_Undef, const bool doStatistics = true);
        void par_nn_strengthening_worker(unsigned int& next_start, unsigned int global_end, SpinLock& balancerLock, std::vector<SpinLock>& var_lock,
                                         struct SubsumeStatsData& stats, std::vector<OccUpdate>& occ_updates, Riss::Heap<VarOrderBVEHeapLt>* heap,
                                         const Riss::Var ignore = var_Undef, const bool doStatistics = true);
        inline Riss::lbool par_nn_strength_check(CoprocessorData& data, std::vector<Riss::CRef>& list, std::deque<Riss::CRef>& localQueue,
                                                 Riss::Clause& strengthener, Riss::CRef cr, Riss::Var fst, std::vector<SpinLock>& var_lock,
                                                 struct SubsumeStatsData& stats, std::vector<OccUpdate>& occ_updates,
                                                 Riss::Heap<VarOrderBVEHeapLt>* heap, const Riss::Var ignore = var_Undef,
                                                 const bool doStatistics = true);
        inline Riss::lbool par_nn_negated_strength_check(CoprocessorData& data, std::vector<Riss::CRef>& list, std::deque<Riss::CRef>& localQueue,
                                                         Riss::Clause& strengthener, Riss::CRef cr, Riss::Lit min, Riss::Var fst,
                                                         std::vector<SpinLock>& var_lock, struct SubsumeStatsData& stats,
                                                         std::vector<OccUpdate>& occ_updates, Riss::Heap<VarOrderBVEHeapLt>* heap,
                                                         const Riss::Var ignore = var_Undef, const bool doStatistics = true);
