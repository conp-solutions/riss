...
namespace Coprocessor {
    ...
    bool EntailedRedundant::process() {
        ...
        for (int i = 0; i < data.getClauses().size(); ++i) {
            ...
            Clause& c = ca[cr];
            if (c.can_be_deleted() || c.size() < config.opt_entailed_minClsSize) {
                continue;
            } // clause does not fulfill criteria
            ...
            DOUT(if (config.entailed_debug > 1) cerr << "c work on clause " << c << " with min literal " << min
                                                     << " toRemoveSoFar: " << data.clss.size() << endl;);
            ...
            for (int i = 0; i < clss.size(); ++i) {
                ...
                if (candidate.can_be_deleted() || candidate.learnt()) {
                    continue;
                } // do work on the formula only!
                if (candidate.size() > c.size() + 1) {
                    continue;
                } // this clause is too large! resolution can remove only a single literal!
                ...
                while (n < candidate.size() && m < c.size()) {
                    if (candidate[n] == c[m]) { // match
                        ...
                        n++;
                        m++;
                    } else if (c[m] < candidate[n]) { // there is a literal inside original, which is not inside candidate - this literal is part of
                                                      // the data.lits literals
                        DOUT(if (config.entailed_debug > 4) cerr << "c " << candidate[n] << " > " << c[m] << " -> new missing: " << data.lits
                                                                 << endl;);
                        data.lits.push_back(c[m]);
                        m++;
                    } else if (candidate[n] < c[m]) { // there is a literal inside candidate, which is not inside original
                        if (resolveLit == lit_Undef) {
                            resolveLit = candidate[n];
                        } else if (resolveLit != lit_Undef) {
                            resolveLit = lit_Error;
                            DOUT(if (config.entailed_debug > 4) cerr << "c " << candidate[n] << " < " << c[m] << " -> resolveLit: " << resolveLit
                                                                     << " => no candidate!" << endl;);
                ...
                if (resolveLit != lit_Error)
                    for (; m < c.size(); ++m) {
                        data.lits.push_back(c[m]);
                    }
                if (resolveLit == lit_Undef) { // candidate subsumes other clause - subsume, continue
                    subsumed++;
                    DOUT(if (config.entailed_debug > 1) cerr << "c " << candidate << " subsumes " << c << "(" << clss[i] << " - " << cr << ")"
                                                             << endl;);
                    ...
                    // find a clause that contains all literals in the vector data.lits, and furthermore contains only literals that appear in the
                    // clause c
                    Lit min2 = data.lits[0];
                    for (int j = 1; j < data.lits.size(); ++j) {
                        min2 = data[min2] <= data[data.lits[j]] ? min2 : data.lits[j];
                    }
                    const vector<CRef>& matches = data.list(min2);
                    for (int j = 0; j < matches.size(); ++j) {
                        const CRef cr2 = matches[j];
                        if (cr2 == cr || cr2 == clss[i]) {
                            continue;
                        } // do not handle same clause twice!
                        const Clause& c2 = ca[cr2];
                        if (c2.can_be_deleted() || c2.size() < data.lits.size() || c2.learnt()) {
                            continue;
                        } // this clause is too small to contain all literals in data.lits
                        ...
                        while (n < data.lits.size() && m < c2.size()) {
                            if (data.lits[n] == c2[m]) { // match
                                ...
                                if (!data.ma.isCurrentStep(toInt(c2[m]))) {
                                    DOUT(if (config.entailed_debug > 3) cerr << "c reject match " << c2 << " , because " << c2[m] << " notin " << c
                                                                             << endl;);
                                ...
                                DOUT(if (config.entailed_debug > 3) cerr << "c reject match " << c2 << " , because " << data.lits[n] << " notin "
                                                                         << c2 << endl;);
                        ...
                        if (n < data.lits.size()) {
                            DOUT(if (config.entailed_debug > 3) cerr << "c reject match " << c2 << " , because " << data.lits[n] << " notin " << c2
                                                                     << endl;);
                        ...
                        DOUT(if (config.entailed_debug > 0) cerr << "c ENT clause " << c << " is resolvent of " << candidate << " and " << c2
                                                                 << endl;);
                        data.clss.push_back(cr);
    ...
    void EntailedRedundant::printStatistics(ostream& stream) {
        stream << "c [STAT] ENT " << processTime << " s, " << removedClauses << " cls, " << subsumed << " subs, " << extraSubs << " extraSubs, "
               << endl;
    }
