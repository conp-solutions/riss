...
namespace Coprocessor {
    ...
    class BoundedVariableElimination : public Technique<BoundedVariableElimination> {
        ...
        struct PostponeReason {
            Riss::Var var, reason;
            PostponeReason(Riss::Var _var, Riss::Var _reason)
                : var(_var)
                , reason(_reason) {
            }
        };
        ...
        // stats variables
        struct ParBVEStats {
            int removedClauses, removedLiterals, createdClauses, createdLiterals, removedLearnts, learntLits, newLearnts, newLearntLits, testedVars,
                anticipations, eliminatedVars, removedBC, blockedLits, removedBlockedLearnt, learntBlockedLit, skippedVars, unitsEnqueued, foundGates,
                usedGates, subsumedClauses, subsumedLiterals, subsumedLearnts, subsumedLearntLiterals, subsimpSteps, strengthtLits,
                strengthtLearntLits;
            ...
            ParBVEStats()
                ...
                , subsumedLiterals(0)
                , subsumedLearnts(0)
                , subsumedLearntLiterals(0)
                , subsimpSteps(0)
                , strengthtLits(0)
                , strengthtLearntLits(0)
                , parBveChecks(0)
                , processTime(0)
                , subsimpTime(0)
                , gateTime(0)
        ...
        int removedClauses, removedLiterals, createdClauses, createdLiterals, removedLearnts, learntLits, newLearnts, newLearntLits, testedVars,
            anticipations, eliminatedVars, removedBC, blockedLits, removedBlockedLearnt, learntBlockedLit, skippedVars, unitsEnqueued, foundGates,
            usedGates, initialClauses, initialLits, clauseCount, litCount, unitCount, elimCount, restarts;
    ...
    public:
        BoundedVariableElimination(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller,
    ...
    protected:
         ...
         *  @param l literal that has been used to remove the clauses during elimination (if l == Riss::lit_Undef, clauses are not added to extension
         * stack)
         */
        inline void removeClauses(CoprocessorData& data, const std::vector<Riss::CRef>& list, const Riss::Lit& l, const int limit,
                                  const bool doStatistics = true);

        /** ths method applies unit propagation during resolution, if possible! */
        inline Riss::lbool resolveSet(CoprocessorData& data, std::vector<Riss::CRef>& positive, std::vector<Riss::CRef>& negative, const int v,
                                      const int p_limit, const int n_limit, Stepper& bveStepper, const bool keepLearntResolvents = false,
        ...
        inline Riss::lbool anticipateElimination(CoprocessorData& data, std::vector<Riss::CRef>& positive, std::vector<Riss::CRef>& negative,
                                                 const int v, const int p_limit, const int n_limit, Riss::vec<int32_t>& pos_stats,
                                                 Riss::vec<int32_t>& neg_stats, int& lit_clauses, int& lit_learnts, int& resolvents,
                                                 Stepper& bveStepper, const bool doStatistics = true);
        ...
        inline void removeBlockedClauses(CoprocessorData& data, const std::vector<Riss::CRef>& list, const int32_t stats[], const Riss::Lit& l,
                                         const int limit, const bool doStatistics = true);

        // parallel functions:
        void par_bve_worker(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, std::deque<Riss::CRef>& strengthQueue,
                            std::deque<Riss::CRef>& sharedStrengthQueue, std::deque<PostponeReason>& postponed, std::vector<SpinLock>& var_lock,
                            ReadersWriterLock& rwlock, ParBVEStats& stats, Riss::MarkArray* gateMarkArray, int& rwlock_count, int& garbageCounter,
                            int64_t& parBVEchecks, const bool force = false, const bool doStatistics = true);

        /** run parallel bve with all available threads */
        void parallelBVE(CoprocessorData& data, const bool doStatistics = true);

        inline void removeClausesThreadSafe(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const std::vector<Riss::CRef>& list,
                                            const Riss::Lit& l, const int limit, SpinLock& data_lock, SpinLock& heap_lock, ParBVEStats& stats,
                                            int& garbageCounter, const bool doStatistics);
        inline Riss::lbool resolveSetThreadSafe(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, std::vector<Riss::CRef>& positive,
                                                std::vector<Riss::CRef>& negative, const int v, const int p_limit, const int n_limit,
                                                Riss::vec<Riss::Lit>& ps, Riss::AllocatorReservation& memoryReservation,
                                                std::deque<Riss::CRef>& strengthQueue, ParBVEStats& stats, SpinLock& data_lock, SpinLock& heap_lock,
                                                int expectedResolvents, int64_t& bveChecks, const bool doStatistics,
                                                const bool keepLearntResolvents = false);
        inline Riss::lbool anticipateEliminationThreadsafe(CoprocessorData& data, std::vector<Riss::CRef>& positive,
                                                           std::vector<Riss::CRef>& negative, const int v, const int p_limit, const int n_limit,
                                                           Riss::vec<Riss::Lit>& resolvent, Riss::vec<int32_t>& pos_stats,
                                                           Riss::vec<int32_t>& neg_stats, int& lit_clauses, int& lit_learnts, int& new_clauses,
                                                           int& new_learnts, SpinLock& data_lock, ParBVEStats& stats, int64_t& bveChecks,
                                                           const bool doStatistics);
        inline void removeBlockedClausesThreadSafe(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const std::vector<Riss::CRef>& list,
                                                   const int32_t _stats[], const Riss::Lit& l, const int limit, SpinLock& data_lock,
                                                   SpinLock& heap_lock, ParBVEStats& stats, int& garbageCounter, const bool doStatistics);

        // Special subsimp implementations for par bve:
        void par_bve_strengthening_worker(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const Riss::Var ignore,
                                          std::vector<SpinLock>& var_lock, ReadersWriterLock& rwlock, std::deque<Riss::CRef>& sharedStrengthQueue,
                                          std::deque<Riss::CRef>& localQueue, Riss::MarkArray& dirtyOccs, ParBVEStats& stats, int& rwlock_count,
                                          int& garbageCounter, const bool strength_resolvents, const bool doStatistics);

        // Special propagation for par bve
        Riss::lbool par_bve_propagate(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const Riss::Var ignore,
                                      std::vector<SpinLock>& var_lock, ReadersWriterLock& rwlock, Riss::MarkArray& dirtyOccs,
                                      std::deque<Riss::CRef>& sharedSubsimpQueue, ParBVEStats& stats, int& rwlock_count, int& garbageCounter,
                                      const bool doStatistics);

        inline Riss::lbool strength_check_pos(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const Riss::Var ignore,
                                              std::vector<Riss::CRef>& list, std::deque<Riss::CRef>& sharedStrengthQueue,
                                              std::deque<Riss::CRef>& localQueue, Riss::Clause& strengthener, Riss::CRef cr, Riss::Var fst,
                                              std::vector<SpinLock>& var_lock, Riss::MarkArray& dirtyOccs, ParBVEStats& stats, int& garbageCounter,
                                              const bool strength_resolvents, const bool doStatistics);

        inline Riss::lbool strength_check_neg(CoprocessorData& data, Riss::Heap<VarOrderBVEHeapLt>& heap, const Riss::Var ignore,
                                              std::vector<Riss::CRef>& list, std::deque<Riss::CRef>& sharedStrengthQueue,
                                              std::deque<Riss::CRef>& localQueue, Riss::Clause& strengthener, Riss::CRef cr, Riss::Lit min,
                                              Riss::Var fst, std::vector<SpinLock>& var_lock, Riss::MarkArray& dirtyOccs, ParBVEStats& stats,
                                              int& garbageCounter, const bool strength_resolvents, const bool doStatistics);
