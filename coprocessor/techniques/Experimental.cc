...
namespace Coprocessor {
    ...
    ExperimentalTechniques::ExperimentalTechniques(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
                                                   Solver& _solver)
    ...
    bool ExperimentalTechniques::process() {
        ...
        for (int i = 0; i < data.getClauses().size() && !data.isInterupted(); ++i) {
            ...
            if (c.can_be_deleted() || c.size() != 2) {
                continue;
            } // clause does not fulfill criteria, here we want to delete binary clauses
            ...
            for (int j = 0; j < c.size(); ++j) {
                ...
                if (solver.value(x) != l_Undef) {
                    continue;
                } // do not perform this check, clause is satisfied anyways
...

                {
                    ...
                    for (int iteration = 0; iteration < 20 && !data.isInterupted(); ++iteration) {
                        ...
                        if (!foundNewPures) {
                            break;
                        } // stop here, because no new pure literals have been added
                    }
                }
