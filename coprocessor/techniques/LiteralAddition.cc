...
namespace Coprocessor {
    ...
    LiteralAddition::LiteralAddition(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
    ...
    void LiteralAddition::asymemtricLiteralAddition() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) { // one literals that appear at least twice, otherwise it doesnt work
            if (data[mkLit(v, false)] > 1)
                if (!laHeap.inHeap(toInt(mkLit(v, false)))) {
                    nextRoundLits.push_back(mkLit(v, false));
                }
            if (data[mkLit(v, true)] > 1)
                if (!laHeap.inHeap(toInt(mkLit(v, true)))) {
        ...
        do {
            ...
            if (iterations >= config.alaIterations) {
                break;
            } // stop here!
            iterations++;
            ...
            while (laHeap.size() > 0 && (data.unlimited() || config.alaLimit > alaSteps) && !data.isInterupted()) {
                ...
                DOUT(if (config.opt_la_debug) cerr << "c CLA work on literal " << right << " with " << data.list(right).size() << " clauses "
                                                   << endl;);
                ...
                for (int i = 0; i < listSize; ++i) {
                    ...
                    if (cfor.can_be_deleted() || cfor.learnt()) {
                        continue;
                    } // do not work on uninteresting clauses!

                    if (!config.ala_binary && cfor.size() == 2) {
                        continue;
                    } // do not perform ALA for binary clauses!
                    ...
                    for (int j = 0; j < data.list(right).size(); ++j) {
                        if (i == j) {
                            continue;
                        } // do not work on the same literal twice!
                        ...
                        if (d.can_be_deleted() || d.learnt()) {
                            continue;
                        } // do not work on uninteresting clauses!
                        if (d.size() != c.size()) {
                            continue;
                        } // in this case, d cannot contain c \setminus e
                        alaSteps++;
                        ...
                        DOUT(if (config.opt_la_debug) cerr << "c intermediate comparison: " << c << "[" << ci << "] and " << d << "[" << di
                                                           << "], failed: " << failed << " lit e: " << e << endl;);

                        if (ci < c.size()) { // all literals inside di have to be consumed already here to make the above loop abort!
                            if (ci + 1 < c.size() || e != lit_Undef) {
                                failed = true;
                            } // cannot extend with two literals!
                            else {
                                e = c[ci];
                            } // the remaining literal is the extension literal!
                        }
        ...
        } while (nextRoundLits.size() > 0 && (data.unlimited() || config.alaLimit > alaSteps) &&
    ...
    void LiteralAddition::coverdLiteralAddition() {
        ...
        do {
            ...
            if (iterations >= config.claIterations) {
                break;
            } // stop here!
            iterations++;
                    ...
                    for (int j = 0; j < data.list(left).size(); ++j) {
                        ...
                        if (!tautologicResolvent(c, d, right)) {
                            if (!canCla) { // simply copy all literals from d except right into data.lits
                                ...
                                data.ma.nextStep();
                                for (int k = 0; k < d.size(); ++k) {
                                    data.ma.setCurrentStep(toInt(d[k]));
                                } // mark all literals of this clause
                    ...
                    if (data.lits.size() > 0 && canCla) { // there is something to be added for the clause c!
                        ...
                        if (data.lits.size() > config.claStepSize) { // reduce number of literals somehow
                            ...
                            if (keptLiterals > config.claStepMax) {
                                keptLiterals = config.claStepMax;
                            }                                                        // cut off the remaining literals
                        ...
                        if (!isTaut) { // do not want to perform CCE here!
                            ...
                            DOUT(if (config.opt_la_debug) cerr << "c CLA turned clause " << ca[data.list(right)[i]] << " into the new clause "
                                                               << ca[newClause] << endl;);
                            // add new clause
        ...
        } while (nextRoundLits.size() > 0 && (data.unlimited() || config.claLimit > claSteps) &&
                 !data.isInterupted()); // repeat until all literals have been seen until a fixed point is reached!
    ...
    bool LiteralAddition::process() {
        ...
        if (config.opt_la_cla) {
            coverdLiteralAddition();
        }
        if (config.opt_la_ala) {
            asymemtricLiteralAddition();
        }
    ...
    void LiteralAddition::printStatistics(ostream& stream) {
        cerr << "c [STAT] CLA " << claTime.getCpuTime() << " seconds, " << claSteps << " steps, " << claTestedLits << " testLits, "
             << claExtendedClauses << " extClss, " << claExtensions << " extLits, " << possibleClaExtensions << " possibles, " << endl;
        cerr << "c [STAT] ALA " << alaTime.getCpuTime() << " seconds, " << alaSteps << " steps, " << alaTestedLits << " testLits, " << alaExtensions
             << " extensions, " << endl;
    }
