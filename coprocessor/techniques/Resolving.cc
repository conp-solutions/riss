...
namespace Coprocessor {
    ...
    void Resolving::printStatistics(ostream& stream) {
        stream << "c [STAT] RES " << processTime << " s, " << addedTern2 + addedTern3 + addedBinaries << " cls+, " << addedBinaries << " bin+, "
               << addedTern2 << " tern2+, " << addedTern3 << " tern3+, "

               << res3steps << " res3teps, " << removedViaSubsubption << " subsCls, " << detectedDuplicates << " duplicates, " << endl;
    }
    ...
    void Resolving::ternaryResolve() {
        ...
        while (resHeap.size() > 0 && !data.isInterupted() && (data.unlimited() || addedTern2 + addedTern3 < config.opt_res3_newCls) &&
            ...
            for (int i = toIgnore[toInt(p)]; i < data.list(p).size(); ++i) {
                for (int j = toIgnore[toInt(n)]; j < data.list(n).size(); ++j) {
                    if (i < seen[toInt(p)] && j < seen[toInt(n)]) {
                        continue;
                    } // do not re-check already seen clauses!
                    resolvent.clear();
                    ...
                    res3steps++;
                    if (!data.unlimited() && res3steps > config.opt_res3_steps) {
                        goto endTernResolve;
                    }
                    if (resolve(c, d, v, resolvent)) {
                        continue;
                    }
                    ...
                    if (resolvent.size() == 0) {
                        ...
                        if (data.enqueue(resolvent[0]) == l_False) {
                            return;
                        }
    ...
    bool Resolving::hasDuplicate(vector<CRef>& list, const vec<Lit>& c) {
        for (int i = 0; i < list.size(); ++i) {
            Clause& d = ca[list[i]];
            if (d.can_be_deleted() || (!config.opt_res3_use_subs && d.size() != c.size())) {
                continue;
            }
            int j = 0;
            if (d.size() == c.size()) {
        ...
        for (Var v = 0; v < data.nVars() && !data.isInterupted(); ++v) {
            for (int p = 0; p < 2; ++p) {
                ...
                for (int i = 0; i < data.lits.size(); ++i) {
                    ...
                    if (l == lit_Undef) {
                        if (i + 1 < data.lits.size() && data.lits[i + 1] != lit_Undef) {
                            data.lits.push_back(lit_Undef);
                            level++;
                            continue;
                        } else {
                    ...
                    for (int j = 0; j < big[toInt(l)].size(); ++j) {
                        ...
                        if (data.ma.isCurrentStep(toInt(k))) {
                            continue;
                        }
                        data.ma.setCurrentStep(toInt(k));
                        data.lits.push_back(k);
                        DOUT(if (config.res3_debug_out) cerr << "c found transitive " << startLit << " -> " << k << "  @ " << level << " via " << l
                                                             << endl;);
                    }
                }
                ...
                DOUT(if (config.res3_debug_out) cerr << "c found " << data.lits.size() << " new (" << directLits << " direct) literals for "
                                                     << startLit << " with " << level << " levels" << endl;);
                ...
                if (use + directLits > data.lits.size()) {
                    use = data.lits.size() - directLits;
                } // limit literals that can be used to the number of present literals
            ...
            for (int p = 0; p < 2; ++p) {
                ...
                for (int j = seen[toInt(thisLit)]; j < big[toInt(thisLit)].size(); ++j) {
                    ...
                    if (k == ps[0]) {
                        ...
                        if (data.enqueue(ps[0]) == l_False) {
                            return;
                        } // add the unit clause we found, if failure, stop procedure
                    } else {
                        ps[1] = k;
                        if (k < thisLit) {
                            continue;
