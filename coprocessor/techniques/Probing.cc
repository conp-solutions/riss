...
namespace Coprocessor {
    ...
    Probing::Probing(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data, Propagation& _propagation,
                     EquivalenceElimination& _ee, Solver& _solver)
    ...
    bool Probing::process() {
        ...
        if ((!config.pr_probe && !config.pr_vivi && !config.pr_lcm) || !data.ok()) {
            return data.ok();
        }
        ...
        if (solver.propagate() != CRef_Undef) {
            ...
            for (int i = 0; i < solver.varFlags.size(); ++i) {
                solver.varFlags[i].polarity = polarity[i].polarity;
            }
                ...
                for (int p = 0; p < 2; ++p) {
                    ...
                    for (int i = 0; i < clauseList.size(); ++i) {
                        if (ca[clauseList[i]].can_be_deleted()) {
                            continue;
                        } // skip clause that we do not care about any more
        ...
        while (solver.qhead < solver.trail.size()) {
            ...
            for (i = j = (Watcher*)ws, end = i + ws.size(); i != end;) {
                if (i->isBinary()) {
                    *j++ = *i++;
                    continue;
                } // skip binary clauses (have been propagated before already!}
                ...
                Lit commonDominator =
                    (config.pr_LHBR ? solver.vardata[var(false_lit)].dom : lit_Error); // inidicate whether lhbr should still be performed
                // Look for new watch:
                for (int k = 2; k < c.size(); k++)
                    if (solver.value(c[k]) != l_False) {
                        ...
                        if (commonDominator != lit_Error) { // do only, if its not broken already - and if limit is not reached
                            commonDominator =
                                (commonDominator == lit_Undef ? solver.vardata[var(c[k])].dom
                ...
                *j++ = w;
                if (solver.value(first) == l_False) {
                    ...
                    if (config.pr_LHBR) {
                        solver.vardata[var(first)].dom = solver.vardata[var(first)].dom;
                    } // set dominator for this variable!

                    // check lhbr!
                    if (commonDominator != lit_Error &&
                        commonDominator != lit_Undef) { // no need to ask for permission, because commonDominator would be lit_Error anyways
                        solver.oc.clear();
                        ...
                        lhbr_news++;
                        if (willSubsume) {
                            // created a binary clause that subsumes this clause  FIXME: actually replace this clause here instead of creating a new
                            // one! adapt code below!
                            if (c.mark() == 0) {
                        ...
                        if (clearnt) {
                            ...
                            ca[cr2].activity() = cactivity;
                        } else {
                            solver.clauses.push(cr2);
                        }
            ...
            ws.shrink_(i - j);
            for (int k = 0; k < solver.clssToBump.size(); ++k) {
                solver.attachClause(solver.clssToBump[k]);
            } // add lhbr clauses lazily
            solver.clssToBump.clear();
        }
    ...
    bool Probing::prAnalyze(CRef confl) {
        ...
        if (config.pr_uip == 0 && solver.decisionLevel() == 0) {
            return false;
        }
        ...
        do {
            DOUT(if (config.pr_debug_out > 3) cerr << "c next conflict [" << reason.getReasonC() << "] with pathC= " << pathC << " and level "
            ...
            DOUT(if (config.pr_debug_out > 3) {
                ...
                for (int i = 0; i < ca[reason.getReasonC()].size(); ++i) {
                    cerr << " " << ca[reason.getReasonC()][i] << "@" << solver.level(var(ca[reason.getReasonC()][i]));
                }
                cerr << endl;
            });
    ...
    bool Probing::prDoubleLook(Lit l1decision) {
        ...
        for (int i = 0; i < doubleLiterals.size(); ++i) {
            ...
            if (solver.value(v) != l_Undef || data.ma.isCurrentStep(v)) {
                continue;
            }                          // no need to check assigned variable!
            ...
            DOUT(if (config.pr_debug_out > 1) cerr << "c conflict after level 2 propagate " << posLit << " present? " << (thisConflict != CRef_Undef)
                                                   << " trail elements: " << solver.trail.size() << endl;);
            ...
            if (thisConflict != CRef_Undef) {
                ...
                if (data.lits.size() == 0) {
                    ...
                    if (data.lits.size() == 1) {
                        ...
                        backtrack_level = solver.level(var(data.lits[1]));
                    }
                }

                DOUT(if (config.pr_debug_out > 0) {
                    cerr << "c learnt clause at level " << solver.decisionLevel() << " with jump to " << backtrack_level << " after decideing "
                         << l1decision << " and " << posLit << " ";
                    for (int i = 0; i < data.lits.size(); ++i) {
                        cerr << " " << data.lits[i];
                    }
                    cerr << endl;
                ...
                if (cClause != CRef_Undef) {
                    ...
                    if (!data.ok()) {
                        return true;
                    } // interrupt loop, if UNSAT has been found!
                    l1learntUnit += learntUnits.size();
                    if (solver.propagate() != CRef_Undef) {
                        data.setFailed();
                        return true;
                    } // UNSAT has been found - current state is level 0
            ...
            probeChecks++;
            thisConflict = prPropagate();
            DOUT(if (config.pr_debug_out > 1) cerr << "c conflict after level 2 propagate " << negLit << " present? " << (thisConflict != CRef_Undef)
                                                   << " trail elements: " << solver.trail.size() << endl;);
            DOUT(if (config.pr_debug_out > 1) {
                cerr << "c current trail: ";
                for (int k = 0; k < solver.trail.size(); ++k) {
            ...
            if (solver.decisionLevel() != 2) {
                cerr << "c L2 lookahead resulted in decision level " << solver.decisionLevel() << endl;
            }
            ...
            if (config.pr_necBinaries) { // if necessary assignments should be generated during probing, do it here!
                for (int i = solver.trail_lim[1] + 1; i < solver.trail.size(); ++i) {
                    ...
                    DOUT(if (config.pr_debug_out > 1) cerr << "c compare variable (level 1)" << tv + 1
                                                           << ": pos = " << toInt(prL2Positive[tv].assigns)
                                                           << " neg = " << toInt(solver.varFlags[tv].assigns) << endl;);
    ...
    void Probing::printStatistics(ostream& stream) {
        stream << "c [STAT] PROBING(1) " << processTime << " s, " << l1implied << " implied " << l1failed << " l1Fail, " << l1learntUnit
               << " l1Units " << l1ee << " l1EE " << l2implied << " l2implied " << l2failed << " l2Fail " << l2ee << " l2EE " << lhbr_news
               << " pr-lhbrs, " << probeLHBRs << " vivi-lhbrs " << endl;

        stream << "c [STAT] PROBING(2) " << probes << " probes " << probeCandidates << " probeCands " << l2probes << " l2probes " << totalL2cand
               << " l2cands " << probeChecks << " checks " << endl;

        stream << "c [STAT] VIVI " << viviTime << "  s, " << viviLits << " lits, " << viviCls << " cls, " << viviCands << " candidates, "
               << viviChecks << " checks, " << viviSize << " viviSize" << endl;

        stream << "c [STAT] VIVI-LCM " << lcmTime << "  s, " << lcmLits << " lits, " << lcmCls << " cls, " << lcmSteps << " checks, " << endl;
    }
    ...
    void Probing::probing() {
        ...
        if (variableHeap.size() == 0) {
            big.fillSorted(variableHeap, data, false, true);
        }
        ...
        do {
            modifiedFormula = false;
            for (int index = 0; index < variableHeap.size() && !data.isInterupted() && (probeLimit > probeChecks || data.unlimited()) && data.ok();
                 ++index) {
                ...
                probeChecks++;
                thisConflict = prPropagate();
                DOUT(if (config.pr_debug_out > 1) cerr << "c conflict after propagate " << posLit << " present? " << (thisConflict != CRef_Undef)
                                                       << " trail elements: " << solver.trail.size() << endl;);
                if (thisConflict != CRef_Undef) {
                    l1failed++;
                    if (!prAnalyze(thisConflict)) {
                ...
                probeChecks++;
                thisConflict = prPropagate();
                DOUT(if (config.pr_debug_out > 1) cerr << "c conflict after propagate " << negLit << " present? " << (thisConflict != CRef_Undef)
                                                       << " trail elements: " << solver.trail.size() << endl;);
                if (thisConflict != CRef_Undef) {
                    l1failed++;
                    if (!prAnalyze(thisConflict)) {
                ...
                if (data.outputsProof()) {
                    printDRUPwarning(cerr, "necessary assignments and equivalences from Probing cannot be handled easily with DRAT");
                }
                data.lits.clear();
                ...
                for (int i = solver.trail_lim[0] + 1; i < solver.trail.size(); ++i) {
                    // check whether same polarity in both trails, or different polarity and in both trails
                    const Var tv = var(solver.trail[i]);
                    DOUT(if (config.pr_debug_out > 1) cerr << "c compare variable (level 0) " << tv + 1 << ": pos = " << toInt(prPositive[tv].assigns)
                                                           << " neg = " << toInt(solver.varFlags[tv].assigns) << endl;);
                ...
                DOUT(if (config.pr_debug_out > 1) {
                    cerr << "c trail: " << solver.trail.size() << " trail_lim[0] " << solver.trail_lim[0] << " decisionLevel "
                         << solver.decisionLevel() << endl;
                ...
                solver.cancelUntil(0);
                // add all found implied literals!
                if (doubleLiterals.size() > 0) {
                    data.addCommentToProof("literals that have been implied during double look ahead");
                }
                for (int i = 0; i < doubleLiterals.size(); ++i) {
        ...
        if (config.pr_keepLearnts != 2) {
            for (int i = 0; i < l2conflicts.size(); ++i) {
                ...
                if (config.pr_keepLearnts == 0) {
                    ...
                    if (!c.can_be_deleted()) {
                        data.addToProof(ca[l2conflicts[i]], true);
                    } // delete from proof
                    c.set_delete(true);
        ...
        if (config.pr_keepImplied != 2) {
            for (int i = 0; i < l2implieds.size(); ++i) {
                ...
                if (config.pr_keepImplied == 0) {
                    ...
                    if (!c.can_be_deleted()) {
                        data.addToProof(ca[l2implieds[i]], true);
                    } // delete from proof
                    c.set_delete(true);
    ...
    void Probing::clauseVivificationLCM() {
        ...
        for (int iteration = 0; iteration < config.pr_lcm_iterations; ++iteration) {
            ...
            for (; processedClauses < clauses.size() && data.ok()          // unsat?
                ...
                lcmSteps += c.size(); // somewhat keep track of steps. number of propagated literals would be a better estimate, but is not accessible
                                      // from Coprossor easily
    ...
    void Probing::clauseVivification() {
        ...
        for (uint32_t i = 0; i < data.getClauses().size() && (data.unlimited() || viviLimit > viviChecks) && !data.isInterupted(); ++i) {
            ...
            DOUT(if (config.pr_debug_out > 2) cerr << "c available watches for " << clause[0] << " : " << solver.watches[~clause[0]].size() << " and "
                                                   << clause[1] << " : " << solver.watches[~clause[1]].size() << endl;);
            ...
            for (int j = 0; j < data.lits.size(); ++j) {
                ...
                if (viviConflict) {
                    break;
                } // found a subsuming sub clause -> stop
                else if (viviNextLit) {
                    continue;
                } // found a tautologic literal -> continue with next literal!
            ...
            if (subClauseLimit != sameClause.size()) {
                ...
                if (!viviConflict) {
                    data.lits.push_back(removedLit);
                }
