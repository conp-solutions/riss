...
namespace Coprocessor {
    ...
    BoundedVariableElimination::BoundedVariableElimination(CP3Config& _config, Riss::ClauseAllocator& _ca, Riss::ThreadController& _controller,
    ...
    void BoundedVariableElimination::printStatistics(ostream& stream) {
        stream << "c [STAT] BVE(1) " << processTime << " s, " << subsimpTime << " s spent on subsimp, " << testedVars << " vars tested, "
               << anticipations << " anticipations, " //  = tested vars?
               << skippedVars << " vars skipped, " << stepper.getCurrentSteps() << " checks, " << endl;
        stream << "c [STAT] BVE(2) " << removedClauses << " rem cls, "
               << "with " << removedLiterals << " lits, " << removedLearnts << " learnts rem, "
               << "with " << learntLits << " lits, " << createdClauses << " new cls, "
               << "with " << createdLiterals << " lits, " << newLearnts << " new learnts, "
               << "with " << newLearntLits << " lits, " << endl;
        stream << "c [STAT] BVE(3) " << eliminatedVars << " vars eliminated, " << unitsEnqueued << " units enqueued, " << removedBC << " BC removed, "
               << "with " << blockedLits << " lits, " << removedBlockedLearnt << " blocked learnts removed, "
               << "with " << learntBlockedLit << " lits, " << endl;
        stream << "c [STAT] BVE(4) " << foundGates << " gateDefs, " << usedGates << " usedGates, " << gateTime << " gateSeconds, " << endl;
        stream << "c [STAT] BVE(5) " << nClsIncreases << " incElims, " << nClsKeep << " keepElims, " << nClsDecreases << " decElims, "
               << totallyAddedClauses << " totalAdds," << endl;
        for (int i = 0; i < parStats.size(); ++i) {
            ParBVEStats& s = parStats[i];
            stream << "c [STAT] BVE(1)-T" << i << " " << s.processTime << " s, " << s.subsimpTime << " s spent on subsimp, " << s.upTime
                   << " s spent on up, " << s.lockNeighborTime << " s locking & neighb-calc, " << s.mereLockingTime << " s mere locking, "
                   << s.testedVars << " vars tested, " << s.anticipations << " anticipations, " //  = tested vars?
                   << s.skippedVars << " vars skipped, " << s.parBveChecks << " checks," << endl;
            stream << "c [STAT] BVE(2)-T" << i << " " << s.removedClauses << " rem cls, "
                   << "with " << s.removedLiterals << " lits, " << s.removedLearnts << " learnts rem, "
                   << "with " << s.learntLits << " lits, " << s.createdClauses << " new cls, "
                   << "with " << s.createdLiterals << " lits, " << s.newLearnts << " new learnts, "
                   << "with " << s.newLearntLits << " lits, " << endl;
            stream << "c [STAT] BVE(3)-T" << i << " " << s.eliminatedVars << " vars eliminated, " << s.unitsEnqueued << " units enqueued, "
                   << s.removedBC << " BC removed, "
                   << "with " << s.blockedLits << " lits, " << s.removedBlockedLearnt << " blocked learnts removed, "
                   << "with " << s.learntBlockedLit << " lits, " << endl;
            stream << "c [STAT] BVE(4)-T" << i << " " << s.foundGates << " gateDefs, " << s.usedGates << " usedGates, " << s.gateTime
                   << " gateSeconds, " << endl;
        }
    }

    void BoundedVariableElimination::progressStats(CoprocessorData& data, const bool cputime) {
        if (!config.opt_print_progress || !config.opt_printStats) {
            return;
        }
        ...
        cerr << "c [STAT] BVEprogress: restarts: " << restarts << ", clauses: " << clauseCount << " ("
             << ((double)clauseCount / (double)initialClauses * 100) << " %), "
    ...
    void BoundedVariableElimination::sequentiellBVE(CoprocessorData& data, const bool force, const bool doStatistics) {
        ...
        if (doStatistics) {
            subsimpTime = cpuTime() - subsimpTime;
        }
        ...
        while ( // repeat loop only, if not already interrupted
            ...
            if (config.opt_bve_verbose > 0)
                cerr << "c sequentiel bve on " << ((config.opt_bve_heap != 2) ? variable_heap->size() : variable_queue.size()) << " variables"
                     << endl;
    ...
    void BoundedVariableElimination::bve_worker(CoprocessorData& data, Stepper& workerStepper, const bool force, const bool doStatistics) {
        while ( // repeat loop only until being interrupted
            ...
            if (config.opt_bve_findGate) {
                ...
                if (doStatistics && foundGate) {
                    foundGates++;
                }
            }

            // only eliminate if there is a gate
            if (!foundGate && config.bve_funcDepOnly) {
                continue;
            }
            ...
            if (!force) {
                ...
                if (pos_count != 0 && neg_count != 0) {
                    ...
                    anticipateResult = anticipateElimination(data, pos, neg, v, p_limit, n_limit, pos_stats, neg_stats, lit_clauses, lit_learnts,
                                                             resolvents, workerStepper);
                    if (anticipateResult == l_False) {
                        return;
                    } // level 0 conflict found while anticipation TODO ABORT
                }
            ...
            if (config.opt_bve_growTotal != INT32_MAX && config.opt_bve_growTotal > config.opt_bve_grow) {
                compareNumber += (config.opt_bve_growTotal - totallyAddedClauses); // have one stable reference value
            }

            // we do allow growth
            bool reducedClss = resolvents <= compareNumber; // && resolvents > 0 ;
            DOUT(if (config.opt_bve_verbose > 2) cerr << "c BVE reduced clauses " << reducedClss << " resolvents: " << resolvents << " posC: "
                                                      << pos_count << " negC: " << neg_count << " grow: " << config.opt_bve_grow << endl;);

            if (totallyAddedClauses > config.opt_bve_growTotal) {
                DOUT(if (config.opt_bve_verbose > 2) cerr << "c BVE block increase due to totally added clauses " << totallyAddedClauses << " vs "
                                                          << config.opt_bve_growTotal << endl;);
                reducedClss = false;
            } // stop increasing!
            if (reducedClss) {
            ...
            if ((force || doResolve)    // clauses or literals should be reduced and we did
                ...
                if (doStatistics) {
                    usedGates = (foundGate ? usedGates + 1 : usedGates);
                } // statistics
                if (config.opt_bve_verbose > 1) {
     ...
     */
    inline void BoundedVariableElimination::removeClauses(Coprocessor::CoprocessorData& data, const vector<Riss::CRef>& list, const Lit& l,
                                                          const int limit, const bool doStatistics) {
        for (int cr_i = 0; cr_i < list.size(); ++cr_i) {
            ...
            if (!c.can_be_deleted()) {
                ...
                successfulSimplification();
                c.set_delete(true);
                if (l != lit_Undef && !c.learnt() /*&& cr < limit*/) {
     ...
     */
    inline lbool BoundedVariableElimination::anticipateElimination(CoprocessorData& data, vector<CRef>& positive, vector<CRef>& negative, const int v,
                                                                   const int p_limit, const int n_limit, vec<int32_t>& pos_stats,
                                                                   vec<int32_t>& neg_stats, int& lit_clauses, int& lit_learnts, int& resolvents,
                                                                   Stepper& bveStepper, const bool doStatistics) {
        if (config.opt_bve_verbose > 2) {
        ...
        if (config.opt_bve_earlyAbort) {
            ...
            if (config.opt_bve_growTotal != INT32_MAX && config.opt_bve_growTotal > config.opt_bve_grow) {
                clausesToUse += (config.opt_bve_growTotal - totallyAddedClauses); // have one stable reference value
                if (config.opt_bve_verbose > 2) {
                    cerr << "c adapt limit due to total grow factor: " << clausesToUse << " by adding "
                         << (config.opt_bve_growTotal - totallyAddedClauses) << endl;
                }
            }
        }
        ...
        for (int cr_p = 0; cr_p < positive.size(); ++cr_p) {
            ...
            if (config.opt_bve_verbose > 2) {
                cerr << "c [" << cr_p << "/" << positive.size() << "] : " << p << endl;
            }
            for (int cr_n = 0; cr_n < negative.size(); ++cr_n) {
                ...
                if (binariesOnly && n.size() > 2) {
                    continue;
                } // we are already over the clause limit, but we are still looking for units
                int newLits = tryResolve(p, n, v);
                if (config.opt_bve_verbose > 2) {
                    cerr << "c vs [" << cr_n << "/" << negative.size() << "] : " << n << endl;
                }
                ...
                if (newLits > 1) {
                    ...
                    if (config.opt_bve_earlyAbort &&
                        resolvents > clausesToUse) { // indicate that we interrupted the process, hence, the BCE information is not valid,
                                                     // furthermore, we continue only on binary clauses
                        binariesOnly = true;
                        if (config.opt_bve_verbose > 2) {
                            cerr << "c reached clauses to use limit (" << clausesToUse << ") with resolvents: " << resolvents << endl;
                        }
                    }
     ...
     */
    lbool BoundedVariableElimination::resolveSet(CoprocessorData& data, vector<CRef>& positive, vector<CRef>& negative, const int v,
                                                 const int p_limit, const int n_limit, Stepper& bveStepper, const bool keepLearntResolvents,
            ...
            for (int cr_n = 0; cr_n < negative.size(); ++cr_n) {
                ...
                if (!resolve(p, n, v, ps)) {
                    if (ps.size() == 1) {
                        ...
                        if (config.opt_bve_verbose > 2) { // expensive!
                            for (int i = 0; i < ps.size(); ++i) {
                                for (int j = i + 1; j < ps.size(); ++j) {
                                    if (ps[i] == ps[j]) {
                                        cerr << "c resolvent of clauses " << p << " and " << n << " result in duplicate lits clause! (" << i << ","
                                             << j << "): " << endl;
                                        for (i = 0; i < ps.size(); ++i) {
                                            cerr << " " << ps[i];
                                        }
                        ...
                        if (config.opt_bve_verbose > 1) {
                            cerr << "c from resolution with" << p << " and " << n << endl;
                        }
                        ...
                        if (config.opt_bve_verbose > 1) {
                            cerr << "c add resolvent [" << cr << "] " << resolvent << endl;
                        }
     ...
     */
    inline void BoundedVariableElimination::removeBlockedClauses(CoprocessorData& data, const vector<Riss::CRef>& list, const int32_t stats[],
        ...
        for (unsigned ci = 0; ci < list.size(); ++ci) {
            ...
            if (stats[ci] == 0) {
                ...
                if (!c.learnt() /*&& ci < limit*/) {
                    if (config.opt_bve_verbose > 2) {
                        cerr << "c remove blocked clause " << ca[list[ci]] << endl;
                    }
    ...
    bool BoundedVariableElimination::findGates(CoprocessorData& data, const Var v, int& p_limit, int& n_limit, double& _gateTime, MarkArray* helper) {
        ...
        if (data.list(mkLit(v, false)).size() < 3 && data.list(mkLit(v, true)).size() < 3) {
            return false;
        }
        if (data.list(mkLit(v, false)).size() < 2 || data.list(mkLit(v, true)).size() < 2) {
            return false;
        }
        ...
        for (uint32_t pn = 0; pn < 2; ++pn) {
            ...
            for (uint32_t i = 0; i < pList.size(); ++i) {
                ...
                if (config.opt_bve_verbose > 0) {
                    cerr << "c mark " << ~other << " by " << clause << endl;
                }
            }
            for (uint32_t i = 0; i < nList.size(); ++i) {
                ...
                for (; j < clause.size(); ++j) {
                    ...
                    if (cLit == nLit) {
                        continue;
                    } // do not consider variable that is to eliminate
                    if (!markArray.isCurrentStep(toInt(cLit))) {
                        break;
                    }
                }
                if (j == clause.size()) {
                    ...
                    if (config.opt_bve_verbose > 0) {
                        cerr << "c [BVE] found " << (pn == 0 ? "pos" : "neg") << " gate with size " << j << " p: " << pList.size()
                             << " n:" << nList.size() << " :=" << clause << endl;
                    }
                    // setup values
                    ...
                    if (config.opt_bve_verbose > 0) {
                        cerr << "unmark literals of clause " << clause << endl;
                    }
                    for (uint32_t k = 0; k < clause.size(); ++k) {
                        if (config.opt_bve_verbose > 0) {
                            cerr << "unmark " << clause[k] << endl;
                        }
                    ...
                    for (uint32_t k = 0; k < pList.size(); ++k) {
                        ...
                        const Clause& clause = ca[cr];
                        if (clause.learnt() || clause.can_be_deleted() || clause.size() != 2) {
                            continue;
                        }
                        Lit other = clause[0] == pLit ? clause[1] : clause[0];
                        if (!markArray.isCurrentStep(toInt(~other))) {
                            ...
                            if (config.opt_bve_verbose > 0)
                                cerr << "c do not work with " << other << " and clause " << clause << ", because already re-marked" << endl;
                        }
                    }
                    ...
                    if (failPrint || config.opt_bve_verbose > 1) {
                        ...
                        for (int i = 0; i < data.list(mkLit(v, false)).size(); ++i) {
                            if (data.list(mkLit(v, false))[i] == CRef_Undef) {
                                continue;
                            }
                            if (ca[data.list(mkLit(v, false))[i]].can_be_deleted()) {
                                continue;
                            } else {
                                cerr << i << " : " << ca[data.list(mkLit(v, false))[i]] << endl;
                            }
                        }
                        cerr << "for " << mkLit(v, true) << endl;
                        for (int i = 0; i < data.list(mkLit(v, true)).size(); ++i) {
                            if (data.list(mkLit(v, true))[i] == CRef_Undef) {
                                continue;
                            }
                            if (ca[data.list(mkLit(v, true))[i]].can_be_deleted()) {
                                continue;
                            } else {
                                cerr << i << " : " << ca[data.list(mkLit(v, true))[i]] << endl;
                            }
                        }
                    }
