...
namespace Coprocessor {
    ...
    BlockedClauseElimination::BlockedClauseElimination(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller,
                                                       CoprocessorData& _data, Coprocessor::Propagation& _propagation)
    ...
    void BlockedClauseElimination::coverdLiteralAddition() {
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            ...
            if (data[mkLit(v, false)] > 0)
                if (!bceHeap.inHeap(toInt(mkLit(v, false)))) {
                    bceHeap.insert(toInt(mkLit(v, false)));
                }
            if (data[mkLit(v, true)] > 0)
                if (!bceHeap.inHeap(toInt(mkLit(v, true)))) {
        ...
        while (bceHeap.size() > 0 && (data.unlimited() || config.bceLimit > bceSteps) && !data.isInterupted()) {
            ...
            for (int i = 0; i < listSize; ++i) {
                ...
                if (!isLeastFrequent) {
                    continue;
                } // do not work with this clause on that literal, because its not among the least frequent literals!
                ...
                if (data.lits.size() > 0 && canCla) { // there is something to be added for the clause c!
                    ...
                    for (int k = 0; k < data.lits.size(); k++) {
                        data.ma.setCurrentStep(toInt(data.lits[k]));
                    }
                    ...
                    if (!isTaut) { // do not want to perform CCE here!
                        ...
                        DOUT(if (config.opt_bce_debug) cerr << "c CLA turned clause " << ca[data.list(right)[i]] << " into the new clause "
                                                            << ca[newClause] << endl;);
                        // add new clause
    ...
    void BlockedClauseElimination::blockedClauseElimination() {
        ...
        do {
            ...
            while (bceHeap.size() > 0 && (data.unlimited() || config.bceLimit > bceSteps) && !data.isInterupted()) {
                ...
                DOUT(if (config.opt_bce_debug) cerr << "c BCE work on literal " << right << " with " << data.list(right).size() << " clauses "
                                                    << endl;);
                data.lits.clear(); // used for covered literal elimination
                for (int i = 0; i < data.list(right).size(); ++i) {
                    if (data.hasToPropagate()) {
                        break;
                    } // we found a unit, stop working on BCE for now
                    Clause& c = ca[data.list(right)[i]];
                    ...
                    for (int j = 0; j < data.list(left).size(); ++j) {
                        ...
                        Clause& d = ca[data.list(left)[j]];
                        if (d.can_be_deleted()) {
                            DOUT(if (config.opt_bce_debug) cerr << "c BCE/CLE do not compare with clause " << d << " as it is already deleted"
                                                                << endl;);
                            continue;
                        } // do not work on uninteresting clauses!
                        bceSteps++;
                        ...
                        DOUT(if (config.opt_bce_debug) cerr << "c BCE/CLE compare with clause " << d << " (" << j << "/" << data.list(left).size()
                                                            << ")" << endl;);
                        ...
                        if (tautLit == lit_Undef) { // the resolvent is not a tautology
                            isBlocked =
                                false; // from here on, the given clause is not a blocked clause since a non-tautologic resolvent has been produced
                            DOUT(if (config.opt_bce_debug) cerr << "c    clause is not BCE, CLE lits: " << data.lits.size() << " , canCLE: " << canCle
                                                                << endl;);
                            ...
                            if (config.opt_bce_cle) { // compute CLE of the clause
                                ...
                                data.lits.resize(keptCle); // remove all the other literals
                                DOUT(if (config.opt_bce_debug) cerr << "c resolving " << c << " with " << d << " keeps the CLE literals " << data.lits
                                                                    << endl;);
                                if (keptCle == 0) {
                                    break;
                                } // can stop here, because no CLE, and not blocked!
                            }
                        } else {
                            // compute cle?
                            if (config.opt_bce_cle) {
                                DOUT(if (config.opt_bce_debug) cerr << "c      handle tautologic resolvent during CLE with " << data.lits
                                                                    << " CLE lits left " << endl;);
                                if (data.lits.size() > 0) { // we have found a tautologic resolvent, take care of the set of literals for CLE!
                                    ...
                                    if (config.opt_bce_cle_conservative) {
                                        ...
                                        while (k < data.lits.size() &&
                                               l < d.size()) { // since both data.lits and d are sorted, intersection can be calculated like this
                            ...
                            if (config.opt_bce_bcm && !d.learnt()) {
                                tautologyReasonLiterals.setCurrentStep(
                                    toInt(tautLit)); // store that this literal is necessary for the clause to be blocked
                            }
                        }
                        DOUT(if (config.opt_bce_debug) cerr << "c BCE/CLE finished iteration with clause " << d << " (" << j << "/"
                                                            << data.list(left).size() << ")" << endl;);
                    }
                    DOUT(if (config.opt_bce_debug) cerr << "c resolved " << c << " with about " << data.list(left).size()
                    ...
                    if (config.opt_bce_bce && isBlocked) { // the clause c is blocked wrt. the current formula
                        if (!config.opt_bce_bcm) {         // perform usual BCE
                            ...
                            if (!c.learnt()) {
                                DOUT(if (config.opt_bce_verbose > 2) cerr << "c remove with blocking literal " << right << " blocked clause "
                        ...
                        } else if (c.size() > 2) { // perform blocked clause minimization (BCM) (remove all literals that do not contribute to being
                                                   // blocked). is the same as adding a blocked clause and performing subsumption
                            ...
                            for (int k = 0; k < c.size(); ++k) {
                                if (c[k] == right ||
                                    (deletedSomeLiteral && data.outputsProof())) { // make sure we only remove one literal in a step ... TODO teach
                                                                                   // proof printing how to handle multi-BCM clauses
                                ...
                                if (!tautologyReasonLiterals.isCurrentStep(toInt(c[k]))) { // found a literal that can be removed
                                    if (!deletedSomeLiteral) { // tell proof about removed blocked clause (the other clause will be added below, after
                                                               // minimization)
                                        data.addCommentToProof("blocked clause during BCM");
                                        data.addToProof(
                                            c,
                                            true); // delete the original clause - the minimized clause can still be added by blocked clause addition
                                    ...
                                    data.removedLiteral(c[k]);   // update counters for the literal
                        ...
                        if (config.opt_bce_cle && canCle && data.lits.size() > 0) {
                            ...
                            if (data.outputsProof()) { // store the original clause for deleting it from the proof
                                ...
                                for (int m = 0; m < c.size(); ++m) {
                                    data.getSolver()->oc.push(c[m]);
                                }
                                // only remove a single literal, as DRAT does not support multiple literals ... // TODO do a step by step procedure in
                                // future for DRAT!
                                data.lits.resize(1);
                            }
                            ...
                            while (k < c.size() && l < data.lits.size()) {
                                if (c[k] == data.lits[l]) {
                                    // remove the literal from the clause and remove the clause from that literals structures, as well as decrease the
                                    // occurrence counter
                                    ...
                                    l++;
                                } // if ( data.lits[l] < c[k] ) // the only left possibility!
                            }
                            for (; k < c.size(); ++k) {
                                c[keptLiterals++] = c[k];
                            } // keep the remaining literals as well!
                            assert((keptLiterals + data.lits.size() == c.size()) && "the size of the clause should not shrink without a reason");
                            if (c.size() != data.lits.size()) {
                                modifiedClause = true;
                            }
                            ...
                            data.addToProof(c, false,
                                            right); // add the new clause after c[k] has been removed - has been resolved on literal "right", hence,
                                                    // do write that literal to the first position!
                            data.addToProof(data.getSolver()->oc, true); // remove the clause of the shape it had before
                            DOUT(if (config.opt_bce_debug) cerr << "c into clause " << c << " by removing " << data.lits.size()
                            ...
                            for (l = 0; l < data.lits.size(); ++l) {
                                if (bceHeap.inHeap(toInt(~data.lits[l])))
                                    bceHeap.update(toInt(~data.lits[l])); // there is the chance for ~right to become blocked!
                                else {
                    ...
                    if (data.hasToPropagate()) {
                        ...
                        if (prevSize != data.list(right).size()) {
                            i = -1; // start the current list over, if propagation did something to the list size (do not check all clauses of the
                                    // list!)!
                        }
                    }
    ...
    bool BlockedClauseElimination::process() {
        ...
        if (!data.unlimited() && (data.nVars() > config.opt_bce_vars && data.getClauses().size() + data.getLEarnts().size() > config.opt_bce_cls &&
        ...
        if (config.opt_bce_bce || config.opt_bce_cle) {
            blockedClauseElimination();
        }

        if (config.opt_bce_cla) {
            coverdLiteralAddition();
        }
    ...
    void BlockedClauseElimination::printStatistics(ostream& stream) {
        cerr << "c [STAT] BCE " << bceTime.getCpuTime() << " seconds, " << bceSteps << " steps, " << testedLits << " testLits, " << remBCE
             << " remBCE, " << bcm_cls << " BCMs, " << bcm_cls_cands << " BCMcands, " << bcm_lits << " BCMlits, " << endl;
        cerr << "c [STAT] CLE " << remCLE << " remCLE, " << cleUnits << " cleUnits, " << endl;
        cerr << "c [STAT] BCE-CLA " << claTime.getCpuTime() << " seconds, " << claSteps << " steps, " << claTestedLits << " testLits, "
             << claExtendedClauses << " extClss, " << claExtensions << " extLits, " << possibleClaExtensions << " possibles, " << endl;
    }
