...
namespace Coprocessor {
    ...
    FourierMotzkin::FourierMotzkin(CP3Config& _config, ClauseAllocator& _ca, ThreadController& _controller, CoprocessorData& _data,
    ...
    bool FourierMotzkin::process() {
        ...
        if (data.outputsProof()) {
            printDRUPwarning(cerr, "FM cannot produce DRUP/DRAT proofs yet");
        }
        ...
        for (Var v = 0; v < data.nVars(); ++v) {
            if (!heap.inHeap(toInt(mkLit(v, false)))) {
                heap.insert(toInt(mkLit(v, false)));
            }
            if (!heap.inHeap(toInt(mkLit(v, true)))) {
                heap.insert(toInt(mkLit(v, true)));
            }
        }
        ...
        while (heap.size() > 0 && (data.unlimited() || (fmLimit > steps && config.opt_fmSearchLimit > searchSteps)) && !data.isInterupted()) {
            ...
            if (size < 2) {
                continue;
            } // cannot result in a AMO of required size -> skip!
            ...
            for (int i = 0; i < size; ++i) {
                ...
                if (data[l] < 2) {
                    continue;
                } // cannot become part of AMO!
                if (big.getSize(~l) < 2) {
                    continue;
                } // cannot become part of AMO!
                if (inAmo.isCurrentStep(toInt(l))) {
                    continue;
                } // avoid duplicates!
                inAmo.setCurrentStep(toInt(l));
                data.lits.push_back(l); // l is implied by ~right -> canidate for AMO(right,l, ... )
                if (data.lits.size() > config.opt_fmMaxAMO) {
                    break;
                } // do not collect more literals for the AMO (heuristic cut off...)
            }
            DOUT(if (config.fm_debug_out > 2) cerr << "c implieds: " << data.lits.size() << ", namely " << data.lits << endl;);
            if (data.lits.size() < 3 || config.opt_fmSearchLimit <= searchSteps) {
                continue;
            } // do not consider this variable, because it does not hit enough literals

            // TODO: should sort list according to frequency in binary clauses - ascending, so that small literals are removed first, increasing the
            // chance for this more frequent ones to stay!
            ...
            for (int i = 0; i < data.lits.size(); ++i) { // keep the very first, since it created the list!
                ...
                if (l == lit_Undef) {
                    continue;
                } // do not handle removed literals!
                ...
                if (config.opt_rem_first) {
                    ...
                    for (; j < data.lits.size(); ++j) {
                        if (i != j && data.lits[j] != lit_Undef && !inAmo.isCurrentStep(toInt(data.lits[j]))) {
                            break;
                        }
                        searchSteps++;
                    }
                    if (j != data.lits.size()) {
                        DOUT(if (config.fm_debug_out > 0) cerr << "c reject [" << i << "]" << data.lits[i] << ", because failed with [" << j << "]"
                                                               << data.lits[j] << endl;);
                        data.lits[i] = lit_Undef; // if not all literals are covered, disable this literal!
                    } else {
                        DOUT(if (config.fm_debug_out > 0) cerr << "c keep [" << i << "]" << data.lits[i] << " which hits [" << j << "] literas"
                                                               << endl;);
                    }
                } else {
                    for (int j = 0; j < size2; ++j) {
                    ...
                    for (; j < data.lits.size(); ++j) {
                        if (data.lits[j] == lit_Undef) {
                            continue;
                        } // do not process this literal!
                        searchSteps++;
                        ...
                        if (!inAmo.isCurrentStep(toInt(data.lits[j])) // not in AMO with current literal
                            ...
                            DOUT(if (config.fm_debug_out > 0) cerr << "c reject [" << j << "]" << data.lits[j] << ", because failed with [" << i
                                                                   << "]" << data.lits[i] << endl;);
                            data.lits[j] = lit_Undef; // if not all literals are covered, disable this literal!
                        } else {
                            DOUT(if (config.fm_debug_out > 0) cerr << "c keep [" << j << "]" << data.lits[j] << " which is hit by literal "
                                                                   << data.lits[i] << "[" << i << "] " << endl;);
                        }
                    }
                }
            ...
            for (int i = 0; i < data.lits.size(); ++i)
                if (data.lits[i] == lit_Undef) {
                    data.lits[i] = data.lits[data.lits.size() - 1];
                    data.lits.pop_back();
                    --i;
                }
            ...
            if (data.lits.size() == 2) {
                continue;
            } // do not consider trivial constraints!
            ...
            if (config.opt_fm_prooftrace) {
                cerr << "c FM proof (AMO) add " << cards[cards.size() - 1].getID() << " [" << cards[cards.size() - 1].getParentL() << " , "
                     << cards[cards.size() - 1].getParentR() << "] : " << cards[cards.size() - 1].ll << " <= " << cards[cards.size() - 1].k << " + "
                     << cards[cards.size() - 1].lr << endl;
            }
            if (cards.size() >= config.opt_fm_max_constraints) {
                break;
            }

            if (config.opt_fm_multiVarAMO && data.lits.size() > 1 && config.opt_fm_avoid_duplicates && big.getSize(right) > 0) {
                heap.insert(toInt(right));
            } // this literal might have more cliques
        }
        ...
        if (config.opt_atMostTwo) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                ...
                for (int p = 0; p < 2; ++p) {                      // for both polarities
                    ...
                    if (!config.opt_multiVarAMT && inAmo.isCurrentStep(toInt(l))) {
                        continue;
                    } // each literal has the chance to be in one constraint!
                    DOUT(if (config.fm_debug_out > 1) cerr << "c consider literal " << l << " with " << data.list(l).size() << " clauses" << endl;);
                    data.lits.clear();
                    data.ma.nextStep(); // prepare set!
                    data.lits.push_back(l);
                    data.ma.setCurrentStep(toInt(l)); // add current literal!
                    ...
                    for (int i = 0; i < data.list(l).size(); ++i) { // collect all literals that occur with l in a ternary clause!
                        ...
                        if (c.can_be_deleted() || c.size() != 3) {
                            continue;
                        } // look for interesting, ternary clauses only!
                        if (c[0] != l) {
                            continue;
                        } // consider only clauses, where l is the smallest literal?! (all other AMT's would have been found before!)
                        if (!config.opt_multiVarAMT && (inAmo.isCurrentStep(toInt(c[1])) || inAmo.isCurrentStep(toInt(c[2])))) {
                            continue;
                        } // do not use this ternary clause any more!
                        for (int j = 1; j < 3; ++j) {
                            searchSteps++;
                            if (c[j] != l && !data.ma.isCurrentStep(toInt(c[j]))) {
                                data.lits.push_back(c[j]);
                        ...
                        for (int j = i + 1; j < data.lits.size(); ++j) {
                            ...
                            for (int k = j + 1; k < data.lits.size(); ++k) {
                                ...
                                for (; m < data.list(l0).size(); ++m) { // collect all literals that occur with l in a ternary clause!
                                    ...
                                    if (c[1] == l1 && c[2] == l2) {
                                        break;
                                    } // found corresponding clause! - l0 < l1 -> needs to be c[0]
                                }
                                if (m == data.list(l0).size()) { // did not find triple l0,l1,l2
                                    for (j = i; j + 1 < data.lits.size(); ++j) {
                                        data.lits[j] = data.lits[j + 1];
                                    }                     // move all remaining literals one position to front // TODO: can be implemented faster!
                    ...
                    if (data.lits.size() > 3) {
                        for (int i = 0; i < data.lits.size(); ++i) {
                            ...
                            if (!config.opt_multiVarAMT) {
                                inAmo.setCurrentStep(toInt(data.lits[i]));
                            } // disallow the current literal to participate in another constraint as well!
                        }
                        foundAmts++;
                        ...
                        if (config.opt_fm_prooftrace) {
                            cerr << "c FM proof (AM2) add " << cards.back().getID() << " [" << cards.back().getParentL() << " , "
                                 << cards.back().getParentR() << "] : " << cards.back().ll << " <= " << cards.back().k << " + " << cards.back().lr
                                 << endl;
                        }
                        if (cards.size() >= config.opt_fm_max_constraints) {
                            goto finishAMT;
        ...
        for (int i = 0; i < cards.size(); ++i) {
            for (int j = 0; j < cards[i].ll.size(); ++j) {
                leftHands[toInt(cards[i].ll[j])].push_back(i);
            }
            for (int j = 0; j < cards[i].lr.size(); ++j) {
                rightHands[toInt(cards[i].lr[j])].push_back(i);
            }
        }
        ...
        if (config.opt_findUnit) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                        ...
                        while (n1 < v1.size() && n2 < v2.size()) {
                            if (v1[n1] == v2[n2]) {                         // same literal in two amos with a different literal -> have to be unit!
                                if (!inAmo.isCurrentStep(toInt(~v1[n1]))) { // store each literal only once in the queue
                                    ...
                                    if (l_False == data.enqueue(~v1[n1])) {
                                        return modifiedFormula;
                                    } // enqueing this literal failed -> finish!
                                }
                ...
                if (!propagateCards(unitQueue, leftHands, rightHands, cards, inAmo)) {
                    return modifiedFormula;
                }
            }
        }
        ...
        if (config.opt_merge) {
            ...
            for (Var v = 0; v < data.nVars(); ++v) {
                const Lit pl = mkLit(v, false), nl = mkLit(v, true);
                if (leftHands[toInt(pl)].size() > 2 || leftHands[toInt(nl)].size() > 2) {
                    continue;
                }                                                                          // only if the variable has very few occurrences
                for (int i = 0; i < leftHands[toInt(pl)].size() && steps < fmLimit; ++i) { // since all cards are compared, be careful!
                    steps++;
                    if (cards[leftHands[toInt(pl)][i]].invalid()) {
                        continue;
                    }
                    if (!cards[leftHands[toInt(pl)][i]].amo()) {
                        continue;
                    } // can do this on AMO only
                    for (int j = 0; j < leftHands[toInt(nl)].size(); ++j) {
                        ...
                        if (data.lits.size() == 0) {
                            continue;
                        } // no AMO, if there are no literals inside!
                        DOUT(if (config.fm_debug_out > 0) cerr << "c deduced AMO " << data.lits << " from AMO " << card1.ll << " and AMO " << card2.ll
                                                               << endl;);

                        // check whether there are similar variables inside the constraint, if so - drop it!
                        bool hasComplements = false;
                        for (int k = 0; k + 1 < data.lits.size(); ++k)
                            if (data.lits[k] == ~data.lits[k + 1]) {
                                DOUT(if (config.fm_debug_out > 2) cerr << "c deduced AMO contains complementary literals - set all others to false!"
                                                                       << endl;);
                                Var uv = var(data.lits[k]);
                                for (int k = 0; k < data.lits.size(); ++k) {
                                    if (var(data.lits[k]) == uv) {
                                        continue;
                                    }                                                 // do not enqueue complementary literal!
                        ...
                        cards[cards.size() - 1].setParents(
                            cards[leftHands[toInt(pl)][i]].getID(),
                            cards[leftHands[toInt(nl)][j]].getID()); // set the parents of the constraint to reconstruct the proof
                        if (config.opt_fm_prooftrace) {
                            cerr << "c FM proof (MERGE) add " << cards.back().getID() << " [" << cards.back().getParentL() << " , "
                                 << cards.back().getParentR() << "] : " << cards.back().ll << " <= " << cards.back().k << " + " << cards.back().lr
                                 << endl;
                        }
                        for (int k = 0; k < data.lits.size(); ++k) {
                            leftHands[toInt(data.lits[k])].push_back(index);
                        }
                        if (cards.size() >= config.opt_fm_max_constraints) {
                            break;
                        }
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            for (int j = 0; j < c.ll.size(); ++j) {
                data.ma.setCurrentStep(var(c.ll[j]));
            }
            for (int j = 0; j < c.lr.size(); ++j) {
                data.ma.setCurrentStep(var(c.lr[j]));
            }
        }
        ...
        for (int i = 0; i < data.getClauses().size(); ++i) {
            ...
            for (int j = 0; j < c.size(); ++j) {
                if (data.ma.isCurrentStep(var(c[j]))) { // a literal of this clause took part in an AMO
                    ...
                    if (config.opt_fm_prooftrace) {
                        cerr << "c FM proof (UseClause) add " << cards.back().getID() << " [" << cards.back().getParentL() << " , "
                             << cards.back().getParentR() << "] : " << cards.back().ll << " <= " << cards.back().k << " + " << cards.back().lr
                             << endl;
                    }
                    DOUT(if (config.fm_debug_out > 1) cerr << "c clause " << c << " leads to card " << cards[cards.size() - 1].ll
                                                           << " <= " << cards[cards.size() - 1].k << " + " << cards[cards.size() - 1].lr << endl;);
                    for (int j = 0; j < c.size(); ++j) {
                        rightHands[toInt(c[j])].push_back(index);
                        if (!heap.inHeap(toInt(c[j]))) {
                            heap.insert(toInt(c[j]));
                        }
                    }
                    usedClauses++;
                    break;
        ...
        while (heap.size() > 0 && (data.unlimited() || (fmLimit > steps && !sizeAbort)) && !data.isInterupted()) {
            /** garbage collection */
            if (config.opt_fm_garbageColelct && needsGarbageCollect == iter &&
                invalidCards * 4 > validCards) { // perform garbage collection only if at least 25% are garbage
                ...
                int k = 0; // number of kept constraints
                for (int i = 0; i < newAMOs.size(); ++i)
                    if (rewrite[newAMOs[i]] != -1) {
                        newAMOs[k++] = rewrite[newAMOs[i]];
                    }
                newAMOs.resize(k);
                k = 0; // remove unused constraints!
                for (int i = 0; i < newALOs.size(); ++i)
                    if (rewrite[newALOs[i]] != -1) {
                        newALOs[k++] = rewrite[newALOs[i]];
                    }
                newALOs.resize(k);
                k = 0; // remove unused constraints!
                for (int i = 0; i < newALKs.size(); ++i)
                    if (rewrite[newALKs[i]] != -1) {
                        newALKs[k++] = rewrite[newALKs[i]];
                    }
                newALKs.resize(k); // remove unused constraints!
                if (data.unlimited()) {
                    needsGarbageCollect = -1;
                } // if unlimited, reset garbage collection
                validCards = cards.size();
                invalidCards = 0; // reset counters
            } else {
            ...
            if (leftHands[toInt(toEliminate)].size() == 0 || rightHands[toInt(toEliminate)].size() == 0) {
                pureAmoLits++;
                continue; // only if the variable has very few occurrences. cannot handle pure here, because its also inside the formula at other
                          // places!
            }
            if (config.opt_cutOff && ((leftHands[toInt(toEliminate)].size() > 5 && rightHands[toInt(toEliminate)].size() > 15) ||
                ...
                continue;
            } // do not eliminate this variable, if it looks too expensive
            ...
            for (int i = 0; i < leftHands[toInt(toEliminate)].size() && steps < fmLimit && !sizeAbort;
                 ++i) { // since all cards are compared, be careful!
                steps++;
                ...
                for (int j = 0; j < rightHands[toInt(toEliminate)].size() && !sizeAbort; ++j) {
                    if (rightHands[toInt(toEliminate)][j] == leftHands[toInt(toEliminate)][i]) {
                        DOUT(if (config.fm_debug_out > 2) cerr
                                 << "c irregular literal " << toEliminate << " with constraints (" << rightHands[toInt(toEliminate)][j]
                                 << " == " << leftHands[toInt(toEliminate)][i] << "): " << endl
                                 << " and  " << cards[rightHands[toInt(toEliminate)][j]].ll << " <= " << cards[rightHands[toInt(toEliminate)][j]].k
                                 << " + " << cards[rightHands[toInt(toEliminate)][j]].lr << endl;);
                        irregular++;
                        continue;
                    ...
                    if (cards.size() >= config.opt_fm_max_constraints) {
                        if (needsGarbageCollect == -1) {
                            needsGarbageCollect = iter;
                        } else if (iter > needsGarbageCollect) {
                            sizeAbort = true;
                        }
                    }
                    ...
                    for (int p = 0; p < 2; ++p) {
                        ...
                        while (n1 < v1.size() && n2 < v2.size()) {
                            ...
                            if (v1[n1] == v2[n2]) { // same literal in two amos with a different literal -> have to be unit!
                                ...
                                // TODO: enqueue, later remove from all cards!
                                data.lits.push_back(
                                    v1[n1]); // keep them // FIXME: duplicate literal is dropped, is this a problem? yes on the right side!
                                // if( p > 0 ) extraK ++; // keep one literal only, but for the right side assume that the other literal is set
                                // (approximation)
                                hasDuplicates = true;
                                n1++;
                                n2++;
                                ...
                                extraK = (p == 0) ? extraK - 1 : extraK + 1; // depending on whether the complementary pair is removed on the left or
                                                                             // the right side of the <= operator, decrease or increase!
                                n1++;
                                n2++;
                            } else if (v1[n1] < v2[n2]) {
                    ...
                    if (hasDuplicates) {
                        DOUT(if (config.fm_debug_out > 0) cerr
                                 << "c new card would have duplicates - drop it (can be fixed with full FM algorithm which uses weights)" << endl
                                 << "c  from " << card1.ll << " <= " << card1.k << " + " << card1.lr << endl
                                 << "c   and " << card2.ll << " <= " << card2.k << " + " << card2.lr << endl
                                 << endl;);
                    ...
                    while (nl < thisCard.ll.size() && nr < thisCard.lr.size()) {
                        DOUT(if (config.fm_debug_out > 2) cerr << "c compare ll[" << nl << "] = " << thisCard.ll[nl] << " << with lr[" << nr << "]"
                                                               << thisCard.lr[nr] << endl;);
                        if (thisCard.ll[nl] == thisCard.lr[nr]) { // do not keep the same literal!
                            ...
                            nl++;
                            nr++; // push both, because there are no complementary literals on one side only
                        } else if (thisCard.ll[nl] < thisCard.lr[nr]) {
                            thisCard.ll[kl++] = thisCard.ll[nl];
                            nl++; // keep this literal on the left side!
                        } else {
                            thisCard.lr[kr++] = thisCard.lr[nr];
                            nr++; // keep this literal on the right side!
                        }
                    }
                    for (; nr < thisCard.lr.size(); ++nr) {
                        thisCard.lr[kr++] = thisCard.lr[nr];
                    }
                    for (; nl < thisCard.ll.size(); ++nl) {
                        thisCard.ll[kl++] = thisCard.ll[nl];
                    }
                    ...
                    if (config.opt_fm_prooftrace) {
                        cerr << "c FM proof (FM) add " << thisCard.getID() << " [" << thisCard.getParentL() << " , " << thisCard.getParentR()
                             << "] : " << thisCard.ll << " <= " << thisCard.k << " + " << thisCard.lr << endl;
                    }

                    DOUT(if (config.fm_debug_out > 0) cerr << "c resolved new CARD " << thisCard.ll << " <= " << thisCard.k << " + " << thisCard.lr
                                                           << "  (unit: " << thisCard.isUnit() << " taut: " << thisCard.taut() << ", failed: "
                                                           << thisCard.failed() << "," << (int)thisCard.lr.size() + thisCard.k << ")" << endl
                                                           << "c from " << card1.ll << " <= " << card1.k << " + " << card1.lr << endl
                                                           << "c and  " << card2.ll << " <= " << card2.k << " + " << card2.lr << endl
                                                           << endl;);
                    if (thisCard.taut()) {
                        ...
                        if (config.opt_newAmo) {
                            newAMOs.push_back(index);
                        }
                        newAmos++;
            ...
            if ((cards.size() - oldSize >
                 leftHands[toInt(toEliminate)].size() + rightHands[toInt(toEliminate)].size() + config.opt_fmGrow) // local increase to large
                ...
                if (config.opt_keepAllNew) { // copy the new AMOs that should be rejected
                    ...
                    discardedNewAmos += (newAMOs.size() - oldNewSize);
                }
            ...
            for (int p = 0; p < 2; ++p) {
                ...
                while (indexes.size() > 0) {
                    ...
                    for (int j = 0; j < card.ll.size(); ++j) { // remove all lls from left hand!
                        ...
                        for (int k = 0; k < leftHands[toInt(l)].size(); ++k) {
                            if (leftHands[toInt(l)][k] == removeIndex) {
                                DOUT(if (config.fm_debug_out > 3) cerr << "c from leftHand " << l << "( " << j << "/" << card.ll.size() << "," << k
                                                                       << "/" << leftHands[toInt(l)].size() << ")" << endl;);
                                leftHands[toInt(l)][k] = leftHands[toInt(l)][leftHands[toInt(l)].size() - 1];
                                leftHands[toInt(l)].pop_back();
                                break;
                            }
                        }
                    ...
                    for (int j = 0; j < card.lr.size(); ++j) { // remove all lls from right hand!
                        ...
                        for (int k = 0; k < rightHands[toInt(l)].size(); ++k) {
                            if (rightHands[toInt(l)][k] == removeIndex) {
                                DOUT(if (config.fm_debug_out > 3) cerr << "c from rightHands " << l << "( " << j << "/" << card.lr.size() << "," << k
                                                                       << "/" << rightHands[toInt(l)].size() << ")" << endl;);
                                rightHands[toInt(l)][k] = rightHands[toInt(l)][rightHands[toInt(l)].size() - 1];
                                rightHands[toInt(l)].pop_back();
                                break;
                            }
                        }
                        DOUT(if (config.fm_debug_out > 3) cerr << "c finished literal " << l << "( " << j << "/" << card.ll.size() << ")" << endl;);
                    ...
                    invalidCards++;
                    validCards--; // garbage collection counters
                }
            }
            ...
            for (int i = oldSize; i < cards.size(); ++i) {
                ...
                for (int j = 0; j < card.ll.size(); ++j) {
                    leftHands[toInt(card.ll[j])].push_back(i);
                }
                for (int j = 0; j < card.lr.size(); ++j) {
                    rightHands[toInt(card.lr[j])].push_back(i);
                }
            }
            ...
            DOUT(if (false && (const char*)config.stepbystepoutput != nullptr) {
        ...
        if (data.ok() && config.opt_newAmo > 0 && (newAMOs.size() > 0 || rejectedNewAmos.size() > 0)) {
            ...
            if (config.opt_newAmo > 1) {
                big.generateImplied(data);
            } // try to avoid adding redundant clauses!

            DOUT(if (config.fm_debug_out > 0) cerr << "c analyze new and rejected AMOs, actual card.constraints: " << cards.size()
                                                   << " rejectedNew: " << rejectedNewAmos.size() << endl;);
            ...
            for (int p = 0; p < 2; ++p) {
                ...
                for (int i = 0; i < (p == 0 ? newAMOs.size() : rejectedNewAmos.size()); ++i) {
                    ...
                    DOUT(if (config.fm_debug_out > 0) cerr << "c check clauses for AMO " << c.ll << " <= " << c.k << " + " << c.lr
                                                           << "  card index: p: " << p << " index: " << (p == 0 ? newAMOs[i] : i) << endl;);
                    for (int j = 0; j < c.ll.size(); ++j) {
                        for (int k = j + 1; k < c.ll.size(); ++k) {
                            bool present = false;
                            if (config.opt_newAmo == 2) {
                                present = big.implies(c.ll[j], ~c.ll[k]) || big.implies(~c.ll[k], c.ll[j]);
                            } // fast stamp check
                            if (!present) {
                                present = big.isChild(c.ll[j], ~c.ll[k]) || big.isChild(c.ll[k], ~c.ll[j]);
                            }               // slower list check
                            if (!present) { // if the information is not part of the formula yet, add the clause!
                                ...
                                DOUT(if (config.fm_debug_out > 0) cerr << "c add clause (" << __LINE__ << ")[" << tmpRef << "]" << ca[tmpRef]
                                                                       << endl;);
        ...
        if (data.ok() && config.opt_newAlo > 0 && (newALOs.size() > 0 || rejectedNewAlos.size() > 0)) {
            for (int p = 0; p < 2; ++p) {
                for (int i = 0; i < (p == 0 ? newALOs.size() : rejectedNewAlos.size()); ++i) {
                    ...
                    modifiedFormula = true;
                    if (c.lr.size() == 1) {
                        if (data.enqueue(c.lr[0]) == l_False) {
                            return modifiedFormula;
                        }
        ...
        if (data.ok() && config.opt_newAlk > 0 && (newALKs.size() > 0 || rejectedNewAlks.size() > 0)) {
            for (int p = 0; p < 2; ++p) {
                for (int i = 0; i < (p == 0 ? newALKs.size() : rejectedNewAlks.size()); ++i) {
                    ...
                    bool isSat = false;
                    for (int j = 0; j < c.lr.size(); ++j) {
                        if (data.value(c.lr[j]) == l_True) {
                            isSat = true;
                            break;
                        } else if (data.value(c.lr[j]) == l_Undef) {
                            c.lr[k++] = c.lr[j];
                        } // keep only undefined literals!
                    }
                    if (isSat) {
                        continue;
                    }               // do not consider satisfied constraints!
                    c.lr.resize(k); // keep only unassigned literals!
                    if (c.lr.size() == 0) {
                        data.setFailed();
                        return modifiedFormula;
                    } else if (c.lr.size() == 1) {
                        if (data.enqueue(c.ll[0]) == l_False) {
                            return modifiedFormula;
                        }
                    } else if (!hasDuplicate(c.lr)) {
        ...
        if (config.opt_fm_printtrace != 0) {
            ...
            for (int i = 0; i < cards.size(); ++i) {
                ...
                for (int j = 0; j < basis.size(); ++j) {
                    ...
                    if (c.getParentL() == -1 && c.getParentR() == -1) {
                        formulaConstraints++;
                    }
                }
                cerr << "c constraint " << thisCard.getID() << " depends on " << basis.size() << " (" << formulaConstraints << ") constraints"
                     << endl;
                if (config.opt_fm_printtrace > 1) {
                    cerr << thisCard.getID() << " [" << thisCard.getParentL() << " , " << thisCard.getParentR() << "] : " << thisCard.ll
                         << " <= " << thisCard.k << " + " << thisCard.lr << endl;
                    for (int j = 0; j < basis.size(); ++j) {
                        CardC& c = cards[basis[j]];
    ...
    void FourierMotzkin::findCardsSemantic(vector<FourierMotzkin::CardC>& cards, vector<std::vector<int>>& leftHands) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 1; j < c.size(); ++j)
                if (c[0] < minLit) {
                    minLit = c[0];
                } // if its the minimum, later disabling clauses is cheaper
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            if (!thisCard.amk()) {
                continue;
            } // disable only clauses of constraints
            data.ma.nextStep();
            for (int j = 0; j < thisCard.ll.size(); ++j) {
                data.ma.setCurrentStep(toInt(~thisCard.ll[j]));
            } // mark all complements of the current constraint
            ...
            for (int j = 0; j < thisCard.ll.size(); ++j) { // for all the literals in at least the original constraint, but also in its extension
                for (int k = 0; k < occ[toInt(~thisCard.ll[j])].size();
                     ++k) { // and all the clauses that have this literal as smallest literal (watch 1 scheme, each clause only once)
                    Clause& candidate = ca[occ[toInt(~thisCard.ll[j])][k]]; // current delete candidate
                    if (candidate.mark() != 0) {
                        continue;
                    } // candidate contains more literals than have been marked
                    if (config.opt_noReduct && candidate.size() > thisCard.ll.size()) {
                        continue;
                    } // there cannot be falsified literals inside the clauses
                    bool failed = false;
                    for (int m = 0; m < candidate.size(); ++m) { // check whether the current clause has all the necessary literals
                        if (!config.opt_noReduct && solver.value(candidate[m]) == l_False) {
                            continue;
                        } // do not care about disabled literals (if there are no units around)
                        if (!data.ma.isCurrentStep(toInt(candidate[m]))) {
                            failed = true;
                            break;
                        } // another literal was found, keep this clause!
                    }
                    if (!failed) {
                        DOUT(if (config.opt_semDebug) cerr << "c disable with literal " << thisCard.ll[j] << " clause["
                                                           << occ[toInt(~thisCard.ll[j])][k] << "] " << candidate << endl;);
        ...
        for (int i = 0; i < n && (data.unlimited() || (semSteps < config.opt_semSearchLimit));
             ++i) { // for all the clauses (all of them are still watched), and within the limits
            Clause& c = ca[tmpA[i]];
            semSteps++;
            if (c.mark() != 0) {
                continue;
            } // this clause has been used for a card constraint already
            // propagated already     if( solver.satisfied(c) ) continue;   // do not use clauses, which are already satisfied by units in the solver

            if (!data.unlimited() && (c.size() < config.opt_minCardClauseSize || c.size() > config.opt_maxCardClauseSize)) {
                continue;
            } // reject certain sizes of cardinality constraints!

            // otherwise, setup the current constraint ...
            // for a clause C = (a,b,c,d,e), the card constraint is -a,-b,-c,-d,-e <= 4 generated, since at most 4 out of these complements can be
            // satisfied
            cc.clear();  // new constraint
            degree = -1; // from the negated literals of the clause, at most |C| - 1 literals can be true at the same time, because otherwise the
                         // clause falsifies them again
            for (int j = 0; j < c.size(); ++j) {
                if (solver.value(c[j]) ==
                    l_Undef) {           // handle only literals, that are not falsified yet already (or where the parameter says it does not care)
            ...
            if (degree > USEABLE_BITS) { // data structures do not support constraints with more than USEABLE_BITS bits (127 or 63)
                ...
                if (!didPrint) {
                    cerr << "c cannot handle constraints with more than USEABLE_BITS literals!" << endl;
                    didPrint = false;
                } // print this error message once!
                continue;
            }
            ...
            DOUT(if (config.opt_semDebug) cerr << endl
                                               << endl
                                               << "c start with card constraint " << cc << " <= " << degree << "  --- by clause [" << tmpA[i] << "] "
                                               << c << endl;);
            ...
            do { // try to extend the card constraint with new literals by unit propagation
                ...
                if (!data.unlimited() && semSteps >= config.opt_semSearchLimit) {
                    break;
                } // stop with the current state of the current cardinality constraint
                ...
                for (int j = 0; j < degree; ++j) {
                    bitField = bitField << 1;
                    bitField = (bitField | 1);
                } // set degree bits in the number, make sure the least siginificant bit is always set!
                ...
                while (true) { // find another k-bit number, until all possible combinations inside the range have been tested
                    if (firstIteration ||
                        ((bitField & 1) !=
                         0)) { // consider this combination if its either the first iteration, or if the least siginificant bit is set
                        //      if( config.opt_semDebug ) cerr << "c probe " << probes << "  with bitfield " << bitField << " and with trail: " <<
                        //      solver.trail << endl;
                        ...
                        for (int j = 0; j < cc.size(); ++j) {             // add all literals where a bit in "bitField" is set
                            ...
                            if ((bitField & (1ull << testPos)) != 0ull) { // if the right bit is set
                                DOUT(if (config.opt_semDebug) cerr << "c assume lit " << cc[j] << " , undefined: " << (solver.value(cc[j]) == l_Undef)
                                                                   << endl;);
                                semUnits++;
                        ...
                        DOUT(if (config.opt_semDebug) cerr << "c call propagate [" << bitField << "] " << probes << " (fail:" << failedProbes
                                                           << ") first: " << firstProbe << " : with decL " << solver.decisionLevel()
                                                           << " and trail size " << solver.trail.size() << endl;);
                        ...
                        if (confl == CRef_Undef) { // no conflict -> build intersection
                            ...
                            if (firstProbe) {  // first probe -> initialize intersection
                                for (int j = solver.trail_lim[0] + degree; j < solver.trail.size(); ++j) {
                                    intersection.setCurrentStep(toInt(solver.trail[j]));
                                } // build intersection with vector and markArray wrt. the implied literals (not the "degree" enqueued ones)
                                for (int j = 0; j < cc.size(); ++j) {
                                    DOUT(if (config.opt_semDebug) cerr << "c reset lit " << ~cc[j] << " (set before "
                                                                       << intersection.isCurrentStep(toInt(~cc[j])) << ")" << endl;);
                                    intersection.reset(toInt(~cc[j])); // do not add literals of the constraint to the intersection
                                }
                                firstProbe = false;
                            }
                            // build intersection
                            for (int j = solver.trail_lim[0]; j < solver.trail.size();
                                 ++j) { // put only data.lits from the trail into data.lits, if they are already in the intersection (markArray)
                                if (intersection.isCurrentStep(toInt(solver.trail[j]))) {
                                    data.lits.push_back(solver.trail[j]);
                                }
                            }
                            intersection.nextStep(); // update markArray to new intersection
                            for (int j = 0; j < data.lits.size(); ++j) {
                                intersection.setCurrentStep(toInt(data.lits[j]));
                            } // set flag for all literals in the "new" intersection
                            DOUT(if (false && config.opt_semDebug) cerr << "c kept " << data.lits.size()
                                                                        << " literals in the intersection: " << data.lits << endl;);
                            ...
                            if (degree == 1) {
                                ...
                                if (!didit) {
                                    cerr << "c found a failed literal! use it!" << endl;
                                    didit = true;
                                }
                            }
                        ...
                        if (!firstProbe && data.lits.size() == 0) {
                            DOUT(if (config.opt_semDebug) cerr
                                     << "c stop, as we have an empty list of literals after and we are not at the first successful probe any more"
                                     << endl;);
                            break;
                        }    // intersection has been initialized, but became empty -> no commonly implied literal -> finished with the current
                             // constraint
                    } else { // end probing current combination
                        DOUT(if (config.opt_semDebug) cerr
                                 << "c skip current bit field, as it is not the first iteration, and the least significant bit is not set" << endl;);
                    }
                    ...
                    tmp = (tmp << shift);
                    DOUT(if (config.opt_semDebug) cerr << "c continue with bitfield " << bitField << "( cmp. to. "
                                                       << ((LONG_INT)1 << (LONG_INT)cc.size()) << " == " << tmp << ") and cc.size: " << cc.size()
                                                       << endl;);
                    if (bitField >= tmp) {
                        break;
                    }
                ...
                semTotalProbes += probes;
                semTotalFailedProbes += failedProbes; // stats
                if (inStepLimit && firstProbe && probes > 0 &&
                    (probes == failedProbes)) { // works only when nothing has been propagated yet! // TODO: what happens if this occurs after a
                    ...
                    //        cerr << "c failed " << failedProbes << " out of " << probes << " during first iteration, with clause " <<  ca[ tmpA[i] ]
                    //        << endl;
                    // assert ( cc.size() == ca[ tmpA[i] ].size() && "works only if we did not add a literal before!" );
                    DOUT(if (config.opt_semDebug) cerr << "c none of the configurations succeeds -> decrease degree by one to " << degree - 1
                                                       << endl;);
                ...
                // if the intersection of all of those is not empty, choose one literal (heuristically), and add it to the constraint -> next
                // iteration!
                ...
                if (!data.unlimited() && semSteps >= config.opt_semSearchLimit) {
                    data.lits.clear();
                    break;
                } // stop here due to limits
            } while (
                extendLit != lit_Undef &&
                (data.unlimited() ||
                 data.lits.size() < config.opt_maxCardSize)); // repeat as long as something has been found and the card.constraint is not too long
            ...
            if (cc.size() > ca[tmpA[i]].size() ||
                degree < origDegree) {              // if this constraint is not simply a clause, but something has been added or changed
                sort(cc);                           // necessary in Coprocessor
                cards.push_back(CardC(cc, degree)); // add the constraint to the data base
                if (config.opt_fm_prooftrace) {
                    cerr << "c FM proof (FindSem) add " << cards.back().getID() << " [" << cards.back().getParentL() << " , "
                         << cards.back().getParentR() << "] : " << cards.back().ll << " <= " << cards.back().k << " + " << cards.back().lr << endl;
                }
                for (int j = 0; j < cards[cards.size() - 1].ll.size(); ++j) {
                    leftHands[toInt(cards[cards.size() - 1].ll[j])].push_back(cards.size() - 1);
                } // register card constraint in data structures
                  //             if( degree < origDegree ) cerr << "c found card constraint " << cc << "  <= " << degree << "     to " << cards[
                  //             cards.size() - 1 ].ll << " <= " << cards[ cards.size() - 1 ].k << " + " << cards[ cards.size() - 1 ].lr << endl;
                DOUT(if (config.opt_semDebug) cerr << "c found card constraint " << cc << "  <= " << degree << endl;);
                intersection.nextStep();
                semExtendedCards++;
                semExtendLits += (cc.size() - ca[tmpA[i]].size()); // stats
                ...
                for (int j = 0; j < data.lits.size(); ++j) { // for all the literals in at least the original constraint, but also in its extension
                    for (int k = 0; k < occ[toInt(~data.lits[j])].size();
                         ++k) { // and all the clauses that have this literal as smallest literal (watch 1 scheme, each clause only once)
                        Clause& candidate = ca[occ[toInt(~data.lits[j])][k]]; // current delete candidate
                        if (candidate.mark() != 0) {
                            continue;
                        } // candidate contains more literals than have been marked
                        if (config.opt_noReduct && candidate.size() > data.lits.size()) {
                            continue;
                        } // there cannot be falsified literals inside the clauses
                        bool failed = false;
                        for (int m = 0; m < candidate.size(); ++m) { // check whether the current clause has all the necessary literals
                            ...
                            if (!intersection.isCurrentStep(toInt(candidate[m]))) {
                                failed = true;
                                break;
                            } // another literal was found, keep this clause!
                        }
                        if (!failed) {
                            DOUT(if (config.opt_semDebug) cerr << "c disable with literal " << data.lits[j] << " clause["
                                                               << occ[toInt(~data.lits[j])][k] << "] " << candidate << endl;);
                            candidate.mark(1); // set this clause to "can be deleted"
    ...
    void FourierMotzkin::removeSubsumedAMOs(vector<FourierMotzkin::CardC>& cards, vector<std::vector<int>>& leftHands) {
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            if (c.taut()) {
                c.invalidate();
            } // do not use this constraint any more!
            if (!c.amo()) {
                continue;
            } // do only for AMOs
            if (c.ll.size() == 0) {
            ...
            for (int j = 1; j < c.ll.size(); ++j)
                if (leftHands[toInt(c.ll[j])].size() < leftHands[toInt(min)].size()) {
                    min = c.ll[j];
                }
            ...
            for (int m = 0; m < leftHands[toInt(min)].size(); ++m) {
                ...
                if (!ref.amo() || ref.taut() || leftHands[toInt(min)][m] == i) {
                    continue;
                } // look only for amos, and do not compare itself with the current AMO
                ...
                while (j < c.ll.size() && k < rl.size()) {
                    if (c.ll[j] < rl[k]) {
                        ...
                        k++;
                    } // simply jump over lits that are in ref, but not in lits
                    else {
                        ++j;
                        ++k;
                    }
                ...
                if (j == c.ll.size()) {
                    c.invalidate();
                } // invalidate each AMO that has been found to be redundant
            }
        }
    }
    ...
    void FourierMotzkin::deduceALOfromAmoAloMatrix(vector<FourierMotzkin::CardC>& cards, vector<std::vector<int>>& leftHands) {
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            if (!cards[i].amo()) {
                continue;
            }                             // not an AMO, test next AMO, use only AMOs with size > 2 (heuristic!)
            ...
            for (int j = 0; j < listAsize; ++j) { // for B in ALO_l
                ...
                if (!isMin) {
                    continue;
                } // do not use the current clause, because it contains a smaller literal!
                ...
                for (int k = 0; k < A.ll.size(); ++k) { // for kLit in A, each lit in A should hit an ALO that hits als AMOs
                    ...
                    for (int m = 0; m < data.list(kLit).size(); ++m) {
                        ...
                        for (int n = 0; n < ALOk.size(); ++n) {
                            if (data.ma.isCurrentStep(toInt(ALOk[n]))) {
                                hitAlready = true;
                                break;
                            }
                        }
                ...
                DOUT(if (config.fm_debug_out > 2) {
                    ...
                    for (int k = 0; k < data.clss.size(); ++k) {
                        cerr << "c [" << k << "]: " << ca[data.clss[k]] << endl;
                    }
                });
                if (searchSteps > config.opt_fmSearchLimit) {
                    break;
                }
                if (data.clss.size() < A.ll.size()) {
                    continue;
                }
                ...
                for (int k = 0; k < B.size(); ++k) {
                    ...
                    for (int m = 0; m < leftHands[toInt(bLit)].size(); ++m) {
                        ...
                        if (C.ll.size() != A.ll.size()) {
                            continue;
                        } // not a good size!
                        bool hitAlready = false;
                        for (int n = 0; n < C.ll.size(); ++n) {
                            if (data.ma.isCurrentStep(toInt(C.ll[n]))) {
                                hitAlready = true;
                                break;
                            }
                        }
                ...
                if (nextB || amoCands.size() != B.size()) {
                    continue;
                } // has to find enough AMOs
                if (searchSteps > config.opt_fmSearchLimit) {
                    break;
                }
                ...
                for (int k = 0; k < data.clss.size(); ++k) {
                    ...
                    for (int m = 0; m < C.size(); ++m) {
                        ...
                        for (int n = 0; n < amoCandsLocal.size(); ++n) {
                            ...
                            for (int o = 0; o < D.ll.size(); ++o) {
                                if (D.ll[o] == mLit) {
                                    DOUT(if (config.fm_debug_out > 2) cerr << "c lit " << mLit << " from ALO hits lit " << D.ll[o] << " from AMO"
                                                                           << endl;);
                                    containsMLit = true;
                    ...
                    if (amoCandsLocal.size() != 0 || nextB) {
                        break;
                    } // if S != 0, goto next B
                }
                if (nextB || amoCandsLocal.size() != 0) {
                    continue;
                } // did not hit all lits in last iteration

                // here, the new AMOs can be added!
                DOUT(if (config.fm_debug_out > 2) {
                    cerr << "c can add the following ALOs, due to" << endl;
                    for (int k = 0; k < data.clss.size(); ++k) {
                        cerr << "c ALO " << ca[data.clss[k]] << endl;
                    }
                    for (int k = 0; k < amoCands.size(); ++k) {
                        cerr << "c AMO " << cards[amoCands[k]].ll << endl;
                    }
                });
                for (int k = 0; k < amoCands.size(); ++k) {
    ...
    void FourierMotzkin::findTwoProduct(vector<FourierMotzkin::CardC>& cards, BIG& big, vector<std::vector<int>>& leftHands) {
        if (config.opt_fmSearchLimit <= searchSteps) {
            return;
        } // if limit is reached, invalidate current AMO candidate
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            if (!cards[i].amo() || cards[i].invalid()) {
                continue;
            }                             // not an AMO, test next AMO, use only AMOs with size > 2 (heuristic!)
            const Lit a = cards[i].ll[0]; // since sorted, the first literal in ll is the smallest literal
            ...
            if (big.getSize(~a) == 0) {
                continue;
            } // not this AMO, because the smallest literal does not imply other literals
            Lit l = ~aList[0];
            ...
            for (int j = 0; j < big.getSize(l); ++j) { // for b in BIG(l), b!=a
                ...
                for (int k = 0; k < leftHands[toInt(b)].size(); ++k) { // for B in AMOs_b, A != B
                    if (leftHands[toInt(b)][k] <= i) {
                        DOUT(if (config.fm_debug_out > 3) cerr << "c reject AMO " << cards[leftHands[toInt(b)][k]].ll << " because of position "
                                                               << endl;);
                    ...
                    for (int m = 0; m < B.ll.size(); ++m) { // for literal k in B
                        ...
                        const Lit bLit =
                            B.ll[m]; // each literal in B should imply a set of literals, where each of these literals hits a different lit in A
                        searchSteps++;
                        ...
                        for (int n = 0; n < big.getSize(~bLit);
                             ++n) { // check whether this literal implies a set of literal, which hit all different literals from A
                            searchSteps++;
                            const Lit hitLit = ~big.getArray(~bLit)[n];
                            if (newAmoLits.isCurrentStep(toInt(hitLit))) {
                                continue;
                            } // do not collect such a literal twice!

                            DOUT(if (config.fm_debug_out > 3) cerr << "c test hitLit " << hitLit << " which implies " << big.getSize(hitLit)
                                                                   << " lits" << endl;);
                    ...
                    if (data.lits.size() > 1) { // if there is a global AMO
                        ...
                        for (int m = 0; m + 1 < data.lits.size(); ++m) {
                            if (data.lits[m] == ~data.lits[m + 1]) { // all complements of the remaining literals inside this AMO are units
                                for (int o = 0; o < m; ++o) {
                                    ...
                                    if (l_False == data.enqueue(~data.lits[o])) {
                                        return;
                                    }
                                }
                        ...
                        if (doUseConstraint) {               // the constraint that is added is not redundant
                            ...
                            if (!amoExistsAlready(data.lits, leftHands, cards)) {
                                ...
                                DOUT(if (config.fm_debug_out > 2) cerr << "c   based on " << cards[i].ll << " and AMO " << B.ll
                                                                       << endl;); // here, the reference is still working
                                ...
                                if (config.opt_fm_prooftrace) {
                                    cerr << "c FM proof (2Prod) add " << cards.back().getID() << " [" << cards.back().getParentL() << " , "
                                         << cards.back().getParentR() << "] : " << cards.back().ll << " <= " << cards.back().k << " + "
                                         << cards.back().lr << endl;
                                }
                                twoPrAmos++;
    ...
    bool FourierMotzkin::amoExistsAlready(const vector<Lit>& lits, vector<std::vector<int>>& leftHands, vector<FourierMotzkin::CardC>& cards) {
        ...
        for (int i = 1; i < lits.size(); ++i)
            if (leftHands[toInt(lits[i])] < leftHands[toInt(min)]) {
                min = lits[i];
            }
        ...
        for (int i = 0; i < leftHands[toInt(min)].size(); ++i) {
            ...
            if (j == lits.size()) {
                return true;
            } // found a AMO that is at least as strong as the current one (might contain more lits!)
        }
    ...
    bool FourierMotzkin::propagateCards(vec<Lit>& unitQueue, vector<std::vector<int>>& leftHands, vector<std::vector<int>>& rightHands,
                                        vector<FourierMotzkin::CardC>& cards, MarkArray& inAmo) {
        // propagate method
        while (unitQueue.size() > 0) {
            Lit p = unitQueue.last();
            unitQueue.pop();
            for (int p1 = 0; p1 < 2; ++p1) {
                ...
                for (int p2 = 0; p2 < 2; ++p2) {
                    ...
                    for (int i = 0; i < indexes.size(); ++i) {
                        ...
                        if (c.invalid()) {
                            continue;
                        } // do not consider this constraint any more!
                        ...
                        for (int j = 0; j < c.ll.size(); ++j) {
                            if (data.value(c.ll[j]) == l_True) {
                                DOUT(if (config.fm_debug_out > 2) cerr << "c since " << c.ll[j] << " == top, reduce k from " << c.k << " to "
                                                                       << c.k - 1 << endl;);
                                c.k--;
            ...
            leftHands[toInt(p)].clear();
            leftHands[toInt(~p)].clear();
            rightHands[toInt(p)].clear();
            rightHands[toInt(~p)].clear();
        }
        return true;
    }
    ...
    void FourierMotzkin::printStatistics(ostream& stream) {
        stream << "c [STAT] FM " << processTime << " s, " << amoTime << " amo-s, " << fmTime << " fm-s, " << amtTime << " amt-s, " << foundAmos
               << " amos, " << foundAmts << " amts, " << sameUnits << " sameUnits, " << deducedUnits << " deducedUnits, " << propUnits
               << " propUnits, " << usedClauses << " cls, " << steps << " steps, " << searchSteps << " searchSteps, " << addDuplicates << " addDups, "
               << endl
               << "c [STAT] FM(2) " << irregular << " irregulars, " << pureAmoLits << " pureAmoLits, " << cardDiff << " diff, " << discardedCards
               << " discards, " << newCards << " addedCs, " << removedCards << " removedCs, " << newAmos << " newAmos, " << discardedNewAmos
               << " discardedNewAmos, " << endl
               << "c [STAT] FM(3) " << addedBinaryClauses << " newAmoBinCls, " << addedClauses << " newCls, " << newAlos << " newAlos, " << newAlks
               << " newAlks, " << detectedDuplicates << " duplicates, " << garbageCollects << " garbageCollecst, " << endl
               << "c [STAT] FM(4) " << twoPrTime << " 2PrdTime, " << twoPrAmos << " 2PrAmos, " << twoPrAmoLits << " 2PrLits, " << deduceAloTime
               << " dedAloTime, " << dedAlos << " dedAlos, " << endl
               << "c [STAT] FM(5)"
               << " time: " << semTime << " s,"
               << "steps: " << semSteps << " cards: " << semExtendedCards << " failed: " << semFailedExtendTries << " extLits: " << semExtendLits
               << " redDegree: " << semReducedDegrees << " probes: " << semTotalProbes << " conflictProbes: " << semTotalFailedProbes
               << " savedPreCls: " << semNrPreDisabledClauses << " savedPostCls: " << semNrDisabledClauses << " units: " << semUnits << endl;
    }
