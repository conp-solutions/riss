...
class TwinAllocator {
    ...
    int findBin(uint32_t& size) {
        ...
        for (; bin < SLAB_BINS; bin++) {
            if (size <= binMaxSize(bin)) {
                break;
            }
        }
        //    std::cerr << "c used bin: " << bin << " for size " << size << " results in block: " << bin - SLAB_BINS << " with blocksize " << ( 1 <<
        //    bin) << std::endl;
        size = binMaxSize(bin);
    ...
    void initFirstPages() {
        ...
        for (uint32_t i = 0; i < SLAB_BINS; ++i) {
            ...
            // std::cerr << "set lastCell[" << i << "] to "  << std::hex << _lastCell[i] << std::dec <<  ", page(" << pageElements << " eles) base: "
            // << std::hex << _lastPage[i] << std::dec << std::endl;
            _head[i] = _lastPage[i];
        }
...
inline void* TwinAllocator::initPage(int bin, uint32_t size) {
    ...
    *(_lastPage[bin]) = ptr;
    ...
    // std::cerr << "set lastCell[" << bin << "] to "  << std::hex << _lastCell[bin] << std::dec <<  ", page(" << pageElements << " eles) base: " <<
    // std::hex << _lastPage[bin] << std::dec << std::endl;
...
inline void* TwinAllocator::get(uint32_t size) {
    ...
    if (_nextCell[bin] != _lastCell[bin]) {
        _nextCell[bin] = ((*(_nextCell[bin])) == 0) ? /* TODO: fix this expression according to bin!*/ /*_nextCell[bin] + sizeof( void* )*/

                             reinterpret_cast<void**>(reinterpret_cast<unsigned long>(_nextCell[bin]) + size)

                                                    : (void**)(*(_nextCell[bin]));
    } else {
        if (nullptr == initPage(bin, size)) {
            return nullptr;
        } // tell outside that there was no more memory
    }
...
inline void* TwinAllocator::resize(void* adress, uint32_t new_size, uint32_t size) {
    ...
    if (adress != 0 && new_size <= SLAB_MALLOC_MAXSIZE && findBin(size) == findBin(new_size)) {
        return adress;
    }
    ...
    if (mem == nullptr) {
        return nullptr;
    } // if there is no more space, tell the outside
