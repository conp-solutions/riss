...
namespace Riss {
    ...
    private:
        ...
        void writeModelToStream(std::ostream& outputStream,
                                const Riss::vec<Riss::lbool>& truthValues); // write the current "disallow-clause" as model into the stream
    ...
    public:
        ...
        void setShared() {
            assert(models.size() == 0 && "cannot set shared after first models have been found already");
            enumerateParallel = true;
        }

        /** do we search for models in a parallel setup ? */
        bool isShared() const {
            return enumerateParallel;
        }
        ...
        int64_t foundModels() const {
            return models.size();
        }

        /** set number of models to be found ( 0 ^= INT64_MAX )*/
        void setMaxModels(const int64_t m) {
            lock();
            maximalModels = (m == 0 ? INT64_MAX : m);
            unlock();
        }
        ...
        */
        bool addModel(int authorID, Riss::vec<Riss::lbool>& newmodel, uint64_t modelhash, Riss::vec<Riss::Lit>* blockingClause = nullptr,
                      Riss::vec<Riss::Lit>* fullModel = nullptr);
         ...
         */
        bool reveiveModel(int authorID, uint64_t modelIndex, Riss::vec<Riss::lbool>& receivedTruthValues,
                          Riss::vec<Riss::Lit>& receivedBlockingClause);
    ...
    inline void EnumerateMaster::notifyReachedAllModels() {
        lock();
        maximalModels = models.size();
        unlock();
    }

    inline void EnumerateMaster::storeFullModel(const vec<Lit>& fullModel) {
    ...
    inline void EnumerateMaster::storeBlockingClause(int authorID, vec<Lit>& clause) {
        SharedClause sc;
    ...
    inline bool EnumerateMaster::addModel(int authorID, vec<lbool>& newmodel, uint64_t modelhash, vec<Lit>* blockingClause, vec<Lit>* fullModel) {
        ...
        if (!enumerateParallel) {
            ...
            for (int i = 0; i < modelHashes.size(); ++i) {
                if (modelHashes[i] != modelhash) {
                    successfulBloom++;
                    continue;
                } // bloom filter, quickly check whether two models are not the same
                bool different = false;
                if (useProjection) {
                    for (int j = 0; j < projectionVariables.size(); ++j) { // compare only projection variables
                        if (newmodel[j] != (*models[i])[projectionVariables[j]]) {
                            different = true;
                            break;
                        }
                    }
            ...
            if (!foundModel) {
                ...
                assert(models.size() == modelHashes.size() && models.size() >= fullModels.size() &&
                       "the numbers have to be the same, there cannot be more full models");
    ...
    inline bool EnumerateMaster::foundEnoughModels() {
        bool result = false;
        lock();
    ...
    inline int EnumerateMaster::assignClientID() {
        lock();
    ...
    inline int EnumerateMaster::projectionSize() const {
        return projectionVariables.size();
    }

    inline Var EnumerateMaster::projectionVariable(int index) const {
        assert(index >= 0 && projectionVariables.size() > index && "stay in bounds");
        return projectionVariables[index];
    }

    inline int EnumerateMaster::minimizeBlocked() const {
        return mType;
    }

    inline bool EnumerateMaster::hasMoreModels(uint64_t localModels) const {
    ...
    inline bool EnumerateMaster::reveiveModel(int authorID, uint64_t modelIndex, vec<lbool>& receivedTruthValues, vec<Lit>& receivedBlockingClause) {
        assert(models.size() > modelIndex && "the requested model has to be present");
        if (!enumerateParallel) {
            return false;
        } // tell that there is no blocking clause to be received (as we currently run in sequential mode)
        lock();
    ...
    inline uint64_t EnumerateMaster::checkNewModelsEvery() const {
        return checkForModelsEveryX;
    }

    inline int EnumerateMaster::minimizeReceived() const {
