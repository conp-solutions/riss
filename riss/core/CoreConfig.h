...
namespace Riss {
    ...
    public:
        ...
        IntOption
            opt_update_lbd;     // update LBD during 0=propagation,1=learning,2=never (if during propagation, then during learning is not necessary!)
        ...
        BoolOption
            opt_useOriginal; // operate on the original formula instead of working on the simplified formula (do not share information in this case!)
        ...
        IntOption opt_probing_step_width; // After how many steps the solver should perform failed literals and detection of necessary assignments.
                                          // (default 32) If set to '0', no inprocessing is performed.
        ...
        DoubleOption
            opt_ics_LBDpercent; // only look at a clause if its LBD is less than this percent of the average of the clauses that are looked at
        DoubleOption
            opt_ics_SIZEpercent; // only look at a clause if its size is less than this percent of the average size of the clauses that are looked at
#ifndef NDEBUG
