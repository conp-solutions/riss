...
namespace Riss {
    ...
    inline void Solver::setCommunication(Communicator* comm) {
        ...
        communicationClient.checkLiterals =
            communication->checkLiterals; // allow sending with literals/variables (to check whether sound wrt inprocessing)
        communicationClient.useDynamicLimits =
            communicationClient.useDynamicLimits || communication->useDynamicLimits; // one of the two overwrites the other
    ...
    inline void Solver::resetLastSolve() {
        clearInterrupt();
        cancelUntil(0);
        budgetOff();
    }
     ...
     */
    inline bool Solver::addLearnedClause(vec<Lit>& ps, bool bump) {
        ...
        if (ps.size() == 0) {
            ...
            if (bump) {
                ca[cr].activity() += cla_inc;
            } // same as in claBumpActivity
            if (communicationClient.keepLonger) {
                ca[cr].resetCanBeDel();
            } // keep this clause for at least one removal round
            ca[cr].setLBD(communicationClient.lbdFactor > 0
                              ? (double)ca[cr].size() * communicationClient.lbdFactor
                              : -communicationClient.lbdFactor * ((double)sumLearnedClauseLBD / (double)sumLearnedClauseSize) *
                                    (double)ca[cr].size()); // set LBD for received clause based on heuristics (TODO: also consider level cache)
            learnts.push(cr);
            attachClause(cr);
        }
        return true;
    ...
    inline unsigned Solver::currentDependencyLevel() const {
#ifdef PCASSO
...
#ifdef PCASSO
        int
        Solver::updateSleep(T* toSend, int toSendSize, int dependencyLevel, bool multiUnits, bool equivalences)
#else
        int
        Solver::updateSleep(T* toSend, int toSendSize, bool multiUnits, bool equivalences)
#endif
    // int Solver::updateSleep(vec< Lit >* toSend, bool multiUnits)
    {
        ...
        if (toSend == 0 && communicationClient.currentTries++ < communicationClient.receiveEvery) {
            return 0;
        }
        ...
        if (!communication->isWorking()) {
            if (communication->isAborted()) {
                interrupt();
                if (verbosity > 0)
                    std::cerr << "c [THREAD] " << communication->getID() << " aborted current search due to flag by master" << std::endl;
                return -1;
            }
        ...
        if (!communication->getDoSend() && (!communication->getDoReceive() || !communicationClient.doReceive)) {
            return 0;
        }
        ...
        if (communication->getDoSend() && toSend != 0) { // send
            ...
            for (int i = 0; i < toSendSize; ++i) { // repeat until allowed, stay in clause
                ...
                if (communication->checkLiterals && (varFlags[v].modifiedPositiveModels || varFlags[v].modifiedNegativeModels)) {
                    rejectSend = true;
                } // we modified models with this literal
                if (v >= communication->getFormulaVariables()) {
                    rejectSend = true;
                } // checks whether a variable is too high (not in the original formula)
                if (!rejectSend) {
                    ...
                    if (multiUnits || equivalences) {
                        ...
                        break;
                    } // do not share a clause with that literal
                }
            }
            ...
            if (rejectSend && !multiUnits && !equivalences) { // do nothing here, as there are variables in the clause that should not be sent
                ...
                if (keep <= 1 && equivalences) {
                    return 0;
                }                                   // do not share equivalence of one literal
            ...
            if (toSendSize == 0) {
                cerr << "c send clause of size 0 (" << toSendSize << "), multiUnit: " << multiUnits << " eqs: " << equivalences << endl;
            }
#endif
            ...
            if (decisionLevel() != 0) {
                return 0;
            } // receive clauses only at level 0!
            ...
            communication->receiveClauses(ca, communicationClient.receiveClauses, communicationClient.receivedUnits,
                                          communicationClient.unitDependencies, communicationClient.receivedEquivalences,
                                          communicationClient.eeDependencies, vi); // receive multiple things!
#else
            communication->receiveClauses(ca, communicationClient.receiveClauses, communicationClient.receivedUnits,
                                          communicationClient.receivedEquivalences, vi); // receive multiple things!
#endif
            // if( communicationClient.receiveClauses.size()  > 5 ) std::cerr << "c [THREAD] " << communication->getID() << " received " <<
            // communicationClient.receiveClauses.size() << " clauses." << std::endl;
...
#ifdef PCASSO
                    eqInfo.addEquivalenceClass(communicationClient.receivedEquivalences, communicationClient.eeDependencies,
                                               false); // use fast-import for pcasso, do not share again
#else
                    eqInfo.addEquivalenceClass(communicationClient.receivedEquivalences,
                                               false); // tell object about these equivalences, do not share again, ignore setting a dependency
#endif
            ...
            for (unsigned i = 0; i < communicationClient.receiveClauses.size(); ++i) {
                ...
                if (c.size() < 2) {
                    ...
                    for (int j = 0; j < c.size(); ++j) { // propagate inside the clause!
                        if (var(c[j]) > nVars()) {
                            //                     std::cerr << "c shared variable " << var(c[j]) << "[" << j << "] is greater than " << nVars() <<
                            //                     std::endl;
                            assert(false && "received variables have to be smaller than maximum!");
                        }
                        //                 cerr << "c look at literal " << c[j] << " sat: " << (value(c[j]) == l_True) << " unsat: " << (value(c[j])
                        //                 == l_False) << endl;
                        if (value(c[j]) == l_True) {
                            c.mark(1);
                            break;
                        } // this clause is already satisfied -> remove it! (we are on level 0!)
                        else if (value(c[j]) == l_False) {
                            c[j] = c[c.size() - 1];
                            --j;
                            c.shrink(1);
                        } // this literal is mapped to false -> remove it! repeat check for this position
                        else {
                    ...
                    if (c.mark() == 0) {
                        ...
                        if (c.size() > 1 && communicationClient.refineReceived) {
                            ...
                            DOUT(if (localDebug) {
                                std::cerr << "c received clause(" << i << "): " << c << std::endl;
                                {
                                    ...
                                    for (int k = 0; k < c.size(); ++k) {
                                        s << " " << (value(c[k]) == l_True) << "/" << (value(c[k]) == l_False);
                                    }
                                    s << std::endl;
                                    std::cerr << s.str();
                                }
                            ...
                            shrinkedClause = reverseLearntClause(c, lbd, dependency);
#endif
                            // std::cerr << "c shrinked received clause(" << i << "): " << c << " first sat: " << (value(c[0]) == l_True) <<
                            // std::endl;
                            communication->vivifiedLiterals += beforeSize - c.size();
                            if (shrinkedClause &&
                                communicationClient.resendRefined) { // re-share clause (should be performed only by one thread per group
// std::cerr << "c recend received clause(" << i << "): " << c << std::endl;
                        ...
                        if (c.size() == 0) {
                            ...
                            addToProof(ca[communicationClient.receiveClauses[i]]); // the shared clause stays in the solver, hence add this clause to
                                                                                   // the proof!
                            ...
                            if (communicationClient.keepLonger) {
                                ca[communicationClient.receiveClauses[i]].resetCanBeDel();
                            } // keep this clause for at least one removal round
                            ca[communicationClient.receiveClauses[i]].setLBD(
                                communicationClient.lbdFactor > 0
                                    ? (double)ca[communicationClient.receiveClauses[i]].size() * communicationClient.lbdFactor
                                    : -communicationClient.lbdFactor * ((double)sumLearnedClauseLBD / (double)sumLearnedClauseSize) *
                                          (double)ca[communicationClient.receiveClauses[i]]
                                              .size()); // set LBD for received clause based on heuristics (TODO: also consider level cache)
    ...
    inline void Solver::updateDynamicLimits(bool failed, bool sizeOnly) {
        if (!communicationClient.useDynamicLimits) {
            return;
        } // do not do anything with the ratios
        if (!failed) {
            communicationClient.succesfullySend++;
        }
        ...
        communicationClient.currentSendSizeLimit =
            (failed ? communicationClient.currentSendSizeLimit * (1.0 + communicationClient.sizeChange)
                    : communicationClient.currentSendSizeLimit - communicationClient.currentSendSizeLimit * communicationClient.sizeChange);
        // check bound
        communicationClient.currentSendSizeLimit = communicationClient.currentSendSizeLimit < communicationClient.sendSize
                                                       ? communicationClient.sendSize
                                                       : communicationClient.currentSendSizeLimit;
        communicationClient.currentSendSizeLimit = communicationClient.currentSendSizeLimit > communicationClient.sendMaxSize
                                                       ? communicationClient.sendMaxSize
                                                       : communicationClient.currentSendSizeLimit;

        if (fulfillRatio) {
            initLimits();
        } // we have hit the ratio, tigthen limits again!

        //   if( sizeOnly ) {
        //     communicationClient.currentSendLbdLimit = communicationClient.currentSendLbdLimit < communicationClient.sendLbd    ?
        //     communicationClient.sendLbd    : communicationClient.currentSendLbdLimit; communicationClient.currentSendLbdLimit =
        //     communicationClient.currentSendLbdLimit > communicationClient.sendMaxLbd ? communicationClient.sendMaxLbd :
        //     communicationClient.currentSendLbdLimit; return;
        //   }

        // fail -> increase geometrically, success, decrease geometrically!
        communicationClient.currentSendLbdLimit =
            (failed ? communicationClient.currentSendLbdLimit * (1.0 + communicationClient.lbdChange)
                    : communicationClient.currentSendLbdLimit - communicationClient.currentSendLbdLimit * communicationClient.lbdChange);
        // check bound
        communicationClient.currentSendLbdLimit = communicationClient.currentSendLbdLimit < communicationClient.sendLbd
                                                      ? communicationClient.sendLbd
                                                      : communicationClient.currentSendLbdLimit;
        communicationClient.currentSendLbdLimit = communicationClient.currentSendLbdLimit > communicationClient.sendMaxLbd
                                                      ? communicationClient.sendMaxLbd
                                                      : communicationClient.currentSendLbdLimit;

        return;
    }

    inline void Solver::initVariableProtection() {
        if (communication != 0) {
            communication->initProtect(assumptions, nVars());
        }
    }

    inline void Solver::initLimits() {
