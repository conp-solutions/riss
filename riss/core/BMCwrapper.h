...
namespace Riss {
    ...
    public:
        ...
        void add(int lit) {
            if (lit != 0) {
                currentClause.push(lit > 0 ? mkLit(lit - 1, false) : mkLit(-lit - 1, true));
        ...
        void assume(int lit) {
            if (lit != 0) {
                currentAssumptions.push(lit > 0 ? mkLit(lit - 1, false) : mkLit(-lit - 1, true));
            }
        }
        ...
        int deref(int lit) {
            ...
            const int iret =
                ret == l_False ? -1 : (ret == l_True ? 1 : 0); // if the value is undefined, map it to true! (important for the output state!)
            return iret;
        }

        void setMaxVar(int maxVar) {
            while (solver->nVars() < maxVar) {
                solver->newVar();
            }
        }
