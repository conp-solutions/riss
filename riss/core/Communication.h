...
namespace Riss {
    ...
    private:
        ...
        struct poolItem {
            ...
            poolItem()
                ...
                , multiunits(0)
                , equivalence(
                      0) /** the initial author is invalid, so that it can be seen whether a clause in the ringbuffer has been added by solver */
#ifdef PCASSO
                , dependencyLevel(0)
#endif
            {
    ...
    public:
        ...
        unsigned getCurrentPosition() const {
            return ((addHereNext == 0) ? poolSize - 1 : addHereNext - 1);
        }
        ...
        void addClause(int authorID, const T& clause, const int& clauseSize, const int& dependencyLevel, bool multiUnits = false,
...
#endif
        {
            ...
            if (pool[addHereNext].author != ~0U && proofMaster != 0) {
                proofMaster->delFromProof(poolClause, lit_Undef, -1, false);
            } // can work only on the global proof
        ...
        template <typename T>
#ifdef PCASSO
        void incorporateReceiveItem(unsigned position, Riss::ClauseAllocator& allocator, std::vector<Riss::CRef>& clauses, vec<Lit>& receivedUnits,
                                    vec<int>& receivedUnitsDependencies, vec<Lit>& receivedEquivalences, vec<int>& receivedEquivalencesDependencies,
                                    T& receiveData){
#else
        void incorporateReceiveItem(unsigned position, Riss::ClauseAllocator& allocator, std::vector<Riss::CRef>& clauses, vec<Lit>& receivedUnits,
                                    vec<Lit>& receivedEquivalences, T& receiveData) {
#endif
        ...
        for (int j = 0; j < eeSCC.size(); ++j) {
            if (receiveData.canBeReceived(eeSCC[j])) { // we are allowed to receive that unit clause due to simplification
                ...
                usedSCCliterals++;
#ifdef PCASSO
                if (usedSCCliterals > 1) {
                    receiveData.setDependency(var(eeSCC[j]), getDependency(position));
                } // store dependency level
#endif
            }
        }
        if (usedSCCliterals == 1) {
            receivedEquivalences.pop();
        } // remove the single literal again, as its a trivial SCC
        else {
        ...
        for (int i = 0; i < lits.size(); ++i) { // check soundness of receiving
            if (!receiveData.canBeReceived(lits[i])) {
                return;
            } // if a literal in the clause is locked, do not receive it
        }
...
unsigned receiveClauses(int authorID, unsigned lastSeenIndex, Riss::ClauseAllocator& allocator, std::vector<Riss::CRef>& clauses,
                        vec<Lit>& receivedUnits, vec<int>& receivedUnitsDependencies, vec<Lit>& receivedEquivalences,
                        vec<int>& receivedEquivalencesDependencies, T& receiveData)
#else
        unsigned receiveClauses(int authorID, unsigned lastSeenIndex, Riss::ClauseAllocator& allocator, std::vector<Riss::CRef>& clauses,
                                vec<Lit>& receivedUnits, vec<Lit>& receivedEquivalences, T& receiveData)
#endif
{
...
#ifdef PCASSO
                    incorporateReceiveItem(i, allocator, clauses, receivedUnits, receivedUnitsDependencies, receivedEquivalences,
                                           receivedEquivalencesDependencies, receiveData); // receive one element, and its dependencies
#else
                            incorporateReceiveItem(i, allocator, clauses, receivedUnits, receivedEquivalences,
                                                   receiveData); // receive one element, add info to collecting data strucutures
#endif
                }
            }
...
public:
    ...
    SleepLock& getMasterLock() {
        return masterLock;
    };

    /** set the handle for the proof master in the ringbuffer */
    void setProofMaster(ProofMaster* pm) {
        clauseBuffer.setProofMaster(pm);
    }

    /** return a reference to the ringbuffer
     */
    ClauseRingBuffer& getBuffer() {
        return clauseBuffer;
    }

    /** return a reference to the ringbuffer
     */
    ClauseRingBuffer& getSpecialBuffer() {
        return specialBuffer;
    }

    /** return a reference to the ringbuffer
     */
    ClauseRingBuffer* getExtraBuffer() {
        return extraClauseBuffer;
    }

    /** return a reference to the ringbuffer
     */
    ClauseRingBuffer* getExtraSpecialBuffer() {
        return extraSpecialBuffer;
    }
...
public:
    ...
    void setParent(TreeReceiver* outerParent) {
        parent = outerParent;
    }

    void setData(CommunicationData* outerData) {
        data = outerData;
    }
     ...
     * @param receiveData object that can tell per variable whether receiving is ok, get set dependency value per variable (for Pcasso, clauses have
     * their value stored already)
     */
    template <typename T>
#ifdef PCASSO
    void receiveClauses(Riss::ClauseAllocator& ca, std::vector<Riss::CRef>& clauses, vec<Lit>& receivedUnits, vec<int>& receivedUnitsDependencies,
                        vec<Lit>& receivedEquivalences, vec<int>& receivedEquivalencesDependencies, T& receiveData)
#else
        void receiveClauses(Riss::ClauseAllocator& ca, std::vector<Riss::CRef>& clauses, vec<Lit>& receivedUnits, vec<Lit>& receivedEquivalences,
                            T& receiveData)
#endif
    {
...
#ifdef PCASSO
        if (data != nullptr) {
            lastSeenSpecialIndex = data->getSpecialBuffer().receiveClauses(data->getSpecialBuffer().allReceivAuthor(), lastSeenSpecialIndex, ca,
                                                                           clauses, receivedUnits, receivedUnitsDependencies, receivedEquivalences,
                                                                           receivedEquivalencesDependencies, receiveData);
            lastSeenIndex =
                data->getBuffer().receiveClauses(data->getBuffer().allReceivAuthor(), lastSeenIndex, ca, clauses, receivedUnits,
                                                 receivedUnitsDependencies, receivedEquivalences, receivedEquivalencesDependencies, receiveData);
        }
        if (parent != nullptr) {
            parent->receiveClauses(ca, clauses, receivedUnits, receivedUnitsDependencies, receivedEquivalences, receivedEquivalencesDependencies,
                                   receiveData);
        } // receive from parent, if activated
#else
            if (data != nullptr) {
                lastSeenSpecialIndex = data->getSpecialBuffer().receiveClauses(data->getSpecialBuffer().allReceivAuthor(), lastSeenSpecialIndex, ca,
                                                                               clauses, receivedUnits, receivedEquivalences, receiveData);
                lastSeenIndex = data->getBuffer().receiveClauses(data->getSpecialBuffer().allReceivAuthor(), lastSeenIndex, ca, clauses,
                                                                 receivedUnits, receivedEquivalences, receiveData);
            }
            if (parent != nullptr) {
                parent->receiveClauses(ca, clauses, receivedUnits, receivedEquivalences, receiveData);
            }
#endif
    }
};
...
public:
    ...
    bool generateProof() const {
        return proofMaster != 0;
    }
    ...
    bool getDoSend() const {
        return doSend;
    }
    void setDoSend(bool ds) {
        doSend = ds;
    }
    bool getDoReceive() const {
        return doReceive;
    }
    void setDoReceive(bool dr) {
        doReceive = dr;
    }

    /** tell the number of shared variables */
    void setFormulaVariables(const int formulaVariables) {
        originalVars = formulaVariables;
    }

    int getFormulaVariables() const {
        return originalVars;
    }
    ...
    Riss::lbool getReturnValue() const {
        return returnValue;
    }
    ...
    bool isInterruptForced() const {
        return state == interruptedForce;
    }
    bool isWaiting() const {
        return state == waiting;
    }
    bool isDoReceive() const {
        return state == doReceiveFromMaster;
    }
    bool isFinishedReceiving() const {
        return state == finishedReceiving;
    }
 ...
 */
    ...
    void addClause(const T& clause, const int& toSendSize, const int& dependencyLevel, const V& variableInformation, bool multiUnits = false,
                   bool equivalences = false)
#else
        template <typename T> // can be either clause or vector
    ...
    {
#ifdef PCASSO
        //        assert(!multiUnits && "remove this assertion when method makes sure that all units have the same dependency");   // either set the
        //        highest vor all, or sort and add multiple items
        if (!multiUnits && !equivalences) {
            ...
            if (data->getExtraBuffer() != nullptr) {
                data->getExtraBuffer()->addClause(data->getExtraBuffer()->specialAuthor(), clause, toSendSize,
                                                  dependencyLevel); // usual special buffer
            }
        } else {
            ...
            if (data->getExtraSpecialBuffer() != nullptr) {
                data->getExtraSpecialBuffer()->addClause(data->getExtraSpecialBuffer()->specialAuthor(), clause, toSendSize, dependencyLevel,
                                                         multiUnits, equivalences); // special buffer
            }
        }
#else
            if (!multiUnits && !equivalences) {
                ...
                if (data->getExtraSpecialBuffer() != nullptr) {
                    data->getExtraSpecialBuffer()->addClause(data->getExtraSpecialBuffer()->specialAuthor(), clause, toSendSize, multiUnits,
                                                             equivalences); // special buffer
                }
            }
#endif
    }
...
// receive from special buffer first
#ifdef PCASSO
        lastSeenSpecialIndex =
            data->getSpecialBuffer().receiveClauses(id, lastSeenSpecialIndex, ca, clauses, receivedUnits, receivedUnitsDependencies,
                                                    receivedEquivalences, receivedEquivalencesDependencies, receiveData);
        lastSeenIndex = data->getBuffer().receiveClauses(id, lastSeenIndex, ca, clauses, receivedUnits, receivedUnitsDependencies,
                                                         receivedEquivalences, receivedEquivalencesDependencies, receiveData);
...
#else
            lastSeenSpecialIndex =
                data->getSpecialBuffer().receiveClauses(id, lastSeenSpecialIndex, ca, clauses, receivedUnits, receivedEquivalences, receiveData);
