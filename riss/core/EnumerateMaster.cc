...
EnumerateMaster::~EnumerateMaster() {
    ...
    for (int i = 0; i < blockingClauses.size(); ++i) {
        delete blockingClauses[i].clause;
    }
    for (int i = 0; i < fullModels.size(); ++i) {
        delete fullModels[i];
    }
}
    ...
    if (projectionFileName != "") {
        ...
        for (int i = 0; i < projectionVariables.size(); ++i) {
            ...
            if (coprocessor != nullptr) {
                coprocessor->freezeExtern(projectionVariables[i]);
            }
