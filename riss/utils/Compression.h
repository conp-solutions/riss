...
namespace Riss {
    ...
    class Compression {
        ...
        bool isDecompressingModel; // indicate that we are currently decompressing for the model, so that newVar calls will not result in increasing
                                   // the mappings
    ...
    public:
        ...
        void setDecompressing(bool isDecompressing) {
            isDecompressingModel = isDecompressing;
        }

        bool isDecompressing() const {
            return isDecompressingModel;
        }
         ...
         */
        inline uint32_t postvars() const {
            return forward.size();
        }
         ...
         */
        inline Var newVar() {
            if (!isAvailable() || isDecompressing()) {
                return 0;
            }
        ...
        inline Lit importLit(const Lit& lit) const {
            if (toInt(lit) < 0) {
                return lit;
            } // take care of lit_Undef and lit_Error!
            if (isAvailable()) {
