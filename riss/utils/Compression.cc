...
namespace Riss {
    ...
    bool Compression::deserialize(const string& filename, bool verbose) {
        ...
        while (stream >> var) {
            // validate mapping
            assert(var >= -1 && var < (int)variables && "Mapped variable must not be larger than number of variables");
        ...
        if (tmpMapping.size() != variables) {
            cerr << "c Error: Mapping has invalid size. Variables=" << variables << ", mapped=" << tmpMapping.size() << endl;
            return false;
        }
        ...
        while (stream >> unit) {
            ...
            // validate lbool
            assert((value == l_True || value == l_False || value == l_Undef) && "Unknown lifted boolean value");
        ...
        if (tmpTrail.size() != variables) {
            cerr << "c Error: Trail has invalid size. Variables=" << variables << ", trail=" << tmpTrail.size() << endl;
            return false;
        }
    ...
    void Compression::update(vector<Var>& _mapping, vector<lbool>& _trail) {
        // get the highest possible variable in the compressed formula (the last element in the mapping), have to scan as we do not know the details
        // in this method
        Var highestVar = 0;
        ...
        if (!isAvailable()) {
            ...
            for (Var var = _mapping.size(); var > 0; var--) {
                if (mapping[var - 1] != UNIT) { // we found a variable that is eliminated. as we started from the end, this is the highest variable in
        ...
        if (forward.size() != 0) {
            for (Var from = 0, to = 0; from < mapping.size(); ++from) {
                ...
                if (mapping[from] != UNIT) {
                    assert(((forward.size() == 0 && to == 0) || to < forward.size()) &&
                           "Compressed variable name must not be larger than forward mapping");
